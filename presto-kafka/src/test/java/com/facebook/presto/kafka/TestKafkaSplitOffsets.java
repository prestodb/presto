...
public class TestKafkaSplitOffsets
{
    ...
    private void produceRecordsWithTimestamps()
    {
        try (KafkaProducer<Long, Object> producer = embeddedKafka.createProducer()) {
            ...
            producer.send(new ProducerRecord<>(topicName, 0, T2, 2L, "msg-2"));
            producer.send(new ProducerRecord<>(topicName, 0, T3, 3L, "msg-3"));
            producer.flush();
        }
    }
            ...
            throws Exception
    {
        ...
        assertEquals(split.getEnd(), 3L,
                "With no filter, split should end at the log end offset (3 records written)");
    }
            ...
            throws Exception
    {
        ...
        assertEquals(split.getStart(), 1L,
                "startTimestamp=T2 should resolve to offset 1 (second record)");
        assertEquals(split.getEnd(), 3L,
            ...
            throws Exception
    {
        ...
        assertEquals(split.getEnd(), 1L,
                "endTimestamp=T2 should resolve to offset 1 (second record)");
    }
            ...
            throws Exception
    {
        ...
        assertEquals(split.getStart(), 1L,
                "startTimestamp=T2 should resolve to offset 1");
        assertEquals(split.getEnd(), 2L,
                "endTimestamp=T3 should resolve to offset 2");
    }
            ...
            throws Exception
    {
        ...
        assertEquals(split.getEnd(), 3L, "timestamp well beyond timestamp for the published messages implies we reached the end of stream");
            ...
            throws Exception
    {
        ...
        assertEquals(split.getEnd(), 1L,
                "endTimestamp=T2 should resolve to offset 1");
    }
            ...
            throws Exception
    {
        ...
        assertEquals(split.getEnd(), 3L, "Long.MAX_VALUE implies we fetch endOffsets");
    }
}
