...
public class TestRawDecoder
{
    ...
    private Map<KafkaColumnHandle, KafkaFieldDecoder<?>> buildMap(List<KafkaColumnHandle> columns)
    {
        ImmutableMap.Builder<KafkaColumnHandle, KafkaFieldDecoder<?>> decoderBuilder = ImmutableMap.builder();
        for (KafkaColumnHandle column : columns) {
            decoderBuilder.put(column, DEFAULT_FIELD_DECODER);
        }
    ...
    public void testFixedWithString()
    {
        ...
        // these only work for single byte codings...
