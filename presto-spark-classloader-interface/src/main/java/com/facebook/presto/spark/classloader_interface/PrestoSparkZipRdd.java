        ...
        extends ZippedPartitionsBaseRDD<Tuple2<Integer, PrestoSparkRow>>
{
    ...
    public scala.collection.Iterator<Tuple2<Integer, PrestoSparkRow>> compute(Partition split, TaskContext context)
    {
        ...
        List<Iterator<Tuple2<Integer, PrestoSparkRow>>> iterators = unmodifiableList(IntStream.range(0, rdds.size())
                .mapToObj(i -> rdds.get(i).iterator(partitions.get(i), context))
                .map(JavaConversions::asJavaIterator)
                .collect(toList()));
        return asScalaIterator(function.apply(iterators));
    }
