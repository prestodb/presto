        ...
        extends AbstractTestQueryFramework
{
            ...
            throws Exception
    {
        ...
        //arrowFlightQueryRunner.getDefaultSession().getSystemProperty()
            ...
            throws Exception
    {
        ...
        return ArrowFlightQueryRunner.createQueryRunner(getServerPort(), getNativeWorkerSystemProperties(), coordinatorProperties, getExternalWorkerLauncher(prestoServerPath.toString(), getServerPort(), "src/test/resources/server.crt"));
    }
    ...
    @Ignore("Caused by: org.jdbi.v3.core.statement.UnableToExecuteStatementException: org.h2.jdbc.JdbcSQLSyntaxErrorException: Column \"LINENUMBER\" must be in the GROUP BY list; SQL statement:\n" +
            "SELECT linenumber, NULL FROM lineitem GROUP BY 1 ORDER BY 1 [90016-199] [statement:\"SELECT linenumber, NULL FROM lineitem GROUP BY 1 ORDER BY 1\", rewritten:\"SELECT linenumber, NULL FROM lineitem GROUP BY 1 ORDER BY 1\", parsed:\"ParsedSql{sql='SELECT linenumber, NULL FROM lineitem GROUP BY 1 ORDER BY 1', parameters=ParsedParameters{positional=false, parameterNames=[]}}\", arguments:{positional:{}, named:{}, finder:[]}]\n")
    @Test
    ...
    @Ignore("Conflicting output with expected")
    @Test
    public void testPad()
    {
                        ...
                        Files.write(catalogDirectoryPath.resolve(format("%s.properties", ARROW_FLIGHT_CATALOG)),
                                format("connector.name=%s\n" +
                                       "arrow-flight.server=localhost\n" +
                                        ...
                                        "arrow-flight.server.verify=false", ARROW_FLIGHT_CONNECTOR, flightServerPort).getBytes());
