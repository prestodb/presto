...
public class PrestoSparkNativeServiceFactory
        extends PrestoSparkServiceFactory
{
    @Override
    protected List<Module> getAdditionalModules(PrestoSparkConfiguration configuration)
    {
        List<Module> result = super.getAdditionalModules(configuration);
        result.add(new NativeExecutionModule());
        return result;
    }
}
