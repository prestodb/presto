        ...
        implements QueryRunner
{
    ...
    public DistributedQueryRunner(Session defaultSession, int nodeCount, Map<String, String> extraProperties)
            throws Exception
    {
        this(false, defaultSession, nodeCount, extraProperties, ImmutableMap.of(), DEFAULT_SQL_PARSER_OPTIONS, ENVIRONMENT, Optional.empty());
    }
            ...
            throws Exception
    {
        ...
        try {
            ...
            for (int i = 1; i < nodeCount; i++) {
                TestingPrestoServer worker = closer.register(createTestingPrestoServer(discoveryServer.getBaseUrl(), false, resourceManagerEnabled, false, extraProperties, parserOptions, environment, baseDataDir));
                servers.add(worker);
            }
            ...
            coordinator = closer.register(createTestingPrestoServer(discoveryServer.getBaseUrl(), false, resourceManagerEnabled, true, extraCoordinatorProperties, parserOptions, environment, baseDataDir));
    ...
    public static class Builder
    {
        ...
        public DistributedQueryRunner build()
                throws Exception
        {
            return new DistributedQueryRunner(resourceManagerEnabled, defaultSession, nodeCount, extraProperties, coordinatorProperties, parserOptions, environment, baseDataDir);
        }
    }
}
