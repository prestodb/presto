            ...
            implements ResultsSession<MaterializedResult>
    {
        ...
        public void addResults(QueryResults results)
        {
            ...
            if (results.getData() != null) {
                rows.addAll(transform(results.getData(), getDataToRowFunction()));
            }
        }
        ...
        private Function<List<Object>, MaterializedRow> getDataToRowFunction()
        {
            return new Function<List<Object>, MaterializedRow>()
            {
                ...
                {
                    List<Type> rowTypes = types.get();
                    checkState(rowTypes != null, "Data without types received!");
                    checkArgument(data.size() == rowTypes.size(), "columns size does not match types size");
