...
{
    private static final Session TDIGEST_SESSION = builder(TEST_SESSION).setSystemProperty("tdigest_enabled", "true").build();
    ...
    {
        assertQuery(TDIGEST_SESSION, format("SELECT value_at_quantile(%s_agg(CAST(orderkey AS DOUBLE)), 0.5E0) > 0 FROM lineitem", type), "SELECT true");
        assertQuery(TDIGEST_SESSION, format("SELECT value_at_quantile(%s_agg(CAST(quantity AS DOUBLE)), 0.5E0) > 0 FROM lineitem", type), "SELECT true");
        assertQuery(TDIGEST_SESSION, format("SELECT value_at_quantile(%s_agg(CAST(quantity AS DOUBLE)), 0.5E0) > 0 FROM lineitem", type), "SELECT true");
        assertQuery(TDIGEST_SESSION, format("SELECT value_at_quantile(%s_agg(CAST(orderkey AS DOUBLE), 2), 0.5E0) > 0 FROM lineitem", type), "SELECT true");
        assertQuery(TDIGEST_SESSION, format("SELECT value_at_quantile(%s_agg(CAST(quantity AS DOUBLE), 3), 0.5E0) > 0 FROM lineitem", type), "SELECT true");
        assertQuery(TDIGEST_SESSION, format("SELECT value_at_quantile(%s_agg(CAST(quantity AS DOUBLE), 4), 0.5E0) > 0 FROM lineitem", type), "SELECT true");
        assertQuery(TDIGEST_SESSION, format("SELECT value_at_quantile(%s_agg(CAST(orderkey AS DOUBLE), 2, 0.0001E0), 0.5E0) > 0 FROM lineitem", type), "SELECT true");
        assertQuery(TDIGEST_SESSION, format("SELECT value_at_quantile(%s_agg(CAST(quantity AS DOUBLE), 3, 0.0001E0), 0.5E0) > 0 FROM lineitem", type), "SELECT true");
        assertQuery(TDIGEST_SESSION, format("SELECT value_at_quantile(%s_agg(CAST(quantity AS DOUBLE), 4, 0.0001E0), 0.5E0) > 0 FROM lineitem", type), "SELECT true");
    }
    ...
    {
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(%s_agg(CAST(orderkey AS DOUBLE)), 0.5E0) > 0 FROM lineitem GROUP BY partkey", type),
                "SELECT partkey, true FROM lineitem GROUP BY partkey");
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(%s_agg(CAST(quantity AS DOUBLE)), 0.5E0) > 0 FROM lineitem GROUP BY partkey", type),
                "SELECT partkey, true FROM lineitem GROUP BY partkey");
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(%s_agg(CAST(quantity AS DOUBLE)), 0.5E0) > 0 FROM lineitem GROUP BY partkey", type),
                "SELECT partkey, true FROM lineitem GROUP BY partkey");
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(%s_agg(CAST(orderkey AS DOUBLE), 2), 0.5E0) > 0 FROM lineitem GROUP BY partkey", type),
                "SELECT partkey, true FROM lineitem GROUP BY partkey");
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(%s_agg(CAST(quantity AS DOUBLE), 3), 0.5E0) > 0 FROM lineitem GROUP BY partkey", type),
                "SELECT partkey, true FROM lineitem GROUP BY partkey");
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(%s_agg(CAST(quantity AS DOUBLE), 4), 0.5E0) > 0 FROM lineitem GROUP BY partkey", type),
                "SELECT partkey, true FROM lineitem GROUP BY partkey");
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(%s_agg(CAST(orderkey AS DOUBLE), 2, 0.0001E0), 0.5E0) > 0 FROM lineitem GROUP BY partkey", type),
                "SELECT partkey, true FROM lineitem GROUP BY partkey");
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(%s_agg(CAST(quantity AS DOUBLE), 3, 0.0001E0), 0.5E0) > 0 FROM lineitem GROUP BY partkey", type),
                "SELECT partkey, true FROM lineitem GROUP BY partkey");
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(%s_agg(CAST(quantity AS DOUBLE), 4, 0.0001E0), 0.5E0) > 0 FROM lineitem GROUP BY partkey", type),
                "SELECT partkey, true FROM lineitem GROUP BY partkey");
    }
    ...
    {
        assertQuery(TDIGEST_SESSION,
                format("SELECT value_at_quantile(merge(%s), 0.5E0) > 0 FROM (SELECT partkey, %s_agg(CAST(orderkey AS DOUBLE)) as %s FROM lineitem GROUP BY partkey)",
                type,
                type,
                type),
                "SELECT true");
    ...
    {
        assertQuery(TDIGEST_SESSION, format("SELECT partkey, value_at_quantile(merge(%s), 0.5E0) > 0 " +
