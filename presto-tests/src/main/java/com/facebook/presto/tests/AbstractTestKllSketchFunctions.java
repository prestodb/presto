        ...
        extends AbstractTestQueryFramework
{
    ...
    /**
     * Cross-engine constant-fold test: simulates the scenario where the Java coordinator
     * builds (or constant-folds) a kllsketch literal and the worker (native or Java) executes
     * sketch_kll_rank / sketch_kll_quantile on it.
     *
     * <p>The hex literals below are the canonical Java-serialized bytes for each type
     * (values 0..99 for numeric, 'a'..'z' for varchar, i%3==0 for boolean, k=200).
     * They are identical to the bytes produced by the native engine, which is the core
     * cross-engine compatibility guarantee.
     *
     * <p>When this test runs via presto-native-tests (TestKllSketchFunctions), the worker
     * is a real native process — so the CAST literal is constant-folded by the Java
     * coordinator and the scalar functions execute on the native side, exercising the
     * full Java-coordinator → native-worker byte boundary.
     */
    @Test
    public void testKllSketchCrossEngineConstantFold()
    {
        // CHECKSTYLE:OFF: LineLength
        // BIGINT: Java-serialized KllItemsSketch<Long>, values 0..99, k=200
        ...
        // DOUBLE: Java-serialized KllItemsSketch<Double>, values 0.0..99.0, k=200
        ...
        // VARCHAR: Java-serialized KllItemsSketch<String> with ArrayOfStringsSerDe, 'a'..'z', k=200
        ...
        // BOOLEAN: Java-serialized KllItemsSketch<Boolean> with bit-packed ArrayOfBooleansSerDe
        // (~34 true, ~66 false). This is the most critical type: Java bit-packs, native must
        // decode correctly via deserializeBoolSketch().
