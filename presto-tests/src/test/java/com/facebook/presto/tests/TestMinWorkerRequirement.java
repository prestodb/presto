...
public class TestMinWorkerRequirement
{
            ...
            throws Exception
    {
        try (DistributedQueryRunner queryRunner = TpchQueryRunner.builder()
                .setCoordinatorProperties(ImmutableMap.of("query-manager.initialization-required-workers", "5"))
                .setNodeCount(4)
                .build()) {
    ...
    {
        try (DistributedQueryRunner queryRunner = TpchQueryRunner.builder()
                .setExtraProperties(ImmutableMap.of("node-scheduler.include-coordinator", "false"))
                .setCoordinatorProperties(ImmutableMap.of("query-manager.initialization-required-workers", "4"))
                .setNodeCount(4)
                .build()) {
            queryRunner.execute("SELECT 1");
            fail("Expected exception due to insufficient active worker nodes");
