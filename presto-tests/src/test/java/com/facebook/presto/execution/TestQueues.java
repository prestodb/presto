...
public class TestQueues
{
    ...
    private QueryId createQuery(DistributedQueryRunner queryRunner, Session session, String sql)
    {
        return queryRunner.getCoordinator().getQueryManager().createQuery(session, sql).getQueryId();
    }

    private void cancelQuery(DistributedQueryRunner queryRunner, QueryId queryId)
    {
        queryRunner.getCoordinator().getQueryManager().cancelQuery(queryId);
    }

    private void waitForQueryState(DistributedQueryRunner queryRunner, QueryId queryId, QueryState queryState)
