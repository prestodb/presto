        ...
        extends AbstractSqlInvokedFunctionNamespaceManager
{
    ...
    protected synchronized SqlInvokedFunction createSqlInvokedFunction(String functionName, JsonBasedUdfFunctionMetadata jsonBasedUdfFunctionMetaData)
    {
        ...
        List<TypeSignature> parameterTypeList = convertApplicableTypeToVariable(jsonBasedUdfFunctionMetaData.getParamTypes(), types);
        ...
        TypeSignature outputType = convertApplicableTypeToVariable(jsonBasedUdfFunctionMetaData.getOutputType(), types);
        ...
        Optional<AggregationFunctionMetadata> aggregationFunctionMetadata =
                jsonBasedUdfFunctionMetaData.getAggregateMetadata()
                        .map(metadata -> new AggregationFunctionMetadata(
                                convertApplicableTypeToVariable(metadata.getIntermediateType(), types),
    ...
    public static TypeSignature convertApplicableTypeToVariable(TypeSignature typeSignature, List<Type> types)
    {
        List<TypeSignature> typeSignaturesList = convertApplicableTypeToVariable(ImmutableList.of(typeSignature), types);
        checkArgument(!typeSignaturesList.isEmpty(), "Type signature list is empty for : " + typeSignature);
        return typeSignaturesList.get(0);
    }

    public static List<TypeSignature> convertApplicableTypeToVariable(List<TypeSignature> typeSignatures, List<Type> types)
    {
            ...
            List<Type> types)
    {
        ...
        for (TypeSignatureParameter parameter : typeSignatureParameterList) {
            ...
            if (parameterTypeSignature.getParameters().isEmpty()) {
                boolean changeTypeToVariable = isDecimalTypeBase(typeSignature.getBase()) && isValidType(parameterTypeSignature, types);
    ...
    private static boolean isValidType(TypeSignature parameterType, List<Type> types)
    {
