...
public class TestOrcFileRewriter
{
    ...
    @Test
    public void testRewriteNoTypeInformationInMetadataOfSourceFile()
            throws Exception
    {
        ...
        File file = new File(temporary, randomUUID().toString());
        try (OrcFileWriter writer = new OrcFileWriter(columnIds, columnTypes, file, orcFileMetadataCodec, false)) {
            List<Page> pages = rowPagesBuilder(columnTypes)
                    .row(123L, "hello")
                    .row(777L, "sky")
                    .build();
