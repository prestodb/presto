        ...
        implements ConnectorPageSourceProvider
{
    ...
    public ConnectorPageSource createPageSource(ConnectorTransactionHandle transactionHandle, ConnectorSession session, ConnectorSplit split, List<ColumnHandle> columns)
    {
        ...
        Map<Long, Type> chunkColumnTypes = raptorSplit.getChunkColumnTypes();
        Optional<CompressionType> compressionType = raptorSplit.getCompressionType();

        if (raptorSplit.getShardUuids().size() == 1) {
            UUID shardUuid = raptorSplit.getShardUuids().iterator().next();
            return createPageSource(shardUuid, bucketNumber, columns, predicate, attributes, transactionId, chunkColumnTypes, compressionType);
        }

        Iterator<ConnectorPageSource> iterator = raptorSplit.getShardUuids().stream()
                .map(shardUuid -> createPageSource(shardUuid, bucketNumber, columns, predicate, attributes, transactionId, chunkColumnTypes, compressionType))
                .iterator();
            ...
            Optional<CompressionType> compressionType)
    {
        ...
        return storageManager.getPageSource(shardUuid, bucketNumber, columnIds, columnTypes, predicate, attributes, transactionId, chunkColumnTypes, compressionType);
    }
}
