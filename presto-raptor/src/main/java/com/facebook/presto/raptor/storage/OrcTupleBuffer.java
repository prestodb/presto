...
public class OrcTupleBuffer
{
    ...
    public OrcTupleBuffer(List<Long> columnIds, List<StorageType> columnTypes, Optional<Long> sampleWeightColumnId)
    {
    ...
    public Writable finishRecordAndReset()
    {
        checkState(field != -1, "not in record");
        checkState(field == fieldCount, "not all fields set");
        field = -1;
        return serializer.serialize(row, tableInspector);
    }
