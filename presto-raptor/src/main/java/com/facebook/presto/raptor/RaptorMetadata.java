        ...
        implements ConnectorMetadata
{
    ...
    public Map<String, ConnectorColumnHandle> getColumnHandles(ConnectorTableHandle tableHandle)
    {
        ...
        for (TableColumn tableColumn : dao.listTableColumns(raptorTableHandle.getTableId())) {
            ...
            builder.put(tableColumn.getColumnName(),
                    new RaptorColumnHandle(connectorId, tableColumn.getColumnName(), tableColumn.getColumnId()));
        }
    ...
    public ConnectorColumnHandle getColumnHandle(ConnectorTableHandle tableHandle, String columnName)
    {
        ...
        return new RaptorColumnHandle(connectorId, columnName, columnId);
    }
    ...
    public ConnectorTableHandle createTable(ConnectorSession session, final ConnectorTableMetadata tableMetadata)
    {
        ...
        RaptorColumnHandle sampleWeightColumnHandle = null;
        if (tableMetadata.isSampled()) {
            sampleWeightColumnHandle = new RaptorColumnHandle(connectorId, SAMPLE_WEIGHT_COLUMN_NAME, tableMetadata.getColumns().size() + 1);
        }
        return new RaptorTableHandle(
                connectorId,
    ...
    @Override
    public void alterTableRename(ConnectorTableHandle tableHandle, final String databaseName, final String tableName,
                                    final String targetDbName, final String targetTableName)
    {
        final RaptorTableHandle raptorHandle = checkType(tableHandle, RaptorTableHandle.class, "tableHandle");
        dbi.inTransaction(new VoidTransactionCallback()
        {
            ...
            {
                MetadataDao dao = handle.attach(MetadataDao.class);
                MetadataDaoUtils.alterTableRename(dao, databaseName, tableName, targetDbName, targetTableName);
            }
        });
    }
