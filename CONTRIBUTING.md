...
**Scope (optional):** The area of code affected. All acceptable scopes are defined in [.github/workflows/conventional-commit-check.yml](.github/workflows/conventional-commit-check.yml). Common scopes include `parser`, `analyzer`, `planner`, `spi`, `connector`, `function`, `native`, and `testing`.

An acceptable scope may also be any connector included in this repository, such as `hive` and `iceberg`.
