...
/**
 * Represents a HBase specific {@link ConnectorSplit}. Each split is mapped to a segment file on disk (based off the segment offset start() and end() values) so that
 * a partition can be processed by reading segment files from partition leader. Otherwise, a HBase topic could only be processed along partition boundaries.
 * <p/>
 * When planning to process a HBase topic with Presto, using smaller than the recommended segment size (default is 1G) allows Presto to optimize early and process a topic
