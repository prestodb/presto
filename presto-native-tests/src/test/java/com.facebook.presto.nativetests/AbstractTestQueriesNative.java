        ...
        extends AbstractTestQueries
{
    ...
    private static final String hashGenerationUnsupportedError = ".*Scalar function name not registered: presto.default.\\$operator\\$.*hash.*";
    private static final String arrayComparisonUnsupportedError = ".*ARRAY comparison not supported for values that contain nulls.*";
    private static final String createHllFunctionNotRegistered = ".*Scalar function name not registered: presto.default.create_hll, called with arguments.*";
    ...
    public void testJoinPrefilter()
    {
...

        {
            ...
            assertQueryFails(session, testQuery, hashGenerationUnsupportedError, true);
        }
    ...
    @Override
    @Test(expectedExceptions = {RuntimeException.class, PrestoException.class}, expectedExceptionsMessageRegExp = createHllFunctionNotRegistered)
    ...
    {
        assertQueryFails("SELECT cardinality(merge(create_hll(custkey))) FROM orders", createHllFunctionNotRegistered, true);
    }
    ...
    public void testMergeHyperLogLogGroupBy()
    {
        assertQueryFails(
                "SELECT orderstatus, cardinality(merge(create_hll(custkey))) " +
                        "FROM orders " +
                        "GROUP BY orderstatus", createHllFunctionNotRegistered, true);
    }
    ...
    public void testMergeHyperLogLogWithNulls()
    {
        assertQueryFails("SELECT cardinality(merge(create_hll(IF(orderstatus = 'O', custkey)))) FROM orders",
                createHllFunctionNotRegistered, true);
    }
...
//        assertQuery("SELECT (SELECT avg(totalprice) FROM orders GROUP BY custkey, orderdate HAVING avg(totalprice) < a) FROM (VALUES 900) t(a)");
    ...
    public void testTryWithLambda()
    {
        ...
        assertQueryFails("SELECT TRY(apply(5 + RANDOM(1), x -> x + 1) / 0)", applyNotRegisteredError, true);
