        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testMapTopNValues()
    {
        ...
        assertQuery("WITH input(c0) AS ( VALUES (MAP(ARRAY[CAST(-2147483648 AS INTEGER), CAST(0 AS INTEGER), CAST(2147483647 AS INTEGER)], ARRAY[1,1,1])) ) SELECT map_top_n_values(c0, 2, (x, y) -> x) AS result FROM input",
                "WITH input(c0) AS ( VALUES (MAP(ARRAY[CAST(-2147483648 AS INTEGER), CAST(0 AS INTEGER), CAST(2147483647 AS INTEGER)], ARRAY[1,1,1])) ) SELECT map_top_n_values(c0, 2, (x, y) -> CAST(IF(x < y, -1, IF(x = y, 0, 1)) AS INTEGER)) AS result FROM input");

        assertQueryFails(
                "SELECT map_top_n_values(c0, BIGINT '6455219767830808341') FROM (VALUES (MAP(ARRAY[1, 2], ARRAY[ARRAY[null, null], ARRAY[1, 2]]))) t(c0)",
                ".* Ordering nulls is not supported Top-level Expression: (native|presto)\\.default\\.map_top_n_values\\(field, 6455219767830808341:BIGINT\\)");
    }
}
