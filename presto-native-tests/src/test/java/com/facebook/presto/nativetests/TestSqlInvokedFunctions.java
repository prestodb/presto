        ...
        extends AbstractTestSqlInvokedFunctions
{
    ...
    public void testMapTopNKeys()
    {
            ...
            assertQuery("WITH input(c0) AS ( VALUES (MAP(ARRAY[-2147483648, 0, 2147483647], ARRAY[1,1,1])) ) SELECT map_top_n_keys(c0, 2, (x, y) -> x) AS result FROM input",
                    "WITH input(c0) AS ( VALUES (MAP(ARRAY[-2147483648, 0, 2147483647], ARRAY[1,1,1])) ) SELECT map_top_n_keys(c0, 2, (x, y) -> CAST(IF(x < y, -1, IF(x = y, 0, 1)) AS INTEGER)) AS result FROM input");
    }
}
