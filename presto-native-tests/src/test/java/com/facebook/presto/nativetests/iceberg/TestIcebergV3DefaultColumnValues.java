        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testDefaultForHistoricalRows(boolean pushdownFilterEnabled)
    {
        ...
        try {
            ...
            assertUpdate(session, String.format("DROP TABLE IF EXISTS %s", tableName));
        }
    }

    @Test(dataProvider = "pushdownFilterEnabled")
    public void testNewRowsWithoutExplicitValueReturnNull(boolean pushdownFilterEnabled)
    {
        Session session = sessionWithPushdown(pushdownFilterEnabled);
    ...
    {
        assertUpdate(session, String.format("CREATE TABLE %s %s WITH (\"format-version\" = '3', format = 'PARQUET')", tableName, tableDefinition));
        assertUpdate(session, String.format("INSERT INTO %s %s", tableName, values), rowCount);
    }
}
