        ...
        implements RecordCursor
{
    ...
    private boolean nextRow(String keyString)
    {
        ...
        keyData = keyString.getBytes(StandardCharsets.UTF_8);
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            switch (split.getKeyDataType()) {
                case STRING: {
                    ...
                    log.debug("Scanning new Redis keys from cursor %s . %d values read so far",
                            cursor, totalValues);
