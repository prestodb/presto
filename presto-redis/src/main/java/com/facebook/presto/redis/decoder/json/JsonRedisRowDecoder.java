...
/**
 * JSON specific Redis row decoder.
 */
public class JsonRedisRowDecoder
        implements RedisRowDecoder
{
    ...
    @Inject
    JsonRedisRowDecoder(ObjectMapper objectMapper)
    {
    ...
    public boolean decodeRow(byte[] data, Map<String, String> dataMap, Set<RedisFieldValueProvider> fieldValueProviders, List<RedisColumnHandle> columnHandles, Map<RedisColumnHandle, RedisFieldDecoder<?>> fieldDecoders)
    {
        ...
        for (RedisColumnHandle columnHandle : columnHandles) {
            ...
            RedisFieldDecoder<JsonNode> decoder = (RedisFieldDecoder<JsonNode>) fieldDecoders.get(columnHandle);
    ...
    private static JsonNode locateNode(JsonNode tree, RedisColumnHandle columnHandle)
    {
