...
import static com.facebook.presto.elasticsearch.ElasticsearchErrorCode.ELASTIC_SEARCH_MAX_HITS_EXCEEDED;
        ...
        implements RecordCursor
{
    ...
    private List<SearchHit> sendElasticQuery(ElasticsearchQueryBuilder queryBuilder)
    {
        ...
        if (response.getHits().getTotalHits() > maxHits) {
            throw new PrestoException(ELASTIC_SEARCH_MAX_HITS_EXCEEDED, "The number of hits for the query: " + response.getHits().getTotalHits() + " exceeds the configured max hits: " + maxHits);
        }
