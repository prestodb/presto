        ...
        implements ConnectorMetadata
{
    ...
    public Map<String, ColumnHandle> getColumnHandles(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        for (ColumnMetadata column : table.getColumnsMetadata()) {
            ...
            int position = metadata.getOrdinalPosition() == -1 ? index : metadata.getOrdinalPosition();
            columnHandles.put(metadata.getName(), new ElasticsearchColumnHandle(column.getName(), column.getType(), metadata.getJsonPath(), metadata.getJsonType(), position, metadata.isList()));
            index++;
        }
