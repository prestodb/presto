 ...
 */
package com.facebook.presto.maxcompute.adapter;
    ...
    public CompletableFuture<?> appendPage(Page page)
    {
        ...
        for (int position = 0; position < page.getPositionCount(); position++) {
            ...
            for (int i = 0; i < page.getChannelCount(); i++) {
                writeField(position, page.getBlock(i), columnTypes.get(i));
            }
    ...
    private void writeField(int position, Block block, Type type)
    {
        ...
        if (type.getJavaType() == boolean.class) {
            ...
            if (type instanceof DecimalType) {
                recordSink.appendDecimal(type.getLong(block, position));
            }
            else {
            ...
            if (type instanceof DecimalType) {
                recordSink.appendDecimal(type.getSlice(block, position));
            }
            else {
