        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testUpdateColumnType()
    {
        try {
            ...
            DerivedColumnSpecList derivedColumnSpecList = IcebergTableProperties.getDerivedColumnSpec(getConnectorTableMetadata("test2", txnSession).getProperties());
            Optional<DerivedColumnSpec> derivedColumnSpec = derivedColumnSpecList.getDerivedColumnSpecs().stream().filter(x -> x.getDerivedColumnName().equals("c1_derived")).findFirst();
            assertTrue(derivedColumnSpec.isPresent(), "Derived column spec for column c1_derived must be present.");
            assertEquals(derivedColumnSpec.get().getDerivedColumnReturnType(), "decimal(19,2)");
    ...
    public void testShowCreateTableIncludesDerivedColumns()
    {
        try {
            ...
            assertUpdate("ALTER TABLE test_ddl ADD COLUMN c2_derived2 VARCHAR GENERATED ALWAYS AS upper(c2) PERSISTENT");
            ...
            assertTrue(ddl.contains("\"c2_derived2\" varchar GENERATED ALWAYS AS \"upper\"(c2) PERSISTENT"));
    ...
    // TODO: We are allowing setting derived column property directly until we have a dedicated DDL command for it.
    // Once we have DDL, this test is expected to cause exceptions as follows.
    // (expectedExceptions = RuntimeException.class, expectedExceptionsMessageRegExp = ".*Updating property presto.derived-columns.spec.json is not supported currently.*")
    @Test
    public void testSetPropertyIsAllowedForAddDerivedColumn()
    {
        try {
            assertUpdate("CREATE TABLE Test2 (Col1 BIGINT, Col2 VARCHAR, Col1_Derived decimal(13,1) AS (CAST(Col1 AS decimal) * DECIMAL '10.5') PERSISTENT)");
            assertUpdate("INSERT INTO Test2 VALUES (123, 'B', 123 * 10.5), (120, 'C', 120 * 10.5)", 2);
            ...
            MaterializedResult result = computeActual("SHOW CREATE TABLE Test2");
            String ddl = (String) result.getOnlyValue();
            assertTrue(ddl.contains("\"col1_derived\" decimal(13,1)"));
            assertUpdate(format("ALTER TABLE IF EXISTS Test2 SET PROPERTIES (\"%s\" = JSON '%s')", DERIVED_COLUMN_EXPRESSION_SPEC,
                    DERIVED_COLUMN_SPEC_JSON_CODEC.toJson(new DerivedColumnSpecList(ImmutableList.of(
                            new DerivedColumnSpec(DerivedColumnType.PERSISTENT, "col1 * 2.0", "col1_derived", 3, "decimal(13,1)"))))));
            result = computeActual("SHOW CREATE TABLE Test2");
            ddl = (String) result.getOnlyValue();
            assertTrue(ddl.contains("\"col1_derived\" decimal(13,1) AS col1 * 2.0"));
        }
        finally {
            assertUpdate("DROP TABLE IF EXISTS Test2");
        }
    ...
    {
        Session txnSession = assertStartTransaction(session, "START TRANSACTION");
        ConnectorTableMetadata connectorTableMetadata = getConnectorTableMetadata(tableName, txnSession);
    ...
    private ConnectorTableMetadata getConnectorTableMetadata(String tableName, Session txnSession)
    {
        ...
        return getQueryRunner().getMetadata().getTableMetadata(txnSession, handle).getMetadata();
    }
}
