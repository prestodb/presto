        ...
        extends AbstractTestQueryFramework
{
            ...
            throws Exception
    {
        assertQueryFails(
                ...
                "Cross-catalog materialized views are not enabled. Materialized view iceberg.test_schema.test_cross_catalog_fail_mv cannot be created from a base table tpch.sf1.region in a different catalog tpch.");
    }
    ...
    @Test
    public void testCrossCatalogMaterializedViewFreshToStale()
            throws Exception
    {
        assertUpdate("CREATE MATERIALIZED VIEW test_cross_catalog_fresh_to_stale_mv " +
                "WITH (" +
                "cross_catalog_materialized_views_enabled = true, " +
                "use_timestamp_based_staleness = true, " +
                "staleness_window = '2s' " +
        ...
        assertUpdate("REFRESH MATERIALIZED VIEW test_cross_catalog_fresh_to_stale_mv", 5);

        assertQuery(
                "SELECT freshness_state FROM information_schema.materialized_views " +
                        "WHERE table_schema = 'test_schema' AND table_name = 'test_cross_catalog_fresh_to_stale_mv'",
                "SELECT 'FULLY_MATERIALIZED'");

        assertQuery("SELECT regionkey, name FROM test_cross_catalog_fresh_to_stale_mv WHERE regionkey = 1",
                "VALUES (1, 'AMERICA')");
        ...
        assertUpdate("DROP MATERIALIZED VIEW test_cross_catalog_fresh_to_stale_mv");
    }

    @Test
    public void testMaterializedViewStaysWithinStalenessWindow()
            throws Exception
    {
        assertUpdate("CREATE TABLE test_base_table_2 (id BIGINT, name VARCHAR)");
        assertUpdate("INSERT INTO test_base_table_2 VALUES (10, 'alpha'), (20, 'beta')", 2);

        assertUpdate("CREATE MATERIALIZED VIEW test_stays_fresh_mv " +
                "WITH (" +
                "use_timestamp_based_staleness = true, " +
                "staleness_window = '10s' " +
                ") " +
                "AS SELECT id, name FROM test_base_table_2");

        assertUpdate("REFRESH MATERIALIZED VIEW test_stays_fresh_mv", 2);

        assertQuery(
                "SELECT freshness_state FROM information_schema.materialized_views " +
                        "WHERE table_schema = 'test_schema' AND table_name = 'test_stays_fresh_mv'",
                "SELECT 'FULLY_MATERIALIZED'");
        ...
        assertUpdate("DROP MATERIALIZED VIEW test_stays_fresh_mv");
        assertUpdate("DROP TABLE test_base_table_2");
    }
}
