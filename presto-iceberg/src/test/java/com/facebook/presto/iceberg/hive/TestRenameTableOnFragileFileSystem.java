...
public class TestRenameTableOnFragileFileSystem
{
    ...
    private static final JsonCodec<DatabaseMetadata> DATABASE_CODEC = JsonCodec.jsonCodec(DatabaseMetadata.class);
    private static final JsonCodec<TableMetadata> TABLE_CODEC = JsonCodec.jsonCodec(TableMetadata.class);
            ...
            throws Exception
    {
        ...
        try {
            ...
            try {
                ...
                fail("Rename table should not fail: " + e.getMessage());
            }
            ...
            throws Exception
    {
        ...
        try {
            ...
            hdfsEnvironment.setFailSignal(FailSignal.DELETE);
            try {
                icebergHiveMetadata.renameTable(connectorSession, icebergTableHandle, new SchemaTableName(newSchemaName, newTableName));
            }
            catch (Exception e) {
            ...
            schemaTableNames = icebergHiveMetadata.listTables(connectorSession, Optional.of(originSchemaName));
            assertFalse(schemaTableNames.contains(new SchemaTableName(originSchemaName, originTableName)));
            schemaTableNames = icebergHiveMetadata.listTables(connectorSession, Optional.of(newSchemaName));
            assertTrue(schemaTableNames.contains(new SchemaTableName(newSchemaName, newTableName)));
            ...
            throws Exception
    {
        ...
        try {
            ...
            hdfsEnvironment.setFailSignal(FailSignal.COPY);
            try {
                icebergHiveMetadata.renameTable(connectorSession, icebergTableHandle, new SchemaTableName(newSchemaName, newTableName));
                fail("Rename table should fail here");
            }
            catch (Exception e) {
                assertTrue(e.getMessage().contains("Could not rename table, because of fail to copy directory: "));
            }
            ...
            throws Exception
    {
        ...
        try {
            ...
            hdfsEnvironment.setFailSignal(FailSignal.RENAME);
            try {
                icebergHiveMetadata.renameTable(connectorSession, icebergTableHandle, new SchemaTableName(newSchemaName, newTableName));
                fail("Rename table should fail here.");
            }
            catch (Exception e) {
                assertTrue(e.getMessage().contains("Could not rename table, because of fail to rename file "));
            }
