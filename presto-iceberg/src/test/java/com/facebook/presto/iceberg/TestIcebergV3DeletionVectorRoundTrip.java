...
import static com.facebook.presto.iceberg.IcebergAbstractMetadata.enumerateExistingDeletionVectors;
 ...
 * {@link IcebergAbstractMetadata#replaceDeletionVectors} and the spec/partition-carrying
 * reconstruction in {@code IcebergAbstractMetadata.toIcebergDeletionVector}. It drives that logic
 * against a real Iceberg table (unpartitioned, partitioned, and on a branch) using hand-built
 * Puffin DV entries — the same technique {@code TestIcebergV3.testPuffinDeletionVectorsNotSupported}
 * uses — and asserts no duplicate DVs accumulate. The partitioned case specifically guards the
 * reconstruction carrying the real spec and partition: Iceberg prunes candidate manifests by
 * {@code (specId, partition)} before matching entries for removal, so an unpartitioned
 * reconstruction would silently leave the prior DV behind.
 */
        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testRepeatedDeletionVectorReplacementUnpartitioned()
    {
        ...
        try {
            ...
            for (int round = 0; round < MUTATION_ROUNDS; round++) {
                ...
                Multiset<String> dvs = puffinDeletionVectorsByDataFile(table, table.currentSnapshot());
    ...
    public void testRepeatedDeletionVectorReplacementPartitioned()
    {
        ...
        try {
            ...
            for (int round = 0; round < MUTATION_ROUNDS; round++) {
                commitReplacementDeletionVector(table, dataFile, round);
                Multiset<String> dvs = puffinDeletionVectorsByDataFile(table, table.currentSnapshot());
                // The removal reconstructs the prior DV with the real spec and partition. Iceberg
                // prunes candidate manifests by (specId, partition) before matching, so an
                // unpartitioned reconstruction would skip the manifest holding the prior DV and
                // leave a second DV behind.
                assertEquals(dvs.count(dataFile.path), 1,
    ...
    /**
     * Regression test for deletion-vector enumeration on a branch. A branch mutation reads from
     * the branch head, not {@code currentSnapshot()} (the main head), so enumerating the latter
     * would miss the branch's own prior DV: it would never be removed (two DVs on one data file)
     * and the worker would seed from main's DV, resurrecting rows already deleted on the branch.
     */
    @Test
    public void testDeletionVectorEnumerationIsScopedToTheBranch()
    {
        String tableName = "test_v3_dv_roundtrip_branch";
        ...
        try {
            ...
            // Mutate only the branch. Main stays at mainSnapshotId with no deletion vectors.
            commitReplacementDeletionVector(table, dataFile, 0, Optional.of(branchName));
            long branchSnapshotId = table.snapshot(branchName).snapshotId();
            assertEquals(table.currentSnapshot().snapshotId(), mainSnapshotId,
                    "the branch mutation must not move the main branch");

            // The read snapshot beginDelete/beginMerge resolve for `table@branch`.
            assertEquals(
                    resolveSnapshotIdByName(table, new IcebergTableName(
                            tableName, IcebergTableType.DATA, Optional.empty(), Optional.of(branchName), Optional.empty())),
                    Optional.of(branchSnapshotId));

            // Enumerating the branch head sees the branch's DV; enumerating main does not.
            assertEquals(
                    enumerateExistingDeletionVectors(table, Optional.of(branchSnapshotId)).keySet(),
                    ImmutableSet.of(dataFile.path));
            assertEquals(
                    enumerateExistingDeletionVectors(table, Optional.of(mainSnapshotId)),
                    ImmutableMap.of(),
                    "main carries no deletion vectors, so enumerating it for a branch mutation would drop the branch's prior DV");

            // Further branch mutations must keep replacing the branch's own DV.
            for (int round = 1; round < MUTATION_ROUNDS; round++) {
                commitReplacementDeletionVector(table, dataFile, round, Optional.of(branchName));
                Multiset<String> dvs = puffinDeletionVectorsByDataFile(table, table.snapshot(branchName));
                assertEquals(dvs.count(dataFile.path), 1,
                        "branch data file must carry exactly one deletion vector after round " + round);
                assertEquals(dvs.elementSet().size(), 1, "no other data file should gain a DV");
            }
            assertEquals(puffinDeletionVectorsByDataFile(table, table.snapshot(mainSnapshotId)).size(), 0,
                    "main must be unaffected by branch mutations");
        }
        finally {
            dropTable(tableName);
        }
    ...
    {
        commitReplacementDeletionVector(table, dataFile, round, Optional.empty());
    }

    private void commitReplacementDeletionVector(Table table, DataFileInfo dataFile, int round, Optional<String> branch)
    {
        Snapshot readSnapshot = branch.map(table::snapshot).orElseGet(table::currentSnapshot);
        Map<String, com.facebook.presto.iceberg.delete.DeleteFile> existingDeletionVectors =
                enumerateExistingDeletionVectors(table, Optional.of(readSnapshot.snapshotId()));
        ...
        FileMetadata.Builder builder = FileMetadata.deleteFileBuilder(dataFile.spec)
                .ofPositionDeletes()
                .withPath(dataFile.path + ".dv" + round + (branch.map(b -> "." + b).orElse("")) + ".puffin")
        ...
        rowDelta.validateFromSnapshot(readSnapshot.snapshotId());
        ...
        replaceDeletionVectors(
                rowDelta,
                Optional.of(readSnapshot.snapshotId()),
    ...
    /**
     * Enumerates the Puffin deletion vectors in {@code snapshot} keyed by referenced data file.
     */
    private static Multiset<String> puffinDeletionVectorsByDataFile(Table table, Snapshot snapshot)
    {
