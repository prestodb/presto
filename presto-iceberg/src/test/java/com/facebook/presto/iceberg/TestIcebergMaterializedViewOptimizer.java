        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testJoinPredicatePushdown()
    {
        try {
            ...
            assertPlan("SELECT * FROM mv_join",
                    output(
                            exchange(
                                    filter(constrainedTableScan("__mv_storage__mv_join",
                                            ImmutableMap.of(
                                    ...
                                    project(join(
                                            ...
                                            anyTree(constrainedTableScan("join_customers",
                                                    ImmutableMap.of("reg_date", singleValue(VARCHAR, utf8Slice("2024-01-01"))),
                                                    ImmutableMap.of())))),
                                    project(join(
                                            anyTree(constrainedTableScan("join_orders",
                                                    ImmutableMap.of("order_date", singleValue(VARCHAR, utf8Slice("2024-01-02"))),
                                                    ImmutableMap.of())),
                                            anyTree(constrainedTableScan("join_customers",
                                                    ImmutableMap.of("reg_date", create(SortedRangeSet.copyOf(VARCHAR, ImmutableList.of(
                                                            lessThan(VARCHAR, utf8Slice("2024-01-01")),
                                                            greaterThan(VARCHAR, utf8Slice("2024-01-01")))), false)),
                                                    ImmutableMap.of())))))));
    ...
    public void testIntersectPredicatePushdown()
    {
        try {
            ...
            // For INTERSECT with stale data on both tables, we still need two union branches
            // (one for each table's stale partition) since each table has independent stale data.
            // Each branch has: FilterProject -> Aggregation(FINAL) -> LocalExchange ->
            //                    [Project -> ... -> Aggregation(PARTIAL) -> table1,
            //                     Project -> ... -> Aggregation(PARTIAL) -> table2]
            // The anyTree with multiple children matches exchange nodes with those children.
            PlanMatchPattern intersectBranchPattern = project(
                    filter(
                            aggregation(
                                    ImmutableMap.of(),
                                    FINAL,
            ...
            assertPlan(skipStorageSession, "SELECT * FROM mv_intersect",
                    output(exchange(intersectBranchPattern)));
        }
        finally {
    ...
    public void testDeeplyNestedJoins()
    {
        try {
            ...
            PlanMatchPattern innerJoin1 = join(
                    anyTree(tableScan("dnj_orders")),
                    anyTree(tableScan("dnj_customers")));
            PlanMatchPattern innerJoin2 = join(
                    anyTree(tableScan("dnj_products")),
                    anyTree(tableScan("dnj_categories")));
            PlanMatchPattern nestedJoinPattern = join(
                    anyTree(innerJoin1),
                    anyTree(innerJoin2));
            PlanMatchPattern stalePlanSingleTable = project(nestedJoinPattern);

            assertPlan("SELECT * FROM mv_dnj",
                    output(
                            exchange(
                                    tableScan("__mv_storage__mv_dnj"),
                                    stalePlanSingleTable)));
            ...
            assertPlan("SELECT * FROM mv_agg_join",
                    output(
                            exchange(
                                    ...
                                    project(aggregation(ImmutableMap.of(), anyTree(join(anyTree(tableScan("agg_orders")), anyTree(tableScan("agg_customers")))))),
                                    project(aggregation(ImmutableMap.of(), anyTree(join(anyTree(tableScan("agg_orders")), anyTree(tableScan("agg_customers")))))))));
    ...
    public void testExceptPredicatePushdown()
    {
        try {
            ...
            PlanMatchPattern staleBranchPattern = filter(
                    aggregation(
                            ImmutableMap.of(),
                            FINAL,
                            exchange(
    ...
    public void testSelectDistinctMVWithMultipleStaleTables()
    {
        try {
            ...
            assertPlan("SELECT * FROM test_distinct_mv ORDER BY category, region",
                    anyTree(
                            constrainedTableScan("__mv_storage__test_distinct_mv",
                                    ...
                                    ImmutableMap.of()),
                            anyTree(join(
                                    anyTree(tableScan("test_distinct_t1")),
                                    anyTree(constrainedTableScan("test_distinct_t2",
                                            ImmutableMap.of("reg_date", singleValue(DATE, jan2InDays)),
                                            ImmutableMap.of())))),
                            anyTree(join(
                                    anyTree(constrainedTableScan("test_distinct_t1",
                                            ImmutableMap.of("event_date", singleValue(DATE, jan2InDays)),
                                            ImmutableMap.of())),
                                    anyTree(constrainedTableScan("test_distinct_t2",
                                            ImmutableMap.of("reg_date", create(SortedRangeSet.copyOf(DATE, ImmutableList.of(
                                                    lessThan(DATE, jan2InDays),
                                                    greaterThan(DATE, jan2InDays))), false)),
                                            ImmutableMap.of()))))));
        }
    ...
    public void testJoinExceptJoinWithLeftSideStale()
    {
        try {
            ...
            // 2. A stale branch (FilterProject = project(filter(...))) that computes the EXCEPT:
            //    - Project -> Filter -> Aggregate(FINAL) -> LocalExchange[HASH] ->
            //      [Project -> RemoteExchange -> Aggregate(PARTIAL) -> Project -> Join(A,B)]
            //      [Project -> RemoteExchange -> Aggregate(PARTIAL) -> Project -> Join(Values,Values)]
            //    - Left side (A JOIN B) scans A and B with stale partition constraint
            //    - Join condition a.dt = b.dt propagates constraint to B
            //    - Right side (C JOIN D) optimized to Values since no data exists for 2024-01-02
            // Join structure for A JOIN B:
            //   InnerJoin -> RemoteExchange -> TableScan[A]
            //             -> LocalExchange -> RemoteExchange -> TableScan[B]
            ...
            // Join structure for C JOIN D (optimized to Values):
            //   InnerJoin -> LocalExchange -> Values
            //             -> LocalExchange -> Values
            // Use values(ImmutableMap.of()) to match Values with any number of columns
            PlanMatchPattern rightJoinBranch = join(
                    exchange(values(ImmutableMap.of())),
                    exchange(values(ImmutableMap.of())));

            // The EXCEPT branch: project(filter(aggregation(FINAL, exchange(...))))
    ...
    public void testJoinExceptJoinWithRightSideStale()
    {
        try {
            ...
            // The plan should have:
            // 1. MV storage scan with constraint excluding stale partition
            // 2. A stale branch where:
            //    - Left side (A JOIN B) is optimized to Values (no data for 2024-01-02)
            //    - Right side (C JOIN D) has C and D constrained to the stale partition
            // Left side (A JOIN B) optimized to Values since no matching data
            PlanMatchPattern leftJoinBranch = join(
                    exchange(values(ImmutableMap.of())),
                    exchange(values(ImmutableMap.of())));

            // Right side (C JOIN D) with stale constraint on C and D (via join condition)
            PlanMatchPattern rightJoinBranch = join(
                    exchange(constrainedTableScan("jexjr_c",
                            ImmutableMap.of("dt", singleValue(VARCHAR, utf8Slice("2024-01-02"))),
                            ImmutableMap.of())),
                    exchange(exchange(constrainedTableScan("jexjr_d",
                            ImmutableMap.of("dt", singleValue(VARCHAR, utf8Slice("2024-01-02"))),
                            ImmutableMap.of()))));
    ...
    public void testJoinExceptJoinWithBothSidesStale()
    {
        try {
            ...
            // With both sides stale (different partitions), we get two union branches.
            // Branch 1 (for A's stale partition 2024-01-02):
            //   - Left side: A JOIN B with actual TableScans constrained to 2024-01-02
            //   - Right side: C JOIN D optimized to Values (no data for 2024-01-02)
            // Branch 2 (for C's stale partition 2024-01-03):
            //   - Left side: A JOIN B optimized to Values (no data for 2024-01-03)
            //   - Right side: C JOIN D with actual TableScans constrained to 2024-01-03
            ...
            PlanMatchPattern joinCDForBranch1 = join(
                    exchange(values(ImmutableMap.of())),
                    exchange(values(ImmutableMap.of())));

            PlanMatchPattern exceptBranchPatternForA = filter(
                    aggregation(
                            ImmutableMap.of(),
                            FINAL,
                            exchange(
                                    ...
                                    project(exchange(
                                            aggregation(
                                                    ImmutableMap.of(),
                                                    PARTIAL,
                                                    project(joinCDForBranch1)))))));

            PlanMatchPattern joinABForBranch2 = join(
                    exchange(values(ImmutableMap.of())),
                    exchange(values(ImmutableMap.of())));

            PlanMatchPattern joinCDForBranch2 = join(
                    exchange(constrainedTableScan("jexjb_c",
                            ImmutableMap.of("dt", singleValue(VARCHAR, utf8Slice("2024-01-03"))),
                            ImmutableMap.of())),
                    exchange(exchange(constrainedTableScan("jexjb_d",
                            ImmutableMap.of("dt", singleValue(VARCHAR, utf8Slice("2024-01-03"))),
                            ImmutableMap.of()))));

            PlanMatchPattern exceptBranchPatternForC = filter(
                    aggregation(
                            ImmutableMap.of(),
                            FINAL,
                            exchange(
                                    project(exchange(
                                            aggregation(
                                                    ImmutableMap.of(),
                                                    PARTIAL,
                                                    project(joinABForBranch2)))),
                                    project(exchange(
                                            aggregation(
                                                    ImmutableMap.of(),
                                                    PARTIAL,
                                                    project(joinCDForBranch2)))))));
                            ...
                            exchange(
                                    constrainedTableScan("__mv_storage__mv_jexjb",
                                            ImmutableMap.of("dt", create(SortedRangeSet.copyOf(VARCHAR, ImmutableList.of(
                                                    ...
                                                    greaterThan(VARCHAR, utf8Slice("2024-01-03")))), false)),
                                            ImmutableMap.of("dt", "dt", "id", "id", "value", "value")),
                                    project(exceptBranchPatternForA),
                                    project(exceptBranchPatternForC))));
        }
        finally {
