        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testThreeTableJoinMV()
    {
        try {
            ...
            // Structure: LocalExchange[ROUND_ROBIN] with 2 children:
            //   1. InnerJoin with LocalExchange[ROUND_ROBIN] containing 2 delta branches for orders/customers
            //   2. Project -> InnerJoin (similar structure)
            PlanMatchPattern orderCustomerJoin = join(
                    exchange(tableScan("orders")),
                    exchange(exchange(tableScan("customers"))));

            // First branch: Join with nested union for orders/customers delta
            PlanMatchPattern topJoinWithNestedUnion = join(
                    exchange(
                            project(orderCustomerJoin),
                            project(orderCustomerJoin)),
                    exchange(tableScan("products")));

            // Second branch: simpler join structure
            PlanMatchPattern secondBranch = join(
                    exchange(join(
                            exchange(tableScan("orders")),
                            exchange(exchange(tableScan("customers"))))),
                    exchange(tableScan("products")));
            ...
            assertPlan("SELECT * FROM mv_order_details",
                    output(
                            exchange(
                                    ...
                                    exchange(
                                            topJoinWithNestedUnion,
                                            project(secondBranch)))));
    ...
    public void testIntersectWithOnlyLeftSideStale()
    {
        try {
            ...
            // - FilterProject (combined filter+project node)
            //     - Aggregate(FINAL)
                                            ...
                                            project(
                                                    exchange(
                                                            aggregation(
                                                                    ImmutableMap.of(),
                                                                    PARTIAL,
                                                                    tableScan("intersect_left_only1")))),
                                            project(
                                                    exchange(
                                                            aggregation(
                                                                    ImmutableMap.of(),
                                                                    PARTIAL,
                                                                    tableScan("intersect_left_only2"))))))));
    ...
    public void testIntersectWithOnlyRightSideStale()
    {
        try {
            ...
            PlanMatchPattern intersectBranch = project(
                    filter(
                            aggregation(
                                    ImmutableMap.of(),
                                    FINAL,
                                    exchange(
                                            anyTree(
                                                    aggregation(
                                                            ImmutableMap.of(),
                                                            PARTIAL,
                                                            anyTree(constrainedTableScan("intersect_right_only1",
                                                                    ImmutableMap.of("dt", singleValue(VARCHAR, utf8Slice("2024-01-03"))),
                                                                    ImmutableMap.of())))),
                                            anyTree(
                                                    aggregation(
                                                            ImmutableMap.of(),
                                                            PARTIAL,
                                                            anyTree(constrainedTableScan("intersect_right_only2",
                                                                    ImmutableMap.of("dt", singleValue(VARCHAR, utf8Slice("2024-01-03"))),
                                                                    ImmutableMap.of()))))))));
