...
import static com.facebook.presto.iceberg.IcebergUtil.getStartSnapshot;
        ...
        implements ConnectorMetadata
{
            ...
            Optional<Set<ColumnHandle>> desiredColumns)
    {
        ...
        Snapshot startSnapshot = getStartSnapshot(icebergTable);
        ...
        for (Map.Entry<ColumnHandle, Domain> entry : domains.entrySet()) {
            ...
            if (domain.isSingleValue()) {
                ...
                handle = handle.withUpdatedIcebergTableName(prepareIncrementalScan(icebergTable, startSnapshot, name, endSnapshot));
                continue;
            }
            ...
            if (range.isSingleValue()) {
                ...
                handle = handle.withUpdatedIcebergTableName(prepareIncrementalScan(icebergTable, start, name, latestSnapshot));
            }
            else if (!range.isLowUnbounded() && !range.isLowInclusive() && range.isHighUnbounded()) {
                // WHERE $snapshot_sequence_number > X
                long lowerSequenceNumber = ((Number) range.getLowBoundedValue()).longValue() + 1; // > excludes lower bound
                Snapshot start = validateAndFetchEndSnapshot(icebergTable, lowerSequenceNumber);
                handle = handle.withUpdatedIcebergTableName(
                        prepareIncrementalScan(icebergTable, start, name, latestSnapshot));
            }
