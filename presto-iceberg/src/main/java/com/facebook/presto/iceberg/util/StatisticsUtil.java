...
public final class StatisticsUtil
{
     ...
     */
    public static TableStatistics calculateBaseTableStatistics(ConnectorMetadata metadata, TypeManager typeManager, ConnectorSession session, IcebergTableHandle tableHandle, Optional<ConnectorTableLayoutHandle> tableLayoutHandle, List<ColumnHandle> columnHandles, Constraint<ColumnHandle> constraint)
    {
    ...
    public static TableStatistics calculateStatisticsConsideringLayout(FilterStatsCalculatorService filterStatsCalculatorService, RowExpressionService rowExpressionService, TableStatistics baseStatistics, ConnectorSession session, Optional<ConnectorTableLayoutHandle> tableLayoutHandle)
    {
        return tableLayoutHandle.map(IcebergTableLayoutHandle.class::cast).map(layoutHandle -> {
            ...
            RowExpression combinedPredicate = binaryExpression(SpecialFormExpression.Form.AND, ImmutableList.of(layoutHandle.getRemainingPredicate(), translatedPredicate));
            TableStatistics.Builder filteredStatsBuilder = TableStatistics.builder()
                    .setRowCount(baseStatistics.getRowCount());
            Set<ColumnHandle> fullColumnSet = baseStatistics.getColumnStatistics().keySet();
            for (ColumnHandle colHandle : fullColumnSet) {
                IcebergColumnHandle icebergHandle = (IcebergColumnHandle) colHandle;
                if (baseStatistics.getColumnStatistics().containsKey(icebergHandle)) {
                    ColumnStatistics stats = baseStatistics.getColumnStatistics().get(icebergHandle);
