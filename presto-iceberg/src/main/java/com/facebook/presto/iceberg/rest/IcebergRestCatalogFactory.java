        ...
        extends IcebergNativeCatalogFactory
{
    ...
    {
        // The catalog fixes the identity used for every metadata read through its FileIO, so the
        // identity is always part of the key, whatever the session type: extra cache entries are
        // cheap, a leaked credential is not. Extra credentials are included because
        // GcsConfigurationProvider takes its token from them. Deployments with many concurrent
        // users may need a larger iceberg.catalog.cached-catalog-num.
        ConnectorIdentity identity = session.getIdentity();
        return Optional.of(identity.getUser() + "|" + extraCredentialsKey(identity));
    }
