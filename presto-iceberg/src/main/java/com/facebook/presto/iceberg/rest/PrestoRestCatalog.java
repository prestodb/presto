 ...
 * {@link org.apache.iceberg.rest.RESTCatalog} does not expose: every constructor passes
 * {@code ioBuilder = null}, so {@code RESTSessionCatalog.tableFileIO} reuses the one catalog-level
 * FileIO for every table whenever the server returns no per-table config or credentials. A sibling
 * rather than a subclass because {@code RESTCatalog}'s {@code newSessionCatalog} hook runs from
 * its constructor, before a subclass field holding the {@code ioBuilder} would be assigned.
 * <p>
        ...
        implements Catalog, ViewCatalog, SupportsNamespaces, Configurable<Object>, Closeable
{
    ...
    @Override
    public Table registerTable(TableIdentifier identifier, String metadataFileLocation, boolean overwrite)
    {
    ...
    {
        return viewDelegate.registerView(identifier, metadataFileLocation);
    }
