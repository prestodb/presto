...
public class PreCheckClusterSchedulerFactory
        implements SchedulerFactory
{
    ...
    {
        try {
            Bootstrap app = new Bootstrap(new PreCheckSidecarPluginModule());

            Injector injector = app
                    .doNotInitializeLogging()
                    .setRequiredConfigurationProperties(config)
                    .initialize();
            return injector.getInstance(PreCheckClusterScheduler.class);
        }
        catch (Exception e) {
            throwIfUnchecked(e);
