        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testZeroFileCreatorForBucketedTable()
    {
        assertUpdate(
                getSession(),
                format(
                        "CREATE TABLE hive.hive_test.test_hive_orders_bucketed_join_zero_file WITH "
                                + "(bucketed_by=array['orderkey'], bucket_count=8) AS "
                                +
    ...
    private void testBucketedTableWriteSimple(Session session, int inputBucketCount,
            int outputBucketCount)
    {
        assertUpdate(
                session,
                format(
                        "CREATE TABLE hive.hive_test.test_hive_orders_bucketed_simple_input WITH "
                                + "(bucketed_by=array['orderkey'], bucket_count=%s) AS "
                                +
                                "SELECT orderkey, custkey, orderstatus, totalprice, orderdate, orderpriority, "
                                + "clerk, shippriority, comment "
                                +
        ...
        assertUpdate(
                session,
                format(
                        "CREATE TABLE hive.hive_test.test_hive_orders_bucketed_simple_output WITH "
                                + "(bucketed_by=array['orderkey'], bucket_count=%s) AS "
                                +
                                "SELECT orderkey, custkey, orderstatus, totalprice, orderdate, orderpriority, "
                                + "clerk, shippriority, comment "
            ...
            int outputBucketCount)
    {
        assertUpdate(
                session,
                format(
                        "CREATE TABLE hive.hive_test.test_hive_orders_bucketed_aggregation_input WITH "
                                + "(bucketed_by=array['orderkey'], bucket_count=%s) AS "
                                +
                                "SELECT orderkey, custkey, orderstatus, totalprice, orderdate, orderpriority, "
                                + "clerk, shippriority, comment "
        ...
        assertUpdate(
                session,
                format(
                        "CREATE TABLE hive.hive_test.test_hive_orders_bucketed_aggregation_output WITH "
                                + "(bucketed_by=array['orderkey'], bucket_count=%s) AS "
                                +
    ...
    private void testBucketedTableWriteJoin(Session session, int firstInputBucketCount,
            int secondInputBucketCount, int outputBucketCount)
    {
        assertUpdate(
                session,
                format(
                        "CREATE TABLE hive.hive_test.test_hive_orders_bucketed_join_input_1 WITH "
                                + "(bucketed_by=array['orderkey'], bucket_count=%s) AS "
                                +
                                "SELECT orderkey, custkey, orderstatus, totalprice, orderdate, orderpriority, "
                                + "clerk, shippriority, comment "
        ...
        assertUpdate(
                session,
                format(
                        "CREATE TABLE hive.hive_test.test_hive_orders_bucketed_join_input_2 WITH "
                                + "(bucketed_by=array['orderkey'], bucket_count=%s) AS "
                                +
                                "SELECT orderkey, custkey, orderstatus, totalprice, orderdate, orderpriority, "
                                + "clerk, shippriority, comment "
        ...
        assertUpdate(
                session,
                format(
                        "CREATE TABLE hive.hive_test.test_hive_orders_bucketed_join_output WITH "
                                + "(bucketed_by=array['orderkey'], bucket_count=%s) AS "
                                +
    ...
    {
        ((PrestoSparkQueryRunner) getQueryRunner()).getMetastore()
                .dropTable(METASTORE_CONTEXT, schema, table, true);
    }
    ...
    {
        assertQuery(
                "select partkey, count(*) c from lineitem where partkey % 10 = 1 group by partkey having "
                        + "count(*) = 42");
    }
    ...
    {
        assertBucketedQuery(
                "SELECT orderkey, count(*) c FROM lineitem_bucketed WHERE partkey % 10 = 1 GROUP BY "
                        + "orderkey");
    }
    ...
    private List<Session> getTestCompatibleBucketCountSessions()
    {
        return ImmutableList.of(
                Session.builder(getSession())
                        .setSystemProperty(PARTIAL_MERGE_PUSHDOWN_STRATEGY,
                                PUSH_THROUGH_LOW_MEMORY_OPERATORS.name())
                        .build(),
                Session.builder(getSession())
    ...
    public void testJoinUnderUnionALL()
    {
        ...
        assertQuery("SELECT hive.hive_test.partitioned_nation_10.nationkey " +
                        ...
                        "  ON hive.hive_test.partitioned_nation_10.nationkey = hive.hive_test"
                        + ".partitioned_nation_20.nationkey "
                        +
                        "UNION ALL " +
                        ...
                        "  ON hive.hive_test.partitioned_nation_10.nationkey = hive.hive_test"
                        + ".partitioned_nation_30.nationkey ",
                "SELECT m.nationkey " +
                        "FROM nation m " +
                        "JOIN nation n " +
    ...
    {
        assertQuery(
                "SELECT orderkey, 1 FROM orders UNION ALL SELECT orderkey, count(*) FROM orders GROUP BY 1",
                "SELECT orderkey, 1 FROM orders UNION ALL SELECT orderkey, count(*) FROM orders GROUP BY "
                        + "orderkey");
    ...
    {
        assertQuery(
                "SELECT l.orderkey, l.linenumber, p1.brand, p2.brand, p3.brand, p4.brand, p5.brand, p6"
                        + ".brand "
                        +
    ...
    public void testBucketedNWayJoin()
    {
        ...
        assertBucketedQuery(
                "SELECT l.orderkey, l.linenumber, o1.orderstatus, o2.orderstatus, o3.orderstatus, o4"
                        + ".orderstatus, o5.orderstatus, o6.orderstatus "
                        +
                        "FROM lineitem_bucketed l, orders_bucketed o1, orders_bucketed o2, orders_bucketed "
                        + "o3, orders_bucketed o4, orders_bucketed o5, orders_bucketed o6 "
                        +
        ...
        assertBucketedQuery(
                "SELECT l.orderkey, l.linenumber, o1.orderstatus, o2.orderstatus, o3.orderstatus, o4"
                        + ".orderstatus, o5.orderstatus, o6.orderstatus "
                        +
                        "FROM lineitem_bucketed l, orders o1, orders_bucketed o2, orders o3, orders_bucketed "
                        + "o4, orders o5, orders_bucketed o6 "
                        +
                        "WHERE l.orderkey = o1.orderkey " +
                        "AND l.orderkey = o2.orderkey " +
                        "AND l.orderkey = o3.orderkey " +
        ...
        assertBucketedQuery(
                "SELECT l.orderkey, l.linenumber, o1.orderstatus, o2.orderstatus, o3.orderstatus, o4"
                        + ".orderstatus, o5.orderstatus, o6.orderstatus "
                        +
                        "FROM lineitem l, orders o1, orders_bucketed o2, orders o3, orders_bucketed o4, "
                        + "orders o5, orders_bucketed o6 "
                        +
                        "WHERE l.orderkey = o1.orderkey " +
                        "AND l.orderkey = o2.orderkey " +
                        "AND l.orderkey = o3.orderkey " +
    ...
    public void testUnionAll()
    {
        ...
        assertBucketedQuery(
                "SELECT * FROM lineitem_bucketed UNION ALL SELECT * FROM lineitem_bucketed");
    ...
    {
        assertQueryFails("SELECT * FROM orders WHERE custkey / (orderkey - orderkey) = 0",
                "/ by zero");
        assertQueryFails(
    ...
    {
        assertUpdate(
                "CREATE TABLE hive.hive_test.empty_orders_bucketed WITH (bucketed_by=array['orderkey'], "
                        + "bucket_count=11) AS "
                        +
                        "SELECT orderkey, custkey, orderstatus, totalprice, orderdate, orderpriority, clerk, "
                        + "shippriority, comment "
        ...
        assertQuery(
                "SELECT count(*) FROM (SELECT orderkey, count(*) FROM hive.hive_test"
                        + ".empty_orders_bucketed GROUP BY orderkey)",
                "SELECT 0");
    }
    ...
    public void testTableSampleSystem()
    {
        ...
        long sampledRows = (Long) computeActual(
                "SELECT count(*) FROM orders TABLESAMPLE SYSTEM (1)").getOnlyValue();
    ...
    public void testTimeouts()
    {
        ...
        String longRunningCrossJoin = "SELECT count(l1.orderkey), count(l2.orderkey) FROM lineitem "
                + "l1, lineitem l2";

        Session queryMaxRunTimeLimitSession = Session.builder(getSession())
                .setSystemProperty("query_max_run_time", "2s")
                .build();
        assertQueryFails(queryMaxRunTimeLimitSession, longRunningCrossJoin,
                "Query exceeded maximum time limit of 2.00s");
        ...
        Session queryMaxExecutionTimeLimitSession = Session.builder(getSession())
                ...
                .build();
        assertQueryFails(queryMaxExecutionTimeLimitSession, longRunningCrossJoin,
                "Query exceeded maximum time limit of 2.00s");
        ...
        Set<PrestoSparkServiceWaitTimeMetrics> waitTimeMetrics =
                ((PrestoSparkQueryRunner) getQueryRunner()).getWaitTimeMetrics();
        PrestoSparkTestingServiceWaitTimeMetrics testingServiceWaitTimeMetrics =
                (PrestoSparkTestingServiceWaitTimeMetrics) waitTimeMetrics.stream()
    ...
    public void testSmileSerialization()
    {
        ...
        try (QueryRunner queryRunner = createHivePrestoSparkQueryRunner(ImmutableList.of(NATION),
                ImmutableMap.of("spark.smile-serialization-enabled", "true"))) {
            MaterializedResult actual = queryRunner.execute(query);
            assertEqualsIgnoreOrder(actual, computeExpected(query, actual.getTypes()));
        }
        try (QueryRunner queryRunner = createHivePrestoSparkQueryRunner(ImmutableList.of(NATION),
                ImmutableMap.of("spark.smile-serialization-enabled", "false"))) {
            MaterializedResult actual = queryRunner.execute(query);
            assertEqualsIgnoreOrder(actual, computeExpected(query, actual.getTypes()));
        }
    }
    ...
    public void testIterativeSplitEnumeration()
    {
        for (int batchSize = 1; batchSize <= 8; batchSize *= 2) {
            ...
            assertQuery(session,
                    "select partkey, count(*) c from lineitem where partkey % 10 = 1 group by partkey "
                            + "having count(*) = 42");
    ...
    public void testDropTable()
    {
        ...
        assertQueryFails("select count(*) from hive.hive_test.hive_orders1",
                ".* Table hive.hive_test.hive_orders1 does not exist");
    }
    ...
    public void testCreateDropSchema()
    {
        ...
        assertQueryFails("DROP SCHEMA hive.hive_test_new",
                ".* Schema 'hive.hive_test_new' does not exist");
    ...
    public void testCreateAlterTable()
    {
        ...
        MaterializedResult actual = computeActual(
                "SHOW CREATE TABLE hive.hive_test.hive_orders_new");
        assertEquals(createTableSql, actual.getOnlyValue());
        assertQuerySucceeds(
                "ALTER TABLE hive.hive_test.hive_orders_new RENAME TO hive.hive_test.hive_orders_new1");
        assertQueryFails("DROP TABLE hive.hive_test.hive_orders_new",
                ".* Table 'hive.hive_test.hive_orders_new' does not exist");
    ...
    public void testGrants()
    {
        ...
        List<String> grants = actual.getMaterializedRows().stream()
                .map(row -> row.getField(7).toString()).collect(Collectors.toList());
        assertEquals(Ordering.natural().sortedCopy(grants),
                ImmutableList.of("DELETE", "INSERT", "SELECT", "UPDATE"));
        ...
        grants = actual.getMaterializedRows().stream().map(row -> row.getField(7).toString())
    ...
    public void testRoles()
    {
        ...
        List<String> roles = actual.getMaterializedRows().stream()
                .map(row -> row.getField(0).toString()).collect(Collectors.toList());
        ...
        roles = actual.getMaterializedRows().stream().map(row -> row.getField(0).toString())
                .collect(Collectors.toList());
    ...
    public void testAddColumns()
    {
        ...
        assertQuerySucceeds(
                "ALTER TABLE test_add_column ADD COLUMN b bigint COMMENT 'test comment BBB'");
        assertQueryFails("ALTER TABLE test_add_column ADD COLUMN a varchar",
                ".* Column 'a' already exists");
        assertQueryFails("ALTER TABLE test_add_column ADD COLUMN c bad_type",
                ".* Unknown type 'bad_type' for column 'c'");
        assertQuery("SHOW COLUMNS FROM test_add_column",
                "VALUES ('a', 'bigint', '', 'test comment AAA'), ('b', 'bigint', '', 'test comment BBB')");
    ...
    public void testRenameColumn()
    {
        ...
        assertQuerySucceeds(
                "ALTER TABLE test_rename_column RENAME COLUMN orderkey TO new_orderkey");
        assertQuery("SELECT new_orderkey, orderstatus FROM test_rename_column",
                "SELECT orderkey, orderstatus FROM orders");
        assertQueryFails("ALTER TABLE test_rename_column RENAME COLUMN \"$path\" TO test",
                ".* Cannot rename hidden column");
        assertQueryFails(
    ...
    public void testDropColumn()
    {
        ...
        assertUpdate(createTable, "SELECT count(*) FROM orders");
        assertQuery("SELECT orderkey, orderstatus FROM test_drop_column",
                "SELECT orderkey, orderstatus FROM orders");

        assertQueryFails("ALTER TABLE test_drop_column DROP COLUMN \"$path\"",
                ".* Cannot drop hidden column");
        assertQueryFails("ALTER TABLE test_drop_column DROP COLUMN orderstatus",
                "Cannot drop partition columns");
        assertQuerySucceeds("ALTER TABLE test_drop_column DROP COLUMN orderkey");
        assertQueryFails("ALTER TABLE test_drop_column DROP COLUMN custkey",
    ...
    {
        assertQuerySucceeds(
                "CREATE FUNCTION unittest.memory.tan (x int) RETURNS double COMMENT 'tangent "
                        + "trigonometric function' LANGUAGE SQL DETERMINISTIC CALLED ON NULL INPUT RETURN sin"
                        + "(x) / cos(x)");
        ...
        //PoS currently supports one query per session. So temporary function created is not
        // discoverable by another query.
    ...
    public void testCreateType()
    {
        ...
        assertQuerySucceeds(
                "CREATE TYPE unittest.memory.pair3 AS (fst unittest.memory.pair, snd integer)");
        assertQuery(
                "SELECT p.fst.fst FROM(SELECT CAST(ROW(CAST(ROW(1,2) AS unittest.memory.pair), 3) AS "
                        + "unittest.memory.pair3) AS p)",
                "SELECT 1");
        assertQuerySucceeds(
                "CREATE TYPE unittest.memory.pair3Alt AS (fst ROW(fst integer, snd integer), snd integer)");
        assertQuery(
                "SELECT p.fst.snd FROM(SELECT CAST(ROW(ROW(1,2), 3) AS  unittest.memory.pair3Alt) AS p)",
                "SELECT 2");
    }
    ...
    public void testCreatePartitionedTable()
    {
        ...
        MaterializedResult actual = computeActual(
                "SELECT count(*) FROM \"test_partition_table$partitions\"");
        ...
        assertQuerySucceeds(format(
                "CALL system.create_empty_partition('%s', '%s', ARRAY['orderstatus'], ARRAY['%s'])",
                "tpch", "test_partition_table", "x"));
        assertQuerySucceeds(format(
                "CALL system.create_empty_partition('%s', '%s', ARRAY['orderstatus'], ARRAY['%s'])",
                "tpch", "test_partition_table", "y"));
