...
createOrReplaceMaterializedView returns [Statement value]
    : ^(CREATE_OR_REPLACE_MATERIALIZED_VIEW qname createMaterializedViewSource) { $value = new CreateOrReplaceMaterializedView($qname.value, $createMaterializedViewSource.value); }
    ;

createMaterializedViewSource returns [Query value]
    : ^(QUERY s=restrictedSelectStmt) { $value = $s.value; }
    ;
