...
public final class GraphvizPrinter
{
            ...
            extends PlanVisitor<Void, Void>
    {
        ...
        {
            printNode(node, "Window", format("partition by = %s|order by = %s", Joiner.on(", ").join(node.getSpecification().getPartitionBy()), Joiner.on(", ").join(node.getSpecification().getOrderBy())), NODE_COLORS.get(NodeType.WINDOW));
            return node.getSource().accept(this, context);
        }
