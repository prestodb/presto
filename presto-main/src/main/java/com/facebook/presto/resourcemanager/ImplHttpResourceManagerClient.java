        ...
        implements HttpResourceManagerClient
{
    ...
    {
        URI uri = buildUri("/v1/resourcemanager/queryHeartbeat", target,
    ...
    {
        URI uri = buildUri(
                "/v1/resourcemanager/resourceGroupInfo",
                target,
                "excludingNode",
                excludingNode);
        ...
        try {
            ...
            log.info(
                    "ResourceGroupInfo SUCCESS uri=%s excludingNode=%s groups=%d",
                    uri,
                    excludingNode,
                    result.size());
            log.info("ResourceGroupInfo payload=%s", result);
    ...
    {
        URI uri = buildUri("/v1/resourcemanager/nodeHeartbeat", target);
    ...
    {
        URI uri = buildUri("/v1/resourcemanager/memoryPoolInfo", target);
    ...
    {
        URI uri = buildUri("/v1/resourcemanager/resourceGroupRuntimeHeartbeat", target, "node", node);
    ...
    {
        URI uri = buildUri("/v1/resourcemanager/getRunningTaskCount", target);
            ...
            implements ResponseHandler<Void, RuntimeException>
    {
        ...
        public Void handle(Request request, Response response)
        {
            if (response.getStatusCode() >= 400) {
                ...
                throw new ResourceManagerException(errorMessage);
            }
            return null;
        }
    }
