        ...
        implements ClusterStatusSender
{
            ...
            ResourceGroupManager<?> resourceGroupManager)
    {
        ...
        this.thriftResourceManagerClient = (communicationProtocol == InternalCommunicationConfig.CommunicationProtocol.THRIFT) ? requireNonNull(thriftResourceManagerClient, "thriftResourceManagerClient is null") : thriftResourceManagerClient;
    ...
    private void sendQueryHeartbeat(ManagedQueryExecution queryExecution, long sequenceId)
    {
        ...
        if (communicationProtocol == InternalCommunicationConfig.CommunicationProtocol.HTTP) {
            getResourceManagers().forEach(hostAndPort -> {
                try {
                    httpClient.setAddressSelectorContext(hostAndPort.toString()).queryHeartbeat(nodeIdentifier, basicQueryInfo, sequenceId);
                }
                catch (Exception e) {
    ...
    private void sendNodeHeartbeat()
    {
        ...
        if (communicationProtocol == InternalCommunicationConfig.CommunicationProtocol.HTTP) {
            getResourceManagers().forEach(hostAndPort -> {
                try {
                    httpClient.setAddressSelectorContext(hostAndPort.toString()).nodeHeartbeat(nodeStatus);
                }
                catch (Exception e) {
    ...
    public void sendResourceGroupRuntimeHeartbeat()
    {
        ...
        if (communicationProtocol == InternalCommunicationConfig.CommunicationProtocol.HTTP) {
            getResourceManagers().forEach(hostAndPort -> {
                try {
                    httpClient.setAddressSelectorContext(hostAndPort.toString()).resourceGroupRuntimeHeartbeat(internalNodeManager.getCurrentNode().getNodeIdentifier(), resourceGroupRuntimeInfos);
                }
                catch (Exception e) {
                    log.error(e, "Failed to send node heartbeat to resource manager at %s", hostAndPort);
                }
