        ...
        implements ConnectorSplitManager
{
    ...
    @Inject
    public JdbcSplitManager(NodeManager nodeManager)
    {
    ...
    {
        return handle instanceof JdbcTableHandle;
    }
    ...
    public PartitionResult getPartitions(TableHandle table, TupleDomain tupleDomain)
    {
        ...
        checkArgument(table instanceof JdbcTableHandle, "TableHandle must be an SystemTableHandle");
        JdbcTableHandle systemTableHandle = (JdbcTableHandle) table;
        ...
        ImmutableList<Partition> partitions = ImmutableList.<Partition>of(new JdbcPartition(systemTableHandle, bindings));
    ...
    public SplitSource getPartitionSplits(TableHandle table, List<Partition> partitions)
    {
        ...
        checkArgument(partition instanceof JdbcPartition, "Partition must be a jdbc partition");
        JdbcPartition systemPartition = (JdbcPartition) partition;
        ...
        for (Entry<ColumnHandle, Comparable<?>> entry : systemPartition.getFilters().entrySet()) {
            JdbcColumnHandle systemColumnHandle = (JdbcColumnHandle) entry.getKey();
        ...
        if (systemTable.isDistributed()) {
            ...
            for (Node node : nodeManager.getActiveNodes()) {
                splits.add(new JdbcSplit(systemPartition.tableHandle, filters.build(), node.getHostAndPort()));
            }
            return new FixedSplitSource(null, splits.build());
        }

        HostAddress address = nodeManager.getCurrentNode().getHostAndPort();
        Split split = new JdbcSplit(systemPartition.tableHandle, filters.build(), address);
        return new FixedSplitSource(null, ImmutableList.of(split));
    }

    public static class JdbcPartition
            implements Partition
    {
        private final JdbcTableHandle tableHandle;
        private final Map<ColumnHandle, Comparable<?>> filters;

        public JdbcPartition(JdbcTableHandle tableHandle, Map<ColumnHandle, Comparable<?>> filters)
        {
