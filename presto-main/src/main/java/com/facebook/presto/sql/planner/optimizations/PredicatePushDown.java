        ...
        implements PlanOptimizer
{
    ...
    public PredicatePushDown(Metadata metadata, SqlParser sqlParser)
    {
        ...
        this.effectivePredicateExtractor = new EffectivePredicateExtractor(new RowExpressionDomainTranslator(metadata), metadata.getTypeAndFunctionManager(), metadata.getTypeManager());
    ...
    public static RowExpression createDynamicFilterExpression(String id, VariableReferenceExpression input, TypeAndFunctionManager typeAndFunctionManager)
    {
        return call(
            ...
            extends SimplePlanRewriter<RowExpression>
    {
                ...
                Session session)
        {
            ...
            this.logicalRowExpressions = new LogicalRowExpressions(determinismEvaluator, new FunctionResolution(metadata.getTypeAndFunctionManager()), metadata.getTypeAndFunctionManager());
            this.typeManager = metadata.getTypeManager();
            this.typeAndFunctionManager = metadata.getTypeAndFunctionManager();
            this.externalCallExpressionChecker = new ExternalCallExpressionChecker(typeAndFunctionManager);
        }
        ...
        private boolean isInliningCandidate(RowExpression expression, ProjectNode node)
        {
            ...
            return dependencies.entrySet().stream()
                    .allMatch(entry -> (entry.getValue() == 1 && !node.getAssignments().get(entry.getKey()).accept(new ExternalCallExpressionChecker(typeAndFunctionManager), null)) ||
        ...
        public PlanNode visitJoin(JoinNode node, RewriteContext<RowExpression> context)
        {
            ...
            if (newJoinPredicate.equals(FALSE_CONSTANT)) {
                newJoinPredicate = buildEqualsExpression(typeAndFunctionManager, constant(0L, BIGINT), constant(1L, BIGINT));
            }
            ...
            if (dynamicFilterEnabled) {
                DynamicFiltersResult dynamicFiltersResult = createDynamicFilters(node, equiJoinClauses, idAllocator, metadata.getTypeAndFunctionManager());
                ...
                TypeAndFunctionManager typeAndFunctionManager)
        {
            ...
            if (node.getType() == INNER) {
                ...
                for (JoinNode.EquiJoinClause clause : equiJoinClauses) {
                    ...
                    predicatesBuilder.add(createDynamicFilterExpression(id, probeSymbol, typeAndFunctionManager));
        ...
        private static RowExpression createDynamicFilterExpression(String id, VariableReferenceExpression input, TypeAndFunctionManager typeAndFunctionManager)
        {
            return call(
                    typeAndFunctionManager,
                    DynamicFilters.DynamicFilterPlaceholderFunction.NAME,
                ...
                TypeAndFunctionManager typeAndFunctionManager)
        {
            ...
            ImmutableList.Builder<RowExpression> predicatesBuilder = ImmutableList.builder();
            String id = idAllocator.getNextId().toString();
            predicatesBuilder.add(createDynamicFilterExpression(id, probeVariable, typeAndFunctionManager));
        ...
        private InnerJoinPushDownResult processInnerJoin(RowExpression inheritedPredicate, RowExpression leftEffectivePredicate, RowExpression rightEffectivePredicate, RowExpression joinPredicate, Collection<VariableReferenceExpression> leftVariables)
        {
            ...
            // Generate equality inferences
            EqualityInference allInference = new EqualityInference.Builder(typeAndFunctionManager, typeManager)
                    .addEqualityInference(inheritedPredicate, leftEffectivePredicate, rightEffectivePredicate, joinPredicate)
                    .build();
            EqualityInference allInferenceWithoutLeftInferred = new EqualityInference.Builder(typeAndFunctionManager, typeManager)
                    .addEqualityInference(inheritedPredicate, rightEffectivePredicate, joinPredicate)
                    .build();
            EqualityInference allInferenceWithoutRightInferred = new EqualityInference.Builder(typeAndFunctionManager, typeManager)
            ...
            for (RowExpression conjunct : new EqualityInference.Builder(typeAndFunctionManager, typeManager).nonInferrableConjuncts(inheritedPredicate)) {
            ...
            for (RowExpression conjunct : new EqualityInference.Builder(typeAndFunctionManager, typeManager).nonInferrableConjuncts(rightEffectivePredicate)) {
            ...
            for (RowExpression conjunct : new EqualityInference.Builder(typeAndFunctionManager, typeManager).nonInferrableConjuncts(leftEffectivePredicate)) {
            ...
            for (RowExpression conjunct : new EqualityInference.Builder(typeAndFunctionManager, typeManager).nonInferrableConjuncts(joinPredicate)) {
        ...
        {
            return buildEqualsExpression(typeAndFunctionManager, equiJoinClause.getLeft(), equiJoinClause.getRight());
        }
        ...
        private boolean isOperation(RowExpression expression, OperatorType type)
        {
            if (expression instanceof CallExpression) {
                Optional<OperatorType> operatorType = typeAndFunctionManager.getFunctionMetadata(((CallExpression) expression).getFunctionHandle()).getOperatorType();
        ...
        private PlanNode visitNonFilteringSemiJoin(SemiJoinNode node, RewriteContext<RowExpression> context)
        {
            ...
            EqualityInference inheritedInference = new EqualityInference.Builder(typeAndFunctionManager, typeManager)
        ...
        private PlanNode visitFilteringSemiJoin(SemiJoinNode node, RewriteContext<RowExpression> context)
        {
            ...
            RowExpression joinExpression = buildEqualsExpression(typeAndFunctionManager, node.getSourceJoinVariable(), node.getFilteringSourceJoinVariable());
            ...
            if (isEnableDynamicFiltering(session)) {
                DynamicFiltersResult dynamicFiltersResult = createDynamicFilters(node.getSourceJoinVariable(), node.getFilteringSourceJoinVariable(), idAllocator, metadata.getTypeAndFunctionManager());
                dynamicFilters = dynamicFiltersResult.getDynamicFilters();
        ...
        {
            return new EqualityInference.Builder(typeAndFunctionManager, typeManager)
