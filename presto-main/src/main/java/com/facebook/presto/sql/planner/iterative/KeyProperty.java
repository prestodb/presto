...
public class KeyProperty
{
     ...
     */
    public static boolean moreGeneral(KeyProperty keyProperty, KeyProperty otherKeyProperty)
    {
        requireNonNull(keyProperty, "Key property must be provided.");
        requireNonNull(otherKeyProperty, "Other key property must be provided.");
        return ((keyProperty.keys.isEmpty() && otherKeyProperty.keys.isEmpty()) ||
                (otherKeyProperty.keys.stream().allMatch(k -> keyProperty.satisfiesKeyRequirement(k))));
    }
    ...
    {
        requireNonNull(keys, "Keys must be provided.");
    ...
    {
        requireNonNull(newKey, "Key must be provided.");
    ...
    {
        requireNonNull(toConcatKeyProp, "Key property must not be provided.");
        KeyProperty result = new KeyProperty();
    ...
    {
        return "{" + String.join(",", keys.stream().map(Key::toString).collect(Collectors.toList())) + "}";
    }
}
