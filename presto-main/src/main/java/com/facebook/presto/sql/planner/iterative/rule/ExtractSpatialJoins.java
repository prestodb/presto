...
public class ExtractSpatialJoins
{
            ...
            implements Rule<FilterNode>
    {
        ...
        public Result apply(FilterNode node, Captures captures, Context context)
        {
            ...
            List<CallExpression> spatialFunctions = extractSupportedSpatialFunctions(filter, typeAndFunctionManager);
            ...
            List<CallExpression> spatialComparisons = extractSupportedSpatialComparisons(filter, typeAndFunctionManager);
    ...
    {
        FunctionMetadata spatialComparisonMetadata = metadata.getTypeAndFunctionManager().getFunctionMetadata(spatialComparison.getFunctionHandle());
        ...
        if (spatialComparisonMetadata.getOperatorType().get() == OperatorType.LESS_THAN || spatialComparisonMetadata.getOperatorType().get() == OperatorType.LESS_THAN_OR_EQUAL) {
            ...
            if (radiusVariables.isEmpty() || (rightVariables.containsAll(radiusVariables) && containsNone(leftVariables, radiusVariables))) {
                ...
                FunctionHandle flippedHandle = getFlippedFunctionHandle(spatialComparison, metadata.getTypeAndFunctionManager());
                newComparison = new CallExpression(
            ...
            PageSourceManager pageSourceManager)
    {
        ...
        if (kdbTree.isPresent()) {
            ...
            if (alignment > 0) {
                newLeftNode = addPartitioningNodes(context, typeAndFunctionManager, newLeftNode, leftPartitionVariable.get(), kdbTree.get(), newFirstArgument, Optional.empty());
                newRightNode = addPartitioningNodes(context, typeAndFunctionManager, newRightNode, rightPartitionVariable.get(), kdbTree.get(), newSecondArgument, radius);
            }
            else {
                newLeftNode = addPartitioningNodes(context, typeAndFunctionManager, newLeftNode, leftPartitionVariable.get(), kdbTree.get(), newSecondArgument, Optional.empty());
                newRightNode = addPartitioningNodes(context, typeAndFunctionManager, newRightNode, rightPartitionVariable.get(), kdbTree.get(), newFirstArgument, radius);
            }
        }
    ...
    private static PlanNode addPartitioningNodes(Context context, TypeAndFunctionManager typeAndFunctionManager, PlanNode node, VariableReferenceExpression partitionVariable, KdbTree kdbTree, RowExpression geometry, Optional<RowExpression> radius)
    {
        Assignments.Builder projections = Assignments.builder();
        for (VariableReferenceExpression outputVariable : node.getOutputVariables()) {
            projections.put(outputVariable, outputVariable);
        ...
        FunctionHandle castFunctionHandle = typeAndFunctionManager.lookupCast(CAST, VARCHAR.getTypeSignature(), KDB_TREE.getTypeSignature());
