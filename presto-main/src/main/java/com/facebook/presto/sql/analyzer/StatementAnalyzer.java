...
class StatementAnalyzer
{
            ...
            extends DefaultTraversalVisitor<Scope, Optional<Scope>>
    {
        ...
        protected Scope visitCreateFunction(CreateFunction node, Optional<Scope> scope)
        {
            ...
            TypeWithName returnType = functionAndTypeManager.getSemanticType(parseTypeSignature(node.getReturnType()));
            ...
            if (node.getBody() instanceof Return) {
                ...
                TypeWithName bodyType = analyzeExpression(returnExpression, functionScope).getExpressionTypes().get(NodeRef.of(returnExpression));
                ...
                if (expressionPhysicalType instanceof ArrayType) {
                    ...
                    if (!SystemSessionProperties.isLegacyUnnest(session) && elementType instanceof RowType) {
                        ((RowType) elementType).getFields().stream()
                                .map(field -> Field.newUnqualified(field.getName(), new TypeWithName(field.getType())))
                                .forEach(outputFields::add);
                    }
                    else {
                        outputFields.add(Field.newUnqualified(Optional.empty(), new TypeWithName(elementType)));
                    }
                }
                else if (expressionPhysicalType instanceof MapType) {
                    outputFields.add(Field.newUnqualified(Optional.empty(), new TypeWithName(((MapType) expressionPhysicalType).getKeyType())));
                    outputFields.add(Field.newUnqualified(Optional.empty(), new TypeWithName(((MapType) expressionPhysicalType).getValueType())));
                }
                else {
        ...
        protected Scope visitTable(Table table, Optional<Scope> scope)
        {
            ...
            for (ColumnMetadata column : tableMetadata.getColumns()) {
                Field field = Field.newQualified(
                        ...
                        new TypeWithName(column.getType()),
                        column.isHidden(),
                        Optional.of(name),
                        Optional.of(column.getName()),
                        false);
        ...
        private Scope processView(Table table, Optional<Scope> scope, QualifiedObjectName name, Optional<ViewDefinition> optionalView)
        {
            ...
            analysis.addRelationCoercion(table, outputFields.stream().map(Field::getType).toArray(TypeWithName[]::new));
        ...
        protected Scope visitSetOperation(SetOperation node, Optional<Scope> scope)
        {
            ...
            TypeWithName[] outputFieldTypes = relationScopes.get(0).getRelationType().getVisibleFields().stream()
                    .map(Field::getType)
                    .toArray(TypeWithName[]::new);
            ...
            for (Scope relationScope : relationScopes) {
                ...
                for (int i = 0; i < descFieldSize; i++) {
                    TypeWithName descFieldType = relationType.getFieldByIndex(i).getType();
                    Optional<TypeWithName> commonSuperType = metadata.getFunctionAndTypeManager().getCommonSuperType(outputFieldTypes[i], descFieldType);
            ...
            for (int i = 0; i < node.getRelations().size(); i++) {
                ...
                for (int j = 0; j < relationType.getVisibleFields().size(); j++) {
                    TypeWithName outputFieldType = outputFieldTypes[j];
                    TypeWithName descFieldType = relationType.getFieldByIndex(j).getType();
        ...
        private boolean isExpensiveUnionDistinct(SetOperation setOperation, TypeWithName[] outputTypes)
        {
        ...
        protected Scope visitJoin(Join node, Optional<Scope> scope)
        {
            ...
            if (node.getType() == Join.Type.CROSS || node.getType() == Join.Type.IMPLICIT) {
                ...
                TypeWithName clauseType = expressionAnalysis.getType(expression);
        ...
        private Scope analyzeJoinUsing(Join node, List<Identifier> columns, Optional<Scope> scope, Scope left, Scope right)
        {
            ...
            for (Identifier column : columns) {
                ...
                Optional<TypeWithName> type = metadata.getFunctionAndTypeManager().getCommonSuperType(leftField.get().getType(), rightField.get().getType());
        ...
        protected Scope visitValues(Values node, Optional<Scope> scope)
        {
            ...
            for (Expression row : node.getRows()) {
                if (row instanceof Row) {
                    ...
                    for (int i = 0; i < items.size(); i++) {
                        TypeWithName expectedType = new TypeWithName(fieldTypes.get(i));
                        Expression item = items.get(i);
                        TypeWithName actualType = analysis.getType(item);
                    ...
                    TypeWithName actualType = analysis.getType(row);
                    TypeWithName expectedType = new TypeWithName(fieldTypes.get(0));
                    if (!actualType.equals(expectedType)) {
            ...
            List<Field> fields = fieldTypes.stream()
                    .map(valueType -> Field.newUnqualified(Optional.empty(), new TypeWithName(valueType)))
                    .collect(toImmutableList());
        ...
        private void analyzeHaving(QuerySpecification node, Scope scope)
        {
            if (node.getHaving().isPresent()) {
                ...
                TypeWithName predicateType = expressionAnalysis.getType(predicate);
        ...
        private List<Expression> analyzeGroupBy(QuerySpecification node, Scope scope, List<Expression> outputExpressions)
        {
            if (node.getGroupBy().isPresent()) {
                ...
                for (Expression expression : expressions) {
                    TypeWithName type = analysis.getType(expression);
            ...
            for (SelectItem item : node.getSelect().getSelectItems()) {
                if (item instanceof AllColumns) {
                    ...
                    for (Field field : fields) {
                        ...
                        TypeWithName type = expressionAnalysis.getType(expression);
                    ...
                    TypeWithName type = expressionAnalysis.getType(column.getExpression());
                    if (node.getSelect().isDistinct() && !type.isComparable()) {
