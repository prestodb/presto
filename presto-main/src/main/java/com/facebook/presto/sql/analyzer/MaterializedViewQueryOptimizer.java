        ...
        extends AstVisitor<Node, Void>
{
    ...
    protected Node visitQuerySpecification(QuerySpecification node, Void context)
    {
        if (!node.getFrom().isPresent()) {
            log.debug("Visited QuerySpecification had no from field: \n%s", node);
            return node;
        }
        ...
        if (from instanceof AliasedRelation
            ...
            return rewriteQuerySpecificationIfCompatible(node, (Table) aliasedRelation.getRelation());
        }
    ...
    protected Node visitQuery(Query node, Void context)
    {
        ...
        boolean withSame = !node.getWith().isPresent() || (node.getWith().get()) == newWith.get();
            ...
            extends AstVisitor<Node, Void>
    {
        ...
        public QuerySpecification rewrite(QuerySpecification querySpecification)
        {
            ...
            try {
                ...
                log.warn("Failed to rewrite query with materialized view with following exception: %s", e.getMessage());
                return querySpecification;
            }
        }
