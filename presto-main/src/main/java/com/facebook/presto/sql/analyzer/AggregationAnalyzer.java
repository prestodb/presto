...
class AggregationAnalyzer
{
            ...
            extends AstVisitor<Boolean, Void>
    {
        ...
        {
            if (metadata.getTypeAndFunctionManager().getFunctionMetadata(analysis.getFunctionHandle(node)).getFunctionKind() == AGGREGATE) {
                ...
                if (!node.getWindow().isPresent()) {
                    List<FunctionCall> aggregateFunctions = extractAggregateFunctions(analysis.getFunctionHandles(), node.getArguments(), metadata.getTypeAndFunctionManager());
