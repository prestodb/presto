...
public class PushDownDereferences
{
    ...
    abstract class ExtractProjectDereferences<N extends PlanNode>
            implements Rule<N>
    {
        private final Class<N> aClass;

        ExtractProjectDereferences(Class<N> aClass)
        {
            this.aClass = aClass;
        }
            ...
            extends ExtractProjectDereferences<FilterNode>
    {
        ...
        protected FilterNode rewrite(Context context, FilterNode node, BiMap<DereferenceExpression, VariableReferenceExpression> expressions)
        {
            ...
            ImmutableMap<VariableReferenceExpression, RowExpression>
                    dereferencesMap =
                    expressions.inverse().entrySet().stream().collect(toImmutableMap(Map.Entry::getKey,
                            entry -> castToRowExpression(entry.getValue())));
            Assignments assignments = Assignments.builder().putAll(identityAssignmentsAsSymbolReferences(source.getOutputVariables())).putAll(dereferencesMap).build();
            ProjectNode projectNode = new ProjectNode(context.getIdAllocator().getNextId(), source, assignments);

            RowExpression filter = castToRowExpression(ExpressionTreeRewriter.rewriteWith(new DereferenceReplacer(expressions), castToExpression(node.getPredicate())));
            ...
            extends ExtractProjectDereferences<JoinNode>
    {
        ...
        protected JoinNode rewrite(Context context, JoinNode joinNode, BiMap<DereferenceExpression, VariableReferenceExpression> expressions)
        {
            ...
            return new JoinNode(
                    ...
                    joinNode.getFilter().map(expression -> castToRowExpression(ExpressionTreeRewriter.rewriteWith(new PushDownDereferences.DereferenceReplacer(expressions), castToExpression(expression)))),
            ...
            implements Rule<ProjectNode>
    {
        ...
        public Result apply(ProjectNode node, Captures captures, Context context)
        {
            ...
            Set<VariableReferenceExpression> childSourceSymbols = child.getSources().stream().map(PlanNode::getOutputVariables).flatMap(Collection::stream).collect(toImmutableSet());
            ...
            for (Map.Entry<VariableReferenceExpression, RowExpression> entry : node.getAssignments().entrySet()) {
                if (OriginalExpressionUtils.isExpression(entry.getValue())) {
                    builder.put(entry.getKey(), castToRowExpression(ExpressionTreeRewriter.rewriteWith(new DereferenceReplacer(pushdownDereferences), castToExpression(entry.getValue()))));
                }
                else {
    ...
    {
        public PushDownDereferenceThrough(Class<N> aClass)
        {
            super(Pattern.typeOf(aClass));
        }
        ...
        protected Result pushDownDereferences(Context context, N targetNode, BiMap<DereferenceExpression, VariableReferenceExpression> expressions)
        {
            ...
            ProjectNode projectNode = new ProjectNode(
                    context.getIdAllocator().getNextId(),
                    source,
                    Assignments.builder().putAll(identityAssignmentsAsSymbolReferences(source.getOutputVariables())).putAll(dereferencesMap).build());
        ...
        protected Result pushDownDereferences(Context context, ProjectNode projectNode, BiMap<DereferenceExpression, VariableReferenceExpression> expressions)
        {
            ...
            return Result.ofPlanNode(
                    new ProjectNode(context.getIdAllocator().getNextId(),
                            ...
                            Assignments.builder().putAll(projectNode.getAssignments()).putAll(dereferencesMap).build()));
        }
    }
            ...
            extends PushdownDereferencesInProject<UnnestNode>
    {
        ...
        protected Result pushDownDereferences(Context context, UnnestNode unnestNode, BiMap<DereferenceExpression, VariableReferenceExpression> expressions)
        {
            ...
            ImmutableMap<VariableReferenceExpression, RowExpression>
                    ...
                    expressions.inverse().entrySet().stream().collect(toImmutableMap(Map.Entry::getKey,
                            entry -> castToRowExpression(entry.getValue())));
            Assignments assignments = Assignments.builder().putAll(identityAssignmentsAsSymbolReferences(unnestNode.getSource().getOutputVariables())).putAll(dereferencesMap).build();
            ...
            UnnestNode newUnnest = new UnnestNode(context.getIdAllocator().getNextId(),
                    source,
                    ImmutableList.<VariableReferenceExpression>builder().addAll(unnestNode.getReplicateVariables()).addAll(expressions.values()).build(),
                    unnestNode.getUnnestVariables(),
                    unnestNode.getOrdinalityVariable());
            return Result.ofPlanNode(newUnnest);
        }
    }
    ...
    private static PlanNode createProjectBelow(PlanNode planNode, Assignments dereferences, PlanNodeIdAllocator idAllocator)
    {
        ...
        return new ProjectNode(idAllocator.getNextId(), planNode, Assignments.builder().putAll(identityAssignmentsAsSymbolReferences(planNode.getOutputVariables())).putAll(dereferences).build());
    }
    ...
    private static Map<DereferenceExpression, VariableReferenceExpression> getDereferenceSymbolMap(Collection<RowExpression> expressions, Context context, Metadata metadata, SqlParser sqlParser)
    {
        ...
        // TODO DereferenceExpression Base will cause unnecessary rewritten of same expression.
        // E.g. [msg.foo, msg.foo.bar] => [exp_1, exp_1.bar] => ...
