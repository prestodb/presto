...
import static com.facebook.presto.sql.rewrite.MaterializedViewOptimizationRewriterUtils.optimizeQueryUsingMaterializedView;
        ...
        implements StatementRewrite.Rewrite
{
    ...
    {
        return (Statement) new MaterializedViewOptimizationRewriter.Visitor(metadata, session, parser, accessControl).process(node, null);
    }
            ...
            extends AstVisitor<Node, Void>
    {
        ...
        protected Node visitQuery(Query query, Void context)
        {
            if (SystemSessionProperties.isQueryOptimizationWithMaterializedViewEnabled(session)) {
                Optional<Query> optimizeQuery = optimizeQueryUsingMaterializedView(metadata, session, sqlParser, accessControl, query);
