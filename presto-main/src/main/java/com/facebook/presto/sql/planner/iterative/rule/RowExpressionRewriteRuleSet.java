...
{
    public interface RowExpressionRewriter
    {
        RowExpression rewrite(RowExpression expression, Rule.Context context);

        default AggregationNode.Aggregation rewrite(AggregationNode.Aggregation aggregation, Rule.Context context)
        {
            return new AggregationNode.Aggregation(
                    new CallExpression(
                            ...
                            aggregation.getArguments().stream().map(argument -> rewrite(argument, context)).collect(toImmutableList())),
                    aggregation.getFilter().map(filter -> rewrite(filter, context)),
            ...
            implements Rule<AggregationNode>
    {
        ...
        public Result apply(AggregationNode node, Captures captures, Context context)
        {
            ...
            for (Map.Entry<VariableReferenceExpression, AggregationNode.Aggregation> entry : node.getAggregations().entrySet()) {
                AggregationNode.Aggregation rewritten = rewriter.rewrite(entry.getValue(), context);
