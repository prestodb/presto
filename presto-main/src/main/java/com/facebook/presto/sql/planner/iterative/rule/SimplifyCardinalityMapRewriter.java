            ...
            extends ExpressionRewriter<Object>
    {
        ...
        public Expression rewriteFunctionCall(FunctionCall node, Object context, ExpressionTreeRewriter<Object> treeRewriter)
        {
            if (node.getName().equals(QualifiedName.of("cardinality")) &&
                    node.getChildren().get(0) instanceof FunctionCall &&
                    ((FunctionCall) node.getChildren().get(0)).getName().equals(QualifiedName.of("map_values"))) {
                FunctionCall nested_node = (FunctionCall) node.getChildren().get(0);
                return new FunctionCall(QualifiedName.of("cardinality"), ImmutableList.of(nested_node.getArguments().get(0)));
            }
            return node;
        }
    }
