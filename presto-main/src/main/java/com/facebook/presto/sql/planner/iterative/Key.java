...
public class Key
{
    ...
    {
        requireNonNull(variables, "Variables must not be provided.");
        checkArgument(!variables.isEmpty(), "Key variables should not be empty.");
        if (variables.isEmpty()) {
            throw new IllegalArgumentException("Unique key variables cannot be empty.");
        }
    ...
    {
        return "{" +
                String.join(",", variables.stream().map(VariableReferenceExpression::toString).collect(Collectors.toList()))
                + "}";
    }
}
