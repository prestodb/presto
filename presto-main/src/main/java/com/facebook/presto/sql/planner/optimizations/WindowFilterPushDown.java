        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanRewriter<Void>
    {
        ...
        public PlanNode visitWindow(WindowNode node, RewriteContext<Void> context)
        {
            ...
            if (canReplaceWithRowNumber(node, metadata.getTypeAndFunctionManager())) {
        ...
        public PlanNode visitLimit(LimitNode node, RewriteContext<Void> context)
        {
            ...
            else if (source instanceof WindowNode && canOptimizeWindowFunction((WindowNode) source, metadata.getTypeAndFunctionManager()) && isOptimizeTopNRowNumber(session)) {
        ...
        private static boolean canReplaceWithRowNumber(WindowNode node, TypeAndFunctionManager typeAndFunctionManager)
        {
            return canOptimizeWindowFunction(node, typeAndFunctionManager) && !node.getOrderingScheme().isPresent();
        }

        private static boolean canOptimizeWindowFunction(WindowNode node, TypeAndFunctionManager typeAndFunctionManager)
        {
            ...
            return isRowNumberMetadata(typeAndFunctionManager, typeAndFunctionManager.getFunctionMetadata(node.getWindowFunctions().get(rowNumberVariable).getFunctionHandle()));
        }

        private static boolean isRowNumberMetadata(TypeAndFunctionManager typeAndFunctionManager, FunctionMetadata functionMetadata)
        {
            FunctionHandle rowNumberFunction = typeAndFunctionManager.lookupFunction("row_number", ImmutableList.of());
            return functionMetadata.equals(typeAndFunctionManager.getFunctionMetadata(rowNumberFunction));
        }
    }
}
