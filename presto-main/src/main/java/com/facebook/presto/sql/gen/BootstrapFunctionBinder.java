...
public class BootstrapFunctionBinder
{
    ...
    public FunctionBinding bindOperator(OperatorType operatorType, ByteCodeNode getSessionByteCode, List<ByteCodeNode> arguments, List<Type> argumentTypes)
    {
        ...
        checkArgument(operatorInfo != null, "Unknown operator %s%s", operatorType, argumentTypes);
    ...
    {
        OperatorInfo operatorInfo = metadata.getOperator(OperatorType.CAST, targetType, ImmutableList.of(sourceType));
        checkArgument(operatorInfo != null, "Cast from %s to %s is not supported", sourceType, targetType);
    ...
    {
        FunctionBinding functionBinding = bindOperator(operatorInfo.getOperatorType().toString(), getSessionByteCode, arguments, operatorInfo.getFunctionBinder());
