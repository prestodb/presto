...
public class ExpressionInterpreter
{
            ...
            extends AstVisitor<Object, Object>
    {
        ...
        protected Object visitInPredicate(InPredicate node, Object context)
        {
            ...
            if (!inListCache.containsKey(valueList)) {
                if (valueList.getValues().stream().allMatch(Literal.class::isInstance) &&
                    ...
                    set = FastutilSetHelper.toFastutilHashSet(objectSet, type(node.getValue()), metadata.getTypeAndFunctionManager());
                }
        ...
        protected Object visitArithmeticUnary(ArithmeticUnaryExpression node, Object context)
        {
            ...
            switch (node.getSign()) {
                case PLUS:
                    return value;
                case MINUS:
                    FunctionHandle operatorHandle = metadata.getTypeAndFunctionManager().resolveOperatorHandle(OperatorType.NEGATION, fromTypes(types(node.getValue())));
                    MethodHandle handle = metadata.getTypeAndFunctionManager().getBuiltInScalarFunctionImplementation(operatorHandle).getMethodHandle();
        ...
        protected Object visitNullIfExpression(NullIfExpression node, Object context)
        {
            ...
            FunctionHandle firstCast = metadata.getTypeAndFunctionManager().lookupCast(CAST, firstType.getTypeSignature(), commonType.getTypeSignature());
            FunctionHandle secondCast = metadata.getTypeAndFunctionManager().lookupCast(CAST, secondType.getTypeSignature(), commonType.getTypeSignature());
        ...
        protected Object visitFunctionCall(FunctionCall node, Object context)
        {
            ...
            switch (functionMetadata.getImplementationType()) {
                case BUILTIN:
                    ...
                    break;
                case SQL:
                    Expression function = getSqlFunctionExpression(functionMetadata, (SqlInvokedScalarFunctionImplementation) metadata.getTypeAndFunctionManager().getScalarFunctionImplementation(functionHandle), metadata, session.getSqlFunctionProperties(), node.getArguments());
        ...
        public Object visitCast(Cast node, Object context)
        {
            ...
            FunctionHandle operator = metadata.getTypeAndFunctionManager().lookupCast(CAST, sourceType.getTypeSignature(), targetType.getTypeSignature());
