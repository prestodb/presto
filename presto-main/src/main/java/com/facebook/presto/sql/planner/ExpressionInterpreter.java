...
public class ExpressionInterpreter
{
    ...
    public static ExpressionInterpreter expressionInterpreter(Expression expression, Metadata metadata, Session session, Map<NodeRef<Expression>, TypeWithName> expressionTypes)
    {
        return new ExpressionInterpreter(expression, metadata, session, expressionTypes, false);
    }

    public static ExpressionInterpreter expressionOptimizer(Expression expression, Metadata metadata, Session session, Map<NodeRef<Expression>, TypeWithName> expressionTypes)
    {
    ...
    public static Object evaluateConstantExpression(Expression expression, TypeWithName expectedType, Metadata metadata, Session session, List<Expression> parameters)
    {
        ...
        TypeWithName actualType = analyzer.getExpressionTypes().get(NodeRef.of(expression));
        ...
        Map<NodeRef<Expression>, TypeWithName> coercions = ImmutableMap.<NodeRef<Expression>, TypeWithName>builder()
    ...
    private static Object evaluateConstantExpression(
            ...
            Map<NodeRef<Expression>, TypeWithName> coercions,
    ...
    private ExpressionInterpreter(Expression expression, Metadata metadata, Session session, Map<NodeRef<Expression>, TypeWithName> expressionTypes, boolean optimize)
    {
    ...
    {
        private final Map<NodeRef<Expression>, TypeWithName> generatedExpressionTypes = new HashMap<>();
        ...
        protected Object visitSimpleCaseExpression(SimpleCaseExpression node, Object context)
        {
            ...
            TypeWithName operandType = type(node.getOperand());
        ...
        private boolean isEqual(Object operand1, TypeWithName type1, Object operand2, TypeWithName type2)
        {
        ...
        private void addGeneratedExpressionType(Expression expression, TypeWithName type)
        {
            generatedExpressionTypes.put(NodeRef.of(expression), type);
        }

        private TypeWithName type(Expression expression)
        {
            TypeWithName type = generatedExpressionTypes.get(NodeRef.of(expression));
            if (type != null) {
                return type;
            }
            return expressionTypes.get(NodeRef.of(expression));
        ...
        protected Object visitInPredicate(InPredicate node, Object context)
        {
            ...
            if (hasUnresolvedValue) {
                TypeWithName type = type(node.getValue());
        ...
        protected Object visitNullIfExpression(NullIfExpression node, Object context)
        {
            ...
            TypeWithName firstType = type(node.getFirst());
            TypeWithName secondType = type(node.getSecond());
            ...
            TypeWithName commonType = functionAndTypeManager.getCommonSuperType(firstType, secondType).get();
        ...
        protected Object visitLikePredicate(LikePredicate node, Object context)
        {
            ...
            if (pattern instanceof Slice && (escape == null || escape instanceof Slice) && !isLikePattern((Slice) pattern, (Slice) escape)) {
                ...
                TypeWithName valueType = type(node.getValue());
                TypeWithName patternType = new TypeWithName(createVarcharType(unescapedPattern.length()));
                FunctionAndTypeManager functionAndTypeManager = metadata.getFunctionAndTypeManager();
                Optional<TypeWithName> commonSuperType = functionAndTypeManager.getCommonSuperType(valueType, patternType);
                ...
                TypeWithName superType = commonSuperType.get();
        ...
        public Object visitCast(Cast node, Object context)
        {
            ...
            TypeWithName targetType = metadata.getFunctionAndTypeManager().getSemanticType(parseTypeSignature(node.getType()));
            if (targetType == null) {
            ...
            TypeWithName sourceType = type(node.getExpression());
            if (value instanceof Expression) {
        ...
        private List<TypeWithName> types(Expression... types)
        {
        ...
        private Object invokeOperator(OperatorType operatorType, List<TypeWithName> argumentTypes, List<Object> argumentValues)
        {
        ...
        private Expression toExpression(Object base, TypeWithName type)
        {
            return literalEncoder.toExpression(base, type.getType());
        }

        private boolean isSerializable(Object value, TypeWithName type)
        {
