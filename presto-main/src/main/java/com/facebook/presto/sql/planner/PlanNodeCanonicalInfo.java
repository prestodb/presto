            ...
            PlanCanonicalInfoProvider planCanonicalInfoProvider)
    {
        ...
        for (PlanCanonicalizationStrategy strategy : historyBasedPlanCanonicalizationStrategyList(session)) {
            for (PlanNode node : forTree(PlanNode::getSources).depthFirstPreOrder(root)) {
                ...
                Optional<List<InputTableCacheKey>> inputTableCacheKeys = planCanonicalInfoProvider.getInputTableCacheKey(session, statsEquivalentPlanNode, strategy, true);
                if (hash.isPresent() && inputTableStatistics.isPresent()) {
                    result.add(new CanonicalPlanWithInfo(new CanonicalPlan(statsEquivalentPlanNode, strategy), new PlanNodeCanonicalInfo(hash.get(), inputTableStatistics, inputTableCacheKeys)));
                }
            }
        }
        return result.build();
