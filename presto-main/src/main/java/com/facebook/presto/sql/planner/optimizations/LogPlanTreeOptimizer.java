        ...
        implements PlanOptimizer
{
    ...
    public PlanOptimizerResult optimize(PlanNode plan, Session session, TypeProvider types, VariableAllocator variableAllocator, PlanNodeIdAllocator idAllocator, WarningCollector warningCollector)
    {
        ...
        return optimizerResult(plan, true);
    }
            ...
            extends InternalPlanVisitor<Void, Integer>
    {
        ...
        {
            output(indent, "[%d:%d] [%s] project, Assignments (%s), Outputs (%s):",
        ...
        {
            output(indent, "[%d:%d] [%s] filter (predicate = %s)",
                    node.getSourceLocation().map(SourceLocation::getLine).orElse(-1),
                    node.getSourceLocation().map(SourceLocation::getColumn).orElse(-1),
                    node.getId(),
                    node.getPredicate());
        ...
        {
            output(indent, "[%d:%d] [%s] join (%s)(%s), Equi-join condition(%s), Filter (%s), Outputs(%s):",
                    node.getSourceLocation().map(SourceLocation::getLine).orElse(-1),
                    node.getSourceLocation().map(SourceLocation::getColumn).orElse(-1),
                    node.getId(),
                    node.getType(),
        ...
        {
            output(indent, "[%d:%d] [%s] exchange (scope=%s, partitioning=%s)",
                    node.getSourceLocation().map(SourceLocation::getLine).orElse(-1),
                    node.getSourceLocation().map(SourceLocation::getColumn).orElse(-1),
                    node.getId(),
                    node.getScope(),
        ...
        public Void visitTableScan(TableScanNode node, Integer indent)
        {
            ...
            output(indent, "[%d:%d] [%s] scan (%s) Constraints [Current (%s), Enforced(%s)]",
                    node.getSourceLocation().map(SourceLocation::getLine).orElse(-1),
                    node.getSourceLocation().map(SourceLocation::getColumn).orElse(-1),
                    node.getId(),
        ...
        {
            output(indent, "groupid(%s) : [%s]", node.getGroupIdVariable(), node.getGroupingColumns());
            return visitPlan(node, indent + 1);
        }
