        ...
        extends PlanOptimizer
{
            ...
            extends PlanNodeRewriter<IndexSourceRewriter.Context>
    {
        ...
        public static Optional<PlanNode> rewriteWithIndex(PlanNode planNode, Set<Symbol> lookupSymbols, IndexManager indexManager, SymbolAllocator symbolAllocator, PlanNodeIdAllocator idAllocator)
        {
            ...
            PlanNode rewritten = PlanRewriter.rewriteWith(new IndexSourceRewriter(indexManager, symbolAllocator, idAllocator), planNode, new Context(lookupSymbols, TRUE_LITERAL, success));
            if (success.get()) {
        ...
        public PlanNode rewriteTableScan(TableScanNode node, Context context, PlanRewriter<Context> planRewriter)
        {
            ...
            DomainTranslator.ExtractionResult result = DomainTranslator.fromPredicate(context.getInheritedPredicate(), symbolAllocator.getTypes(), node.getAssignments());
            ...
            Expression unresolvedExpression = DomainTranslator.toPredicate(resolvedIndex.getUnresolvedTupleDomain(), inverseAssignments);
