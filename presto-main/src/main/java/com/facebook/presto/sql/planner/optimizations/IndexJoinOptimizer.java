        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanRewriter<IndexSourceRewriter.Context>
    {
        ...
        {
            RowExpressionDomainTranslator.ExtractionResult decomposedPredicate = domainTranslator.fromPredicate(session.toConnectorSession(), predicate);
            Map<VariableReferenceExpression, ColumnHandle> variableReferenceExpressionColumnHandleMap = node.getAssignments()
                    .entrySet()
                    .stream()
                    .collect(toImmutableMap(entry -> entry.getKey(), Map.Entry::getValue));

            TupleDomain<ColumnHandle> simplifiedConstraint = decomposedPredicate.getTupleDomain()
                    .transform(variableName -> variableReferenceExpressionColumnHandleMap.get(variableName))
            ...
            Map<ColumnHandle, VariableReferenceExpression> inverseAssignments = node.getAssignments().entrySet().stream()
                    .collect(toImmutableMap(Map.Entry::getValue, entry -> entry.getKey()));
