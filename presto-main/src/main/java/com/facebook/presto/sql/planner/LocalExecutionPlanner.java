...
public class LocalExecutionPlanner
{
            ...
            extends PlanVisitor<Void, PhysicalOperation>
    {
        ...
        public PhysicalOperation visitTableScan(TableScanNode node, Void context)
        {
            ...
            for (Map.Entry<Symbol, ColumnHandle> entry : node.getAssignments().entrySet()) {
                ...
                ColumnHandle column = entry.getValue();
        ...
        public PhysicalOperation visitJoin(JoinNode node, Void context)
        {
            ...
            int probeChannel = leftSource.getLayout().get(left).getChannel(); // TODO: verify "left" symbol is in a channel by itself

            PhysicalOperation rightSource = node.getRight().accept(this, context);
            int buildChannel = rightSource.getLayout().get(right).getChannel(); // TODO: verify "right" symbol is in a channel by itself
        ...
        public PhysicalOperation visitSink(SinkNode node, Void context)
        {
            ...
            if (needsProjection) {
                return pack(source, ImmutableList.<Symbol>of(), types);
            }
        ...
        private AggregationFunctionDefinition buildFunctionDefinition(AggregationNode node, PhysicalOperation source, Symbol symbol, FunctionCall call)
        {
            ...
            FunctionHandle functionHandle = node.getFunctions().get(symbol);
            return metadata.getFunction(functionHandle).bind(arguments);
        }
        ...
        private PhysicalOperation planGlobalAggregation(AggregationNode node, PhysicalOperation source)
        {
            ...
            for (Map.Entry<Symbol, FunctionCall> entry : node.getAggregations().entrySet()) {
                ...
                functionDefinitions.add(buildFunctionDefinition(node, source, symbol, entry.getValue()));
                aggregationOutputSymbols.add(symbol);
            }

            aggregationOperator = new AggregationOperator(source.getOperator(), node.getStep(), functionDefinitions);
    ...
    {
        ImmutableMap.Builder<Symbol, Input> mappings = ImmutableMap.builder();
        ImmutableList.Builder<ProjectionFunction> projections = ImmutableList.builder();

        int channel = 0;
        for (Symbol symbol : Sets.difference(source.getLayout().keySet(), ImmutableSet.copyOf(symbols))) {
            // TODO: this code splits composite channels into one field per channel. Fix it so that it preserves the layout of channels for fields not in the list
            projections.add(ProjectionFunctions.singleColumn(types.get(symbol).getRawType(), source.getLayout().get(symbol)));
            mappings.put(symbol, new Input(channel, 0));
            channel++;
        }

        // append a projection that packs all the input symbols into a single channel
        ...
        for (Symbol symbol : symbols) {
            ...
            mappings.put(symbol, new Input(channel, field));
            field++;
        }
        if (!packedProjections.isEmpty()) {
        ...
        return new PhysicalOperation(operator, mappings.build());
    }
