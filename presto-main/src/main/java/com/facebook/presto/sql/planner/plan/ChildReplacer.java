        ...
        extends PlanVisitor<List<PlanNode>, PlanNode>
{
    ...
    @Override
    public PlanNode visitEnforceUniqueColumns(EnforceUniqueColumns node, List<PlanNode> newChildren)
    {
        checkArgument(newChildren.size() == 1, "expected newChildren to contain 1 node");
        return new EnforceUniqueColumns(node.getId(), Iterables.getOnlyElement(newChildren), node.getMostSigBitsSymbol(), node.getLeastSigBitsSymbol());
    }
}
