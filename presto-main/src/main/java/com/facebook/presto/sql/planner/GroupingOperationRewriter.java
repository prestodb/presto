...
public final class GroupingOperationRewriter
{
    ...
    public static Expression rewriteGroupingOperation(GroupingOperation expression, QuerySpecification queryNode, Analysis analysis, FunctionRegistry functionRegistry, Optional<Symbol> groupIdSymbol)
    {
        ...
        if (analysis.getGroupingSets(queryNode).size() == 1) {
            ...
            List<Expression> groupingResults = groupingSetDescriptors.stream()
                    ...
                    .map(groupingResult -> shouldUseIntegerReturnType(expression) ? new GenericLiteral(INTEGER, groupingResult) : new LongLiteral(groupingResult))
                    .collect(toImmutableList());
            ...
            List<Expression> newGroupingArguments = ImmutableList.of(
                    ...
                    new ArithmeticBinaryExpression(ADD, groupIdSymbol.get().toSymbolReference(), new GenericLiteral(StandardTypes.BIGINT, "1"))
            );
            QualifiedName functionName = shouldUseIntegerReturnType(expression) ? QualifiedName.of(INTEGER_GROUPING) : QualifiedName.of(BIGINT_GROUPING);
            FunctionCall rewritten = new FunctionCall(functionName, newGroupingArguments);

            Type arrayType = shouldUseIntegerReturnType(expression) ? IntegerType.INTEGER : BigintType.BIGINT;
            List<TypeSignatureProvider> functionArgumentTypes = Arrays.asList(
                    new TypeSignatureProvider((new ArrayType(arrayType)).getTypeSignature()),
                    new TypeSignatureProvider(BIGINT.getTypeSignature())
            );
            resolveFunction(rewritten, functionArgumentTypes, functionRegistry);
