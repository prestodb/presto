...
public class EquivalenceClassProperty
{
    ...
    {
        equivalenceClassHeads = new HashMap<>();
     ...
     * @return True if equivalenceClassProperty is more general than otherEquivalenceClassProperty or False otherwise.
     */
    public static boolean moreGeneral(EquivalenceClassProperty equivalenceClassProperty, EquivalenceClassProperty otherEquivalenceClassProperty)
    {
        requireNonNull(equivalenceClassProperty, "Equivalence class property must be provided.");
        requireNonNull(otherEquivalenceClassProperty, "Other equivalence class property must be provided.");
        //every equivalence class of other is a subset of some equivalence class of the first
        return ((equivalenceClassProperty.equivalenceClasses.isEmpty() && otherEquivalenceClassProperty.equivalenceClasses.isEmpty()) ||
                (otherEquivalenceClassProperty.equivalenceClasses.entrySet().stream().allMatch(e -> {
                    final Set<RowExpression> otherEqClass = new HashSet<>();
                    otherEqClass.add(e.getKey());
                    otherEqClass.addAll(e.getValue());
                    return equivalenceClassProperty.equivalenceClasses.entrySet().stream().anyMatch(e2 -> {
                        final Set<RowExpression> eqClass = new HashSet<>();
                        eqClass.add(e2.getKey());
                        eqClass.addAll(e2.getValue());
                        return eqClass.containsAll(otherEqClass);
                    });
                })));
    }
    ...
    {
        requireNonNull(expression, "Expression must be provided.");
        checkArgument((expression instanceof VariableReferenceExpression || expression instanceof ConstantExpression),
                "Row expression must be a VariableReferenceExpression or a ConstantExpression.");
    ...
    {
        requireNonNull(head, "Head expression must be provided.");
    ...
    public void update(EquivalenceClassProperty equivalenceClassProperty)
    {
        ...
        equivalenceClassProperty.equivalenceClasses.entrySet().forEach(eqClass -> {
            ...
            members.stream().forEach(member -> updateLocal(head, member));
        });
    }
    ...
    public boolean update(RowExpression predicate)
    {
        ...
        LogicalRowExpressions.extractConjuncts(predicate).stream()
                .filter(CallExpression.class::isInstance)
                .map(CallExpression.class::cast)
                .filter(e -> (isVariableEqualVariableExpression(e) || isVariableEqualConstantExpression(e)))
                .forEach(e -> updateLocal(e.getArguments().get(0), e.getArguments().get(1)));
        return updated;
    }

    private boolean isVariableEqualVariableExpression(RowExpression expression)
    {
        requireNonNull(expression, "Expression must be provided.");
        return expression instanceof CallExpression
                && isEqualExpression((CallExpression) expression)
    ...
    private boolean isVariableEqualConstantExpression(RowExpression expression)
    {
        requireNonNull(expression, "Expression must be provided.");
        return expression instanceof CallExpression
                && isEqualExpression((CallExpression) expression)
                && ((CallExpression) expression).getArguments().size() == 2
                && ((((CallExpression) expression).getArguments().get(0) instanceof VariableReferenceExpression
                && ((CallExpression) expression).getArguments().get(1) instanceof ConstantExpression)
                || (((CallExpression) expression).getArguments().get(0) instanceof ConstantExpression
                && ((CallExpression) expression).getArguments().get(1) instanceof VariableReferenceExpression));
    }

    private boolean isEqualExpression(CallExpression callExpression)
    {
        requireNonNull(callExpression, "Call expression must be provided.");
        ...
        if ((functionHandle instanceof BuiltInFunctionHandle)) {
            ...
            Signature signature = builtInFunctionHandle.getSignature();
            return signature.getName().equals(EQUAL.getFunctionName());
        }
        return false;
    }
    ...
    {
        updateLocal(firstExpression, secondExpression);
    }
     ...
     */
    private void updateLocal(RowExpression firstExpression, RowExpression secondExpression)
    {
        requireNonNull(firstExpression, "First expression must be provided.");
        requireNonNull(secondExpression, "Second expression must be provided.");
        checkArgument((firstExpression instanceof VariableReferenceExpression || firstExpression instanceof ConstantExpression),
                "First row expression must be a VariableReferenceExpression or a ConstantExpression.");
        checkArgument((secondExpression instanceof VariableReferenceExpression || secondExpression instanceof ConstantExpression),
                "Second row expression must be a VariableReferenceExpression or a ConstantExpression.");
    ...
    {
        requireNonNull(head1, "First head expression must be provided.");
        requireNonNull(head2, "Second head expression must be provided.");
    ...
    {
        requireNonNull(head, "Head expression must be provided.");
        requireNonNull(headClass, "Head expression class members must be provided.");
        requireNonNull(headOfOtherEqClass, "Other head expression must be provided.");
        requireNonNull(otherEqClass, "Other head expression class members must be provided.");
    ...
    public EquivalenceClassProperty project(Map<VariableReferenceExpression, VariableReferenceExpression> inverseVariableMappings)
    {
        ...
        for (Map.Entry<RowExpression, List<RowExpression>> entry : equivalenceClasses.entrySet()) {
            ...
            if (!projectedMembers.isEmpty()) {
                ...
                if (projectedMembers.isEmpty()) {
                    //do not store trivial equivalence classes
                    continue;
                }

                projectedEquivalenceClassProperty.equivalenceClasses.put(projectedHead, projectedMembers);
                final RowExpression finalProjectedHead = projectedHead;
                projectedMembers.stream().forEach(m -> projectedEquivalenceClassProperty.equivalenceClassHeads.put(m, finalProjectedHead));
            }
        }
    ...
    {
        return "{" +
                "EquivalenceClassHeads{" +
                String.join(",", equivalenceClassHeads.entrySet().stream().map(e -> e.getKey().toString() + ":" + e.getValue().toString()).collect(Collectors.toList())) +
                "}" +
                ", EquivalenceClasses{" +
                String.join(",", equivalenceClasses.entrySet().stream().map(e -> e.getKey().toString() + ":" + e.getValue().toString()).collect(Collectors.toList())) +
                "}}";
    }
}
