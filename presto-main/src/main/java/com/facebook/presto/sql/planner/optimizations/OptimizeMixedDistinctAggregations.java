            ...
            for (Map.Entry<VariableReferenceExpression, Aggregation> entry : node.getAggregations().entrySet()) {
                if (entry.getValue().getMask().isPresent()) {
                    ...
                    Aggregation aggregation = new Aggregation(
                            new CallExpression(
                                    "arbitrary",
                                    metadata.getTypeAndFunctionManager().lookupFunction("arbitrary", fromTypes(ImmutableList.of(argument.getType()))),
                            ...
                            false,
                            Optional.empty());
                    QualifiedFunctionName functionName = metadata.getTypeAndFunctionManager().getFunctionMetadata(entry.getValue().getFunctionHandle()).getName();
