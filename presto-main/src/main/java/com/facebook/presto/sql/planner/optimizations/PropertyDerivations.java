...
import static com.facebook.presto.sql.planner.Partitioning.ArgumentBinding.expressionBinding;
                    ...
                    if (probeProperties.getNodePartitioning().isPresent()) {
                        ...
                        for (Partitioning.ArgumentBinding argument : nodePartitioning.getArguments()) {
                            if (argument.isVariable()) {
                                ...
                                throw new IllegalStateException("Argument binding is neither symbol nor constant");
                            }
                        }

                        return ActualProperties.builder()
                                .global(partitionedOn(new Partitioning(nodePartitioning.getHandle(), outputArguments.build()), Optional.empty()))
                                .build();
                    }
                    return ActualProperties.builder()
