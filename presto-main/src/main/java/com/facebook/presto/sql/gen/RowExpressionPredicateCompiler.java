        ...
        implements PredicateCompiler
{
    ...
    private final LoadingCache<RowExpression, Supplier<Predicate>> filterCache;

    @Inject
    public RowExpressionPredicateCompiler(Metadata metadata)
    {
        this(metadata, 10_000);
    }

    public RowExpressionPredicateCompiler(Metadata metadata, int filterCacheSize)
    {
    ...
    @Override
    public Supplier<Predicate> compilePredicate(RowExpression predicate, Optional<String> classNameSuffix)
    {
        if (filterCache == null) {
            return compileFilterInternal(predicate, classNameSuffix);
        }
    ...
    private Supplier<Predicate> compileFilterInternal(RowExpression filter, Optional<String> classNameSuffix)
    {
        ...
        ClassDefinition classDefinition = defineFilterClass(result.getRewrittenExpression(), inputChannels, callSiteBinder, classNameSuffix);

        Class<? extends Predicate> filterClass;
        try {
            filterClass = defineClass(classDefinition, Predicate.class, callSiteBinder.getBindings(), getClass().getClassLoader());
        }
        catch (Exception e) {
    ...
    {
        return makeClassName(Predicate.class.getSimpleName(), classNameSuffix);
    }

    private ClassDefinition defineFilterClass(RowExpression filter, int[] inputChannels, CallSiteBinder callSiteBinder, Optional<String> classNameSuffix)
    {
    ...
    {
        Map<LambdaDefinitionExpression, LambdaBytecodeGenerator.CompiledLambda> compiledLambdaMap = generateMethodsForLambda(classDefinition, callSiteBinder, cachedInstanceBinder, filter);
