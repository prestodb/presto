...
{
    private final Map<Rule, RuleStats> stats = new HashMap<>();

    public void registerAll(Collection<Rule> rules)
    {
        for (Rule rule : rules) {
            stats.put(rule, new RuleStats());
        }
    }

    public void record(Rule rule, long nanos, boolean match)
    {
        stats.get(rule).record(nanos, match);
    }

    void export(MBeanExporter exporter)
    {
        for (Map.Entry<Rule, RuleStats> entry : stats.entrySet()) {
            String name = ObjectNames.builder(IterativeOptimizer.class)
                    .withProperty("rule", entry.getKey().getClass().getSimpleName())
                    .build();
    ...
    {
        for (Rule rule : stats.keySet()) {
            String name = ObjectNames.builder(IterativeOptimizer.class)
                    .withProperty("rule", rule.getClass().getSimpleName())
                    .build();
