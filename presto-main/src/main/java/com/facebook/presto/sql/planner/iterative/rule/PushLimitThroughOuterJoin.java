        ...
        implements Rule<LimitNode>
{
    ...
    private static final Pattern<LimitNode> PATTERN =
            limit()
                    .with(source().matching(
                            join()
                                    .with(type().matching(type -> isLeftOuter(type) || isRightOuter(type)))
                                    .capturedAs(CHILD)));
    ...
    public Result apply(LimitNode parent, Captures captures, Context context)
    {
        ...
        if (isLeftOuter(joinNode.getType()) && !isLimited(left, context.getLookup(), parent.getCount())) {
            left = new LimitNode(context.getIdAllocator().getNextId(), left, parent.getCount(), false);
        }

        if (isRightOuter(joinNode.getType()) && !isLimited(right, context.getLookup(), parent.getCount())) {
            right = new LimitNode(context.getIdAllocator().getNextId(), right, parent.getCount(), false);
        }
    ...
    private static boolean isLeftOuter(Type type)
    {
        return type == LEFT || type == FULL;
    }

    private static boolean isRightOuter(Type type)
    {
