...
public final class ExpressionDomainTranslator
{
            ...
            extends AstVisitor<ExtractionResult, Boolean>
    {
        ...
        {
            Map<NodeRef<Expression>, TypeWithName> expressionTypes = analyzeExpression(comparison);
        ...
        {
            Map<NodeRef<Expression>, TypeWithName> expressionTypes = analyzeExpression(cast);
            TypeWithName actualType = expressionTypes.get(NodeRef.of(cast.getExpression()));
            TypeWithName expectedType = expressionTypes.get(NodeRef.<Expression>of(cast));
            return metadata.getFunctionAndTypeManager().canCoerce(actualType, expectedType);
        }

        private Map<NodeRef<Expression>, TypeWithName> analyzeExpression(Expression expression)
        {
    ...
    private static TypeWithName typeOf(Expression expression, Session session, Metadata metadata, SemanticTypeProvider types)
    {
        Map<NodeRef<Expression>, TypeWithName> expressionTypes = ExpressionAnalyzer.getExpressionTypes(session, metadata, new SqlParser(), types, expression, emptyList(), WarningCollector.NOOP);
