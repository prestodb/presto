        ...
        implements ExpressionOptimizer
{
    ...
    public RowExpression optimize(RowExpression rowExpression, Level level, ConnectorSession session)
    {
        if (level.ordinal() <= OPTIMIZED.ordinal()) {
            return toRowExpression(rowExpression.getSourceLocation(), new DelegatingRowExpressionInterpreter(rowExpression, metadata, session, level, expressionManager.getRowExpressionInterpreter()).optimize(), rowExpression.getType());
        }
    ...
    {
        DelegatingRowExpressionInterpreter interpreter = new DelegatingRowExpressionInterpreter(expression, metadata, session, level, expressionManager.getRowExpressionInterpreter());
