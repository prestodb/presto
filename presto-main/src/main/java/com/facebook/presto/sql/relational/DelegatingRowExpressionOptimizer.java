        ...
        implements ExpressionOptimizer
{
    ...
    @Override
    public RowExpression optimize(RowExpression rowExpression, Level level, ConnectorSession session, Function<VariableReferenceExpression, Object> variableResolver)
    {
        ...
        for (int i = 0; i < maxOptimizationAttempts; i++) {
            ...
            rowExpression = delegate.optimize(toRowExpression(rowExpression, rowExpression.getType()), level, session, variableResolver);
            if (originalExpression.equals(rowExpression)) {
                break;
            }
        }
        return toRowExpression(rowExpression.getSourceLocation(), rowExpression, rowExpression.getType());
    }
}
