...
public class PlanPrinter
{
            ...
            extends PlanVisitor<Integer, Void>
    {
        ...
        {
            print(indent, "- MarkDistinct(%s) => [%s]", node.getMarkerSymbol(), formatOutputs(node.getOutputSymbols()));
            print(indent + 1, "[%s] => %s", formatOutputs(node.getDistinctSymbols()), node.getMarkerSymbol());
            return processChildren(node, indent + 1);
        }
