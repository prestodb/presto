...
public class PlanPrinter
{
            ...
            extends PlanVisitor<Integer, Void>
    {
        ...
        public Void visitJoin(JoinNode node, Integer indent)
        {
            ...
            print(indent, "- %s[%s] => [%s] %s", node.getType().getJoinLabel(), Joiner.on(" AND ").join(joinExpressions), formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
        ...
        {
            print(indent, "- SemiJoin[%s = %s] => [%s] %s", node.getSourceJoinSymbol(), node.getFilteringSourceJoinSymbol(), formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
        ...
        public Void visitIndexJoin(IndexJoinNode node, Integer indent)
        {
            ...
            print(indent, "- %sIndexJoin[%s] => [%s] %s", node.getType().getJoinLabel(), Joiner.on(" AND ").join(joinExpressions), formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
        ...
        {
            print(indent, "- DistinctLimit%s[%s] => [%s] %s", node.isPartial() ? "Partial" : "", node.getLimit(), formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
            return processChildren(node, indent + 1);
        }
        ...
        public Void visitAggregation(AggregationNode node, Integer indent)
        {
            ...
            print(indent, "- Aggregate%s%s%s => [%s] %s", type, key, sampleWeight, formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
        ...
        {
            print(indent, "- GroupId%s => [%s] %s", node.getGroupingSets(), formatOutputs(node.getOutputSymbols()), formatCost(node));
            return processChildren(node, indent + 1);
        }
        ...
        {
            print(indent, "- MarkDistinct[distinct=%s marker=%s] => [%s] %s", formatOutputs(node.getDistinctSymbols()), node.getMarkerSymbol(), formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
            return processChildren(node, indent + 1);
        }
        ...
        public Void visitWindow(WindowNode node, Integer indent)
        {
            ...
            print(indent, "- Window[%s] => [%s] %s", Joiner.on(", ").join(args), formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
        ...
        public Void visitTopNRowNumber(TopNRowNumberNode node, Integer indent)
        {
            ...
            print(indent, "- TopNRowNumber[%s limit %s] => [%s] %s", Joiner.on(", ").join(args), node.getMaxRowCountPerPartition(), formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
        ...
        public Void visitRowNumber(RowNumberNode node, Integer indent)
        {
            ...
            print(indent, "- RowNumber[%s] => [%s] %s", Joiner.on(", ").join(args), formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
        ...
        {
            print(indent, "- Filter[%s] => [%s] %s", node.getPredicate(), formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
            return processChildren(node, indent + 1);
        }
        ...
        {
            print(indent, "- Project => [%s] %s", formatOutputs(node.getOutputSymbols()), formatCost(node));
            printStats(indent + 2, node.getId());
