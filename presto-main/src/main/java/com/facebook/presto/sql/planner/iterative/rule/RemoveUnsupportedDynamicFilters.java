...
import static com.facebook.presto.sql.ExpressionUtils.extractDynamicFilters;
        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanRewriter<Set<DynamicFilter>>
    {
        ...
        public PlanNode visitJoin(JoinNode node, RewriteContext<Set<DynamicFilter>> context)
        {
            ...
            Optional<Expression> filter = join.getFilter();
            if (filter.isPresent()) {
                Expression original = filter.get();
            ...
            if (!filter.equals(join.getFilter()) || !assignments.equals(join.getDynamicFilterAssignments())) {
                return new JoinNode(
                        join.getId(),
                        join.getType(),
                        join.getLeft(),
