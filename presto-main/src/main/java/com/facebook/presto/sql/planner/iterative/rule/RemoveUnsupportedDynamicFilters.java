        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanRewriter<Set<DynamicFilterExpression>>
    {
        ...
        public PlanNode visitJoin(JoinNode node, RewriteContext<Set<DynamicFilterExpression>> context)
        {
            ...
            Assignments assignments = join.getDynamicFilterSource().getDynamicFilterAssignments();
            Set<DynamicFilterExpression> removedFilters = context.get();
            if (!removedFilters.isEmpty()) {
                Assignments originalAssignments = join.getDynamicFilterSource().getDynamicFilterAssignments();
            ...
            if (!filterOptional.equals(join.getFilter()) || !assignments.equals(join.getDynamicFilterSource().getDynamicFilterAssignments())) {
                return new JoinNode(
                        join.getId(),
                        join.getType(),
                        join.getLeft(),
                        ...
                        new DynamicFilterSource(join.getDynamicFilterSource().getDynamicSourceId(), assignments));
            }
            return join;
        }
        ...
        private Assignments cleanupAssignments(PlanNodeId joinId, Assignments assignments, Set<DynamicFilterExpression> removedFilters)
        {
            ...
            while (filtersIterator.hasNext()) {
                ...
                if (removedFilter.getSourceId().equals(joinId.toString())) {
                    Symbol filterSymbol = new Symbol(removedFilter.getDynamicFilterSymbol());
