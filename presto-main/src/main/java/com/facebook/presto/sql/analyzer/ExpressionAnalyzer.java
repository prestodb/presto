...
public class ExpressionAnalyzer
{
            ...
            extends AstVisitor<Type, AnalysisContext>
    {
        ...
        protected Type visitCurrentTime(CurrentTime node, AnalysisContext context)
        {
            ...
            if (node.getType() == CurrentTime.Type.LOCALTIME) {
                type = TIME;
            }
            if (node.getType() == CurrentTime.Type.TIMESTAMP) {
                type = TIMESTAMP_WITH_TIME_ZONE;
            }
            if (node.getType() == CurrentTime.Type.LOCALTIMESTAMP) {
                type = TIMESTAMP;
            }
        ...
        protected Type visitGenericLiteral(GenericLiteral node, AnalysisContext context)
        {
            ...
            try {
                OperatorInfo operator = metadata.getOperator(OperatorType.CAST, type, ImmutableList.of(VARCHAR));
