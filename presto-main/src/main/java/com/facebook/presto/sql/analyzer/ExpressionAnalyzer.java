...
public class ExpressionAnalyzer
{
            ...
            extends AstVisitor<Type, Void>
    {
        ...
        protected Type visitSearchedCaseExpression(SearchedCaseExpression node, Void context)
        {
            ...
            return getType(node, "clauses", types);
        }
        ...
        protected Type visitSimpleCaseExpression(SimpleCaseExpression node, Void context)
        {
            ...
            for (WhenClause whenClause : node.getWhenClauses()) {
                Type whenOperand = process(whenClause.getOperand(), context);
                if (!comparableTypes(operand, whenOperand)) {
        ...
        protected Type visitCoalesceExpression(CoalesceExpression node, Void context)
        {
            ...
            return getType(node, "operands", operandTypes);
        }

        private Type getType(Node node, String subTypeName, List<Type> subTypes)
        {
            subTypes = ImmutableList.copyOf(filter(subTypes, not(equalTo(Type.NULL))));
            Type firstOperand = Iterables.get(subTypes, 0);
            if (!Iterables.all(subTypes, comparableTypesPredicate(firstOperand))) {
        ...
        public static Predicate<Type> comparableTypesPredicate(final Type type) {
            return new Predicate<Type>() {
                public boolean apply(Type input)
                {
                    return comparableTypes(type, input);
                }
            };
        }

        public static boolean comparableTypes(Type type1, Type type2)
        {
            switch (type1) {
                case LONG:
                case DOUBLE:
                    return isNumericOrNull(type2);
                case STRING:
                    return isStringTypeOrNull(type2);
                case BOOLEAN:
                    return isBooleanOrNull(type2);
                case NULL:
                    return true;
                default:
                    throw new IllegalStateException("Unknown type " + type1);
            }
        }
