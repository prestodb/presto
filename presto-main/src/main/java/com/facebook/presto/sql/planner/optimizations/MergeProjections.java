        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanRewriter<Void>
    {
        ...
        public PlanNode visitProject(ProjectNode node, RewriteContext<Void> context)
        {
            ...
            if (source instanceof ProjectNode) {
                ...
                if (isDeterministic(sourceProject) && !containsTry(node)) {
                    ...
                    for (Map.Entry<Symbol, Expression> projection : node.getAssignmentsMap().entrySet()) {
                        Expression inlined = ExpressionTreeRewriter.rewriteWith(
                                new ExpressionSymbolInliner(sourceProject.getAssignmentsMap()), projection.getValue());
        ...
        {
            return node.getAssignmentsMap().values().stream().allMatch(DeterminismEvaluator::isDeterministic);
        }

        private static boolean containsTry(ProjectNode node)
        {
            return node.getAssignmentsMap().values().stream().anyMatch(TryExpression.class::isInstance);
        }
    }
}
