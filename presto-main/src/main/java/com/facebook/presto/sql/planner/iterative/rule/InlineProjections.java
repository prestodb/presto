        ...
        implements Rule<ProjectNode>
{
    ...
    private Sets.SetView<VariableReferenceExpression> extractInliningTargets(ProjectNode parent, ProjectNode child, Context context)
    {
        ...
        boolean optimizeCommonSubExpression = isOptimizeCommonSubExpression(context.getSession());
        Set<VariableReferenceExpression> singletons = dependencies.entrySet().stream()
                // if optimize_common_sub_expression is not enabled, reference appears just once across all expressions in parent project node
