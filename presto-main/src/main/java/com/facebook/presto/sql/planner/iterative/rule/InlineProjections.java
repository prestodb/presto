        ...
        implements Rule<ProjectNode>
{
    ...
    public Result apply(ProjectNode parent, Captures captures, Context context)
    {
        ...
        Set<VariableReferenceExpression> targets = extractInliningTargets(parent, child, context);
        if (targets.isEmpty()) {
            return Result.empty();
        }
    ...
    private Set<VariableReferenceExpression> extractInliningTargets(ProjectNode parent, ProjectNode child, Context context)
    {
        ...
        // The purpose of inlining is to eliminate unnecessary projects. We can only eliminate a project when it's reduced to identity assignments
        // only. Thus, if there are non-identity assignments left, we consider the inlining target set as empty.

        // Inlining any complex inputs to try expressions would potentially change the semantics of those expressions
        ...
        // remove identity assignments in child because inline has no effects
        Map<VariableReferenceExpression, Long> candidates = dependencies.entrySet().stream()
                .filter(entry -> !isIdentity(child.getAssignments(), entry.getKey()))
                .collect(toImmutableMap(Map.Entry::getKey, Map.Entry::getValue));

        // if there's any dependency that's referenced for more than once but is not a constant, we cannot inline them
        if (candidates.entrySet().stream()
                .anyMatch(entry -> entry.getValue() > 1 && !isConstant(child.getAssignments().get(entry.getKey())))) {
            return ImmutableSet.of();
        }
