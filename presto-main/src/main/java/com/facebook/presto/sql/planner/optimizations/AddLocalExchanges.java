...
import static com.facebook.presto.SystemSessionProperties.isNativeExecutionScalePartitionedWritersThreadsEnabled;
        ...
        implements PlanOptimizer
{
            ...
            extends InternalPlanVisitor<PlanWithProperties, StreamPreferredProperties>
    {
        ...
        public PlanWithProperties visitTableWriter(TableWriterNode tableWrite, StreamPreferredProperties parentPreferences)
        {
            ...
            if (nativeExecution
                    && tableWrite.getTablePartitioningScheme().isPresent()
                    && isNativeExecutionScalePartitionedWritersThreadsEnabled(session)) {
                verify(tableWrite.getTablePartitioningScheme().get().isScaleWriters());
                PlanWithProperties source = accept(tableWrite.getSource(), defaultParallelism(session));
                PlanWithProperties exchange = deriveProperties(
                        partitionedExchange(
                                idAllocator.getNextId(),
