...
public class SplitManager
{
    ...
    private Iterable<SplitAssignments> getImportSplitAssignments(Session session, ImportTableHandle handle, Expression predicate, Map<Symbol, ColumnHandle> mappings)
    {
        ...
        Iterable<List<PartitionChunk>> chunks = retry().allowing(ObjectNotFoundException.class).runUnchecked(new Callable<Iterable<List<PartitionChunk>>>()
        {
            @Override
    ...
    private List<PartitionInfo> getCandidatePartitions(final String sourceName, final String databaseName, final String tableName, Expression predicate, Map<Symbol, String> symbolToColumnName)
    {
        ...
        return retry().allowing(ObjectNotFoundException.class).runUnchecked(new Callable<List<PartitionInfo>>()
        {
            @Override
    ...
    {
        return retry().allowing(ObjectNotFoundException.class).runUnchecked(new Callable<List<String>>()
        {
            @Override
