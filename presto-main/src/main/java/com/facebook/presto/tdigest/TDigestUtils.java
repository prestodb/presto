...
public final class TDigestUtils
{
    ...
    public static boolean isTDigestFunction(String name, List<TypeSignature> signatures)
    {
        if (name.equals(TDigestAggregationFunction.NAME)) {
            return true;
        }

        for (TypeSignature signature : signatures) {
            if (TDIGEST_TYPE.getTypeSignature().equals(signature)) {
                return true;
            }
        }
        return false;
