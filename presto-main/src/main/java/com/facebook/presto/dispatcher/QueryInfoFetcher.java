...
/**
 * Periodically refreshes a remote {@link BasicQueryInfo} from a given coordinator, and can provide a notification of
 * remote query completion or failure via the {@link #addFinalQueryInfoListener(StateMachine.StateChangeListener)} method.
 */
public class QueryInfoFetcher
        implements SimpleHttpResponseCallback<BasicQueryInfo>
{
    private static final Logger log = Logger.get(QueryInfoFetcher.class);
    ...
    public QueryInfoFetcher(
            ...
            // TODO: remove binary transport flag once Thrift migration is complete
            boolean isBinaryTransportEnabled)
    {
        this.onFail = requireNonNull(onFail, "onFail is null");
        this.queryId = requireNonNull(queryId, "queryId is null");
        this.httpClient = requireNonNull(httpClient, "httpClient is null");
        this.queryInfo = new StateMachine<>("query " + queryId, executor, initialQueryInfo);
    ...
    public synchronized void start(InternalNode coordinator)
    {
        ...
        URI location = UriBuilder.fromUri(httpLocationFactory.createQueryLocation(coordinator, queryId))
        ...
        scheduledFuture = updateScheduledExecutor.scheduleWithFixedDelay(() -> {
            try {
                ...
                log.error(t, "Error while scheduling update to QueryInfoFetcher");
            }
        }, 0, 100, MILLISECONDS);
        running = true;
    }
    ...
    public void failed(Throwable cause)
    {
        try (SetThreadName ignored = new SetThreadName("QueryInfoFetcher-%s", queryId)) {
            ...
            try {
                // if query not already done, record error
