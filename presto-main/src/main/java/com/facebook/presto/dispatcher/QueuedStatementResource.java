...
public class QueuedStatementResource
{
                ...
                () -> {
                    try {
                        ...
                        for (Entry<QueryId, Query> entry : ImmutableSet.copyOf(queries.entrySet())) {
                            ...
                            if (!queryManager.getDispatchInfo(entry.getKey()).isPresent()) {
            ...
            @Context UriInfo uriInfo)
    {
        ...
        Query query = new Query(statement, sessionContext, queryManager);
    ...
    private static final class Query
    {
        ...
        public Query(String query, SessionContext sessionContext, DispatcherQueryManager queryManager)
        {
        ...
        private ListenableFuture<?> waitForDispatched()
        {
            ...
            synchronized (this) {
                if (querySubmissionFuture == null) {
                    querySubmissionFuture = queryManager.createQuery(queryId, slug, sessionContext, query);
                }
            ...
            return queryManager.waitForDispatched(queryId);
        }
        ...
        public QueryResults getQueryResults(long token, UriInfo uriInfo, String xForwardedProto)
        {
            ...
            Optional<DispatchInfo> dispatchInfo = queryManager.getDispatchInfo(queryId);
        ...
        {
            querySubmissionFuture.addListener(() -> queryManager.cancelQuery(queryId), directExecutor());
        }
