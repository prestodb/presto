...
public class DispatcherQueryManager
{
    ...
    private <C> void createQueryInternal(QueryId queryId, String slug, SessionContext sessionContext, String query, ResourceGroupManager<C> resourceGroupManager)
    {
        ...
        try {
            ...
            try {
                onQueryCreated(failedDispatchQuery);
            }
            finally {
                onQueryFailure(failedDispatchQuery);
            }
            return;
        }
    ...
    private void onQueryCreated(DispatchQuery dispatchQuery)
    {
    ...
    private void onQueryFailure(DispatchQuery dispatchQuery)
    {
        try {
            ...
            stats.queryFailedWhileQueuedEvent(queryInfo.getQueryStats().getQueuedTime(), Optional.ofNullable(queryInfo.getErrorCode()));
        }
        finally {
