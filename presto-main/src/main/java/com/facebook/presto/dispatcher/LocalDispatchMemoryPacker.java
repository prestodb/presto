...
public class LocalDispatchMemoryPacker
{
    ...
    private final ClusterMemoryPoolManager memoryManager;
    ...
    @Inject
    public LocalDispatchMemoryPacker(ClusterMemoryPoolManager memoryManager, LocalDispatchMemoryPackerConfig config)
    {
    ...
    private synchronized long getAvailableMemoryBytes()
    {
        ...
        return (long) (memoryManager.getClusterMemoryBytes() * (1 + overcommitRatio) - (memoryManager.getClusterTotalMemoryReservation() + recentlyAllocatedBytes));
    }
    ...
    public synchronized void runIfEligible()
    {
        ...
        for (WaitingQuery waitingQuery : ImmutableSet.copyOf(waitingPreallocations.keySet())) {
            long bytesRequired = (long) Math.max(
                    waitingQuery.getEstimatedPeakMemoryBytes() * totalToUserMemoryRatio,
