        ...
        implements AccessControl
{
    ...
    public void checkCanShowTables(TransactionId transactionId, Identity identity, CatalogSchemaName schema)
    {
        ...
        if (entry != null) {
            authorizationCheck(() -> entry.getAccessControl().checkCanShowTables(entry.getTransactionHandle(transactionId), identity.toConnectorIdentity(schema.getCatalogName()), schema.getSchemaName()));
        }
    }
    ...
    public void checkCanGrantTablePrivilege(TransactionId transactionId, Identity identity, Privilege privilege, QualifiedObjectName tableName)
    {
        ...
        CatalogAccessControlEntry entry = getConnectorAccessControl(transactionId, tableName.getCatalogName());
        if (entry != null) {
            authorizationCheck(() -> entry.getAccessControl().checkCanGrantTablePrivilege(entry.getTransactionHandle(transactionId), identity.toConnectorIdentity(tableName.getCatalogName()), privilege, tableName.asSchemaTableName()));
        }
    }
    ...
    public void checkCanRevokeTablePrivilege(TransactionId transactionId, Identity identity, Privilege privilege, QualifiedObjectName tableName)
    {
        ...
        CatalogAccessControlEntry entry = getConnectorAccessControl(transactionId, tableName.getCatalogName());
        if (entry != null) {
            authorizationCheck(() -> entry.getAccessControl().checkCanRevokeTablePrivilege(entry.getTransactionHandle(transactionId), identity.toConnectorIdentity(tableName.getCatalogName()), privilege, tableName.asSchemaTableName()));
        }
    }
    ...
    public void checkCanShowGrants(TransactionId transactionId, Identity identity, QualifiedTablePrefix prefix)
    {
        ...
        if (entry != null) {
            authenticationCheck(() -> entry.getAccessControl().checkCanShowGrants(entry.getTransactionHandle(transactionId), identity.toConnectorIdentity(prefix.getCatalogName()), prefix.getCatalogName(), prefix.asSchemaTablePrefix()));
        }
    }
    ...
    public Set<GrantInfo> filterGrants(TransactionId transactionId, Identity identity, QualifiedTablePrefix prefix, Set<GrantInfo> grantInfos)
    {
        ...
        CatalogAccessControlEntry entry = getConnectorAccessControl(transactionId, prefix.getCatalogName());
        if (entry != null) {
            tableGrants = entry.getAccessControl().filterGrants(entry.getTransactionHandle(transactionId), identity.toConnectorIdentity(prefix.getCatalogName()), prefix.getCatalogName(), prefix.asSchemaTablePrefix(), grantInfos);
        }
        return tableGrants;
    }
    ...
    public void checkCanShowRoles(TransactionId transactionId, Identity identity, String catalogName)
    {
        ...
        authenticationCheck(() -> systemAccessControl.get().checkCanShowRoles(identity, catalogName));
    ...
    public Set<String> filterRoles(TransactionId transactionId, Identity identity, String catalogName, Set<String> roles)
    {
        ...
        Set<String> filteredRoles = systemAccessControl.get().filterRoles(identity, catalogName, roles);
        CatalogAccessControlEntry entry = getConnectorAccessControl(transactionId, catalogName);
        if (entry != null) {
            filteredRoles = entry.getAccessControl().filterRoles(entry.getTransactionHandle(transactionId), identity.toConnectorIdentity(catalogName), catalogName, roles);
        }
    ...
    public void checkCanShowCurrentRoles(TransactionId transactionId, Identity identity, String catalogName)
    {
        ...
        authenticationCheck(() -> systemAccessControl.get().checkCanShowCurrentRoles(identity, catalogName));
    ...
    public void checkCanShowRoleGrants(TransactionId transactionId, Identity identity, String catalogName)
    {
        ...
        authenticationCheck(() -> systemAccessControl.get().checkCanShowRoleGrants(identity, catalogName));
