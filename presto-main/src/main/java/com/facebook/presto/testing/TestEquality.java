    ...
    public static boolean equalToForTests(Type type, Block leftBlock, int leftPosition, Block rightBlock, int rightPosition)
    {
        if (type instanceof MapType) {
            ...
            for (int position = 0; position < rightMapBlock.getPositionCount(); position += 2) {
                ...
                if (!equalToForTests(mapType.getValueType(), leftMapBlock, leftValuePosition, rightMapBlock, rightValuePosition)) {
                    return false;
                }
            }
            ...
            for (int i = 0; i < leftArray.getPositionCount(); i++) {
                ...
                if (leftArray.isNull(i) ^ leftArray.isNull(i)) {
                    return false;
                }
                if (!equalToForTests(arrayType.getElementType(), leftArray, i, rightArray, i)) {
                    return false;
                }
            }
