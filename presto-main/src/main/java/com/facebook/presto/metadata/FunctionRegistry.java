...
public class FunctionRegistry
{
    ...
    public OperatorInfo resolveOperator(OperatorType operatorType, List<? extends Type> argumentTypes)
    {
        ...
        throw new IllegalArgumentException(format("Operator %s(%s) not registered", operatorType, Joiner.on(", ").join(argumentTypes)));
    }

    public OperatorInfo getOperator(OperatorType operatorType, List<? extends Type> argumentTypes, Type returnType)
    {
        Iterable<OperatorInfo> candidates = functions.getOperators(operatorType);
        ...
        throw new IllegalArgumentException(format("Operator %s(%s):%s not registered", operatorType, Joiner.on(", ").join(argumentTypes), returnType));
    }
    ...
    private static List<Type> parameterTypes(Method method)
    {
        ...
        for (int i = 0; i < method.getParameterTypes().length; i++) {
            ...
            // find the explicit type annotation is present
    ...
    public static class FunctionListBuilder
    {
        ...
        private static void checkValidMethod(Method method)
        {
            ...
            // todo add operator validation and reenable this check
