        ...
        implements DataDefinitionTask<CreateMaterializedView>
{
    ...
    public ListenableFuture<?> execute(CreateMaterializedView statement, TransactionManager transactionManager, Metadata metadata, AccessControl accessControl, QueryStateMachine stateMachine, List<Expression> parameters)
    {
        ...
        viewDefinition.setViewToBaseColumnMap(getOriginalColumnMapping(statement.getQuery(), analysis, toSchemaTableName(viewName)));
    ...
    private static Map<TableColumn, TableColumn> getOriginalColumnMapping(Node node, Analysis analysis, SchemaTableName tableName)
    {
        return analysis.getOutputDescriptor(node).getVisibleFields()
                .stream()
                .filter(field -> field.getOriginTable().isPresent() && field.getOriginColumnName().isPresent())
                .collect(toImmutableMap(
                        field -> new TableColumn(tableName, new ColumnMetadata(field.getName().get(), field.getType())),
                        field -> new TableColumn(toSchemaTableName(field.getOriginTable().get()), new ColumnMetadata(field.getOriginColumnName().get(), field.getType()))));
    }
}
