...
public class TaskHandle
{
    ...
    private int level = 0;
    @GuardedBy("this")
    private long taskScheduledNanos;
    @GuardedBy("this")
    ...
    public synchronized long addScheduledNanos(long durationNanos)
    {
        concurrencyController.update(durationNanos, utilizationSupplier.getAsDouble(), runningLeafSplits.size());
        taskScheduledNanos += durationNanos;

        level = computeLevel(taskScheduledNanos);
        levelPriority = splitQueue.getLevelMinPriority(level, taskScheduledNanos);

        levelPriority += durationNanos;
        return levelPriority;
    }

    public synchronized long updateLevelPriority()
    {
        long levelMinPriority = splitQueue.getLevelMinPriority(level, taskScheduledNanos);
        if (levelPriority < levelMinPriority) {
            levelPriority = levelMinPriority;
        }
    ...
    public synchronized long getTaskScheduledNanos()
    {
        return taskScheduledNanos;
    }
