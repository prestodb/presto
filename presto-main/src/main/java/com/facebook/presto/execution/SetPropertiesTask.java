        ...
        implements DDLDefinitionTask<SetProperties>
{
    ...
    {
        QualifiedObjectName tableName = MetadataUtil.createQualifiedObjectName(session, statement, statement.getName());
        ...
        if (statement.getType() == SetProperties.Type.TABLE) {
            Map<String, Object> properties = metadata.getTablePropertyManager().getUserSpecifiedProperties(
                    ...
                    session,
                    metadata,
                    ParameterUtils.parameterExtractor(statement, parameters)).build();
            setTableProperties(tableName, metadata, accessControl, session, properties);
        }
        else {
    ...
    private void setTableProperties(QualifiedObjectName tableName, Metadata metadata, AccessControl accessControl, Session session, Map<String, Object> properties)
    {
