...
public class ResourceManagerResource
{
    ...
    public ListenableFuture<List<ResourceGroupRuntimeInfo>> getResourceGroupInfo(@QueryParam("excludingNode") String excludingNode)
    {
        return executor.submit(() -> {
            List<ResourceGroupRuntimeInfo> result = clusterStateProvider.getClusterResourceGroups(excludingNode);

            log.info("Returning ResourceGroupRuntimeInfo list: size=%s content=%s",
