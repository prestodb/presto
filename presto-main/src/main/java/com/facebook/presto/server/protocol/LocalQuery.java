...
class LocalQuery
        implements Query
{
    private static final Logger log = Logger.get(LocalQuery.class);
    ...
    public static LocalQuery create(
            Session session,
            String slug,
    ...
    {
        LocalQuery result = new LocalQuery(session, slug, queryManager, exchangeClient, dataProcessorExecutor, timeoutExecutor, blockEncodingSerde);
    ...
    private static URI findCancelableLeafStage(QueryInfo queryInfo)
    {
        ...
        return queryInfo.getOutputStage().map(LocalQuery::findCancelableLeafStage).orElse(null);
    }
