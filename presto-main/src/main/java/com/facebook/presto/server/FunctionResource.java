...
public class FunctionResource
{
                ...
                Optional.of(
                        new SqlFunctionId(
                                new QualifiedObjectName(
                                        FUNCTION_CATALOG,
                                        function.getSignature().getName().getSchemaName(),
                                        function.getSignature().getName().getObjectName()),
                                function.getSignature().getArgumentTypes())),
                Optional.of("1"));
    }
    ...
    public static List<TypeSignatureProvider> extractArgumentTypeSignatures(String functionId)
    {
        int openBracketIndex = functionId.indexOf('[');
        int closeBracketIndex = functionId.indexOf(']', openBracketIndex);

        if (openBracketIndex == -1 || closeBracketIndex == -1 || openBracketIndex >= closeBracketIndex) {
            throw new IllegalArgumentException("Invalid functionId format: " + functionId);
        }

        String argumentTypesString = functionId.substring(openBracketIndex + 1, closeBracketIndex).trim();

        List<TypeSignatureProvider> argumentTypeSignatures = new ArrayList<>();
        if (!argumentTypesString.isEmpty()) {
            for (String argumentType : argumentTypesString.split(",")) {
                argumentTypeSignatures.add(new TypeSignatureProvider(TypeSignature.parseTypeSignature(argumentType.trim())));
            }
        }

        return argumentTypeSignatures;
    }
