...
public class TaskCreateOrUpdateResponseHandler<T>
        implements FutureCallback<BaseResponse<T>>
{
    ...
    public TaskCreateOrUpdateResponseHandler(SimpleHttpResponseCallback<T> callback, URI uri, SimpleHttpResponseHandlerStats stats, ErrorCodeSupplier errorCode)
    {
    ...
    public void onSuccess(BaseResponse<T> response)
    {
        ...
        try {
            ...
            else if (response.getStatusCode() == HttpStatus.TOO_MANY_REQUESTS.code()) {
                int retryAfter = Integer.parseInt(response.getHeader("Retry-After"));
                callback.failed(new TaskCreationBackPressureException(retryAfter));
            }
            else {
