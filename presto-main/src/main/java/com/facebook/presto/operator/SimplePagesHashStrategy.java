            ...
            boolean groupByUsesEqualTo)
    {
        ...
        for (Type type : types) {
            distinctFromMethodHandlesBuilder.add(
                    typeAndFunctionManager.getBuiltInScalarFunctionImplementation(typeAndFunctionManager.resolveOperatorHandle(IS_DISTINCT_FROM, fromTypes(type, type))).getMethodHandle());
        }
