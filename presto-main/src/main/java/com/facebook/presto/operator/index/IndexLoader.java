...
public class IndexLoader
{
    ...
    public IndexedData streamIndexDataForSingleKey(UpdateRequest updateRequest)
    {
        ...
        SimplePageSet pageSet = new SimplePageSet(keyTypes, ImmutableList.of(indexKeyTuple));
        PlanNodeId planNodeId = driverFactory.getSourceId().get();
        ScheduledSplit split = new ScheduledSplit(0, planNodeId, new Split(INDEX_CONNECTOR_ID, new ConnectorTransactionHandle() {}, new IndexSplit(pageSet)));
    ...
    private static class IndexSnapshotLoader
    {
        ...
        {
            // Generate a PageSet that only presents index keys that have not been cached and are deduped based on lookupSourceInputChannels
            UnloadedIndexPageSet lookupPageSet = new UnloadedIndexPageSet(pipelineContext.getSession(), indexSnapshotReference.get(), lookupSourceInputChannels, indexTypes, requests, joinCompiler);
            ...
            try (Driver driver = driverFactory.createDriver(pipelineContext.addDriverContext())) {
                ...
                ScheduledSplit split = new ScheduledSplit(0, sourcePlanNodeId, new Split(INDEX_CONNECTOR_ID, new ConnectorTransactionHandle() {}, new IndexSplit(lookupPageSet)));
            ...
            UnloadedIndexPageSet indexKeysPageSet = (lookupSourceInputChannels.equals(allInputChannels))
                    ? lookupPageSet
                    : new UnloadedIndexPageSet(pipelineContext.getSession(), indexSnapshotReference.get(), allInputChannels, indexTypes, requests, joinCompiler);

            // Create lookup source with new data
            IndexSnapshot newValue = indexSnapshotBuilder.createIndexSnapshot(indexKeysPageSet);
            if (newValue == null) {
                clearCachedData();
                return false;
            }
