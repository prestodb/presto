...
import static com.facebook.presto.operator.scalar.DateTimeFunctionsForFixedTimestamp.diffTimestamp;
...
import static com.facebook.presto.type.DateTimeOperatorsForFixedTimestamp.timestampPlusIntervalYearToMonth;
import static java.lang.Math.toIntExact;

public final class SequenceFunctionForFixedTimestamp
{
    private static final Slice MONTH = Slices.utf8Slice("month");

    private SequenceFunctionForFixedTimestamp() {}
            ...
            @SqlType(StandardTypes.INTERVAL_YEAR_TO_MONTH) long step)
    {
        ...
        int length = toIntExact(diffTimestamp(MONTH, start, stop) / step + 1);
        ...
        for (int i = 0; i < length; ++i) {
            BIGINT.writeLong(blockBuilder, timestampPlusIntervalYearToMonth(start, value));
            value += step;
        }
