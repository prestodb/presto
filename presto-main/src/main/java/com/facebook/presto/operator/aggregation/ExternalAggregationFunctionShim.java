...
public final class ExternalAggregationFunctionShim
{
    ...
    public static InternalAggregationFunction createInternalAggregationFunction(
            FunctionHandle functionHandle,
            Function<CatalogSchemaName, Optional<FunctionNamespaceManager<? extends SqlFunction>>> managerProvider)
    {
        requireNonNull(functionHandle);
        requireNonNull(managerProvider);

        return managerProvider.apply(functionHandle.getCatalogSchemaName())
                .map(manager -> getAggregationFunctionImplementation(manager, functionHandle))
                .map(ExternalAggregationFunctionShim::create)
                .orElseThrow(() -> new IllegalStateException("No aggregation implementation found for " + functionHandle));
    }

    private static InternalAggregationFunction create(AggregationFunctionImplementation implementation)
    {
    ...
    private static AggregationFunctionImplementation getAggregationFunctionImplementation(
            FunctionNamespaceManager<?> functionNamespaceManager,
            FunctionHandle functionHandle)
    {
        if (functionNamespaceManager instanceof AggregationFunctionImplementationProvider) {
            return ((AggregationFunctionImplementationProvider) functionNamespaceManager).getAggregationFunctionImplementation(functionHandle);
        }
        return null;
    }
}
