...
public class MultiplePageRecordSet
        implements RecordSet
{
    private final List<Type> types;
    private final List<Page> pages;

    public MultiplePageRecordSet(PageSet pageSet)
    {
        this(pageSet.getColumnTypes(), pageSet.getPages());
    }

    public MultiplePageRecordSet(List<Type> types, List<Page> pages)
    {
        this.types = ImmutableList.copyOf(requireNonNull(types, "types is null"));
        this.pages = requireNonNull(pages, "page is null");
        checkArgument(pages.stream().allMatch(page -> page.getChannelCount() == types.size()), "Types do not match page channels");
    }
    ...
    {
        return new MultiplePageRecordCursor(types, pages);
    }

    public static class MultiplePageRecordCursor
            implements RecordCursor
    {
        ...
        private MultiplePageRecordCursor(List<Type> types, List<Page> pages)
        {
            this.types = ImmutableList.copyOf(requireNonNull(types, "types is null"));
            this.pages = requireNonNull(pages, "page is null");
            checkArgument(pages.stream().allMatch(page -> page.getChannelCount() == types.size()), "Types do not match page channels");
        ...
        public boolean advanceNextPosition()
        {
            ...
            if (position >= pages.get(currentPage).getPositionCount()) {
                currentPage++;
                position = 0;
            }
