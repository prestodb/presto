...
public class LongApproximateCountDistinctAggregation
        extends SqlAggregationFunction
{
    public static final LongApproximateCountDistinctAggregation LONG_APPROXIMATE_COUNT_DISTINCT_AGGREGATION = new LongApproximateCountDistinctAggregation();
    private static final String NAME = "approx_distinct";
    private static final MethodHandle INPUT_FUNCTION = methodHandle(LongApproximateCountDistinctAggregation.class, "input", HyperLogLogState.class, long.class, double.class);
    private static final MethodHandle BLOCK_INPUT_FUNCTION = methodHandle(LongApproximateCountDistinctAggregation.class, "blockInput", HyperLogLogState.class, Block.class, Block.class);
    private static final MethodHandle COMBINE_FUNCTION = methodHandle(LongApproximateCountDistinctAggregation.class, "combine", HyperLogLogState.class, HyperLogLogState.class);
    private static final MethodHandle OUTPUT_FUNCTION = methodHandle(LongApproximateCountDistinctAggregation.class, "output", HyperLogLogState.class, BlockBuilder.class);

    public LongApproximateCountDistinctAggregation()
    {
        super(NAME,
                ImmutableList.of(),
                ImmutableList.of(),
                parseTypeSignature(StandardTypes.BIGINT),
                ImmutableList.of(parseTypeSignature(StandardTypes.BIGINT), parseTypeSignature(StandardTypes.DOUBLE)));
    }

    private static BuiltInAggregationFunctionImplementation generateAggregation(Type type)
    {
        DynamicClassLoader classLoader = new DynamicClassLoader(LongApproximateCountDistinctAggregation.class.getClassLoader());
    ...
    {
        return ImmutableList.of(new ParameterMetadata(STATE), new ParameterMetadata(INPUT_CHANNEL, type), new ParameterMetadata(INPUT_CHANNEL, DOUBLE));
    }

    private static List<ParameterMetadata> createBlockInputParameterMetadata(Type type)
    {
        return ImmutableList.of(new ParameterMetadata(STATE), new ParameterMetadata(NULLABLE_BLOCK_INPUT_CHANNEL, type), new ParameterMetadata(BLOCK_INPUT_CHANNEL, DOUBLE));
    }

    public static void input(HyperLogLogState state, long value, double maxStandardError)
    {
    ...
    public static void blockInput(HyperLogLogState state, Block value, Block maxStandardErrorBlock)
    {
        double maxStandardError = DOUBLE.getDouble(maxStandardErrorBlock, 0);
        HyperLogLog hll = HyperLogLogUtils.getOrCreateHyperLogLog(state, maxStandardError);
        state.addMemoryUsage(-hll.estimatedInMemorySize());
        if (value.mayHaveNull()) {
            for (int i = 0; i < value.getPositionCount(); ++i) {
                ...
                long hash;
                try {
                    if (DOUBLE.getDouble(maxStandardErrorBlock, i) != maxStandardError) {
                        hll = HyperLogLogUtils.getOrCreateHyperLogLog(state, DOUBLE.getDouble(maxStandardErrorBlock, i));
                    }
