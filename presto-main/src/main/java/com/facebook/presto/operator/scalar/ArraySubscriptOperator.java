        ...
        extends ParametricOperator
{
    ...
    {
        Block block = readBlock(array.getInput());
        if (index <= 0 || index > block.getPositionCount()) {
    ...
    {
        return subscript(elementType, array, index);
    }
    ...
    public static <T> T subscript(Type elementType, Slice array, long index)
    {
        if (index <= 0) {
            throw new PrestoException(INVALID_FUNCTION_ARGUMENT, "Index out of bounds");
        }
        ...
        return (T) castValue(elementType, block, position);
    }
}
