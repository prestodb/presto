...
public abstract class AbstractSamplingOperator
        implements Operator {
    private final Operator source;
    private final List<TupleInfo> tupleInfos;

    public AbstractSamplingOperator(List<TupleInfo> tupleInfos, Operator source)
    {
    ...
    public abstract static class AbstractSamplingIterator
        ...
        public AbstractSamplingIterator(Iterable<TupleInfo> tupleInfos, PageIterator pageIterator, OperatorStats operatorStats)
        {
            super(tupleInfos);
        ...
        protected Page iteratorComputeNext()
        {
            ...
            while (!operatorStats.isDone() && !pageBuilder.isFull() && pageIterator.hasNext())
            {
                ...
                samplingRowOriented(blocks, pageBuilder);
            }
        ...
        protected Page cursorComputeNext()
        {
            ...
            pageBuilder.reset();
            samplingRowOriented(cursor, pageBuilder);
            if (pageBuilder.isEmpty())
            {
                return endOfData();
            }
        ...
        protected abstract void samplingRowOriented(Block[] blocks, PageBuilder pageBuilder);

        protected abstract void samplingRowOriented(RecordCursor cursor, PageBuilder pageBuilder);
