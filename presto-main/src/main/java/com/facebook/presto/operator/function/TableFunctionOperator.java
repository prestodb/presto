...
import com.facebook.presto.operator.PageBuffer;
        ...
        implements Operator
{
    ...
    // TODO: Set PageBuffer max to 100 based on other usages. Is this correct?
            ...
            implements WorkProcessor.Transformation<Page, PagesIndex>
    {
        ...
        {
            Page resolvedInput = input.orElse(null);
            if (resetPagesIndex) {
                pagesIndex.clear();
                updateMemoryUsage();
            ...
            if ( resolvedInput == null && pagesIndex.getPositionCount() == 0) {
            ...
            if (resolvedInput != null) {
                ...
                inputPosition = appendCurrentGroup(pagesIndex, hashStrategies, resolvedInput, inputPosition);
                updateMemoryUsage();

                if (inputPosition >= resolvedInput.getPositionCount()) {
                    inputPosition = 0;
    ...
    // TODO: Changed strategies to match latest trino
