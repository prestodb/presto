 ...
 */
/**
 * Created by monicalee on 3/16/15.
 */
        ...
        extends ParametricScalar
{
    ...
    private static final Signature SIGNATURE = new Signature(FUNCTION_NAME, ImmutableList.of(typeParameter("T")), "boolean", ImmutableList.of("array<T>", "T"), false, false);
    ...
    public static boolean contains(Type type, Slice givenArray, Slice value)
    {
        Block arrayBlock = readStructuralBlock(givenArray);
    ...
    public static boolean contains(Type type, Slice givenArray, long value)
    {
        Block arrayBlock = readStructuralBlock(givenArray);
        Block valueBlock = createBlock(type, value);
        for (int i = 0; i < arrayBlock.getPositionCount(); i++) {
    ...
    public static boolean contains(Type type, Slice givenArray, boolean value)
    {
        Block arrayBlock = readStructuralBlock(givenArray);
        Block valueBlock = createBlock(type, value);
        for (int i = 0; i < arrayBlock.getPositionCount(); i++) {
    ...
    public static boolean contains(Type type, Slice givenArray, double value)
    {
        Block arrayBlock = readStructuralBlock(givenArray);
        Block valueBlock = createBlock(type, value);
        for (int i = 0; i < arrayBlock.getPositionCount(); i++) {
