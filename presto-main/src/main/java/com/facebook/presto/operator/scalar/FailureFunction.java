...
public final class FailureFunction
{
    ...
    @Description("Throws an exception with given message")
    @ScalarFunction(value = "fail", hidden = true)
    @SqlType("unknown")
    public static void fail(@SqlType(StandardTypes.VARCHAR) Slice failureMessage)
    {
        throw new PrestoException(StandardErrorCode.GENERIC_USER_ERROR, failureMessage.toStringUtf8());
    }

    @Description("Throws an exception with given error code and message")
    @ScalarFunction(value = "fail", hidden = true)
    @SqlType("unknown")
    public static void fail(
            @SqlType(StandardTypes.INTEGER) long errorCode,
            @SqlType(StandardTypes.VARCHAR) Slice failureMessage)
    {
        for (StandardErrorCode standardErrorCode : StandardErrorCode.values()) {
            if (standardErrorCode.toErrorCode().getCode() == errorCode) {
                throw new PrestoException(standardErrorCode, failureMessage.toStringUtf8());
            }
        }
        throw new PrestoException(StandardErrorCode.GENERIC_USER_ERROR, "Unable to find error for code: " + errorCode);
    }
}
