        ...
        extends StatisticalDigestAggregationFunction
{
    ...
    {
        super(NAME, TDIGEST, createTDigestFactory(), true, typeSignatures);
    }
    ...
    {
        checkArgument(weight > 0, "Weight must be > 0, was %s", weight);
        checkArgument(compression > 0, "Compression factor must be positive, was %s", compression);
    ...
    private static StatisticalDigest getOrCreateTDigest(StatisticalDigestState state, double compression)
    {
        ...
        if (digest == null) {
            digest = new StatisticalTDigest(createTDigest(compression));
