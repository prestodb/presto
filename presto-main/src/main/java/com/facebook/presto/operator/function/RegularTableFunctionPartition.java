        ...
        implements TableFunctionPartition
{
    ...
    private int[] findEndOfData(Optional<Map<Integer, Integer>> markerChannels, List<List<Integer>> requiredChannels, List<PassThroughColumnSpecification> passThroughSpecifications)
    {
        ...
        // Changed to throw PrestoException.
        int maxInputChannel = referencedChannels.stream()
                .mapToInt(Integer::intValue)
                .max()
                .orElseThrow(() -> new PrestoException(FUNCTION_IMPLEMENTATION_ERROR, "No maximum value found"));
        ...
        if (!markerChannels.isPresent() || markerChannels.get().isEmpty()) {
            referencedChannels.stream()
                    .forEach(channel -> result[channel] = partitionEnd - partitionStart);
            return result;
        }

        // Modified orElseThrow to throw PrestoException.
        ...
        for (int markerChannel : ImmutableSet.copyOf(markerChannels.orElseThrow(
                () -> new PrestoException(FUNCTION_IMPLEMENTATION_ERROR, "Channel not found")).values())) {
        ...
        referencedChannels.stream()
                .forEach(channel -> result[channel] = endOfDataPerMarker.get(markerChannels.orElseThrow(
                        () -> new PrestoException(FUNCTION_IMPLEMENTATION_ERROR, "Channel not found")).get(channel)) - partitionStart);
        return result;
    }
    ...
    // Modified to remove record. Getters added.
    public class PassThroughColumnSpecification {
    ...
    private class PartitioningColumnProvider implements PassThroughColumnProvider {
        private final Block partitioningValue;

        public PartitioningColumnProvider(Block partitioningValue) {
            this.partitioningValue = Objects.requireNonNull(partitioningValue, "partitioningValue is null");
        }
