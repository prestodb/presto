...
public class HtmlEntitiesSlice
{
    private HtmlEntitiesSlice() {}
    ...
    static final Slice[] CODEPOINTS_TO_ESCAPED_ENTITIES;

    /**
     * This block loads the html escape dictionaries and transforms them into an array
     * that is mapping a code point to an escaped slice
     */
    static {
        CODEPOINTS_TO_ESCAPED_ENTITIES = new Slice[MAX_CODE_POINT + 1];
        String[][][] dictionaries = new String[][][] {BASIC_ESCAPE,
                                                      ISO8859_1_ESCAPE,
                                                      HTML40_EXTENDED_ESCAPE};
        for (String[][] dictionary : dictionaries) {
            for (String[] keyValuePair : dictionary) {
                Slice key = Slices.wrappedBuffer(keyValuePair[0].getBytes());
                Slice value = Slices.wrappedBuffer(keyValuePair[1].getBytes());
                CODEPOINTS_TO_ESCAPED_ENTITIES[SliceUtf8.getCodePointAt(key, 0)] = value;
            }
        }
    }

    /**
     * Returns a utf8 slice that is an HTML 4 replacement for a codepoint or null if the
     * code point doesn't need to be replaced
     */
    @Nullable
    public static Slice getEscapedSliceForCodepoint(int codePoint)
    {
        return CODEPOINTS_TO_ESCAPED_ENTITIES[codePoint];
    }
}
