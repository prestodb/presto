...
/**
 * This is a fixed-bin histogram of weights breakdowns. For each bin, it stores the number of times
...
public class FixedDoubleBreakdownHistogram
        implements Cloneable, Iterable<FixedDoubleBreakdownHistogram.BucketWeight>
{
    ...
    {
        return
                SizeOf.SIZE_OF_INT + // bucketCount
                        2 * SizeOf.SIZE_OF_DOUBLE + // min, max
                        SizeOf.SIZE_OF_INT + // size
                        indices.length * (SizeOf.SIZE_OF_INT + SizeOf.SIZE_OF_DOUBLE + SizeOf.SIZE_OF_LONG);
    }
    ...
    public void serialize(SliceOutput out)
    {
        ...
        IntStream.range(0, bucketCount).forEach(i -> out.appendInt(indices[i]));
        IntStream.range(0, bucketCount).forEach(i -> out.appendDouble(weights[i]));
        IntStream.range(0, bucketCount).forEach(i -> out.appendLong(counts[i]));
    }
