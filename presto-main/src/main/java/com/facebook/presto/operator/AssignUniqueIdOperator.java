...
import com.facebook.presto.util.LongValueCompressor;
        ...
        implements Operator
{
    ...
    private final List<Type> types;
    private final LongValueCompressor valueCompressor = new LongValueCompressor();
            ...
            AtomicLong valuePool)
    {
        ...
        valueCompressor.store(fullTaskId.getStageId().getId(), 10)
                .store(fullTaskId.getId(), 14);
    ...
    {
        lastCounterValue = valuePool.addAndGet(VALUES_PER_REQUEST);
        counter = lastCounterValue - VALUES_PER_REQUEST;
    }
    ...
    private Page getRowsWithUniqueColumns()
    {
        ...
        outputBlocks[inputPageChannelCount] = createBlockWithValuesFromPool();
    ...
    private Block createBlockWithValuesFromPool()
    {
        ...
        for (int currentPosition = 0; currentPosition < inputPage.getPositionCount(); currentPosition++) {
            ...
            BIGINT.writeLong(leastSigBitsBlock, valueCompressor.build(counter++));
        }
