        ...
        implements Mergeable<WindowInfo>, OperatorInfo
{
    ...
    static class DriverWindowInfoBuilder
    {
        ...
        public synchronized DriverWindowInfo build()
        {
            ...
            long totalRowsCount = indexInfos.stream()
                    .mapToLong(index -> index.getTotalRowsCount())
                    .sum();
            ...
            double averageIndexSize = indexInfos.stream()
                    .mapToLong(index -> index.getSizeInBytes())
            ...
            double squaredDifferencesSizeInPartition = indexInfos.stream()
                    .mapToDouble(index -> index.getSumSquaredDifferences())
                    .sum();
    ...
    private static class IndexInfoBuilder
    {
        ...
        public synchronized Optional<IndexInfo> build()
        {
            ...
            double avgSize = partitions.stream().mapToLong(Integer::longValue).average().orElse(Double.NaN);
    ...
    private static class IndexInfo
    {
        private final long totalRowsCount;
        private final long sizeInBytes;
        private final double sumSquaredDifferences; // sum of (partitionSize - averagePartitionSize)^2 for each partition
        private final long numberOfPartitions;

        public IndexInfo(long totalRowsCount, long sizeInBytes, double sumSquaredDifferences, long numberOfPartitions)
        {
            this.totalRowsCount = totalRowsCount;
            this.sizeInBytes = sizeInBytes;
