        ...
        extends SqlOperator
{
    ...
    public BuiltInScalarFunctionImplementation specialize(BoundVariables boundVariables, int arity, TypeAndFunctionManager typeAndFunctionManager)
    {
        ...
        Type toMapType = typeAndFunctionManager.getParameterizedType(
                "map",
                ImmutableList.of(
                        TypeSignatureParameter.of(toKeyType.getTypeSignature()),
                        TypeSignatureParameter.of(toValueType.getTypeSignature())));

        MethodHandle keyProcessor = buildProcessor(typeAndFunctionManager, fromKeyType, toKeyType, true);
        MethodHandle valueProcessor = buildProcessor(typeAndFunctionManager, fromValueType, toValueType, false);
     ...
     */
    private MethodHandle buildProcessor(TypeAndFunctionManager typeAndFunctionManager, Type fromType, Type toType, boolean isKey)
    {
        ...
        BuiltInScalarFunctionImplementation castImplementation = typeAndFunctionManager.getBuiltInScalarFunctionImplementation(typeAndFunctionManager.lookupCast(CastType.CAST, fromType.getTypeSignature(), toType.getTypeSignature()));
