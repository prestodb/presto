...
public class UnweightedMleStateStrategy
        implements DiscreteEntropyStateStrategy
{
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(UnweightedMleStateStrategy.class).instanceSize();
    private int[] samples;
    private int[] counts;

    public UnweightedMleStateStrategy()
    {
        samples = new int[0];
        counts = new int[0];
    }

    private UnweightedMleStateStrategy(int[] samples, int[] counts)
    {
        this.samples = samples;
        this.counts = counts;
    }

    private UnweightedMleStateStrategy(UnweightedMleStateStrategy other)
    {
    ...
    {
        int size = SizeOf.SIZE_OF_INT + // length
                samples.length * (SizeOf.SIZE_OF_INT + SizeOf.SIZE_OF_INT);
        return SizeOf.SIZE_OF_INT + // length
    ...
    {
        UnweightedMleStateStrategy otherStrategy = (UnweightedMleStateStrategy) other;
        for (int i = 0; i < otherStrategy.samples.length; ++i) {
            increment(otherStrategy.samples[i], otherStrategy.counts[i]);
        }
    }

    public static UnweightedMleStateStrategy deserialize(SliceInput input)
    {
        ...
        return new UnweightedMleStateStrategy(samples, counts);
    }
    ...
    {
        return new UnweightedMleStateStrategy(this);
    }
