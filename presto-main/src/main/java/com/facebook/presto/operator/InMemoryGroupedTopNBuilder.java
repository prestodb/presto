        ...
        implements GroupedTopNBuilder
{
    ...
    private static final Logger log = Logger.get(InMemoryGroupedTopNBuilder.class);
    ...
    {
        IntIterator groupIds = hashSortedGroupIds();

        return new ResultIterator(groupIds);
    }
    ...
    {
        if (this.localUserMemoryContext != null) {
            this.localUserMemoryContext.setBytes(getEstimatedSizeInBytes());
        }
    }
}
