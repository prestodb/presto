        ...
        extends SimpleAggregationFunction
{
    ...
    protected GroupedAccumulator createGroupedAccumulator(Optional<Integer> maskChannel, Optional<Integer> sampleWeightChannel, double confidence, int valueChannel)
    {
        ...
        return new ApproximateSetGroupedAccumulator(valueChannel, maskChannel, sampleWeightChannel);
    }
    ...
    {
        checkArgument(confidence == 1.0, "confidence level must be 1.0");
        return new ApproximateSetAccumulator(valueChannel, maskChannel);
    }
            ...
            extends SimpleAccumulator
    {
        ...
        public ApproximateSetAccumulator(int valueChannel, Optional<Integer> maskChannel)
        {
            ...
            extends SimpleGroupedAccumulator
    {
        ...
        public ApproximateSetGroupedAccumulator(int valueChannel, Optional<Integer> maskChannel, Optional<Integer> sampleWeightChannel)
        {
        ...
        protected void processIntermediate(GroupByIdBlock groupIdsBlock, Block valuesBlock)
        {
            ...
            for (int position = 0; position < groupIdsBlock.getPositionCount(); position++) {
                ...
                // skip null values
                if (!intermediates.isNull()) {
                    add(groupIdsBlock.getGroupId(position), intermediates);
                }
            }
            checkState(!intermediates.advanceNextPosition());
        }
