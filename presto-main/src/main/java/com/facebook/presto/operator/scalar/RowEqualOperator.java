        ...
        extends SqlOperator
{
    ...
    public BuiltInScalarFunctionImplementation specialize(BoundVariables boundVariables, int arity, TypeAndFunctionManager typeAndFunctionManager)
    {
        ...
        return new BuiltInScalarFunctionImplementation(
                ...
                METHOD_HANDLE
                        .bindTo(type)
                        .bindTo(resolveFieldEqualOperators(type, typeAndFunctionManager)));
    }

    public static List<MethodHandle> resolveFieldEqualOperators(RowType rowType, TypeAndFunctionManager typeAndFunctionManager)
    {
        return rowType.getTypeParameters().stream()
                .map(type -> resolveEqualOperator(type, typeAndFunctionManager))
                .collect(toImmutableList());
    }

    private static MethodHandle resolveEqualOperator(Type type, TypeAndFunctionManager typeAndFunctionManager)
    {
        FunctionHandle operator = typeAndFunctionManager.resolveOperatorHandle(EQUAL, fromTypes(type, type));
        BuiltInScalarFunctionImplementation implementation = typeAndFunctionManager.getBuiltInScalarFunctionImplementation(operator);
