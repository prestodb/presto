        ...
        extends ParametricScalar
{
    ...
    public FunctionInfo specialize(Map<String, Type> types, int arity, TypeManager typeManager)
    {
        ...
        for (int i = 0; i < arity; i++) {
            ...
            actualArgumentNames.add(parameterizedTypeName("array", type.getTypeSignature()));

            Class<?> javaType = type.getJavaType();
            if (javaType.isPrimitive()) {
                builder.add(Primitives.wrap(javaType));
            }
            else {
                builder.add(javaType);
            }
        }
        ...
        Signature signature = new Signature(MAP, ImmutableList.<TypeParameter>of(), mapType.getTypeSignature(), actualArgumentNames.build(), false, false);
        List<Boolean> nullableParameters = ImmutableList.copyOf(Collections.nCopies(stackTypes.size(), false));

        return new FunctionInfo(signature, DESCRIPTION, true, METHOD_HANDLE, true, false, nullableParameters);
    }
