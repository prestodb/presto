        ...
        implements Closeable
{
    ...
    public boolean locateEndPosition(List<Type> types, List<Integer> joinChannels, int position)
    {
        while (++currentPosition < pages.peek().getPositionCount()) {
            if (comparePositions(types, joinChannels, pages.peek(), position, joinChannels, pages.peek(), currentPosition) != 0) {
                return true;
            }
        }
        return false;
