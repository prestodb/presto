...
public class TestAccessControlManager
{
    ...
    @Test(expectedExceptions = PrestoException.class, expectedExceptionsMessageRegExp = "Access Denied: Cannot access catalog connector")
            ...
            throws Exception
    {
        ...
        transaction(transactionManager, accessControlManager)
                .execute(transactionId -> {
                    accessControlManager.checkCanAccessCatalog(transactionId, new Identity(USER_NAME, Optional.of(PRINCIPAL)), "connector");
                });
    }

    @Test(expectedExceptions = PrestoException.class, expectedExceptionsMessageRegExp = "Access Denied: Cannot access catalog secured_catalog")
            ...
            throws Exception
    {
        ...
        registerBogusConnector(catalogManager, transactionManager, "connector");
        accessControlManager.addCatalogAccessControl(new ConnectorId("connector"), new AllowConnectorAccessControl());

        transaction(transactionManager, accessControlManager)
                .execute(transactionId -> {
                    accessControlManager.checkCanAccessCatalog(transactionId, new Identity(USER_NAME, Optional.of(PRINCIPAL)), "secured_catalog");
                });
    }
    ...
    public void testShowCatalog()
    {
        ...
        registerBogusConnector(catalogManager, transactionManager, "safe_catalog");
        registerBogusConnector(catalogManager, transactionManager, "secured_catalog");
        registerBogusConnector(catalogManager, transactionManager, "another_secured_catalog");
        accessControlManager.addCatalogAccessControl(new ConnectorId("safe_catalog"), new AllowConnectorAccessControl());
        accessControlManager.addCatalogAccessControl(new ConnectorId("secured_catalog"), new AllowConnectorAccessControl());
        accessControlManager.addCatalogAccessControl(new ConnectorId("another_secured_catalog"), new DenyConnectorAccessControl());
        ...
        transaction(transactionManager, accessControlManager)
                .execute(transactionId -> {
                    Session session = Session.builder(new SessionPropertyManager())
                            ...
                            .setIdentity(new Identity(USER_NAME, Optional.of(PRINCIPAL))).build();
                    Set<String> safe = metadata.getCatalogNames(session).keySet();
                    assertEquals(safe, ImmutableSet.of("safe_catalog"));

                    Set<String> all = transactionManager.getCatalogNames(transactionId).keySet();
