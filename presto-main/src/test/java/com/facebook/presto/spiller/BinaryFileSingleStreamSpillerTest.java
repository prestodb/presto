...
public class BinaryFileSingleStreamSpillerTest
{
    ...
    public void testFileSpiller()
            throws Exception
    {
        try (SingleStreamSpiller spiller = factory.createSingleStreamSpiller(TYPES)) {
    ...
    private void testSimpleSpiller(SingleStreamSpiller spiller)
            throws ExecutionException, InterruptedException
    {
        ...
        testSpiller(TYPES, spiller, firstSpill, secondSpill.toArray(new Page[0]));
    }

    private void testSpiller(List<Type> types, SingleStreamSpiller spiller, List<Page> initialSpill, Page... pageSpills)
            throws ExecutionException, InterruptedException
    {
        ImmutableList.Builder<Page> expectedPagesBuilder = ImmutableList.builder();
        expectedPagesBuilder.addAll(initialSpill);

        spiller.spill(initialSpill.iterator()).get();

        for (Page page : pageSpills) {
            spiller.spill(page).get();
            expectedPagesBuilder.add(page);
        }

        List<Page> expectedPages = expectedPagesBuilder.build();
        List<Page> actualSpilledPages = ImmutableList.copyOf(spiller.getSpilledPages());

        assertEquals(actualSpilledPages.size(), expectedPages.size());

        for (int i = 0; i < actualSpilledPages.size(); i++) {
            assertPageEquals(types, actualSpilledPages.get(i), expectedPages.get(i));
        }
    }
}
