        ...
        extends AbstractTestFunctions
{
    ...
    public void setUp()
    {
        ...
        legacyJsonCastEnabled = new FunctionAssertions(session, featuresConfig, true);
    }
    ...
    public void testJsonToRow()
    {
        ...
        // without legacy json cast, we would reserve the case of field name in json when casting to row type
        assertFunction("CAST(json_extract('{\"1\":[{\"name\": \"John\", \"AGE\": 30}]}', '$') AS MAP<bigint, ARRAY<ROW(name VARCHAR, \"AGE\" VARCHAR)>>)",
                mapType(BIGINT, new ArrayType(RowType.from(ImmutableList.of(
                        RowType.field("name", VARCHAR),
        ...
        assertFunction("CAST(json_extract('{\"1\":[{\"key1\": \"John\", \"KEY1\": \"Johnny\"}]}', '$') AS MAP<bigint, ARRAY<ROW(\"key1\" VARCHAR, \"KEY1\" VARCHAR)>>)",
                mapType(BIGINT, new ArrayType(RowType.from(ImmutableList.of(
        ...
        assertInvalidCast("CAST(json_extract('{\"1\":[{\"key1\": \"John\", \"KEY1\": \"Johnny\"}]}', '$') AS MAP<bigint, ARRAY<ROW(KEY1 VARCHAR, \"key1\" VARCHAR)>>)",
    ...
    public void testRowCast()
    {
        ...
        assertNotSupported("CAST(ROW(1, 2) AS ROW(a BIGINT, A DOUBLE)).a", "Duplicate field: a");
        assertNotSupported("CAST(ROW(1, 2) AS ROW(KEY1 VARCHAR, \"key1\" VARCHAR))", "Duplicate field: key1");
        assertNotSupported("TYPEOF(CAST(row(1, 2) AS ROW(KEY1 VARCHAR, \"key1\" VARCHAR)))", "Duplicate field: key1");
        assertNotSupported("CAST(ROW(1, 2) AS ROW(KEY1 VARCHAR, \"key1\" VARCHAR)).key1", "Duplicate field: key1");
