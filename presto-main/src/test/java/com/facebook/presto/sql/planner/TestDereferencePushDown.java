        ...
        extends BasePlanTest
{
    ...
    @Test
    public void testDereferencePushdownAggregation()
    {
        Session session = Session.builder(getQueryRunner().getDefaultSession()).setSystemProperty("legacy_row_field_ordinal_access", "true").build();
        // java.lang.IllegalStateException: Node ROW ("field".x, "field".y) is not supported
//        assertPlanWithSession("WITH t(msg) AS (SELECT * FROM (VALUES ROW(CAST(ROW(1, 2.0, 3) AS ROW(x BIGINT, y DOUBLE, z BIGINT))))) " +
//                        "SELECT r.field0 from (select max_by(ROW(msg.x, msg.y), msg.z) as r  from t)", session, false,
//                anyTree(
//                        project(ImmutableMap.of("field0", expression("xxx.field0")),
//                                aggregation(ImmutableMap.of("xxx", functionCall("MAX_BY", ImmutableList.of("a", "b"))),
//                                        project(ImmutableMap.of("a", expression("ROW(msg.x, msg.y)"), "b", expression("msg.z")),
//                                                values("msg"))))));
    }
