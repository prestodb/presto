        ...
        extends AstVisitor<Boolean, RowExpression>
{
    ...
    protected Boolean visitComparisonExpression(ComparisonExpression expected, RowExpression actual)
    {
        if (actual instanceof CallExpression) {
            FunctionMetadata functionMetadata = metadata.getTypeAndFunctionManager().getFunctionMetadata(((CallExpression) actual).getFunctionHandle());
    ...
    protected Boolean visitFunctionCall(FunctionCall expected, RowExpression actual)
    {
        ...
        if (!expected.getName().getSuffix().equals(metadata.getTypeAndFunctionManager().getFunctionMetadata(actualFunction.getFunctionHandle()).getName().getFunctionName())) {
            return false;
        }
