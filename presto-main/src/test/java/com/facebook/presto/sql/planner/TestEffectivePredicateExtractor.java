...
public class TestEffectivePredicateExtractor
{
        ...
        PlanNode node = new AggregationNode(newId(),
                filter(baseTableScan,
                        and(
                                ...
                                equals(EV, FV))),
                ImmutableMap.of(
                        CV, count(metadata.getTypeAndFunctionManager()),
                        DV, count(metadata.getTypeAndFunctionManager())),
    ...
    public void testFilter()
    {
        PlanNode node = filter(baseTableScan,
                and(
                        greaterThan(AV, call(metadata.getTypeAndFunctionManager(), "rand", DOUBLE, ImmutableList.of())),
    ...
    private RowExpression compare(OperatorType type, RowExpression left, RowExpression right)
    {
        return call(
                ...
                metadata.getTypeAndFunctionManager().resolveOperatorHandle(type, fromTypes(left.getType(), right.getType())),
                BOOLEAN,
                left,
                right);
    }
