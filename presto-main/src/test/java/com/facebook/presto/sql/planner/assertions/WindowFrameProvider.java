        ...
        implements ExpectedValueProvider<WindowNode.Frame>
{
    ...
    public WindowNode.Frame getExpectedValue(SymbolAliases aliases)
    {
        ...
        return new WindowNode.Frame(
                ...
                //startValue.map(alias -> new VariableReferenceExpression(alias.toSymbol(aliases).getName(), startValueType.orElseGet(() -> BIGINT))),
                toVariableReferenceExpression(aliases, sortKeyForStartComparison, sortKeyForStartComparisonType),
                //sortKeyForStartComparison.map(alias -> new VariableReferenceExpression(alias.toSymbol(aliases).getName(), sortKeyForStartComparisonType.orElseGet(() -> BIGINT))),
                endType,
                toVariableReferenceExpression(aliases, endValue, endValueType),
                //endValue.map(alias -> new VariableReferenceExpression(alias.toSymbol(aliases).getName(), endValueType.orElseGet(() -> BIGINT))),
                toVariableReferenceExpression(aliases, sortKeyForEndComparison, sortKeyForEndComparisonType),
                //sortKeyForEndComparison.map(alias -> new VariableReferenceExpression(alias.toSymbol(aliases).getName(), sortKeyForEndComparisonType.orElseGet(() -> BIGINT))),
    ...
    private Optional<VariableReferenceExpression> toVariableReferenceExpression(SymbolAliases aliases, Optional<SymbolAlias> symbolAlias, Optional<Type> type)
    {
        ...
        if (alias.startsWith("field")) {
            return Optional.of(new VariableReferenceExpression(symbolAlias.get().toString(), variableType));
        }

        return Optional.of(new VariableReferenceExpression(symbolAlias.get().toSymbol(aliases).getName(), variableType));
    }
