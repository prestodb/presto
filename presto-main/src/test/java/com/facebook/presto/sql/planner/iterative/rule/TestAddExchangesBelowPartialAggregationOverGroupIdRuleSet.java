        ...
        extends BaseRuleTest
{
    ...
    {
        testAddExchangesWithoutProjection(1000, 10_000, 1_000_000, "groupingKey3");
        testAddExchangesWithoutProjection(1000, 2_000_000, 1_000_000, "groupingKey2");
        testAddExchangesWithoutProjection(1000, 1000, 1000, "groupingKey1");
    }

    private void testAddExchangesWithoutProjection(double groupingKey1NDV, double groupingKey2NDV, double groupingKey3NDV, String expectedRepartitionSymbol)
    {
        getRuleAssert(groupingKey1NDV, groupingKey2NDV, groupingKey3NDV)
                .matches(exchange(
                        REMOTE_STREAMING,
                        GATHER,
                        aggregation(
    ...
    @Test
    public void testDoesNotFireIfAnySourceSymbolIsMissingStats()
    {
        getRuleAssert(Double.NaN, Double.NaN, Double.NaN).doesNotFire();
        getRuleAssert(1000, 10_000, Double.NaN).doesNotFire();
        getRuleAssert(1000, Double.NaN, 10_000).doesNotFire();
        getRuleAssert(1000, 10_000, Double.NaN).doesNotFire();
    }

    private RuleAssert getRuleAssert(double groupingKey1NDV, double groupingKey2NDV, double groupingKey3NDV)
    {
        RuleTester ruleTester = tester();
        String groupIdSourceId = "groupIdSourceId";
        return ruleTester.assertThat(belowExchangeRule(ruleTester))
