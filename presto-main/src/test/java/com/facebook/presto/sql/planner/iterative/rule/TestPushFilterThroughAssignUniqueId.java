...
public class TestPushFilterThroughAssignUniqueId
{
    private final RuleTester tester = new RuleTester();
    private final Rule rule = new PushFilterThroughAssignUniqueId();

    @Test
    public void testDoesNotFire()
    {
        tester.assertThat(rule)
                .on(p -> p.values(p.symbol("a", BOOLEAN)))
                .doesNotFire();

        tester.assertThat(rule)
                .on(p ->
                        p.filter(new SymbolReference("uid"),
            ...
            throws Exception
    {
        tester.assertThat(rule)
                .on(p ->
                        p.filter(new SymbolReference("a"),
                                p.assignUniqueId(p.symbol("uid", BOOLEAN),
                                        p.values(p.symbol("a", BOOLEAN)))))
        ...
        tester.assertThat(rule)
                .on(p ->
                        p.filter(and(new SymbolReference("a"), new SymbolReference("uid")),
                                p.assignUniqueId(p.symbol("uid", BOOLEAN),
                                        p.values(p.symbol("a", BOOLEAN)))))
                .matches(
                        filter("uid",
