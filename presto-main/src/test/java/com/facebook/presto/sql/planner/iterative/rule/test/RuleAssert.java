...
public class RuleAssert
{
    ...
    public void doesNotFire()
    {
        ...
        if (ruleApplication.wasRuleApplied()) {
            fail(String.format(
                    ...
                    inTransaction(session -> textLogicalPlan(plan, ruleApplication.types, metadata.getFunctionManager(), ruleApplication.statsProvider, node -> UNKNOWN_COST, session, 2))));
        }
    }
    ...
    {
        return inTransaction(session -> textLogicalPlan(plan, types, metadata.getFunctionManager(), statsCalculator, costCalculator, session, 2));
    }
