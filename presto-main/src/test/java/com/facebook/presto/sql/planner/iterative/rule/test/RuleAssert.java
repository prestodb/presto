...
public class RuleAssert
{
    ...
    public void doesNotFire()
    {
        ...
        if (ruleApplication.wasRuleApplied()) {
            fail(String.format(
                    ...
                    inTransaction(session -> textLogicalPlan(plan, ruleApplication.types, metadata.getFunctionRegistry(), StatsAndCosts.empty(), session, "    "))));
        }
    }
    ...
    private String formatPlan(PlanNode plan, TypeProvider types)
    {
        ...
        return inTransaction(session -> textLogicalPlan(plan, types, metadata.getFunctionRegistry(), StatsAndCosts.create(plan, statsProvider, costProvider), session, "    ", false));
    }
