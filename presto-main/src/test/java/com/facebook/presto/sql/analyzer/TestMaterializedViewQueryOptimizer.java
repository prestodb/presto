        ...
        extends AbstractAnalyzerTest
{
    ...
    {
        String originalViewSql = format("SELECT %s.a, %s.b FROM %s JOIN %s ON %s.c = %s.c",
        ...
        originalViewSql = format("SELECT a, b, c FROM %s", BASE_TABLE_1);
        baseQuerySql = format("SELECT %s.a, %s.b FROM %s JOIN %s ON %s.c = %s.c",
                BASE_TABLE_1, BASE_TABLE_2, BASE_TABLE_1, BASE_TABLE_2, BASE_TABLE_1, BASE_TABLE_2);
    ...
    // Valid case that does not work
    // DNF conversion of (A&~B) is invalid due to exponential explosion of sub-expressions
    @Test(enabled = false)
