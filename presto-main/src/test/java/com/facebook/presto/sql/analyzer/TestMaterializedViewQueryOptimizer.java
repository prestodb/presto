...
public class TestMaterializedViewQueryOptimizer
{
    ...
    {
        // Definition of materialized view
        String originalViewSql = format("SELECT a as mv_a, b, c as mv_c, d FROM %s", BASE_TABLE_1);
        String baseQuerySql = format("SELECT a as result_a, b as result_b, c, d FROM %s", BASE_TABLE_1);
        String expectedRewrittenSql = format("SELECT mv_a as result_a, b as result_b, mv_c, d FROM %s", VIEW);
    ...
    public void testWithAllColumnsSelect()
    {
        ...
        String expectedRewrittenSql = format("SELECT * FROM %s", VIEW);
    ...
    {
        // Definition of materialized view
        String originalViewSql = format("SELECT SUM(a * b + c) as mv_sum, MAX(a * b + c) as mv_max, d as mv_d, e FROM %s GROUP BY d, e", BASE_TABLE_1);
        String baseQuerySql = format("SELECT SUM(a * b + c) as sum_of_abc, MAX(a * b + c) as max_of_abc, d, e FROM %s GROUP BY d, e", BASE_TABLE_1);
        String expectedRewrittenSql = format("SELECT SUM(mv_sum) as sum_of_abc, MAX(mv_max) as max_of_abc, mv_d, e FROM %s GROUP BY mv_d, e", VIEW);
    ...
    public void testWithArithmeticBinaryWithAlias()
    {
        // Definition of materialized view
        String originalViewSql = format("SELECT a as mv_a, b, c as mv_c, d FROM %s", BASE_TABLE_1);
        String baseQuerySql = format("SELECT a + b, c / d, a * c - b * d FROM %s", BASE_TABLE_1);
        String expectedRewrittenSql = format("SELECT mv_a + b, mv_c / d, mv_a * mv_c - b * d FROM %s", VIEW);
    ...
    {
        // Definition of materialized view
        String originalViewSql = format("SELECT a as mv_a, b, c, d as mv_d FROM %s", BASE_TABLE_1);
        String baseQuerySql = format("SELECT a, b FROM %s WHERE a < 10 AND c > 10 or d = '2000-01-01'", BASE_TABLE_1);
        String expectedRewrittenSql = format("SELECT mv_a, b FROM %s WHERE mv_a < 10 AND c > 10 or mv_d = '2000-01-01'", VIEW);
    ...
    public void testWithOrderByWithAlias()
    {
        ...
        String originalViewSql = format("SELECT a as mv_a, b, c as mv_c FROM %s", BASE_TABLE_1);
        String baseQuerySql = format("SELECT a, b, c FROM %s ORDER BY c ASC, b DESC, a", BASE_TABLE_1);
        String expectedRewrittenSql = format("SELECT mv_a, b, mv_c FROM %s ORDER BY mv_c ASC, b DESC, mv_a", VIEW);
    ...
    public void testWithFunctionCallInOrderBy()
    {
        ...
        String baseQuerySql = format("SELECT MAX(a), b FROM %s GROUP BY b ORDER BY MAX(a) DESC, b ASC", BASE_TABLE_1);
        String expectedRewrittenSql = format("SELECT MAX(mv_max_a), b FROM %s GROUP BY b ORDER BY MAX(mv_max_a) DESC, b ASC", VIEW);
    ...
    {
        // Definition of materialized view
        String originalViewSql = format("SELECT MAX(id) as mv_max_id, country FROM %s GROUP BY country", BASE_TABLE_1);
        String baseQuerySql = format("SELECT MAX(id), country " +
                "FROM %s " +
                "WHERE country <> 'USA' " +
                "GROUP BY country " +
                "HAVING MAX(id) <=15 " +
                "ORDER BY MAX(id) DESC", BASE_TABLE_1);
        String expectedRewrittenSql = format("SELECT MAX(mv_max_id), country " +
                "FROM %s " +
                "WHERE country <> 'USA' " +
                "GROUP BY country " +
    ...
    public void testWithNoMatchingMaterializedView()
    {
        ...
        String baseQuerySql = format("SELECT a, b FROM %s", BASE_TABLE_1);
        String expectedRewrittenSql = baseQuerySql;
    ...
    @Test
    public void testWithNoMatchingWhereClause()
    {
        // Definition of materialized view
    ...
    @Test
    public void testWithNoWhereClauseInBaseQuery()
    {
        // Definition of materialized view
    ...
    @Test
    public void testWithNoMatchingColumnNameInWhereClause()
    {
        // Definition of materialized view
        String originalViewSql = format("SELECT a, b, c FROM %s", BASE_TABLE_1);
        String baseQuerySql = format("SELECT a, c FROM %s WHERE d = 5", BASE_TABLE_1);
        String expectedRewrittenSql = baseQuerySql;
    ...
    // Currently the rewrite optimizer do not handle cases that contain table alias (in either view definition or base query)
    // TODO: Handle table alias rewrite for view definition and base query.
    @Test
