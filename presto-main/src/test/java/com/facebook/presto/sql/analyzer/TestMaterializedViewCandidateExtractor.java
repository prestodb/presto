...
public class TestMaterializedViewCandidateExtractor
{
    ...
    public void testWithClause()
    {
        String baseTable1 = "base_table_v1v2";
        String baseTable2 = "base_table_v2v3";
        String baseQuerySql = format("WITH with_subquery AS " +
                        "(SELECT sum(x) AS sum_x, y FROM %s GROUP BY y) " +
                        "SELECT with_subquery.sum_x, subquery.min_z, subquery.y FROM " +
                        "with_subquery " +
                        "INNER JOIN " +
                        "(SELECT min(z) AS min_z, y FROM %s GROUP BY y) subquery " +
                        "ON subquery.y = with_subquery.y",
                baseTable1,
                baseTable2);
        assertCandidateMaterializedView(
                baseQuerySql,
