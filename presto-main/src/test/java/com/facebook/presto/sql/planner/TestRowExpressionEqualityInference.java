...
public class TestRowExpressionEqualityInference
{
    ...
    private static final StandardExpressions STANDARD_EXPRESSIONS = new StandardExpressions(METADATA.getFunctionManager());
    ...
    {
        return STANDARD_EXPRESSIONS.compare(OperatorType.GREATER_THAN, expression1, expression2);
    }

    private static RowExpression add(String symbol1, String symbol2)
    {
        return STANDARD_EXPRESSIONS.arithmetic(ADD, variable(symbol1), variable(symbol2));
    }

    private static RowExpression add(RowExpression expression1, RowExpression expression2)
    {
        return STANDARD_EXPRESSIONS.arithmetic(ADD, expression1, expression2);
    }

    private static RowExpression multiply(String symbol1, String symbol2)
    {
        return STANDARD_EXPRESSIONS.arithmetic(MULTIPLY, variable(symbol1), variable(symbol2));
    }

    private static RowExpression multiply(RowExpression expression1, RowExpression expression2)
    {
        return STANDARD_EXPRESSIONS.arithmetic(MULTIPLY, expression1, expression2);
    }

    private static RowExpression equals(String symbol1, String symbol2)
    {
        return STANDARD_EXPRESSIONS.compare(OperatorType.EQUAL, variable(symbol1), variable(symbol2));
    }

    private static RowExpression equals(RowExpression expression1, RowExpression expression2)
    {
        return STANDARD_EXPRESSIONS.compare(OperatorType.EQUAL, expression1, expression2);
    }
    ...
    {
        Preconditions.checkArgument(STANDARD_EXPRESSIONS.isComparison(expression));
        Preconditions.checkArgument(STANDARD_EXPRESSIONS.isOperation(expression, OperatorType.EQUAL));
