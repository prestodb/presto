        ...
        implements Matcher
{
    ...
    private static Map<Symbol, Symbol> extractDynamicFilters(FilterNode filterNode, String expectedSourceId)
    {
        ...
        List<DynamicFilterExpression> dynamicFilters = conjuncts.stream()
        ...
        for (DynamicFilterExpression comparison : dynamicFilters) {
            Symbol left = Symbol.from(comparison.getProbeExpression());
            if (!comparison.getSourceId().equals(expectedSourceId)) {
                return null;
            }

            mappings.put(left, new Symbol(comparison.getDynamicFilterSymbol()));
        }
    ...
    {
        return node.getDynamicFilterSource().getDynamicFilterAssignments()
                .getMap()
                .entrySet()
                .stream()
