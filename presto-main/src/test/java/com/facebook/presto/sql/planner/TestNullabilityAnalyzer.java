...
public class TestNullabilityAnalyzer
{
    ...
    void test()
    {
        // TODO following tests will succeed in RowExpression but will fail in Expression.
...
//        assertNullability("CAST('123' AS VARCHAR)", false);
    ...
    private void assertNullability(String expression, boolean canReturnNullForNotNullInput)
    {
        ...
        Expression desugaredExpression = new DesugarExpressions(TYPES.allVariables()).rewrite(rawExpression);
        RowExpression rowExpression = TRANSLATOR.translate(desugaredExpression, TYPES);
        assertEquals(NullabilityAnalyzer.mayReturnNullOnNonNullInput(rawExpression), canReturnNullForNotNullInput);
        assertEquals(analyzer.mayReturnNullOnNonNullInput(rowExpression), canReturnNullForNotNullInput);
    }
}
