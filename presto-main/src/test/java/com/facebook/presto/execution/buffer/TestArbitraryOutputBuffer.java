...
public class TestArbitraryOutputBuffer
{
            ...
            throws Exception
    {
        ...
        buffer.get(FIRST, 1, sizeOfPages(10)).cancel(true);
        // pages now acknowledged
        assertQueueState(buffer, FIRST, 0, 3);

        // fill the buffer, so that it has 3 pages
        for (int i = 3; i < 13; i++) {
            addPage(buffer, createPage(i));
        }
        ...
        // remove a page
        assertBufferResultEquals(TYPES, getBufferResult(buffer, FIRST, 1, sizeOfPages(1), NO_WAIT), bufferResult(1, createPage(3)));
        ...
        buffer.setOutputBuffers(outputBuffers);
        assertQueueState(buffer, SECOND, 11, 3);
        ...
        buffer.get(SECOND, 1, sizeOfPages(10)).cancel(true);
        assertQueueState(buffer, SECOND, 1, 13);
        ...
        assertQueueState(buffer, FIRST, 1, 13);
        assertQueueState(buffer, SECOND, 1, 13);
        ...
        assertQueueState(buffer, FIRST, 4, 13);
        assertQueueState(buffer, SECOND, 4, 13);

        // pull one page from the second buffer; it excludes the page which is still not acknowledged by the first buffer
        assertBufferResultEquals(TYPES, getBufferResult(buffer, SECOND, 1, sizeOfPages(1), NO_WAIT), bufferResult(1, createPage(14)));
        assertQueueState(buffer, FIRST, 4, 13);
        assertQueueState(buffer, SECOND, 4, 13);

        // acknowledge the page in the first buffer and pull remaining ones
        assertBufferResultEquals(TYPES, getBufferResult(buffer, FIRST, 2, sizeOfPages(10), NO_WAIT), bufferResult(2, createPage(15), createPage(16)));
        assertQueueState(buffer, FIRST, 3, 14);
        assertQueueState(buffer, SECOND, 3, 14);
        ...
        // acknowledge a page from the first buffer, buffer should not be closed
        assertBufferResultEquals(TYPES, getBufferResult(buffer, FIRST, 3, sizeOfPages(10), NO_WAIT), emptyResults(TASK_INSTANCE_ID, 3, true));
        assertQueueState(buffer, FIRST, 1, 16);
        assertQueueState(buffer, SECOND, 1, 16);
        assertFalse(buffer.isFinished());
        ...
        // acknowledge a page from the second queue, should be finished
        assertBufferResultEquals(TYPES, getBufferResult(buffer, SECOND, 2, sizeOfPages(1), NO_WAIT), emptyResults(TASK_INSTANCE_ID, 2, true));
        assertQueueState(buffer, SECOND, 0, 17);
        buffer.abort(SECOND);
        assertQueueClosed(buffer, FIRST, 17);
        assertQueueClosed(buffer, SECOND, 17);
        assertFinished(buffer);

        assertBufferResultEquals(TYPES, getBufferResult(buffer, FIRST, 3, sizeOfPages(10), NO_WAIT), emptyResults(TASK_INSTANCE_ID, 3, true));
        assertBufferResultEquals(TYPES, getBufferResult(buffer, SECOND, 2, sizeOfPages(10), NO_WAIT), emptyResults(TASK_INSTANCE_ID, 2, true));
    }
            ...
            throws Exception
    {
        ...
        assertBufferResultEquals(TYPES, getBufferResult(buffer, FIRST, 1, sizeOfPages(1), NO_WAIT), emptyResults(TASK_INSTANCE_ID, 1, true));
        ...
        buffer.abort(SECOND);
        assertQueueClosed(buffer, SECOND, 1);
        assertFinished(buffer);
        assertBufferResultEquals(TYPES, getBufferResult(buffer, SECOND, 1, sizeOfPages(1), NO_WAIT), emptyResults(TASK_INSTANCE_ID, 1, true));
    }
            ...
            throws Exception
    {
        ...
        assertBufferResultEquals(TYPES, getBufferResult(buffer, FIRST, 2, sizeOfPages(100), NO_WAIT),
                bufferResult(2, createPage(2), createPage(3), createPage(4), createPage(5), createPage(6)));
        assertBufferResultEquals(TYPES, getBufferResult(buffer, FIRST, 3, sizeOfPages(100), NO_WAIT), emptyResults(TASK_INSTANCE_ID, 3, true));
    ...
    public void testBufferCompletion()
            throws Exception
    {
        ArbitraryOutputBuffer sharedBuffer = createArbitraryBuffer(createInitialEmptyOutputBuffers(ARBITRARY), sizeOfPages(5));
        sharedBuffer.setOutputBuffers(createInitialEmptyOutputBuffers(ARBITRARY)
                .withBuffer(FIRST, 0)
                .withNoMoreBufferIds());
        ...
        assertBufferResultEquals(TYPES, getBufferResult(sharedBuffer, FIRST, 0, sizeOfPages(5), MAX_WAIT), bufferResult(0, pages));
    ...
    private static void assertQueueClosed(OutputBuffer buffer, OutputBufferId bufferId, int pagesSent)
    {
    ...
    public static BufferResult bufferResult(long token, List<Page> pages)
    {
        ...
        return new BufferResult(TASK_INSTANCE_ID, token, token + 1, false, pages);
    }
