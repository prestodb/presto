...
public class TestLocalTask
{
            ...
            throws Exception
    {
        ...
        LocalTaskFactory localTaskFactory = createLocalTaskFactory(testingTaskResource);
        LocalTask localTask = createLocalTask(localTaskFactory);

        testingTaskResource.setInitialTaskInfo(localTask.getTaskInfo());
        localTask.start();
    }
            ...
            throws Exception
    {
        ...
        assertTrue(localTask.getTaskInfo().getTaskStatus().getState().isDone(), format("TaskStatus is not in a done state: %s", localTask.getTaskInfo().getTaskStatus()));

        ErrorCode actualErrorCode = getOnlyElement(localTask.getTaskInfo().getTaskStatus().getFailures()).getErrorCode();
        switch (failureScenario) {
            case TASK_MISMATCH:
            case TASK_MISMATCH_WHEN_VERSION_IS_HIGH:
                assertTrue(localTask.getTaskInfo().getTaskStatus().getState().isDone(), format("TaskInfo is not in a done state: %s", localTask.getTaskInfo()));
    ...
    private LocalTask createLocalTask(LocalTaskFactory localTaskFactory)
    {
        return localTaskFactory.createLocalTask(
                TEST_SESSION,
    ...
    private static LocalTaskFactory createLocalTaskFactory(TestingTaskResource testingTaskResource)
            throws Exception
    {
        Bootstrap app = new Bootstrap(
                ...
                new Module()
                {
                    ...
                    @Provides
                    private LocalTaskFactory createLocalTaskFactory(
        ...
        return injector.getInstance(LocalTaskFactory.class);
    }
