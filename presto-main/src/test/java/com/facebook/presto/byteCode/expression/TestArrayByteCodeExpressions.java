...
public class TestArrayByteCodeExpressions
{
    ...
    public void setUp()
            throws Exception
    {
        for (Class<?> aClass : ImmutableList.of(boolean[].class, float[].class, double[].class, int[].class, long[].class, String[].class)) {
            ...
            throws Exception
    {
        ...
        assertByteCodeExpression(newArray(type(int[].class), 5).length(), 5, "new int[]{length = 5}.length");

        assertByteCodeExpressionType(newArray(type(boolean[].class), 5), type(boolean[].class));
        assertByteCodeExpression(newArray(type(boolean[].class), 5).length(), 5, "new boolean[]{length = 5}.length");

        assertByteCodeExpressionType(newArray(type(float[].class), 5), type(float[].class));
        assertByteCodeExpression(newArray(type(float[].class), 5).length(), 5, "new float[]{length = 5}.length");

        assertByteCodeExpressionType(newArray(type(double[].class), 5), type(double[].class));
        assertByteCodeExpression(newArray(type(double[].class), 5).length(), 5, "new double[]{length = 5}.length");
            ...
            throws Exception
    {
        ...
        assertByteCodeExpression(stringArray.getElement(constantInt(0)), "foo", "\"foo bar baz\".split(\" \")[0]");
        assertByteCodeExpression(stringArray.getElement(constantInt(1)), "bar", "\"foo bar baz\".split(\" \")[1]");
        assertByteCodeExpression(stringArray.getElement(constantInt(2)), "baz", "\"foo bar baz\".split(\" \")[2]");
