...
public class TestHashSemiJoinOperator
{
    ...
    public void testSemiJoin(boolean hashEnabled)
    {
        ...
        // expected
        MaterializedResult expected = resultBuilder(driverContext.getSession(), Stream.concat(probeTypes.stream(), ImmutableList.of(BOOLEAN).stream()).collect(toImmutableList()))
                .row(30L, 0L, true)
                .row(31L, 1L, false)
                .row(32L, 2L, false)
                .row(33L, 3L, false)
    ...
    public void testBuildSideNulls(boolean hashEnabled)
    {
        ...
        // expected
        MaterializedResult expected = resultBuilder(driverContext.getSession(),
                Stream.concat(probeTypes.stream(), ImmutableList.of(BOOLEAN).stream()).collect(toImmutableList()))
                .row(1L, true)
                .row(2L, true)
                .row(3L, true)
                .row(4L, null)
