        ...
        extends AbstractTestFunctions
{
    ...
    public void testAnnotations()
    {
        ...
        for (SqlScalarFunction function : sqlScalarFunctions) {
            ...
            Signature signature = ScalarStatsAnnotationProcessor.eraseTypeBounds(parametricScalar.getSignature());
            Map<Signature, ScalarFunctionStatsHeader> scalarStatsHeaderMap = parametricScalar.getScalarHeader().getSignatureToScalarFunctionStatsHeadersMap();
            ScalarFunctionStatsHeader scalarFunctionStatsHeader = scalarStatsHeaderMap.get(signature);
            assertEquals(scalarFunctionStatsHeader.getAvgRowSize(), 2);
            if (function.getSignature().getName().toString().equals("fun2")) {
                assertEquals(scalarFunctionStatsHeader.getDistinctValuesCount(), 2);
                Map<Integer, ScalarPropagateSourceStats> argumentStatsActual = scalarFunctionStatsHeader.getArgumentSourceStatsResolver();
