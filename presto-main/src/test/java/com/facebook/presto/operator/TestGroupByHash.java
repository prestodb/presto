...
public class TestGroupByHash
{
    ...
    public void testGetGroupIds()
            throws Exception
    {
        GroupByHash groupByHash = new GroupByHash(ImmutableList.of(BIGINT), new int[] {0}, 1, 100);
        for (int tries = 0; tries < 2; tries++) {
            for (int value = 0; value < MAX_GROUP_ID; value++) {
                Block block = BlockAssertions.createLongsBlock(value);
                Block hashBlock = TypeUtils.getHashBlock(ImmutableList.<Type>of(BIGINT), block);
    ...
    public void testTypes()
            throws Exception
    {
        GroupByHash groupByHash = new GroupByHash(ImmutableList.of(VARCHAR), new int[] {0}, 1, 100);
            ...
            throws Exception
    {
        ...
        Block hashBlock = TypeUtils.getHashBlock(ImmutableList.<Type>of(VARCHAR), valuesBlock);
        GroupByHash groupByHash = new GroupByHash(ImmutableList.of(VARCHAR), new int[] {0}, 1, 100);
            ...
            throws Exception
    {
        ...
        Block hashBlock = TypeUtils.getHashBlock(ImmutableList.<Type>of(BIGINT), valuesBlock);
        ...
        assertEquals(pageBuilder.build().getPositionCount(), 50);
    }
            ...
            throws Exception
    {
        ...
        Block hashBlock = TypeUtils.getHashBlock(ImmutableList.<Type>of(DOUBLE), valuesBlock);
        GroupByHash groupByHash = new GroupByHash(ImmutableList.of(DOUBLE), new int[] {0}, 1, 100);
        groupByHash.getGroupIds(new Page(valuesBlock, hashBlock));

        Block testBlock = BlockAssertions.createDoublesBlock((double) 3);
        Block testHashBlock = TypeUtils.getHashBlock(ImmutableList.<Type>of(DOUBLE), testBlock);
        assertTrue(groupByHash.contains(0, testHashBlock, testBlock));

        testBlock = BlockAssertions.createDoublesBlock((double) 11);
        testHashBlock = TypeUtils.getHashBlock(ImmutableList.<Type>of(DOUBLE), testBlock);
        assertFalse(groupByHash.contains(0, testHashBlock, testBlock));
    }
            ...
            throws Exception
    {
        ...
        Block hashBlock = TypeUtils.getHashBlock(ImmutableList.<Type>of(DOUBLE, VARCHAR), valuesBlock, stringValuesBlock);
        GroupByHash groupByHash = new GroupByHash(ImmutableList.of(DOUBLE, VARCHAR), new int[] {0, 1}, 2, 100);
        ...
        Block testHashBlock = TypeUtils.getHashBlock(ImmutableList.<Type>of(DOUBLE, VARCHAR), testValuesBlock, testStringValuesBlock);
        assertTrue(groupByHash.contains(0, testHashBlock, testValuesBlock, testStringValuesBlock));
    }
            ...
            throws Exception
    {
        ...
        GroupByHash groupByHash = new GroupByHash(ImmutableList.of(VARCHAR), new int[] {0}, 1, 4);
        groupByHash.getGroupIds(new Page(valuesBlock, hashBlock));

        // Ensure that all groups are present in group by hash
        for (int i = 0; i < valuesBlock.getPositionCount(); i++) {
            assertTrue(groupByHash.contains(i, hashBlock, valuesBlock));
        }
    }
}
