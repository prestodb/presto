...
public class BenchmarkLongSumAggregation
{
    ...
    public void sumAggregation(BenchmarkData data)
    {
    ...
    public static class BenchmarkData
    {
        ...
        private static Block createChannel(int arraySize, Type elementType)
        {
            ...
            else if (elementType.equals(BIGINT_ARRAY)) {
                blockBuilder = elementType.createBlockBuilder(null, 1);
                BlockBuilder arrayBlockBuilder = BIGINT.createBlockBuilder(null, arraySize);
                for (int i = 0; i < arraySize; ++i) {
                    BIGINT.writeLong(arrayBlockBuilder, ARRAY_DATA[i]);
                }
                blockBuilder.appendStructure(arrayBlockBuilder.build());
            }
            else {
                throw new UnsupportedOperationException();
            }
            ...
            throws Throwable
    {
        ...
        data.setup();
        new BenchmarkLongSumAggregation().sumAggregation(data);

        Options options = new OptionsBuilder()
                .verbosity(VerboseMode.NORMAL)
                .warmupMode(WarmupMode.BULK)
                .include(".*" + BenchmarkLongSumAggregation.class.getSimpleName() + ".*")
                .build();
