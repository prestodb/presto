...
{
    protected TypeAndFunctionManager typeAndFunctionManager;
    ...
    {
        typeAndFunctionManager.registerBuiltInFunctions(extractFunctions(plugin.getFunctions()));
    }

    protected void registerTypes(Plugin plugin)
    {
        for (Type type : plugin.getTypes()) {
            typeAndFunctionManager.addType(type);
        }
    }
    ...
    protected final InternalAggregationFunction getFunction()
    {
        ...
        FunctionHandle functionHandle = typeAndFunctionManager.resolveFunction(session.getTransactionId(), qualifyFunctionName(QualifiedName.of(getFunctionName())), parameterTypes);
        return typeAndFunctionManager.getAggregateFunctionImplementation(functionHandle);
    }
