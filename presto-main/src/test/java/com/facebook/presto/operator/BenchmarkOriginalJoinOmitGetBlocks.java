...
/*
    ...
    no null checks
    Benchmark                                                  (dataDistribution)  Mode  Cnt    Score    Error  Units
    BenchmarkOriginalJoinOmitGetBlocks.benchmarkBuildByteHash              random  avgt    7   63.780 ±  5.469  ms/op
    BenchmarkOriginalJoinOmitGetBlocks.benchmarkBuildByteHash       unique_random  avgt    7   39.288 ±  3.806  ms/op
    BenchmarkOriginalJoinOmitGetBlocks.benchmarkBuildHash                  random  avgt    7   89.337 ±  1.284  ms/op
    BenchmarkOriginalJoinOmitGetBlocks.benchmarkBuildHash           unique_random  avgt    7   90.391 ±  4.500  ms/op
    BenchmarkOriginalJoinOmitGetBlocks.benchmarkJoinByteHash               random  avgt    7  107.316 ±  3.911  ms/op
    BenchmarkOriginalJoinOmitGetBlocks.benchmarkJoinByteHash        unique_random  avgt    7  125.502 ±  5.117  ms/op
    BenchmarkOriginalJoinOmitGetBlocks.benchmarkJoinHash                   random  avgt    7  137.299 ±  2.487  ms/op
    BenchmarkOriginalJoinOmitGetBlocks.benchmarkJoinHash            unique_random  avgt    7  171.413 ±  0.785  ms/op
 */
...
public class BenchmarkOriginalJoinOmitGetBlocks
{
    ...
    private static List<Page> BUILD_PAGES;
    ...
    static {
        ...
        BUILD_PAGES = buildPagesBuilder.build();
    }
    ...
    public void testOriginal()
    {
        ...
        context.setup();
        context.buildPages = BUILD_PAGES;
    ...
    public int benchmarkJoin(InMemoryJoinHash joinHash, List<Page> probePages)
    {
        ...
        for (Page probePage : probePages) {
            Block[] block = probePage.getBlocks();

            for (int probePosition = 0; probePosition < probePage.getPositionCount(); probePosition++) {
                long joinPosition = joinHash.getJoinPosition(probePosition, block);
    ...
    private static class InMemoryJoinHash
    {
        ...
        {
            return getJoinPosition(probePosition, probePage, pagesHashStrategy.hashRow(probePosition, probePage.getBlocks()));
        }

        public long getJoinPosition(int probePosition, Block[] probeBlock)
        {
            return getJoinPosition(probePosition, probeBlock, pagesHashStrategy.hashRow(probePosition, probeBlock));
        }

        public long getJoinPosition(int position, Block[] probeBlock, long rawHash)
        {
        ...
        public long getJoinPosition(int position, Page probePage, long rawHash)
        {
            ...
            while (key[pos] != -1) {
                if (positionEqualsCurrentRow(key[pos], (byte) rawHash, position, probePage.getBlocks())) {
                    return key[pos];
                }
                // increment position and mask to handler wrap around
                pos = (pos + 1) & mask;
        ...
        protected boolean positionEqualsCurrentRow(int leftPosition, byte rawHash, int rightPosition, Block... rightBlocks)
        {
