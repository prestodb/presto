        ...
        extends AbstractTestReservoirAggregation
{
    ...
    {
        return ImmutableList.of(StandardTypes.INTEGER, StandardTypes.INTEGER, StandardTypes.DOUBLE, StandardTypes.DOUBLE);
    }
    ...
    public Double getExpectedValue(int start, int length)
    {
        ...
        for (int i = start; i < start + LENGTH_FACTOR * length; i++) {
            ...
            if (i % TRUE_FACTOR != 0) {
                totalTrueWeight += weight;
                trueSamples.add((double) i);
            }
            else {
                totalFalseWeight += weight;
