...
public class TestJsonExtract
{
    ...
    private static String doScalarExtract(String inputJson, String jsonPath)
            throws IOException
    {
        Slice value = JsonExtract.extract(Slices.wrappedBuffer(inputJson.getBytes(Charsets.UTF_8)), generateExtractor(jsonPath, new ScalarValueJsonExtractor()));
    ...
    private static String doJsonExtract(String inputJson, String jsonPath)
            throws IOException
    {
        Slice value = JsonExtract.extract(Slices.wrappedBuffer(inputJson.getBytes(Charsets.UTF_8)), generateExtractor(jsonPath, new JsonValueJsonExtractor()));
        return (value == null) ? null : value.toString(Charsets.UTF_8);
    }
}
