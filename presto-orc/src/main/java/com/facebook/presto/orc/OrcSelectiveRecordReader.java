        ...
        extends AbstractOrcRecordReader<SelectiveStreamReader>
{
            ...
            throws IOException
    {
        ...
        if (!filterFunctionsWithNoInput.isEmpty()){
            positionCount = applyFilterFunctions(positionsToRead, positionCount, filterFunctionsWithNoInput);
        ...
        for (int columnIndex : streamReaderOrder) {
            ...
            if (!filterFunctionsApplied && !hasAnyFilter(columnIndex)) {
                positionCount = applyFilterFunctions(positionsToRead, positionCount, filterFunctionsWithInputs);
                if (positionCount == 0) {
                    break;
                }
    ...
    private int applyFilterFunctions(int[] positions, int positionCount, List<FilterFunction> someFilterFunctions)
    {
        BlockLease[] blockLeases = new BlockLease[hiveColumnIndices.length];
        Block[] blocks = new Block[hiveColumnIndices.length];

        checkArgument(someFilterFunctions.size() > 0);
        if (someFilterFunctions.get(0).getInputChannels().length > 0 ) {
            for (int columnIndex : filterFunctionInputs) {
                if (constantValues[columnIndex] != null) {
        ...
        try {
            ...
            for (FilterFunction function : someFilterFunctions) {
