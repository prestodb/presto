        ...
        implements MetadataReader
{
    ...
    {
        Map<Integer, List<DwrfProto.ColumnEncoding>> columnEncodingsByColumn = new HashMap<>(columnEncodings.size());
        ...
        for (int i = 0; i < columnEncodings.size(); i++) {
            ...
            columnEncodingsByColumn.computeIfAbsent(column, key -> new ArrayList<>()).add(columnEncoding);
        }

        ImmutableList.Builder<ColumnEncoding> resultBuilder = ImmutableList.builder();

        for (Map.Entry<Integer, List<DwrfProto.ColumnEncoding>> entry : columnEncodingsByColumn.entrySet()) {
