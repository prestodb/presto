        ...
        implements ColumnWriter
{
    ...
    private final List<FlatMapValueColumnWriter> valueNodes;
    private final FlatMapValueNodesMap keyToValueNodes;
            ...
            FlatMapValueWriterFactory valueWriterfactory)
    {
        ...
        this.keyToValueNodes = new FlatMapValueNodesMap(keyOrcTypeKind);
    ...
    public void writeColumnarMap(ColumnarMap columnarMap)
    {
        ...
        for (int position = 0; position < columnarMap.getPositionCount(); position++) {
            ...
            final boolean[] hits = new boolean[valueNodes.size() + 1];
            for (int entry = 0; entry < entryCount; entry++) {
                ...
                keyNode.writeBlock(valueBlock);

                if (keyExists) {
                    hits[keyNode.getDwrfSequence()] = true;
                }
            }
