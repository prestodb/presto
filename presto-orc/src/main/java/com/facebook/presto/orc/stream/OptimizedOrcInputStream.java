...
public final class OptimizedOrcInputStream
        extends OrcInputStream
{
    ...
    public OptimizedOrcInputStream(
            ...
            long sliceInputRetainedSizeInBytes)
    {
        ...
        this.bufferMemoryUsage = systemMemoryContext.newLocalMemoryContext(OptimizedOrcInputStream.class.getSimpleName());
        checkArgument(sliceInputRetainedSizeInBytes >= 0, "sliceInputRetainedSizeInBytes is negative");
        systemMemoryContext.newLocalMemoryContext(OptimizedOrcInputStream.class.getSimpleName()).setBytes(sliceInputRetainedSizeInBytes);

        if (!decompressor.isPresent()) {
            long sliceInputPosition = sliceInput.position();
            Slice slice = sliceInput.readSlice(toIntExact(sliceInput.remaining()));
            this.buffer = (byte[]) slice.getBase();
            this.position = toIntExact(slice.getAddress() - ARRAY_BYTE_BASE_OFFSET);
            this.length = slice.length() + position;
            this.uncompressedOffset = position;
