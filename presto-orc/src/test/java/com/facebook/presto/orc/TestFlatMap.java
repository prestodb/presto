        ...
        public List<Map<K, V>> build()
        {
            ...
            for (int i = 0; i < NUM_ROWS; ++i) {
                if (passesFrequencyCheck(nullRowsFrequency, i)) {
                    ...
                    for (int j = 0; j < 3; j++) {
                        V value;
                        int baseValue = (i * 3 + j) % 32;

                        if (j == 0 && passesFrequencyCheck(nullValuesFrequency, i)) {
                            value = null;
                        }
                        else if (mixedEncodings && (baseValue == 1 || j == 2)) {
                            // In order to get a key that uses direct encoding, remove the mod if baseValue (i.e. the key) is 1
                            // In order to ensure IN_DICTIONARY streams are present remove the mod if j is 2
                            value = valueConverter.apply(i * 3 + j);
                        }
                        else {
                            value = valueConverter.apply(baseValue);
                        }

                        row.put(keyConverter.apply(baseValue), value);
                    }
