...
Feature Toggles should allow teams to modify system behavior without changing code. Feature Toggles are configured using [Google Guice](https://github.com/google/guice). The basic definition of toggles is created
...
To allow feature toggle configuration, add four lines in the config.properties file:
...
Switching the feature toggle on or off is done by changing the enabled value from `true` to `false` in the configuration source file:

```
    feature.featureXX.enabled=true
```

The value returned by `isFeatureXXEnabled.get();` will reflect the updated configuration after the refresh period.
...
Strategy-based Toggling allows us to implement custom predicates ([Strategy Pattern](https://en.wikipedia.org/wiki/Strategy_pattern)) to evaluate if a feature is enabled.

Default toggles provided are `AllowAll`, `AllowList`, and an operating system-based toggle strategy.
...
Configuration properties always start with a `feature` followed by a dot, then the feature id. In this example the feature id is `query-cancel`.
...
In this example, change the value of `feature.query-cancel.enabled` to `false` to disable this feature.
Changes are effective after the refresh period. 
