 ...
 */
public class Int64TimeAndTimestampNanosDeltaBinaryPackedValuesDecoder
        implements Int64TimeAndTimestampMicrosValuesDecoder
{
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(Int64TimeAndTimestampNanosDeltaBinaryPackedValuesDecoder.class).instanceSize();

    private final DeltaBinaryPackingValuesReader innerReader;

    private final PackFunction packFunction;

    public Int64TimeAndTimestampNanosDeltaBinaryPackedValuesDecoder(int valueCount, ByteBufferInputStream bufferInputStream, boolean withTimezone)
            throws IOException
    {
    ...
    public void readNext(long[] values, int offset, int length)
    {
        ...
        for (int i = offset; i < endOffset; i++) {
            long curValue = NANOSECONDS.toMillis(innerReader.readLong());
