...
public class Int64TimeAndTimestampNanosPlainValuesDecoder
        implements Int64TimeAndTimestampMicrosValuesDecoder
{
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(Int64TimeAndTimestampNanosPlainValuesDecoder.class).instanceSize();
    ...
    public Int64TimeAndTimestampNanosPlainValuesDecoder(byte[] byteBuffer, int bufferOffset, int length)
    {
        this(byteBuffer, bufferOffset, length, false);
    }

    public Int64TimeAndTimestampNanosPlainValuesDecoder(byte[] byteBuffer, int bufferOffset, int length, boolean withTimezone)
    {
    ...
    public void readNext(long[] values, int offset, int length)
    {
        ...
        while (offset < endOffset) {
            long valueMillis = NANOSECONDS.toMillis(BytesUtils.getLong(localByteBuffer, localBufferOffset));
