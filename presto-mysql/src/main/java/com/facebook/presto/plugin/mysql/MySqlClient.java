        ...
        extends BaseJdbcClient
{
    ...
    public PreparedStatement buildSql(ConnectorSession session, Connection connection, JdbcSplit split, List<JdbcColumnHandle> columnHandles) throws SQLException
    {
        ...
        columnHandles.forEach(handle -> {
            if (handle.getJdbcTypeHandle().getJdbcTypeName().equalsIgnoreCase("geometry")) {
                String columnName = handle.getColumnName();
                supposedColumnExps.put(columnName, "ST_AsBinary(" + columnName + ")");
            }
        });

        ImmutableMap<String, String> columnExpressions = ImmutableMap.<String, String>builder().putAll(supposedColumnExps).build();
