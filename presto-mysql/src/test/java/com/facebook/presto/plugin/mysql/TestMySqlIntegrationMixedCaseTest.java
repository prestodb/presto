        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testCatalogCase()
    {
        ...
        getQueryRunner().execute(session, "CREATE TABLE TEST_CREATE_CATALOG (name VARCHAR(50), id int)");
        getQueryRunner().execute(session, "INSERT INTO TEST_CREATE_CATALOG VALUES('a', 1)");
        assertQuery(session, "SELECT * FROM TEST_CREATE_CATALOG", "VALUES('a', 1)");

        assertUpdate(session, "DROP TABLE IF EXISTS TEST_CREATE_CATALOG");
        assertFalse(getQueryRunner().tableExists(session, "TEST_CREATE_CATALOG"));
    }
