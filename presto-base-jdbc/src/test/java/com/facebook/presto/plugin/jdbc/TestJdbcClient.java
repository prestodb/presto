...
public class TestJdbcClient
{
    ...
    public void testCreateWithNullableColumns()
    {
        ...
        List<ColumnMetadata> expectedColumns = ImmutableList.of(
                ColumnMetadata.builder("columnA", BIGINT).setHidden(false).build(),
                ColumnMetadata.builder("columnB", BIGINT).setNullable(true).setHidden(false).setProperties(emptyMap()).build(),
                ColumnMetadata.builder("columnC", BIGINT).setNullable(false).setHidden(false).setProperties(emptyMap()).build(),
                ColumnMetadata.builder("columnD", DATE).setNullable(false).setHidden(false).setProperties(emptyMap()).build());
    ...
    public void testAlterColumns()
    {
        ...
        SchemaTableName schemaTableName = new SchemaTableName("schema_for_create_table_tests", tableName);
        List<ColumnMetadata> expectedColumns = ImmutableList.of(
                ColumnMetadata.builder("columnA", BIGINT)
        ...
        try {
            ...
            jdbcClient.addColumn(session, JdbcIdentity.from(session), tableHandle, ColumnMetadata.builder("columnB", DOUBLE)
                    .setHidden(false).build());
            assertEquals(jdbcClient.getColumns(session, tableHandle), ImmutableList.of(
                    new JdbcColumnHandle(CONNECTOR_ID, "COLUMNA", JDBC_BIGINT, BigintType.BIGINT, true, Optional.empty()),
