...
public class QueryBuilder
{
            ...
            throws SQLException
    {
        ...
        if (additionalPredicate.isPresent()) {
            ...
            /*  We are passing JdbcTypeHandle as null and later on getting the ReadMapping based on presto type itself since there is no way of getting the JdbcTypeHandle at this place. JdbcTypeHandle can be fetched only from JdbcColumnHandle, and we do not have that object in this lambda. The accumulator has no good use since the corresponding function mapping is not rich enough currently and also addition of accumulator results in duplicate where clauses which does not solve any purpose (at least for [equals operator](https://github.com/prestodb/presto/blob/e1f8b4b7adcbec565215b4d81e688f76634db6ed/presto-base-jdbc/src/main/java/com/facebook/presto/plugin/jdbc/optimization/function/OperatorTranslators.java#L49) ).
                The basic idea behind introducing ReadMapping was to cater to requirements wherein we want to have control over the writeFunctions. This is particularly useful where we try to map some presto type to some other presto type (e.g array type mapped to json) or for mapping unsupported types to string. The visitFilter method in JdbcComputePushdown() is responsible for creating additionalPredicates. The where clause passed in the query ultimately needs to go through the functionMapping [here](https://github.com/prestodb/presto/blob/e1f8b4b7adcbec565215b4d81e688f76634db6ed/presto-expressions/src/main/java/com/facebook/presto/expressions/translator/FunctionTranslator.java#L49) as part of visitFilter execution.
    ...
    {
        if (typeHandle != null) {
            return client.toWriteMapping(client.toPrestoType(session, typeHandle)
                    .orElseThrow(() -> new VerifyException(format("Unsupported type %s with handle %s", type, typeHandle)))
                    .getType()).getWriteFunction();
        }
        return getWriteMappingForAccumulators(type)
