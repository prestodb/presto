...
public final class StandardColumnMappings
{
    ...
    public static WriteMapping charWriteMapping(CharType charType)
    {
        requireNonNull(charType, "charType is null");
    ...
    public static WriteMapping varcharWriteMapping(VarcharType varcharType)
    {
        return createSliceWriteMapping(((statement, index, value) -> statement.setString(index, value.toStringUtf8())));
    }
    ...
    public static Optional<WriteMapping> prestoTypeToWriteMapping(Type type)
    {
        if (type.equals(BOOLEAN)) {
            ...
            return Optional.of(varcharWriteMapping((VarcharType) type));
        }
        else if (type instanceof CharType) {
            return Optional.of(charWriteMapping((CharType) type));
        }
    ...
    public static Optional<WriteMapping> getWriteMappingForAccumulators(Type type)
    {
        if (type instanceof BooleanType) {
            ...
            return Optional.of(decimalWriteMapping(createDecimalType(precision, max(scale, 0))));
        }
        else if (type instanceof CharType) {
            //TODO: check if there is any way of getting the column size here
            return Optional.of(charWriteMapping(createCharType(CharType.MAX_LENGTH)));
        }
        else if (type instanceof VarcharType) {
            //TODO: check if there is any way of getting the actual column size here
            return Optional.of(varcharWriteMapping(createUnboundedVarcharType()));
        }
