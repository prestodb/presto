        ...
        extends AbstractTestQueryFramework
{
    ...
    protected QueryRunner createQueryRunner()
            throws Exception
    {
        DistributedQueryRunner queryRunner = (DistributedQueryRunner) PrestoNativeQueryRunnerUtils.createNativeQueryRunner(true, true, true);
    ...
    public void testPlanCheckerPluginWithNativeCompatibleQueries()
    {
        ...
        assertEquals(target, planCheckerRouterConfig.getNativeRouterURI());
    }
    ...
    public void testPlanCheckerPluginWithNativeIncompatibleQueries()
    {
        ...
        URI target = scheduler.getDestination(
                ...
                .orElseThrow(AssertionError::new);
        assertEquals(target, planCheckerRouterConfig.getJavaRouterURI());
    }
    ...
    public void testPlanCheckerPluginWithUserErrors()
    {
        ...
        try {
            ...
            verifyQueryError(e, "line 1:55: Schema must be specified when session schema is not set", "USER_ERROR");
        }
    }
    ...
    private static void verifyQueryError(PrestoException e, String message, String errorCode)
    {
        QueryError error = QUERY_RESULTS_CODEC.fromJson(e.getMessage()).getError();
        assertNotNull(error);
        assertTrue(error.getMessage().equalsIgnoreCase(message));
        assertTrue(error.getErrorType().equalsIgnoreCase(errorCode));
    }
}
