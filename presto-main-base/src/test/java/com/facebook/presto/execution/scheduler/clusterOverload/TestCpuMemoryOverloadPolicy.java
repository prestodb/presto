...
{
    private static final String CPU_OVERLOAD_METRIC = "cpu.overload";
    private static final String MEMORY_OVERLOAD_METRIC = "memory.overload";
    ...
    @Test
    public void testIsClusterOverloadedDisabled()
    {
        ...
        // Even with overloaded nodes, should return false when disabled
        InternalNodeManager nodeManager = createNodeManager(ImmutableSet.of(createNode("node1", ImmutableMap.of(CPU_OVERLOAD_METRIC, 1.5)), createNode("node2", ImmutableMap.of(MEMORY_OVERLOAD_METRIC, 1.5))));
    ...
    public void testIsClusterOverloadedCountBasedNoOverload()
    {
        ...
        InternalNodeManager nodeManager = createNodeManager(ImmutableSet.of(createNode("node1", ImmutableMap.of(CPU_OVERLOAD_METRIC, 1.5)), createNode("node2", ImmutableMap.of(CPU_OVERLOAD_METRIC, 0.5)), createNode("node3", ImmutableMap.of(MEMORY_OVERLOAD_METRIC, 0.5))));
        assertFalse(policy.isClusterOverloaded(nodeManager));
    }
    ...
    public void testIsClusterOverloadedCountBasedOverload()
    {
        ...
        InternalNodeManager nodeManager = createNodeManager(ImmutableSet.of(createNode("node1", ImmutableMap.of(CPU_OVERLOAD_METRIC, 1.5)), createNode("node2", ImmutableMap.of(MEMORY_OVERLOAD_METRIC, 1.5)), createNode("node3", ImmutableMap.of(CPU_OVERLOAD_METRIC, 0.5))));
    ...
    public void testIsClusterOverloadedPctBasedNoOverload()
    {
        ClusterOverloadConfig config = new ClusterOverloadConfig()
                .setClusterOverloadThrottlingEnabled(true)
                .setAllowedOverloadWorkersPct(0.4)
    ...
    public void testIsClusterOverloadedPctBasedOverload()
    {
        ClusterOverloadConfig config = new ClusterOverloadConfig()
                .setClusterOverloadThrottlingEnabled(true)
                .setAllowedOverloadWorkersPct(0.3)
                .setOverloadPolicyType(OVERLOAD_POLICY_PCT_BASED);
        CpuMemoryOverloadPolicy policy = new CpuMemoryOverloadPolicy(config);

        // 2 out of 5 nodes (40%) are overloaded, allowed is 30%, so overloaded
        InternalNodeManager nodeManager = createNodeManager(ImmutableSet.of(createNode("node1", ImmutableMap.of(CPU_OVERLOAD_METRIC, 1.5)), createNode("node2", ImmutableMap.of(MEMORY_OVERLOAD_METRIC, 1.5)), createNode("node3", ImmutableMap.of(CPU_OVERLOAD_METRIC, 0.5)), createNode("node4", ImmutableMap.of(MEMORY_OVERLOAD_METRIC, 0.5)), createNode("node5", ImmutableMap.of(CPU_OVERLOAD_METRIC, 0.5))));
        assertTrue(policy.isClusterOverloaded(nodeManager));
    }
    ...
    public void testIsClusterOverloadedBothMetricsOverloaded()
    {
        ClusterOverloadConfig config = new ClusterOverloadConfig()
                .setClusterOverloadThrottlingEnabled(true)
                .setAllowedOverloadWorkersCnt(0.0)
                .setOverloadPolicyType(OVERLOAD_POLICY_CNT_BASED);
        CpuMemoryOverloadPolicy policy = new CpuMemoryOverloadPolicy(config);

        // Node has both CPU and memory overloaded, should only count as one overloaded node
        InternalNodeManager nodeManager = createNodeManager(ImmutableSet.of(createNode("node1", ImmutableMap.of(CPU_OVERLOAD_METRIC, 1.5, MEMORY_OVERLOAD_METRIC, 1.5))));
        assertTrue(policy.isClusterOverloaded(nodeManager));
    }
    ...
    private static final Map<String, Map<String, Double>> NODE_METRICS = new HashMap<>();

    private static InternalNode createNode(String nodeId, Map<String, Double> metrics)
    {
        // Store metrics in the map for later retrieval
        NODE_METRICS.put(nodeId, metrics);
    ...
    private static InternalNodeManager createNodeManager(Set<InternalNode> nodes)
    {
        return new InternalNodeManager()
        {
            ...
            {
                return NODE_METRICS.getOrDefault(nodeIdentifier, Collections.emptyMap());
            }
        };
    }
}
