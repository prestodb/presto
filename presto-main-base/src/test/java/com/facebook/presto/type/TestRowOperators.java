        ...
        extends AbstractTestFunctions
{
    ...
    {
        fieldNameInJsonCastDisabled.assertFunction("cast(cast (null as ROW(BIGINT, VARCHAR)) AS JSON)", JSON, null);
        fieldNameInJsonCastDisabled.assertFunction("cast(ROW(null, null) as json)", JSON, "[null,null]");

        fieldNameInJsonCastDisabled.assertFunction("cast(ROW(true, false, null) AS JSON)", JSON, "[true,false,null]");
        ...
        // other miscellaneous tests
        fieldNameInJsonCastDisabled.assertFunction("CAST(ROW(1, 2) AS JSON)", JSON, "[1,2]");
        fieldNameInJsonCastDisabled.assertFunction("CAST(CAST(ROW(1, 2) AS ROW(a BIGINT, b BIGINT)) AS JSON)", JSON, "[1,2]");
        fieldNameInJsonCastDisabled.assertFunction("CAST(ROW(1, NULL) AS JSON)", JSON, "[1,null]");
        fieldNameInJsonCastDisabled.assertFunction("CAST(ROW(1, CAST(NULL AS INTEGER)) AS JSON)", JSON, "[1,null]");
        fieldNameInJsonCastDisabled.assertFunction("CAST(ROW(1, 2.0E0) AS JSON)", JSON, "[1,2.0]");
        fieldNameInJsonCastDisabled.assertFunction("CAST(ROW(1.0E0, 2.5E0) AS JSON)", JSON, "[1.0,2.5]");
        fieldNameInJsonCastDisabled.assertFunction("CAST(ROW(1.0E0, 'kittens') AS JSON)", JSON, "[1.0,\"kittens\"]");
        fieldNameInJsonCastDisabled.assertFunction("CAST(ROW(TRUE, FALSE) AS JSON)", JSON, "[true,false]");
        fieldNameInJsonCastDisabled.assertFunction("CAST(ROW(FALSE, ARRAY [1, 2], MAP(ARRAY[1, 3], ARRAY[2.0E0, 4.0E0])) AS JSON)", JSON, "[false,[1,2],{\"1\":2.0,\"3\":4.0}]");
        fieldNameInJsonCastDisabled.assertFunction("CAST(row(1.0, 123123123456.6549876543) AS JSON)", JSON, "[1.0,123123123456.6549876543]");
    }
