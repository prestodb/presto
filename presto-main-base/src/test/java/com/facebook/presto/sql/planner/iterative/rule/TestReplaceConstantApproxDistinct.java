...
public class TestReplaceConstantApproxDistinct
        extends BaseRuleTest
{
    @Test
    public void testReplaceConstant()
    {
        tester().assertThat(new ReplaceConstantApproxDistinct(getFunctionManager()))
                .on(p -> {
                ...
                .matches(
                    aggregation(
                        ImmutableMap.of("output", functionCall("arbitrary", false, ImmutableList.of(anySymbol()))),
                        SINGLE,
                        project(
                            ImmutableMap.of(
                                "input", expression("'constant'")),
                                // "one", expression("1")),
                            values())));
    }
