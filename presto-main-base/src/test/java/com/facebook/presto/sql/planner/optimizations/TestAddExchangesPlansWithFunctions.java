        ...
        extends BasePlanTest
{
    ...
    private static final SqlInvokedFunction CPP_FOO = new SqlInvokedFunction(
            new QualifiedObjectName("dummy", "unittest", "foo"),
    ...
    private static final SqlInvokedFunction CPP_BAZ = new SqlInvokedFunction(
            new QualifiedObjectName("dummy", "unittest", "baz"),
            ImmutableList.of(new Parameter("x", parseTypeSignature(StandardTypes.BIGINT))),
            parseTypeSignature(StandardTypes.BIGINT),
            "baz(x)",
            RoutineCharacteristics.builder().setLanguage(CPP).setDeterminism(DETERMINISTIC).setNullCallClause(RETURNS_NULL_ON_NULL_INPUT).build(),
    ...
    private static final SqlInvokedFunction JAVA_BAR = new SqlInvokedFunction(
            new QualifiedObjectName("dummy", "unittest", "bar"),
            ImmutableList.of(new Parameter("x", parseTypeSignature(StandardTypes.BIGINT))),
            parseTypeSignature(StandardTypes.BIGINT),
            "foo(x)",
    ...
    private static final SqlInvokedFunction JAVA_FEE = new SqlInvokedFunction(
            new QualifiedObjectName("dummy", "unittest", "fee"),
            ImmutableList.of(new Parameter("x", parseTypeSignature(StandardTypes.BIGINT))),
            parseTypeSignature(StandardTypes.BIGINT),
            "fee(x)",
            RoutineCharacteristics.builder().setLanguage(JAVA).setDeterminism(DETERMINISTIC).setNullCallClause(RETURNS_NULL_ON_NULL_INPUT).build(),
    ...
    {
        // bar is a java function, it is pushed down past the exchange
        assertDistributedPlan("SELECT fee(ordinal_position) FROM information_schema.columns WHERE bar(ordinal_position) = 1",
                anyTree(
                        exchange(REMOTE_STREAMING, GATHER,
                                project(ImmutableMap.of("fee", expression("fee(ordinal_position)")),
                                        filter("bar(ordinal_position) = BIGINT'1'",
                                                tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position")))))));
        // foo is a CPP function, it is not pushed down past the exchange because the source is a system table scan
        // The filter is preserved above the exchange to prove that the filter is not in the same fragment as the system table scan
        assertDistributedPlan("SELECT baz(ordinal_position) FROM information_schema.columns WHERE foo(ordinal_position) = 1",
                anyTree(
                        project(ImmutableMap.of("baz", expression("baz(ordinal_position)")),
                                filter("foo(ordinal_position) = BIGINT'1'",
                                        exchange(REMOTE_STREAMING, GATHER,
                                                tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position")))))));
    }
    ...
    public void testJoinWithCppFunctionDoesNotGetPushedIntoSystemTableScan()
    {
        ...
        assertDistributedPlan(
                "SELECT c1.table_name FROM information_schema.columns c1 JOIN information_schema.columns c2 ON c1.ordinal_position = c2.ordinal_position WHERE bar(c1.ordinal_position) = 1",
                anyTree(
                        exchange(
                                join(INNER, ImmutableList.of(equiJoinClause("ordinal_position", "ordinal_position_4")),
                                        ...
                                        anyTree(
                                                exchange(
                                                        project(
                                                                filter("bar(ordinal_position_4) = BIGINT'1'",
                                                                        tableScan("columns", ImmutableMap.of(
                                                                                "ordinal_position_4", "ordinal_position"))))))))));

        // foo is a CPP function, it is not pushed down past the exchange because the source is a system table scan
        assertDistributedPlan(
                "SELECT baz(c1.ordinal_position) FROM information_schema.columns c1 JOIN information_schema.columns c2 ON c1.ordinal_position = c2.ordinal_position WHERE foo(c1.ordinal_position) = 1",
                output(
                        exchange(
                                project(ImmutableMap.of("baz", expression("baz(ordinal_position)")),
                                        join(INNER, ImmutableList.of(equiJoinClause("ordinal_position", "ordinal_position_4")),
                                                        ...
                                                        exchange(REMOTE_STREAMING, GATHER,
                                                                anyTree(
                                                                        tableScan("columns", ImmutableMap.of(
                                                                                "ordinal_position", "ordinal_position"))))),
                                                anyTree(
                                                        filter("foo(ordinal_position_4) = BIGINT'1'",
                                                                exchange(REMOTE_STREAMING, GATHER,
                                                                        anyTree(
                                                                                tableScan("columns", ImmutableMap.of(
                                                                                        "ordinal_position_4", "ordinal_position")))))))))));
    ...
    public void testMixedFunctionTypesInComplexPredicates()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE bar(ordinal_position) = 1 AND foo(ordinal_position) > 0",
                anyTree(
                        filter("bar(ordinal_position) = BIGINT'1' AND foo(ordinal_position) > BIGINT'0'",
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));

        // Test OR condition with mixed functions - entire predicate should be evaluated after exchange
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE bar(ordinal_position) = 1 OR foo(ordinal_position) = 2",
                anyTree(
                        filter("bar(ordinal_position) = BIGINT'1' OR foo(ordinal_position) = BIGINT'2'",
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    }
    ...
    public void testNestedFunctionCalls()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE bar(foo(ordinal_position)) = 1",
                anyTree(
                        filter("bar(foo(ordinal_position)) = BIGINT'1'",
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));

        // Java function nested inside CPP function - should not push down
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE foo(bar(ordinal_position)) = 1",
                anyTree(
                        filter("foo(bar(ordinal_position)) = BIGINT'1'",
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));

        // Multiple levels of nesting
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE foo(bar(foo(ordinal_position))) = 1",
                anyTree(
                        filter("foo(bar(foo(ordinal_position))) = BIGINT'1'",
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    }
    ...
    public void testMixedSystemAndRegularTables()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns c JOIN nation n ON c.ordinal_position = n.nationkey WHERE foo(c.ordinal_position) = 1",
                output(
                        exchange(
        ...
        assertDistributedPlan(
                "SELECT * FROM nation WHERE foo(nationkey) = 1",
                anyTree(
                        exchange(REMOTE_STREAMING, GATHER,
                                filter("foo(nationkey) = BIGINT'1'",
                                        tableScan("nation", ImmutableMap.of("nationkey", "nationkey"))))));
    }
    ...
    public void testAggregationsWithMixedFunctions()
    {
        ...
        assertDistributedPlan(
                "SELECT DISTINCT foo(ordinal_position) FROM information_schema.columns",
                anyTree(
                        exchange(REMOTE_STREAMING, GATHER,
                                anyTree(
                                        project(
        ...
        assertDistributedPlan(
                "SELECT DISTINCT bar(ordinal_position) FROM information_schema.columns",
                anyTree(
                        exchange(REMOTE_STREAMING, GATHER,
                                anyTree(
                                        project(
    ...
    @Test(enabled = false)  // TODO: Fix the bug where ScanProject combines scan and projection for system tables with CPP functions
    public void testSubqueriesWithCppFunctions()
    {
        // Subquery with CPP function creates a semi-join
        // BUG: ScanProject combines the scan and projection for system tables
        // The exchange that should be between the scan and the join is missing
        // The system table scan should have an exchange before the semi-join when CPP functions are involved

        // Expected structure (but currently failing):
        // - Output
        //   - Exchange
        //     - Filter/Project
        //       - SemiJoin
        //         - Exchange  <-- This is missing!
        //           - TableScan(columns)
        //         - ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE ordinal_position IN (SELECT foo(nationkey) FROM nation)",
                output(
                        exchange(REMOTE_STREAMING, GATHER,
                                anyTree(
                                        exchange(REMOTE_STREAMING, GATHER,
    ...
    public void testComplexPredicateWithMultipleFunctions()
    {
        ...
        // Since the predicate contains CPP functions (foo, baz), the exchange is inserted before the system table scan
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE (foo(ordinal_position) > 0 AND bar(ordinal_position) < 100) OR baz(ordinal_position) = 50",
                anyTree(
                        filter(
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    ...
    public void testProjectionWithMixedFunctions()
    {
        ...
        assertDistributedPlan(
                "SELECT bar(ordinal_position) as java_result, foo(ordinal_position) as cpp_result FROM information_schema.columns",
                anyTree(
                        project(ImmutableMap.of(
                                        "java_result", expression("bar(ordinal_position)"),
                                        "cpp_result", expression("foo(ordinal_position)")),
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    }
    ...
    public void testCaseStatementsWithCppFunctions()
    {
        ...
        assertDistributedPlan(
                "SELECT CASE WHEN foo(ordinal_position) > 0 THEN 'positive' ELSE 'negative' END FROM information_schema.columns",
                output(
                        project(
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
        ...
        // The RemoveRedundantExchanges optimizer removes the redundant exchange
        assertDistributedPlan(
                "SELECT CASE WHEN ordinal_position > 0 THEN foo(ordinal_position) ELSE 0 END FROM information_schema.columns",
                output(
                        project(
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    ...
    public void testWindowFunctionsWithCppFunctions()
    {
        ...
        assertDistributedPlan(
                "SELECT row_number() OVER (PARTITION BY foo(ordinal_position)) FROM information_schema.columns",
                anyTree(
                        exchange(
                                project(
                                        anyTree(
        ...
        assertDistributedPlan(
                "SELECT row_number() OVER (ORDER BY foo(ordinal_position)) FROM information_schema.columns",
                anyTree(
                        exchange(
                                project(
                                        anyTree(
    ...
    public void testMultipleSystemTableJoins()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns c1 " +
                        "JOIN information_schema.columns c2 ON foo(c1.ordinal_position) = foo(c2.ordinal_position)",
                output(
                        exchange(
                                anyTree(
                                        project(
    ...
    public void testInPredicateWithCppFunction()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE foo(ordinal_position) IN (1, 2, 3)",
                anyTree(
                        filter("foo(ordinal_position) IN (BIGINT'1', BIGINT'2', BIGINT'3')",
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    }
    ...
    public void testBetweenPredicateWithCppFunction()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE foo(ordinal_position) BETWEEN 1 AND 10",
                anyTree(
                        filter("foo(ordinal_position) BETWEEN BIGINT'1' AND BIGINT'10'",
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    }
    ...
    public void testNullHandlingWithCppFunctions()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE foo(ordinal_position) IS NULL",
                anyTree(
                        filter("foo(ordinal_position) IS NULL",
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
        ...
        // The RemoveRedundantExchanges optimizer removes the redundant exchange
        assertDistributedPlan(
                "SELECT COALESCE(foo(ordinal_position), 0) FROM information_schema.columns",
                anyTree(
                        project(
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    ...
    @Test
    public void testDistinctWithCppFunction()
    {
        // DISTINCT with CPP function should require exchange
        assertDistributedPlan(
                "SELECT DISTINCT foo(ordinal_position) FROM information_schema.columns",
                anyTree(
                        exchange(REMOTE_STREAMING, GATHER,
    ...
    public void testUnionWithCppFunctions()
    {
        ...
        assertDistributedPlan(
                "SELECT foo(ordinal_position) FROM information_schema.columns " +
                        "UNION ALL SELECT foo(nationkey) FROM nation",
                output(
                        exchange(
                                anyTree(
                                        exchange(REMOTE_STREAMING, GATHER,
    ...
    public void testExistsSubqueryWithCppFunction()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns c WHERE EXISTS (SELECT 1 FROM nation n WHERE foo(c.ordinal_position) = n.nationkey)",
                anyTree(
                        join(
                                anyTree(
                                        exchange(REMOTE_STREAMING, GATHER,
    ...
    public void testLimitWithCppFunction()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns ORDER BY foo(ordinal_position) LIMIT 10",
                output(
                        project(
                                anyTree(
                                        project(
    ...
    public void testCastOperationsWithCppFunctions()
    {
        ...
        // The RemoveRedundantExchanges optimizer removes the redundant exchange
        assertDistributedPlan(
                "SELECT CAST(foo(ordinal_position) AS VARCHAR) FROM information_schema.columns",
                output(
                        project(
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    ...
    public void testArrayConstructorWithCppFunction()
    {
        ...
        assertDistributedPlan(
                "SELECT ARRAY[foo(ordinal_position), baz(ordinal_position)] FROM information_schema.columns",
                anyTree(
                        project(
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    ...
    public void testRowConstructorWithCppFunction()
    {
        ...
        // The RemoveRedundantExchanges optimizer removes the redundant exchange
        assertDistributedPlan(
                "SELECT ROW(foo(ordinal_position), table_name) FROM information_schema.columns",
                anyTree(
                        project(
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of(
    ...
    public void testIsNotNullWithCppFunction()
    {
        ...
        assertDistributedPlan(
                "SELECT * FROM information_schema.columns WHERE foo(ordinal_position) IS NOT NULL",
                anyTree(
                        filter("foo(ordinal_position) IS NOT NULL",
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    }
    ...
    public void testComplexJoinWithMultipleCppFunctions()
    {
        ...
        // The filters are pushed into FilterProject nodes and the join happens on the expression foo(c1.ordinal_position)
        assertDistributedPlan(
                "SELECT c1.table_name, n.name FROM information_schema.columns c1 " +
                        "JOIN nation n ON foo(c1.ordinal_position) = n.nationkey " +
                        "WHERE baz(c1.ordinal_position) > 0 AND foo(n.nationkey) < 100",
                output(
                        exchange(
                                anyTree(
                                        exchange(REMOTE_STREAMING, GATHER,
