        ...
        extends BasePlanTest
{
    ...
    public void testFilterWithCppFunctionDoesNotGetPushedIntoSystemTableScan()
    {
        ...
        assertNativeDistributedPlan("SELECT fee(ordinal_position) FROM information_schema.columns WHERE bar(ordinal_position) = 1",
                anyTree(
        ...
        assertNativeDistributedPlan("SELECT baz(ordinal_position) FROM information_schema.columns WHERE foo(ordinal_position) = 1",
                anyTree(
    ...
    public void testAggregationsWithMixedFunctions()
    {
        ...
        assertNativeDistributedPlan(
                ...
                anyTree(
                        project(ImmutableMap.of("foo", expression("foo(ordinal_position)")),
                                exchange(REMOTE_STREAMING, GATHER,
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
        ...
        assertNativeDistributedPlan(
                ...
                anyTree(
                        exchange(REMOTE_STREAMING, GATHER,
                                project(ImmutableMap.of("bar", expression("bar(ordinal_position)")),
                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position"))))));
    }
    ...
    public void testMultipleSystemTableJoins()
    {
        ...
        assertNativeDistributedPlan(
                ...
                anyTree(
                        exchange(
                                join(INNER, ImmutableList.of(equiJoinClause("foo", "foo_4")),
                                        exchange(
                                                project(ImmutableMap.of("foo", expression("foo")),
                                                        project(ImmutableMap.of("foo", expression("foo(ordinal_position)")),
                                                                exchange(REMOTE_STREAMING, GATHER,
                                                                        tableScan("columns", ImmutableMap.of("ordinal_position", "ordinal_position")))))),
                                        anyTree(
                                                exchange(
                                                        project(ImmutableMap.of("foo_4", expression("foo_4")),
                                                                project(ImmutableMap.of("foo_4", expression("foo(ordinal_position_4)")),
                                                                        exchange(REMOTE_STREAMING, GATHER,
    ...
    public void testComplexJoinWithMultipleCppFunctions()
    {
        ...
        assertNativeDistributedPlan(
                ...
                anyTree(
                        join(INNER, ImmutableList.of(equiJoinClause("foo", "nationkey")),
                                project(ImmutableMap.of("table_name", expression("table_name"), "foo", expression("foo")),
                                        project(ImmutableMap.of("foo", expression("foo(ordinal_position)")),
                                                filter("baz(ordinal_position) > BIGINT'0' AND foo(foo(ordinal_position)) < BIGINT'100'",
                                                        exchange(REMOTE_STREAMING, GATHER,
                                                                tableScan("columns", ImmutableMap.of(
                                                                        "ordinal_position", "ordinal_position",
                                                                        "table_name", "table_name")))))),
                                anyTree(
                                        project(ImmutableMap.of("nationkey", expression("nationkey"),
                                                ...
                                                filter("foo(nationkey) < BIGINT'100'",
