    ...
    public void testTwoDistinctExpressions()
    {
        tester().assertThat(new CombineApproxDistinctFunctions(getMetadata().getFunctionAndTypeManager()))
                ...
                .matches(
                        project(
                                ImmutableMap.of(
                                        "approx_distinct_1", expression("coalesce(element_at(transform_result, 1), CAST(0 AS bigint))"),
                                        "approx_distinct_2", expression("coalesce(element_at(transform_result, 2), CAST(0 AS bigint))")),
                                project(
                                        ImmutableMap.of("transform_result", expression("transform(array_transpose(set_agg_result), x -> cardinality(array_distinct(remove_nulls(x))))")),
                                        aggregation(
                ...
                .matches(
                        project(
                                ImmutableMap.of(
                                        "approx_distinct_1", expression("coalesce(element_at(transform_result, 1), CAST(0 AS bigint))"),
                                        "approx_distinct_2", expression("coalesce(element_at(transform_result, 2), CAST(0 AS bigint))"),
                                        "approx_distinct_3", expression("coalesce(element_at(transform_result, 3), CAST(0 AS bigint))")),
                                project(
                                        ImmutableMap.of("transform_result", expression("transform(array_transpose(set_agg_result), x -> cardinality(array_distinct(remove_nulls(x))))")),
                                        aggregation(
                                                ImmutableMap.of("set_agg_result", PlanMatchPattern.functionCall("set_agg", ImmutableList.of("array_expr"))),
