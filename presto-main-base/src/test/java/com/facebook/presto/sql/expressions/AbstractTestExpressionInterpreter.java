...
public abstract class AbstractTestExpressionInterpreter
{
    ...
    public static final TestingRowExpressionTranslator TRANSLATOR = new TestingRowExpressionTranslator(METADATA);
    public static final BlockEncodingSerde blockEncodingSerde = new BlockEncodingManager();
...
//                "CAST(3.3 AS DECIMAL(20,10))");
    ...
    public void testFailedExpressionOptimization()
    {
        ...
        assertFailedMatches("if(false, 1, 0 / 0)", "cast(fail(8, 'ignored failure message') as integer)");
    ...
    {
        assertThrows(PrestoException.class, () -> optimize("1 / 0"));
    }

    @Test
    public void testArraySubscriptConstantNegativeIndex()
    {
        assertThrows(PrestoException.class, () -> optimize("ARRAY [1, 2, 3][-1]"));
    }

    @Test
    public void testArraySubscriptConstantZeroIndex()
    {
        assertThrows(PrestoException.class, () -> optimize("ARRAY [1, 2, 3][0]"));
    }

    @Test
    public void testMapSubscriptMissingKey()
    {
        assertThrows(PrestoException.class, () -> optimize("MAP(ARRAY [1, 2], ARRAY [3, 4])[-1]"));
    }
    ...
    public static void assertRowExpressionEvaluationEquals(RowExpression left, RowExpression right)
    {
        ...
        if (left instanceof ConstantExpression) {
            ...
            assertCallExpressionEvaluationEquals((CallExpression) left, (CallExpression) right);
        }
        else if (left instanceof SpecialFormExpression) {
            assertTrue(right instanceof SpecialFormExpression);
            assertSpecialFormExpressionEvaluationEquals((SpecialFormExpression) left, (SpecialFormExpression) right);
        }
        else {
            assertTrue(left instanceof LambdaDefinitionExpression);
            assertTrue(right instanceof LambdaDefinitionExpression);
            assertLambdaExpressionEvaluationEquals((LambdaDefinitionExpression) left, (LambdaDefinitionExpression) right);
        }
    }
     ...
     */
    public static void assertRowExpressionEvaluationEquals(Object left, Object right)
    {
        if (right instanceof RowExpression) {
            assertRowExpressionEvaluationEquals((RowExpression) left, (RowExpression) right);
        }
        else {
            // We have constants; directly compare
    ...
    private static void assertCallExpressionEvaluationEquals(CallExpression left, CallExpression right)
    {
        assertEquals(left.getFunctionHandle(), right.getFunctionHandle());
        assertEquals(left.getArguments().size(), right.getArguments().size());
        for (int i = 0; i < left.getArguments().size(); i++) {
            assertRowExpressionEvaluationEquals(left.getArguments().get(i), right.getArguments().get(i));
        }
    }

    private static void assertSpecialFormExpressionEvaluationEquals(SpecialFormExpression left, SpecialFormExpression right)
    {
        assertEquals(left.getForm(), right.getForm());
        assertEquals(left.getArguments().size(), right.getArguments().size());
        for (int i = 0; i < left.getArguments().size(); i++) {
            assertRowExpressionEvaluationEquals(left.getArguments().get(i), right.getArguments().get(i));
        }
    ...
    private static void assertLambdaExpressionEvaluationEquals(LambdaDefinitionExpression left, LambdaDefinitionExpression right)
    {
        assertEquals(left.getArguments(), right.getArguments());
        assertEquals(left.getArgumentTypes(), right.getArgumentTypes());
        assertRowExpressionEvaluationEquals(left.getBody(), right.getBody());
    }
    ...
    public abstract void assertOptimizedEquals(@Language("SQL") String actual, @Language("SQL") String expected);

    public void assertRoundTrip(String expression)
    {
    ...
    public static RowExpression toRowExpression(Expression expression)
    {
        return TRANSLATOR.translate(expression, SYMBOL_TYPES);
    }

    public abstract void assertOptimizedMatches(@Language("SQL") String actual, @Language("SQL") String expected);

    public void assertFailedMatches(@Language("SQL") String actual, @Language("SQL") String expected)
    {
        assertOptimizedMatches(actual, expected);
    }

    public abstract Object optimize(@Language("SQL") String expression);
    ...
    public abstract void assertDoNotOptimize(@Language("SQL") String expression, ExpressionOptimizer.Level optimizationLevel);

    public static Object symbolConstant(Symbol symbol)
    {
    ...
    public abstract void assertEvaluatedEquals(@Language("SQL") String actual, @Language("SQL") String expected);

    public abstract Object evaluate(@Language("SQL") String expression, boolean deterministic);
}
