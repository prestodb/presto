        ...
        extends BaseRuleTest
{
    ...
    @Test
    public void testFiresForIdentityProject()
    {
        // The project maps a -> a (a pure pass-through): computedOutputs is empty, so no project
        // needs to be pushed into the exchange source branches.  The rule falls through to the same
        // pushPartial logic as the base (WITHOUT_PROJECTION) rule, producing:
        //   Exchange -> Project(SUM->SUM) -> Agg(PARTIAL, SUM(a)) -> Values(a)
        tester().assertThat(new PushPartialAggregationThroughExchangeRuleSet(getFunctionManager(), false).withProjectionRule())
                .setSystemProperty(PARTIAL_AGGREGATION_STRATEGY, "AUTOMATIC")
                .on(p -> {
                    VariableReferenceExpression a = p.variable("a", BIGINT);
