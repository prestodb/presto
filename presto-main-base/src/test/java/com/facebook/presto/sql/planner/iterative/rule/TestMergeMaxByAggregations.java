...
import static com.facebook.presto.SystemSessionProperties.MERGE_MAX_BY_AGGREGATIONS;
        ...
        extends BaseRuleTest
{
    ...
    public void testMergeMaxByWithSameComparisonKey()
    {
        tester().assertThat(new MergeMaxByAggregations(getMetadata().getFunctionAndTypeManager()))
                .setSystemProperty(MERGE_MAX_BY_AGGREGATIONS, "true")
    ...
    {
        tester().assertThat(new MergeMaxByAggregations(getMetadata().getFunctionAndTypeManager()))
                .setSystemProperty(MERGE_MAX_BY_AGGREGATIONS, "false")
                .on(p -> p.aggregation(af -> {
                    p.variable("v1", BIGINT);
                    p.variable("v2", BIGINT);
                    p.variable("k", BIGINT);
