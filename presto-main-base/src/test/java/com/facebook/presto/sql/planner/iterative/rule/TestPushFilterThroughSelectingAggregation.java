        ...
        extends BaseRuleTest
{
    ...
    @Test
    public void testDoesNotPushArbitrary()
    {
        // ARBITRARY pushdown is not safe: it changes the distribution of values that can be
        // returned (e.g. HAVING arbitrary(x) = 'foo' with x = 'foo' pushed below would always
        // return 'foo' when previously it might have returned something else).
        tester().assertThat(new PushFilterThroughSelectingAggregation(tester().getMetadata()))
                .setSystemProperty(PUSH_FILTER_THROUGH_SELECTING_AGGREGATION, "true")
                .on(p -> {
                    VariableReferenceExpression x = p.variable("x", BIGINT);
                    VariableReferenceExpression g = p.variable("g", BIGINT);
                    VariableReferenceExpression arbitraryX = p.variable("arbitrary_x", BIGINT);
                    return p.filter(
                            p.rowExpression("arbitrary_x = BIGINT '5'"),
                            p.aggregation(ab -> ab
                                    .source(p.values(x, g))
                                    .addAggregation(arbitraryX, p.rowExpression("arbitrary(x)"))
                                    .singleGroupingSet(g)));
                })
                .doesNotFire();
    }
