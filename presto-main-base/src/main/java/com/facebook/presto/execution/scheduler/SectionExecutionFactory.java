...
public class SectionExecutionFactory
{
            ...
            CTEMaterializationTracker cteMaterializationTracker)
    {
        ...
        if (partitioningHandle.equals(SOURCE_DISTRIBUTION)) {
            ...
            if (hasDpp) {
                setExpectedPartitionsForFilters(dynamicFilterService, session.getQueryId(), plan.getFragment().getRoot(), 1);
            }
    ...
    // The DynamicFilterSourceOperator always runs in the same stage as the JoinNode
    // (created by LocalExecutionPlanner.createCoordinatorDynamicFilterSourceOperatorFactory),
    // so the current stage's task count is the correct expected partitions value,
    // regardless of whether the build side is local or a RemoteSourceNode.
    @VisibleForTesting
