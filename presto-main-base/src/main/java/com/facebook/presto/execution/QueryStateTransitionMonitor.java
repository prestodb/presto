...
public class QueryStateTransitionMonitor
{
    ...
    // TODO: Restore to 2.0 and 10 after testing
    private static final double ANOMALY_THRESHOLD_STDDEV = -999.0;  // TESTING: negative = everything is anomaly
    private static final int MIN_SAMPLES_FOR_ANOMALY_DETECTION = 1;  // TESTING: trigger on first query
    ...
    void checkAndLogAnomaly(QueryId queryId, QueryState state, long durationMillis, CounterStat anomalyCounter)
    {
        ...
        // Add the sample to rolling stats
        rollingStats.addSample(durationMillis);

        // Only perform anomaly detection after we have enough samples
        ...
        // Avoid division by zero or detection when there's no variance
        if (stddev < 1.0) {
            return;
        }
