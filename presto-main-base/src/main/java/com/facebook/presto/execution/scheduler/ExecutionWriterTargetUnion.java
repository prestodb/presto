...
public class ExecutionWriterTargetUnion
{
    ...
    {
        if (executionWriterTargetUnion.getId() == 1) {
            return executionWriterTargetUnion.getCreateHandle();
        }
        else if (executionWriterTargetUnion.getId() == 2) {
            return executionWriterTargetUnion.getInsertHandle();
        }
        else if (executionWriterTargetUnion.getId() == 3) {
            return executionWriterTargetUnion.getDeleteHandle();
        }
        else if (executionWriterTargetUnion.getId() == 4) {
            return executionWriterTargetUnion.getRefreshMaterializedViewHandle();
        }
        else if (executionWriterTargetUnion.getId() == 5) {
            return executionWriterTargetUnion.getUpdateHandle();
        }
        else {
            throw new IllegalArgumentException("Unrecognized execution writer target: " + executionWriterTargetUnion.getId());
        }
    }
