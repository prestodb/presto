        ...
        implements NodeOverloadPolicy
{
    ...
    private final boolean isClusterOverloadThrottlingEnabled;
    ...
    private static final double OVERLOAD_THRESHOLD = 1.0;

    @Inject
    public CpuMemoryOverloadPolicy(ClusterOverloadConfig config)
    {
        this.isClusterOverloadThrottlingEnabled = config.isClusterOverloadThrottlingEnabled();
    ...
    private OverloadStats collectOverloadStats(Set<InternalNode> activeNodes, InternalNodeManager nodeManager)
    {
        ...
        for (InternalNode node : activeNodes) {
            Map<String, Double> metrics = nodeManager.getNodeLoadMetrics(node.getNodeIdentifier());
            String nodeId = node.getNodeIdentifier();

            // Check for CPU overload
            Double cpuValue = metrics.get(CPU_OVERLOAD_METRIC);
            if (cpuValue != null && cpuValue >= OVERLOAD_THRESHOLD) {
                overloadedNodeIds.add(String.format("%s (CPU: %.2f)", nodeId, cpuValue));
                overloadedWorkersCnt++;
                continue;
            }

            // Check for memory overload
            Double memoryValue = metrics.get(MEMORY_OVERLOAD_METRIC);
            if (memoryValue != null && memoryValue >= OVERLOAD_THRESHOLD) {
                overloadedNodeIds.add(String.format("%s (Memory: %.2f)", nodeId, memoryValue));
                overloadedWorkersCnt++;
            }
        }
