...
public final class FunctionExtractor
{
    ...
    public static List<? extends SqlFunction> extractFunctions(Class<?> clazz, CatalogSchemaName functionNamespace)
    {
        if (WindowFunction.class.isAssignableFrom(clazz)) {
            checkArgument(functionNamespace.equals(JAVA_BUILTIN_NAMESPACE), format("Connector specific Window functions are not supported: Class [%s], Namespace [%s]", clazz.getName(), functionNamespace));
        ...
        if (clazz.isAnnotationPresent(AggregationFunction.class)) {
            return SqlAggregationFunction.createFunctionsByAnnotations(clazz, functionNamespace);
        }

        if (clazz.isAnnotationPresent(ScalarFunction.class)) {
            return ScalarFromAnnotationsParser.parseFunctionDefinition(clazz, functionNamespace);
        }
        if (clazz.isAnnotationPresent(ScalarOperator.class)) {
            checkArgument(functionNamespace.equals(JAVA_BUILTIN_NAMESPACE), format("Connector specific Scalar Operator functions are not supported: Class [%s], Namespace [%s]", clazz.getName(), functionNamespace));
        ...
        if (clazz.isAnnotationPresent(SqlInvokedScalarFunction.class)) {
            return SqlInvokedScalarFromAnnotationsParser.parseFunctionDefinition(clazz, functionNamespace);
        }
        ...
        List<SqlFunction> scalarFunctions = ImmutableList.<SqlFunction>builder()
                ...
                .addAll(SqlInvokedScalarFromAnnotationsParser.parseFunctionDefinitions(clazz, functionNamespace))
                .addAll(CodegenScalarFromAnnotationsParser.parseFunctionDefinitions(clazz, functionNamespace))
                .build();
