...
public class NativeFunctionRegistryTool
{
    ...
    @Inject
    public NativeFunctionRegistryTool(
    ...
    public UdfFunctionSignatureMap getNativeFunctionSignatureMap()
    {
        try {
    ...
    private URI getSidecarLocationOnStartup()
    {
        ...
        while (sidecarNode == null && retryCount < MAX_RETRIES) {
            try {
                ...
                retryCount++;
                log.error("Attempt Error getting sidecar node (attempt " + retryCount + "): " + e.getMessage());
