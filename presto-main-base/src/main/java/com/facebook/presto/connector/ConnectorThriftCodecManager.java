...
public class ConnectorThriftCodecManager
{
    ...
    public ConnectorThriftCodec<ConnectorSplit> getConnectorSplitThriftCodec(String connectorId)
    {
        requireNonNull(connectorId, "connectorId is null");
        return Optional.ofNullable(connectorThriftCodecProviders.get(connectorId)).flatMap(ConnectorThriftCodecProvider::getConnectorSplitCodec)
                .orElseThrow(() -> new IllegalArgumentException("Can not find split codec for connector: " + connectorId));
    }

    public ConnectorThriftCodec<ConnectorTransactionHandle> getConnectorTransactionHandleThriftCodec(String connectorId)
    {
        requireNonNull(connectorId, "connectorId is null");
        return Optional.ofNullable(connectorThriftCodecProviders.get(connectorId)).flatMap(ConnectorThriftCodecProvider::getConnectorTransactionHandleCodec)
                .orElseThrow(() -> new IllegalArgumentException("Can not find transactionHandle codec for connector: " + connectorId));
    }

    public boolean isConnectorThriftCodecAvailable(String connectorId)
    {
        requireNonNull(connectorId, "connectorId is null");
        return connectorThriftCodecProviders.containsKey(connectorId);
    }
}
