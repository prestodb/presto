...
public class ConnectorTransactionHandleThriftCodec
        implements ThriftCodec<Split.ConnectorTransactionHandleWrapper>
{
    private static final short JSON_DATA_FIELD_ID = 111;
    private static final String JSON_DATA_FIELD_NAME = "connectorTransactionHandleWrapper";
    private static final String JSON_DATA_STRUCT_NAME = "ConnectorTransactionHandleWrapper";

    private static final ThriftType THRIFT_TYPE = createConnectorSpecificThriftType(Split.ConnectorTransactionHandleWrapper.class, "transactionHandle", "getTransactionHandle");
    private final ConnectorSpecificCodecManager connectorSpecificCodecManager;
    private final JsonCodec<Split.ConnectorTransactionHandleWrapper> jsonCodec;

    @Inject
    public ConnectorTransactionHandleThriftCodec(JsonCodec<Split.ConnectorTransactionHandleWrapper> jsonCodec, ConnectorSpecificCodecManager connectorSpecificCodecManager, ThriftCatalog thriftCatalog)
    {
    ...
    @Override
    public Split.ConnectorTransactionHandleWrapper read(TProtocolReader protocol)
            throws Exception
    {
        ...
        return new Split.ConnectorTransactionHandleWrapper(connectorId, connectorSpecificCodecManager.getConnectorTransactionHandleCodec(connectorId).deserialize(bytes));
    }

    @Override
    public void write(Split.ConnectorTransactionHandleWrapper value, TProtocolWriter protocol)
            throws Exception
    {
        if (connectorSpecificCodecManager.isConnectorSpecificCodecAvailable(value.getConnectorId())) {
            protocol.writeStructBegin(new TStruct("ConnectorTransactionHandleWrapper"));
            ...
            protocol.writeBinary(ByteBuffer.wrap(connectorSpecificCodecManager.getConnectorTransactionHandleCodec(value.getConnectorId()).serialize(value.getTransactionHandle())));
            protocol.writeFieldEnd();
