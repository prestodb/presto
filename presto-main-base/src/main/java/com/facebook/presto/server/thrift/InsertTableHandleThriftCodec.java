        ...
        extends AbstractTypedThriftCodec<ConnectorInsertTableHandle>
{
            ...
            throws Exception
    {
        ...
        checkArgument(byteBuffer.position() == 0, "Thrift read InsertTableHandle byte buffer position not 0");
        byte[] bytes = byteBuffer.array();
        return connectorCodecManager.getInsertTableHandleCodec(connectorId).map(codec -> codec.deserialize(bytes))
                .orElseThrow(() -> new PrestoException(GENERIC_INTERNAL_ERROR, "Failed to deserialize InsertTableHandle"));
    }
