        ...
        extends AbstractTypedThriftCodec<ConnectorDeleteTableHandle>
{
            ...
            throws Exception
    {
        ...
        checkArgument(byteBuffer.position() == 0, "Thrift read DeleteTableHandle byte buffer position not 0");
        byte[] bytes = byteBuffer.array();
        return connectorCodecManager.getDeleteTableHandleCodec(connectorId).map(codec -> codec.deserialize(bytes))
                .orElseThrow(() -> new PrestoException(GENERIC_INTERNAL_ERROR, "Failed to deserialize DeleteTableHandle"));
    }
