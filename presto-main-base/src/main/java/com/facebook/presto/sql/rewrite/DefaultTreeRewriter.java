 ...
 * Expression node reconstruction is not supported and left to the users.
 */
        ...
        extends AstVisitor<Node, C>
{
    ...
    protected Node visitFrameBound(FrameBound node, C context)
    {
        ...
        return value.map(expression -> new FrameBound(node.getType(), expression)).orElseGet(() -> new FrameBound(node.getType()));
    }

    @Override
    protected Node visitFunctionCall(FunctionCall node, C context)
    {
        Optional<Window> window = process(node.getWindow(), context);
        Optional<Expression> filter = process(node.getFilter(), context);
        Optional<OrderBy> orderBy = process(node.getOrderBy(), context);
        List<Expression> arguments = process(node.getArguments(), context);
        if (sameElement(node.getWindow(), window) && sameElement(node.getFilter(), filter) && sameElement(node.getOrderBy(), orderBy) && sameElements(node.getArguments(), arguments)) {
            return node;
        }

        return node.getLocation().isPresent() ? new FunctionCall(node.getLocation().get(), node.getName(), node.getWindow(), node.getFilter(), node.getOrderBy(), node.isDistinct(), node.isIgnoreNulls(), node.getArguments()) :
                new FunctionCall(node.getName(), node.getWindow(), node.getFilter(), node.getOrderBy(), node.isDistinct(), node.isIgnoreNulls(), node.getArguments());
    }
