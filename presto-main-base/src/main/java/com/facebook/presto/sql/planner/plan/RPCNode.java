...
/**
 * Plan node for single-operator async RPC execution.
 *
 * This node has a SINGLE source (the data input). The RPCOperator created
 * from this node handles both dispatch (sending requests) and join (receiving
 * responses) within a single operator.
 *
 ...
 * - arguments: The original argument expressions. Used by the C++ plan
 *   converter to extract argument types and constant values for
 *   AsyncRPCFunction::initialize(). Types cannot come from argumentColumns
 *   alone because those are just column name strings — the expression tree
 *   carries the type information.
 * - argumentColumns: Column names in the source output that contain
 *   pre-evaluated argument values (used by RPCOperator::addInput())
 * - outputVariable: The result variable for RPC responses
 * - streamingMode: The streaming mode for RPC execution (PER_ROW or BATCH)
 * - dispatchBatchSize: For BATCH mode pipelining
 */
@Immutable
        ...
        extends InternalPlanNode
{
     ...
     * - PER_ROW: Each row is dispatched individually, allowing parallel execution
     * - BATCH: All rows are collected and dispatched as a single batch
     */
            ...
            int dispatchBatchSize)
    {
        ...
        // Output includes source outputs plus the RPC result
    ...
    {
        checkArgument(newChildren.size() == 1, "RPCNode requires exactly 1 child");
        return new RPCNode(
                getSourceLocation(),
                getId(),
