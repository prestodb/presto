        ...
        extends RowExpressionRewriteRuleSet
{
            ...
            extends RowExpressionRewriter<Void>
    {
                ...
                if (canRemoveMapCast(fromKeyType, fromValueType, toKeyType, toValueType, node.getArguments().get(1).getType())) {
                    if (functionResolution.isSubscriptFunction(node.getFunctionHandle())) {
                        ...
                        if (node.getArguments().get(1) instanceof CallExpression && functionResolution.isArrayConstructor(((CallExpression) node.getArguments().get(1)).getFunctionHandle())) {
                            ...
                            newIndex = call(functionAndTypeManager, "array_constructor", new ArrayType(INTEGER), newArguments);
                        }
                            ...
                            for (int i = 0; i < arrayValue.getPositionCount(); ++i) {
                                ...
                                arguments.add(tryCast(functionAndTypeManager, mapIndex, INTEGER));
                            }
                            newIndex = call(functionAndTypeManager, "array_constructor", new ArrayType(INTEGER), arguments.build());
                        }
                        if (newIndex != null) {
                            CallExpression mapSubset = call(functionAndTypeManager, "map_subset", castInput.getType(), castInput, newIndex);
        ...
        {
            return fromValueType.equals(toValueType) && fromKeyType.equals(INTEGER) && toKeyType.equals(BIGINT) && (indexType.equals(BIGINT) || (indexType instanceof ArrayType && ((ArrayType) indexType).getElementType().equals(BIGINT)));
        }
    }
}
