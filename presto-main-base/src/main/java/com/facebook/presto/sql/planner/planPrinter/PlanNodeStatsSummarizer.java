    ...
    private static List<PlanNodeStats> getPlanNodeStats(TaskStats taskStats)
    {
        ...
        for (PipelineStats pipelineStats : taskStats.getPipelines()) {
            ...
            for (OperatorStats operatorStats : pipelineStats.getOperatorSummaries()) {
                ...
                long scheduledMillis = operatorStats.getAddInputWallInNanos() + operatorStats.getGetOutputWallInNanos() + operatorStats.getFinishWallInNanos();
                planNodeScheduledMillis.merge(planNodeId, scheduledMillis, Long::sum);

                long cpuMillis = operatorStats.getAddInputCpuInNanos() + operatorStats.getGetOutputCpuInNanos() + operatorStats.getFinishCpuInNanos();
                planNodeCpuMillis.merge(planNodeId, cpuMillis, Long::sum);
                planNodeBlockedMillis.merge(planNodeId, (long) operatorStats.getBlockedWallInNanos(), Long::sum);
                planNodeAddInputMillis.merge(planNodeId, (long) operatorStats.getAddInputWallInNanos(), Long::sum);
                planNodeFinishMillis.merge(planNodeId, (long) operatorStats.getFinishWallInNanos(), Long::sum);
                planNodeGetOutputMillis.merge(planNodeId, (long) operatorStats.getGetOutputWallInNanos(), Long::sum);
