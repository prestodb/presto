...
public class TestRedshiftClient
{
    ...
    @Test
    public void testIsRunAsInvokerAndOwnerParsing()
    {
        RedshiftClient client = createRedshiftClient();

        // Null comment -> INVOKER fallback
        assertEquals(client.isRunAsInvoker(null), true);
        assertEquals(client.parsePrestoOwner(null), Optional.empty());

        // Empty / Non-matching comment -> INVOKER fallback
        assertEquals(client.isRunAsInvoker("some other comment"), true);
        assertEquals(client.parsePrestoOwner("some other comment"), Optional.empty());

        // Presto INVOKER comment
        assertEquals(client.isRunAsInvoker("presto-view-security:I"), true);
        assertEquals(client.parsePrestoOwner("presto-view-security:I"), Optional.empty());

        // Presto DEFINER comment with owner
        assertEquals(client.isRunAsInvoker("presto-view-security:D:alice"), false);
        assertEquals(client.parsePrestoOwner("presto-view-security:D:alice"), Optional.of("alice"));

        // Presto DEFINER comment without owner
        assertEquals(client.isRunAsInvoker("presto-view-security:D"), false);
        assertEquals(client.parsePrestoOwner("presto-view-security:D"), Optional.empty());
    }
