...
public class TestVerifierRewriteQueries
{
    ...
    private ImmutableList<QueryPair> queryPairs;
    ...
    public void testSingleThread()
    {
        ...
        config.setQueryRewriteThreadCount(1);
        List<QueryPair> rewrittenQueries = VerifyCommand.rewriteQueries(parser, config, queryPairs);
    ...
    {
        config.setControlGateway(URL);
        config.setQueryRewriteThreadCount(5);
        List<QueryPair> rewrittenQueries = VerifyCommand.rewriteQueries(parser, config, queryPairs);
        assertEquals(rewrittenQueries.size(), queryPairs.size());
    }
    ...
    public void testQueryRewriteException()
    {
        ...
        Query invalidQuery = new Query(
                ...
                ImmutableMap.of());
        ImmutableList.Builder<QueryPair> builder = ImmutableList.builder();
        builder.addAll(queryPairs);
        builder.add(new QueryPair(QUERY_SUITE, QUERY_NAME, invalidQuery, invalidQuery));
        List<QueryPair> rewrittenQueries = VerifyCommand.rewriteQueries(parser, config, builder.build());
