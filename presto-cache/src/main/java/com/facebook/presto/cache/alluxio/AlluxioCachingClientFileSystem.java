        ...
        implements alluxio.client.file.FileSystem
{
    ...
    public List<URIStatus> listStatus(AlluxioURI path, ListStatusPOptions options)
            throws IOException
    {
        return Arrays.stream(fileSystem.listStatus(toPath(path))).map(this::toUriStatus).collect(Collectors.toList());
    }
    ...
    private URIStatus toUriStatus(FileStatus status)
    {
        ...
        // assign a unique identifier derived from file path
        info.setFileIdentifier(md5().hashString(status.getPath().toString(), UTF_8).toString());
        info.setLength(status.getLen()).setPath(status.getPath().toUri().getPath());
