...
public class TestArrowBatchSource
{
            ...
            throws IOException
    {
        ...
        BlockBuilder timestampBuilder = TIMESTAMP_WITH_TIME_ZONE.createBlockBuilder(null, numValues);
        for (int i = 0; i < numValues; i++) {
            TIME_WITH_TIME_ZONE.writeLong(timeBuilder, packDateTimeWithZone(1111 * i, getTimeZoneKeyForOffset(i)));
            TIMESTAMP_WITH_TIME_ZONE.writeLong(timestampBuilder, packDateTimeWithZone(1111 * i, getTimeZoneKeyForOffset(i)));
        }
        Block timeBlock = timeBuilder.build();
        Block timestampBlock = timestampBuilder.build();
        List<Block> blocks = ImmutableList.of(timeBlock, timestampBlock);

        try (TimeMilliVector timeVector = new TimeMilliVector("time", allocator);
                TimeStampMilliVector timestampVector = new TimeStampMilliVector("timestamp", allocator);
                VectorSchemaRoot expectedRoot = new VectorSchemaRoot(Arrays.asList(timeVector, timestampVector))) {
            for (int i = 0; i < numValues; i++) {
                timeVector.setSafe(i, (int) unpackMillisUtc(timeBlock.getLong(i)));
                timestampVector.setSafe(i, unpackMillisUtc(timestampBlock.getLong(i)));
            }
            expectedRoot.setRowCount(numValues);

            ColumnMetadata timeColumn = ColumnMetadata.builder().setName(timeVector.getName()).setType(TIME_WITH_TIME_ZONE).build();
            ColumnMetadata timestampColumn = ColumnMetadata.builder().setName(timestampVector.getName()).setType(TIMESTAMP_WITH_TIME_ZONE).build();
            List<ColumnMetadata> columns = ImmutableList.of(timeColumn, timestampColumn);
            ...
            implements ConnectorPageSource
    {
        ...
        public TestArrowPageSource(int rowCount, List<Block> blocks, List<ColumnMetadata> columns, int numPages)
        {
