...
public class TestScheduler
{
            ...
            throws Exception
    {
        ...
        for (int i = 0; i < 10_000; i++) {
            URI target = scheduler.getDestination(getRequestInfo("test"), "").orElse(new URI("invalid"));
            ...
            throws Exception
    {
        ...
        URI target1 = scheduler.getDestination(getRequestInfo(user), "").orElse(new URI("invalid"));
        assertTrue(servers.contains(target1));

        URI target2 = scheduler.getDestination(getRequestInfo(user), "").orElse(new URI("invalid"));
        assertTrue(servers.contains(target2));

        URI target3 = scheduler.getDestination(getRequestInfo(user), "").orElse(new URI("invalid"));
            ...
            throws Exception
    {
        ...
        URI target1 = scheduler.getDestination(getRequestInfo("test"), "").orElse(new URI("invalid"));
        assertTrue(servers.contains(target1));
        assertEquals(target1.getPath(), "192.168.0.1");

        URI target2 = scheduler.getDestination(getRequestInfo("test"), "").orElse(new URI("invalid"));
        assertTrue(servers.contains(target2));
        assertEquals(target2.getPath(), "192.168.0.2");

        URI target3 = scheduler.getDestination(getRequestInfo("test"), "").orElse(new URI("invalid"));
        assertTrue(servers.contains(target3));
        assertEquals(target3.getPath(), "192.168.0.3");

        URI target4 = scheduler.getDestination(getRequestInfo("test"), "").orElse(new URI("invalid"));
    ...
    private RequestInfo getRequestInfo(String user)
    {
