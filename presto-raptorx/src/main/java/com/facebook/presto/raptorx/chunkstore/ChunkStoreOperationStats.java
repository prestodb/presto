...
import static com.facebook.presto.raptorx.RaptorErrorCode.RAPTOR_CHUNKSTORE_TIMEOUT;
import static com.facebook.presto.raptorx.RaptorErrorCode.RAPTOR_CHUNK_NOT_FOUND;
import static java.util.concurrent.TimeUnit.MILLISECONDS;

public class ChunkStoreOperationStats
{
    ...
    public <V> V run(Supplier<V> supplier)
    {
        try (TimeStat.BlockTimer ignored = time.time()) {
            ...
            if (e.getErrorCode().equals(RAPTOR_CHUNK_NOT_FOUND.toErrorCode())) {
                successes.update(1);
            }
            else if (e.getErrorCode().equals(RAPTOR_CHUNKSTORE_TIMEOUT.toErrorCode())) {
                timeouts.update(1);
            }
            else {
                failures.update(1);
