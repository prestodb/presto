        ...
        implements JsonReader
{
    ...
    public MapJsonReader(StreamDescriptor streamDescriptor, boolean checkForNulls, DateTimeZone sessionTimeZone)
    {
        this.streamDescriptor = checkNotNull(streamDescriptor, "stream is null");
        this.checkForNulls = checkForNulls;

        keyReader = createJsonMapKeyReader(streamDescriptor.getNestedStreams().get(0), sessionTimeZone);
        valueReader = createJsonReader(streamDescriptor.getNestedStreams().get(1), true, sessionTimeZone);
    }
