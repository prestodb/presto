        ...
        implements DynamicConfigurationProvider
{
    ...
    {
        if (!mappings.get().isAWSS3SecurityMappingEnabled()) {
            return;
        }
        ...
        AWSS3SecurityMapping awsS3SecurityMapping = mappings.get().getAWSS3SecurityMapping(context.getIdentity())
                .orElseThrow(() -> new AccessDeniedException("No matching AWS S3 security mapping"));
