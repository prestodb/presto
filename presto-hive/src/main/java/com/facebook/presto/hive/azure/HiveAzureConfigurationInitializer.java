        ...
        implements AzureConfigurationInitializer
{
    ...
    public HiveAzureConfigurationInitializer(HiveAzureConfig config)
    {
        ...
        if (abfsOAuthClientEndpoint.isPresent() || abfsOAuthClientSecret.isPresent() || abfsOAuthClientId.isPresent()) {
            checkArgument(
                    abfsOAuthClientEndpoint.isPresent() && abfsOAuthClientId.isPresent() && abfsOAuthClientSecret.isPresent() && abfsStorageAccount.isPresent(),
                    "ABFS OAuth2 authentication requires endpoint, client ID, client secret, and a storage account, all must be set.");
        }
    }
