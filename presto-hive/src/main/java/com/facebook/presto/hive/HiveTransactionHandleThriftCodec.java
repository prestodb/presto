        ...
        implements ConnectorThriftCodec<ConnectorTransactionHandle>
{
    ...
    @Override
    public void serialize(ConnectorTransactionHandle handle, TProtocolWriter writer)
    {
        try {
            thriftCodec.write((HiveTransactionHandle) handle, writer);
        }
        catch (Exception e) {
            throw new IllegalArgumentException("Can not serialize HiveTransactionHandle", e);
        }
    }

    @Override
    public ConnectorTransactionHandle deserialize(TProtocolReader reader)
    {
        try {
            return thriftCodec.read(reader);
        }
        catch (Exception e) {
            throw new IllegalArgumentException("Can not deserialize HiveTransactionHandle", e);
        }
    }
}
