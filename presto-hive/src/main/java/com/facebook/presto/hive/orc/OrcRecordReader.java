            ...
            throws IOException
    {
        ...
        for (int i = 0; i < fileStripes.size(); i++) {
            ...
            if (splitOffset <= stripe.getOffset() && stripe.getOffset() < splitEndOffset) {
                // ORC files created by Hive0.11 don't have Stripe Stats, consider full stripe in such cases
                TupleDomain<HiveColumnHandle> stripeDomain;
                if (stripeStats.size() == 0) {
                    stripeDomain = TupleDomain.<HiveColumnHandle>all();
                }
                else {
                    stripeDomain = extractDomain(columnHandleStreamIndex, stripe.getNumberOfRows(), stripeStats.get(i).getColStatsList());
                }
