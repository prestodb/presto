...
import static com.facebook.presto.hive.parquet.ParquetUtils.readIntLittleEndian;
...
public final class ParquetMetadataReader
{
    ...
    private static final byte[] MAGIC = "PAR1".getBytes(Charset.forName("ASCII"));
        ...
        throws IOException
    {
        ...
        try {
            ...
            checkArgument(length >= MAGIC.length + PARQUET_METADATA_LENGTH + MAGIC.length,
                        file.toString() + " is not a valid Parquet File");
            ...
            checkArgument(Arrays.equals(MAGIC, magic), "Not valid Parquet file: " + file.toString() +
                        " expected magic number: " + Arrays.toString(MAGIC) + " got: " + Arrays.toString(magic));
            ...
            checkArgument(metadataIndex >= MAGIC.length && metadataIndex < metadataLengthIndex,
                        "Corrupted Parquet file: " + file.toString() +
                        " metadata index: " + metadataIndex + " out of range");
            inputStream.seek(metadataIndex);
            FileMetaData fileMetaData = readFileMetaData(inputStream);
            List<SchemaElement> schema = fileMetaData.getSchema();
            checkArgument(schema.size() > 0, "Empty Parquet schema in file: " + file.toString());
            ...
            if (rowGroups != null) {
                for (RowGroup rowGroup : rowGroups) {
                    ...
                    checkArgument(columns.size() > 0, "No columns in row group: " + rowGroup.toString());
            ...
            return new ParquetMetadata(new parquet.hadoop.metadata.FileMetaData(messageType,
                                                                                keyValueMetaData,
    ...
    private static void readTypeSchema(Types.GroupBuilder builder, Iterator<SchemaElement> schemaIterator, int typeCount)
    {
        for (int i = 0; i < typeCount; i++) {
            ...
            Types.Builder typeBuilder = null;
            if (element.type == null) {
                typeBuilder = builder.group(Repetition.valueOf(element.repetition_type.name()));
                readTypeSchema((Types.GroupBuilder) typeBuilder,
                                schemaIterator,
                                element.num_children);
            }
            else {
                Types.PrimitiveBuilder primitiveBuilder = builder.primitive(getTypeName(element.type),
                                                                            Repetition.valueOf(element.repetition_type.name()));
    ...
    {
        Set<parquet.column.Encoding> columnEncodings = new HashSet<parquet.column.Encoding>();
