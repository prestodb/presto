        ...
        extends HiveRecordCursor
{
                ...
                if (!column.isPartitionKey() && column.getHiveColumnIndex() < messageType.getFieldCount()) {
                    ...
                    if (parquetType.isPrimitive()) {
                        ...
                        switch (groupType.getOriginalType()) {
                            case LIST:
                                converters.add(new ParquetJsonColumnConverter(new ParquetListJsonConverter(groupType.getName(), null, groupType), i, false));
                                break;
                            case MAP:
                            case MAP_KEY_VALUE: // original versions of Parquet have map and entry swapped
                                converters.add(new ParquetJsonColumnConverter(new ParquetMapJsonConverter(groupType.getName(), null, groupType), i, false));
                                break;
                            case UTF8:
                            case ENUM:
            ...
            extends GroupConverter
    {
        ...
        private final int fieldIndex;
        private final boolean legacyMap;

        private JsonGenerator generator;

        public ParquetJsonColumnConverter(GroupedJsonConverter jsonConverter, int fieldIndex, boolean legacyMap)
        {
