        ...
        extends PartitionLoader
{
            ...
            throws IOException
    {
        ...
        hiveFileInfoIterator.forEachRemaining(hiveFileInfo -> targetParentHiveFileInfos.put(hiveFileInfo.getPath(), hiveFileInfo));

        if (parentPathCached && isUseListDirectoryCache(session)) {
            boolean allPathsExist = currentTargetPaths.stream().map(Path::toString).allMatch(targetParentHiveFileInfos::containsKey);
            ...
            if (!allPathsExist) {
                ...
                hiveFileInfoIterator = directoryLister.list(targetFilesystem, table, targetParent, partition.getPartition(), namenodeStats, hiveDirectoryContext);
                hiveFileInfoIterator.forEachRemaining(hiveFileInfo -> targetParentHiveFileInfos.put(hiveFileInfo.getPath(), hiveFileInfo));
            }
        }

        List<HiveFileInfo> targetPathsHiveFileInfos = currentTargetPaths.stream().map(targetPath -> {
            HiveFileInfo hiveFileInfo = targetParentHiveFileInfos.get(targetPath.toString());
