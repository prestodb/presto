...
public final class OrcDomainExtractor
{
            ...
            RowIndex[] indexes)
    {
        ...
        for (long rows = rowsInStripe; rows > 0; rows -= rowIndexStride) {
            ...
            TupleDomain<HiveColumnHandle> rowGroupTupleDomain = extractDomain(typeManager, columnHandles, indexes, rowGroup, rowsInStride);
            ...
            long rowCount)
    {
        ...
        for (Entry<HiveColumnHandle, Integer> entry : columnHandles.entrySet()) {
            ...
            Domain domain = getDomain(typeManager.getType(columnHandle.getTypeName()), rowCount, rowIndexEntry.hasStatistics() ? rowIndexEntry.getStatistics() : null);
