        ...
        implements ConnectorPageSinkProvider
{
    ...
    public ConnectorPageSink createPageSink(ConnectorTransactionHandle transactionHandle, ConnectorSession session, ConnectorOutputTableHandle outputTableHandle)
    {
        return authentication.doAs(session.getUser(), () -> {
            ConnectorPageSink targetPageSink = targetConnectorPageSinkProvider.createPageSink(transactionHandle, session, outputTableHandle);
    ...
    {
        return authentication.doAs(session.getUser(), () -> {
            ConnectorPageSink targetPageSink = targetConnectorPageSinkProvider.createPageSink(transactionHandle, session, insertTableHandle);
            return new HdfsAuthenticatingPageSink(session, authentication, targetPageSink);
        });
    }
}
