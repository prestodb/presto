        ...
        extends FileSystem
{
    ...
    private static final PrestoS3FileSystemMetricCollector metricCollector = new PrestoS3FileSystemMetricCollector(STATS);
                    ...
                    .run("getS3ObjectMetadata", () -> {
                        try {
                            ...
                            if (e instanceof AmazonS3Exception) {
                                ...
                                if (amazonS3Exception.getStatusCode() == SC_NOT_FOUND) {
                                    return null;
                                }
                                else if (amazonS3Exception.getStatusCode() == SC_FORBIDDEN) {
    ...
    private AmazonS3Client createAmazonS3Client(URI uri, Configuration hadoopConfig, ClientConfiguration clientConfig)
    {
        ...
        try {
            return new AmazonS3Client(new StaticCredentialsProvider(getAwsCredentials(uri, hadoopConfig)), clientConfig, metricCollector);
        }
        catch (IllegalArgumentException ignored) {
        }

        if (useInstanceCredentials) {
            return new AmazonS3Client(new InstanceProfileCredentialsProvider(), clientConfig, metricCollector);
        }
                ...
                return retry()
                        ...
                        .run("getS3Object", () -> {
                            try {
                                ...
                                if (e instanceof AmazonS3Exception) {
                                    AmazonS3Exception amazonS3Exception = (AmazonS3Exception) e;
                                    if (amazonS3Exception.getStatusCode() == SC_FORBIDDEN) {
                                        throw new UnrecoverableS3OperationException(e);
                                    }
                                }
                                throw Throwables.propagate(e);
