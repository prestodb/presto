        ...
        implements ConnectorAccessControl
{
    ...
    {
        Set<HivePrivilegeInfo.HivePrivilege> privilegeSet = metastore.getDatabasePrivileges(identity.getUser(), schemaName).stream()
    ...
    private boolean checkTablePermission(Identity identity, SchemaTableName tableName, HivePrivilegeInfo.HivePrivilege... requiredPrivileges)
    {
        ...
        Set<HivePrivilegeInfo.HivePrivilege> privilegeSet = metastore.getTablePrivileges(identity.getUser(), tableName.getSchemaName(), tableName.getTableName()).stream()
                .map(HivePrivilegeInfo::getHivePrivilege)
                .collect(Collectors.toSet());
        return privilegeSet.containsAll(ImmutableSet.copyOf(requiredPrivileges));
    }
