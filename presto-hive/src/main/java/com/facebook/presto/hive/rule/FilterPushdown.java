        ...
        implements ConnectorPlanOptimizer
{
    ...
    private boolean isPushdownFilterSupported(ConnectorSession session, TableHandle tableHandle)
    {
        ...
        ConnectorMetadata metadata = hiveMetadataProvider.apply((HiveTransactionHandle) tableHandle.getTransaction());
    ...
    {
        HiveMetadata metadata = hiveMetadataProvider.apply((HiveTransactionHandle) tableHandle.getTransaction());
        ...
        TupleDomain<ColumnHandle> entireColumnDomain = decomposedFilter.getTupleDomain()
                .transform(subfield -> isEntireColumn(subfield) ? symbolToColumnMapping.get(subfield.getRootName()) : null)
    ...
    {
        return hiveMetadataProvider.apply((HiveTransactionHandle) tableHandle.getTransaction()).getColumnMetadata(session, tableHandle.getConnectorHandle(), columnHandle).getName();
    }
        ...
        public PlanNode visitFilter(FilterNode node, Void context)
        {
            if (node.getSource() instanceof TableScanNode) {
                ...
                TableScanNode tableScan = new TableScanNode(
                        node.getId(),
                        new TableHandle(handle.getConnectorId(), handle.getConnectorHandle(), handle.getTransaction(), Optional.of(result.getLayout().getHandle())),
