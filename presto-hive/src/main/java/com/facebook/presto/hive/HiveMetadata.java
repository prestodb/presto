        ...
        for (PartitionUpdate partitionUpdate : partitionUpdates) {
            if (partitionUpdate.getName().isEmpty()) {
                ...
                if (partitionUpdate.getUpdateMode() == OVERWRITE) {
                    ...
                    PrincipalPrivileges principalPrivileges = fromHivePrivilegeInfos(metastore.listTablePrivileges(metastoreContext, handle.getSchemaName(), handle.getTableName(), null));

                    // first drop it
                    metastore.dropTable(new HdfsContext(session, handle.getSchemaName(), handle.getTableName()),
                            handle.getSchemaName(),
                            handle.getTableName());
