        ...
        implements GcsConfigurationInitializer
{
    ...
    public void updateConfiguration(Configuration config)
    {
        ...
        if (useGcsAccessToken) {
            ...
            config.set(GCS_CONFIG_PREFIX + ENABLE_SERVICE_ACCOUNTS_SUFFIX.getDeprecatedKeys(), "false");
            config.set(GCS_CONFIG_PREFIX + ACCESS_TOKEN_PROVIDER_IMPL_SUFFIX, GcsAccessTokenProvider.class.getName());
        }
        else if (jsonKeyFilePath != null) {
            // use service account key file
            config.set(GCS_CONFIG_PREFIX + ENABLE_SERVICE_ACCOUNTS_SUFFIX.getDeprecatedKeys(), "true");
            config.set(GCS_CONFIG_PREFIX + SERVICE_ACCOUNT_JSON_KEYFILE_SUFFIX, jsonKeyFilePath);
        }
    }
}
