        ...
        if (!partition.isPresent()) {
            if (table == null) {
                ...
                schema.setProperty(META_TABLE_COLUMN_TYPES, dataColumns.stream()
                        ...
                        .collect(joining(":")));
                writeInfo = locationService.getPartitionOrTableWriteInfo(locationHandle, partition, partitionName);

                if (partitionName.isPresent() && writeInfo.getWriteMode() == STAGE_AND_MOVE_TO_TARGET_DIRECTORY) {
