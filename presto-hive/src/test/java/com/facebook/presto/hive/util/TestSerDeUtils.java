...
public class TestSerDeUtils
{
    ...
    public void testListBlock()
    {
        ...
        Slice expected = sliceOf(new ArrayType(new ArrayType(rowType)), sliceOf(rowType, sliceOf(BIGINT, BIGINT, 8, 9L), sliceOf(BIGINT, BIGINT, 10, 11L)));
    ...
    public void testMapBlock()
    {
        ...
        com.facebook.presto.spi.type.Type rowType = new RowType(ImmutableList.of(BIGINT, BIGINT), Optional.empty());
        Slice expected = sliceOf(new ArrayType(new MapType(VARCHAR, rowType)), sliceOf(VARCHAR, rowType, "fifteen", sliceOf(BIGINT, BIGINT, 16, 17L), "twelve", sliceOf(BIGINT, BIGINT, 13, 14L)));
    ...
    public void testStructBlock()
    {
        ...
        Slice expected = sliceOf(BIGINT, BIGINT, 13, 14L);
        assertEquals(actual, expected);
        ...
        com.facebook.presto.spi.type.Type rowType = new RowType(ImmutableList.of(BIGINT, BIGINT), Optional.empty());
        // it would be fine to use VARCHAR for complex types as long as they're built by ArrayBuilder
        expected = sliceOf(ImmutableList.of(BIGINT, BIGINT, BIGINT, BIGINT, DOUBLE, DOUBLE, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR),
                        1, 2, 3, 4L, 5.01f, 6.001d, "seven", new byte[] {'2'},
                        sliceOf(new ArrayType(rowType), sliceOf(BIGINT, BIGINT, 2, -5L), sliceOf(BIGINT, BIGINT, -10, 0L)),
                        sliceOf(VARCHAR, rowType, "fifteen", sliceOf(BIGINT, BIGINT, -5, -10L), "twelve", sliceOf(BIGINT, BIGINT, 0, 5L)),
                        sliceOf(BIGINT, BIGINT, 18, 19L));
        assertEquals(actual, expected);
    }
            ...
            throws Exception
    {
        ...
        Slice expected = sliceOf(VARCHAR, BIGINT, "bye", 0);
    ...
    private static Slice sliceOf(com.facebook.presto.spi.type.Type elementType, Object... values)
    {
        return StructBuilder.arrayBuilder(elementType).addAll(values).build();
    }

    private static Slice sliceOf(com.facebook.presto.spi.type.Type  keyType, com.facebook.presto.spi.type.Type  valueType, Object... values)
    {
        return StructBuilder.mapBuilder(keyType, valueType).addAll(values).build();
    }

    private static Slice sliceOf(List<com.facebook.presto.spi.type.Type> parameterTypes, Object... values)
    {
