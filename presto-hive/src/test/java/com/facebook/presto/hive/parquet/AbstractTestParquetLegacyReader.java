...
public abstract class AbstractTestParquetLegacyReader
        extends AbstractTestParquetReader
{
    public AbstractTestParquetLegacyReader(ParquetTester tester)
    {
        super(tester);
    }
...
//        tester.testRoundTrip(javaShortObjectInspector, limit(cycle(values), 30_000), SMALLINT);
            ...
            throws Exception
    {
        tester.testRoundTrip(javaByteObjectInspector,
                transform(writeValues, AbstractTestParquetLegacyReader::intToByte),
                AbstractTestParquetLegacyReader::byteToInt,
                INTEGER);

        tester.testRoundTrip(javaShortObjectInspector,
                transform(writeValues, AbstractTestParquetLegacyReader::intToShort),
                AbstractTestParquetLegacyReader::shortToInt,
                INTEGER);

        tester.testRoundTrip(javaIntObjectInspector, writeValues, INTEGER);
        tester.testRoundTrip(javaLongObjectInspector, transform(writeValues, AbstractTestParquetLegacyReader::intToLong), BIGINT);
        tester.testRoundTrip(javaTimestampObjectInspector,
                transform(writeValues, AbstractTestParquetLegacyReader::intToTimestamp),
                transform(writeValues, AbstractTestParquetLegacyReader::intToSqlTimestamp),
                TIMESTAMP);

        tester.testRoundTrip(javaDateObjectInspector,
                transform(writeValues, AbstractTestParquetLegacyReader::intToDate),
                transform(writeValues, AbstractTestParquetLegacyReader::intToSqlDate),
                DATE);
    }
    ...
    public void testBinaryDirectSequence()
            throws Exception
    {
        Iterable<byte[]> writeValues = transform(intsBetween(0, 30_000), compose(AbstractTestParquetLegacyReader::stringToByteArray, Object::toString));
        tester.testRoundTrip(javaByteArrayObjectInspector,
                writeValues,
                transform(writeValues, AbstractTestParquetLegacyReader::byteArrayToVarbinary),
                VARBINARY);
    }
    ...
    public void testBinaryDictionarySequence()
            throws Exception
    {
        Iterable<byte[]> writeValues = limit(cycle(transform(ImmutableList.of(1, 3, 5, 7, 11, 13, 17), compose(AbstractTestParquetLegacyReader::stringToByteArray, Object::toString))), 30_000);
        tester.testRoundTrip(javaByteArrayObjectInspector,
                writeValues,
                transform(writeValues, AbstractTestParquetLegacyReader::byteArrayToVarbinary),
                VARBINARY);
    ...
    public void testEmptyBinarySequence()
            throws Exception
    {
        tester.testRoundTrip(javaByteArrayObjectInspector, limit(cycle(new byte[0]), 30_000), AbstractTestParquetLegacyReader::byteArrayToVarbinary, VARBINARY);
    }
