...
public class TestConnectorOutputTableHandle
{
    ...
    public void TestHiveTransactionHandleInterfaceSerDe()
    {
        ...
        try {
            byte[] bytes0 = serializer.serialize(thriftTransactionHandle);

            ThriftConnectorTransactionHandle deserialized = new ThriftConnectorTransactionHandle();
            deserializer.deserialize(deserialized, bytes0);
            ...
            throws TException
    {
        ...
        for (HiveStorageFormat format : getSupportedHiveStorageFormats()) {
            ...
            TableWriteInfo tableWriteInfo = new TableWriteInfo(Optional.of(handle), Optional.empty(), Optional.empty());

            TSerializer serializer = new TSerializer(new TJSONProtocol.Factory());
            byte[] bytes = serializer.serialize(tableWriteInfo.toThrift());

            TDeserializer deserializer = new TDeserializer(new TJSONProtocol.Factory());
            ThriftTableWriteInfo thriftInfo = new ThriftTableWriteInfo();
            deserializer.deserialize(thriftInfo, bytes);

            TableWriteInfo convertedBack = TableWriteInfo.newTableWriteInfo(thriftInfo);
            ...
            throws TException
    {
        ...
        for (HiveStorageFormat format : getSupportedHiveStorageFormats()) {
            ...
            TSerializer serializer = new TSerializer(new TJSONProtocol.Factory());
            byte[] bytes = serializer.serialize(handle.toThriftInterface());

            TDeserializer deserializer = new TDeserializer(new TJSONProtocol.Factory());
            ThriftExecutionWriterTarget thriftHandle = new ThriftExecutionWriterTarget();
            ...
            throws TException
    {
        ...
        for (HiveStorageFormat format : getSupportedHiveStorageFormats()) {
            ...
            OutputTableHandle outputTableHandle = new OutputTableHandle(CONNECTOR_ID, transaction, hiveOutputTableHandle);
            TSerializer serializer = new TSerializer(new TJSONProtocol.Factory());
            byte[] bytes = serializer.serialize(outputTableHandle.toThrift());

            TDeserializer deserializer = new TDeserializer(new TJSONProtocol.Factory());
            ThriftOutputTableHandle thriftHandle = new ThriftOutputTableHandle();
            deserializer.deserialize(thriftHandle, bytes);
