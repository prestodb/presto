...
public class TestTupleDomainParquetPredicate
{
    ...
    public void testMatchesWithStatistics()
    {
        ...
        RichColumnDescriptor column = new RichColumnDescriptor(new ColumnDescriptor(new String[] {"path"}, PrimitiveTypeName.BINARY, 0, 0), type);
        TupleDomain<ColumnDescriptor> effectivePredicate = getEffectivePredicate(column, createVarcharType(255), value);
        TupleDomainParquetPredicate predicate = new TupleDomainParquetPredicate(effectivePredicate, singletonList(column));
