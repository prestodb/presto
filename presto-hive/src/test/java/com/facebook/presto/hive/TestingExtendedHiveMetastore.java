        ...
        extends UnimplementedHiveMetastore
{
    ...
    public Map<String, Optional<Partition>> getPartitionsByNames(MetastoreContext metastoreContext, String databaseName, String tableName, List<PartitionNameWithVersion> partitionNames)
    {
        ...
        for (PartitionNameWithVersion partitionName : partitionNames) {
            List<String> partitionValues = toPartitionValues(partitionName.getPartitionName());
            String partitionKey = createPartitionKey(databaseName, tableName, partitionValues);
            long time = lastDataCommitTimes.getOrDefault(partitionKey, 0L);
            ...
            if (partition != null) {
                Partition.Builder builder = Partition.builder(partition)
                        .setLastDataCommitTime(time);
                result.put(partitionName.getPartitionName(), Optional.of(builder.build()));
            }
            else {
                result.put(partitionName.getPartitionName(), Optional.empty());
            }
        }
        return result;
    }
