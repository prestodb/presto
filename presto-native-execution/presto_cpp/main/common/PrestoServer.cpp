...
void PrestoServer::run() {
  ...
  if (systemConfig->enableRuntimeMetricsCollection()) {
    ...
    if (folly::Singleton<velox::BaseStatsReporter>::try_get()) {
      ...
      if (bindToNodeInternalAddressOnly) {
        metricsSocketAddress.setFromHostPort(address_, metricsPort);
      } else {
        metricsSocketAddress.setFromLocalPort(metricsPort);
      }

      auto metricsHttpConfig = std::make_unique<http::HttpConfig>(
          metricsSocketAddress, reusePort);

      std::unique_ptr<http::HttpsConfig> metricsHttpsConfig;
      if (useHttps) {
        // Metrics server uses same HTTPS configuration as main server
        const bool http2Enabled =
            SystemConfig::instance()->httpServerHttp2Enabled();
        const std::string clientCaFile =
            SystemConfig::instance()->httpsClientCaFile().value_or("");
        metricsHttpsConfig = std::make_unique<http::HttpsConfig>(
            metricsSocketAddress,
            certPath,
            keyPath,
            ciphers,
            reusePort,
      ...
      // Register only the metrics endpoint on the dedicated server
      metricsHttpServer_->registerGet(
          "/v1/info/metrics",
          [](proxygen::HTTPMessage* /*message*/,
             const std::vector<std::unique_ptr<folly::IOBuf>>& /*body*/,
             proxygen::ResponseHandler* downstream) {
