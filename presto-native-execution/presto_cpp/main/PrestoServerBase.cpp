...
PrestoServerBase::PrestoServerBase(const std::string& configDirectoryPath)
...
PrestoServerBase::~PrestoServerBase() {}

void PrestoServerBase::run() {
...
void PrestoServerBase::initializeVeloxMemory() {
  auto nodeConfig = NodeConfig::instance();
  auto systemConfig = SystemConfig::instance();
...
std::function<folly::SocketAddress()>
PrestoServerBase::discoveryAddressLookup() {
...
void PrestoServerBase::addAdditionalPeriodicTasks() {}

std::shared_ptr<velox::exec::TaskListener> PrestoServerBase::getTaskListener() {
  return nullptr;
}

std::shared_ptr<velox::exec::ExprSetListener>
PrestoServerBase::getExprSetListener() {
  return nullptr;
}

std::vector<std::unique_ptr<proxygen::RequestHandlerFactory>>
PrestoServerBase::getHttpServerFilters() {
...
PrestoServerBase::getHttpStatsFilter() {
  return std::make_unique<http::filters::StatsFilterFactory>();
}

std::vector<std::string> PrestoServerBase::registerConnectors(
...
void PrestoServerBase::registerShuffleInterfaceFactories() {
...
void PrestoServerBase::registerCustomOperators() {
...
void PrestoServerBase::registerFunctions() {
...
void PrestoServerBase::registerVectorSerdes() {
  if (!velox::isRegisteredVectorSerde()) {
    velox::serializer::presto::PrestoVectorSerde::registerVectorSerde();
  }
}

void PrestoServerBase::registerFileSystems() {
  velox::filesystems::registerLocalFileSystem();
}

void PrestoServerBase::registerStatsCounters() {
  registerPrestoCppCounters();
  registerVeloxCounters();
}

std::shared_ptr<velox::connector::Connector>
PrestoServerBase::connectorWithCache(
...
void PrestoServerBase::populateMemAndCPUInfo() {
  auto systemConfig = SystemConfig::instance();
...
void PrestoServerBase::runOperation(
    proxygen::HTTPMessage* message,
    proxygen::ResponseHandler* downstream) {
  try {
...
std::string PrestoServerBase::connectorOperation(
...
void PrestoServerBase::reportMemoryInfo(proxygen::ResponseHandler* downstream) {
  http::sendOkResponse(downstream, json(**memoryInfo_.rlock()));
}

void PrestoServerBase::reportServerInfo(proxygen::ResponseHandler* downstream) {
...
void PrestoServerBase::reportNodeStatus(proxygen::ResponseHandler* downstream) {
  auto systemConfig = SystemConfig::instance();
  const int64_t nodeMemoryGb =
      systemConfig->systemMemoryGb() - cacheRamCapacityGb_;
