 ...
 protected:
  ...
  void validateHttpResponse(
    ...
    http::RequestBuilder()
        ...
        .thenValue(
            [expectedStr, this](std::unique_ptr<http::HttpResponse> response) {
              VELOX_USER_CHECK_EQ(
              ...
              auto sz = resJson.size();
              for (auto i = 0; i < sz; i++) {
                json result = resJson[i];
                json expected = expectedJson[i];
  ...
  void testFile(const std::string& prefix) {
    ...
    auto inputExprs = json::parse(input);
    std::string output =
        slurp(getDataPath(fmt::format("{}Expected.json", prefix)));
    auto expectedExprs = json::parse(output);

    validateHttpResponse(inputExprs.dump(), expectedExprs.dump());
  }
...
TEST_F(PrestoExprEvalTest, simple) {
...
TEST_F(PrestoExprEvalTest, specialFormRewrites) {
  // File SpecialExpressions{Input|Expected}.json contain the input and expected
  // JSON representing the RowExpressions resulting from the following queries:
  // select if(1 < 2, 2, 3);
  // select (1 < 2) and (2 < 3);
  // select (1 < 2) or (2 < 3);
  // The special form expression rewrites is verified here.
