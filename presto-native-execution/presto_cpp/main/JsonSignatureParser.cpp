...
JsonSignatureParser::FunctionSignatureItem parseSignature(
    const folly::dynamic& input,
    std::optional<std::string> subDirectoryName = std::nullopt) {
  VELOX_USER_CHECK(
      input.isObject(),
  ...
  return JsonSignatureParser::FunctionSignatureItem{
      ...
      (entrypoint != nullptr)
          ? entrypoint->asString()
          : (subDirectoryName.has_value() ? "registry" : ""),
...
JsonSignatureParser::JsonSignatureParser(
  ...
  std::string topLevelKey = jsonSignatureScopeToString(scope);
  try {
...
void JsonSignatureParser::parseHelper(
    const folly::dynamic& input,
    std::optional<std::string> subDirName) {
  VELOX_USER_CHECK(
      input.isObject(),
...
void JsonSignatureParser::parse(
  ...
  VELOX_USER_CHECK(
      ...
      "Input signatures should be an object. Got: {}",
      input.typeName());
  if (scope == JsonSignatureScope::DynamiclibrariesUdf) {
