          ...
          [this, taskId, sequence](
            ...
            if (!data.empty() || noMoreData) {
              ...
              std::move(future)
                  ...
                  .thenValue(
                      [this, downstream, taskId, sequence](bool /*value*/) {
                        auto [data, noMoreData] = getData(sequence);
                        VELOX_CHECK(!data.empty() || noMoreData);
                        sendResponse(
                            downstream, taskId, sequence, data, noMoreData);
                      });
 ...
 public:
  ...
  static std::unique_ptr<http::HttpServer> getServer(bool useHttps) {
    if (useHttps) {
      ...
      std::string ciphers = getCiphers(useHttps);
...
TEST_P(PrestoExchangeSourceTestSuite, basic) {
  ...
  auto producerServer = getServer(useHttps);
  ...
  auto exchangeSource = std::make_shared<PrestoExchangeSource>(
      producerUri, 3, queue, pool_.get(), getClientCa(useHttps), getCiphers(useHttps));
...
TEST_P(PrestoExchangeSourceTestSuite, earlyTerminatingConsumer) {
  ...
  auto exchangeSource = std::make_shared<PrestoExchangeSource>(
      makeProducerUri(producerAddress), 3, queue, pool_.get(), getClientCa(useHttps), getCiphers(useHttps));
...
TEST_P(PrestoExchangeSourceTestSuite, exceedingMemoryCapacityForHttpResponse) {
  ...
  queue->noMoreSources();
  auto exchangeSource = std::make_shared<PrestoExchangeSource>(
      makeProducerUri(producerAddress), 3, queue, leafPool.get(), getClientCa(useHttps), getCiphers(useHttps));
...
TEST(PrestoExchangeSourceMemoryTest, memoryAllocationAndUsageCheck) {
  auto rootPool =
