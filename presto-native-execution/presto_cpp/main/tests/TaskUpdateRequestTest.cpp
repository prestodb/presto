...
TEST_F(TaskUpdateRequestTest, optionalField) {
  ...
  ASSERT_EQ(
      *resourceEstimates.executionTime,
      protocol::Duration(100, protocol::TimeUnit::MILLISECONDS));
  ASSERT_EQ(resourceEstimates.cpuTime, nullptr);
  ASSERT_EQ(
      *resourceEstimates.peakMemory,
      protocol::DataSize(1024 * 1024 * 1024, protocol::DataUnit::BYTE));
...
TEST_F(TaskUpdateRequestTest, binaryRemoteSplitFromThrift) {
  ...
  auto remoteSplit =
      std::dynamic_pointer_cast<protocol::RemoteSplit>(split.connectorSplit);
...
TEST_F(TaskUpdateRequestTest, unionExecutionWriterTargetFromThrift) {
  ...
  auto createHandle =
      std::dynamic_pointer_cast<protocol::CreateHandle>(writerTarget);
  ...
  auto* hiveTxnHandle = dynamic_cast<protocol::hive::HiveTransactionHandle*>(
      createHandle->handle.transactionHandle.get());
  ...
  auto* hiveOutputTableHandle =
      dynamic_cast<protocol::hive::HiveOutputTableHandle*>(
          createHandle->handle.connectorHandle.get());
  ...
  ASSERT_EQ(
      ...
      protocol::hive::HiveStorageFormat::ORC);
  ASSERT_EQ(
...
TEST_F(TaskUpdateRequestTest, unionExecutionWriterTargetToThrift) {
  ...
  // Convert to thrift and verify fields. Note that toThrift functions for
...
TEST_F(TaskUpdateRequestTest, fragment) {
  std::string str =
      slurp(getDataPath(BASE_DATA_PATH, "Fragment.thrift.base64"));
  const auto strEnd = str.find_last_not_of(" \t\n\r");
  if (strEnd != std::string::npos) {
    str.erase(strEnd + 1);
  }

  protocol::PlanFragment f =
      json::parse(facebook::velox::encoding::Base64::decode(str));
...
TEST_F(TaskUpdateRequestTest, sessionRepresentation) {
  ...
  ASSERT_EQ(
      sessionRepresentation.unprocessedCatalogProperties["Person1"]["City"],
      "New York");
  ASSERT_EQ(
      sessionRepresentation.unprocessedCatalogProperties["Person2"]["Age"],
      "25");
  ASSERT_EQ(
      sessionRepresentation.unprocessedCatalogProperties["Person3"]["Name"],
      "Bob Smith");
}
