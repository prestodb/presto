...
TEST_F(TaskInfoTest, binaryMetadataUpdates) {
  std::string str = slurp(getDataPath("/github/presto-trunk/presto-native-execution/presto_cpp/main/tests/data/", "MetadataUpdates.json"));
...
TEST_F(TaskInfoTest, taskInfo) {
  std::string str = slurp(getDataPath("/github/presto-trunk/presto-native-execution/presto_cpp/main/tests/data/", "TaskInfo.json"));
  json j = json::parse(str);
  registerPrestoToVeloxConnector(std::make_unique<facebook::presto::HivePrestoToVeloxConnector>("hive"));
  TaskInfo taskInfo = j;
...
TEST_F(TaskInfoTest, operatorStatsEmptyBlockedReason) {
  std::string str = slurp(getDataPath("/github/presto-trunk/presto-native-execution/presto_cpp/main/tests/data/", "OperatorStatsEmptyBlockedReason.json"));
  json j = json::parse(str);
...
TEST_F(TaskInfoTest, operatorStats) {
  std::string str = slurp(getDataPath("/github/presto-trunk/presto-native-execution/presto_cpp/main/tests/data/", "OperatorStats.json"));
  json j = json::parse(str);
  OperatorStats operatorStats = j;
  facebook::presto::thrift::OperatorStats thriftOperatorStats;
  facebook::presto::thrift::toThrift(operatorStats, thriftOperatorStats);
