...
TEST_F(DynamicFunctionRegistererTest, twoFunctions) {
  ...
  // // Create dummy shared library to emulate the shared library path
  // validation. auto filePathForDummySharedFile =
  // exec::test::TempFilePath::create(); std::filesystem::path
  // sharedFilePath(filePathForDummySharedFile->getPath()); auto dummySharedFile
  // = sharedFilePath.replace_extension(kFileExtension);
