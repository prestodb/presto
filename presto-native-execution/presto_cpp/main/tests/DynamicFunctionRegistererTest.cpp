...
class DynamicFunctionRegistererTest : public testing::Test {
 public:
  ...
  void writeToFile(const std::string& filePath, const std::string_view& data) {
    std::ofstream outputFile(filePath);
    outputFile.write(data.data(), data.size());
    outputFile.close();
  }

  std::filesystem::path createConfigFile(
      const std::string_view& json,
      const std::string& path) {
    // Create dummy shared library to emulate the shared library path
    // validation.
    auto dummySharedLibPathStr = fmt::format(
        "{}/test{}", std::string(fs::path(path).parent_path()), kFileExtension);
    std::ofstream{dummySharedLibPathStr}.put('a');

    // Create json config file.
    std::filesystem::path filePathJson(std::string(path).append("/test.json"));
    writeToFile(filePathJson.string(), json);
    return filePathJson;
  }
 ...
 private:
  ...
  // std::shared_ptr<TempFilePath> fsPath;
};
...
TEST_F(DynamicFunctionRegistererTest, twoFunctions) {
  ...
  auto path = exec::test::TempDirectoryPath::create();
  auto pathStr = createConfigFile(json, path->getPath()).string();

  auto signaturesBefore = velox::getFunctionSignatures();

  DynamicLibraryLoader testLoader(pathStr, "/");
  ...
  auto signaturesAfter = velox::getFunctionSignatures();

  assertFunctionExistsAfter(
      signaturesBefore, signaturesAfter, "presto.default.mock1");
  assertFunctionExistsAfter(
      signaturesBefore, signaturesAfter, "presto.default.mock2");

  // Config validations.
  EXPECT_EQ(
      testLoader.compareConfigWithRegisteredFunctionSignatures(
          signaturesBefore),
      0);
}
...
TEST_F(DynamicFunctionRegistererTest, newNamespace) {
  ...
  assertFunctionExistsAfter(
      signaturesBefore, signaturesAfter, "test.namespace.mock3");
