...
TEST_F(KllSketchTest, customKWithNullK) {
  // NULL k rows are silently skipped (matching Java @InputFunction behaviour
  // which is never invoked for NULL args). All rows skipped → NULL result.
...
TEST_F(KllSketchTest, customKInconsistentValues) {
  // Varying k within a group is tolerated: the first k seen initialises the
  // sketch and subsequent different k values are ignored (matching Java
  // behaviour).
  ...
  auto result = AssertQueryBuilder(plan).copyResults(pool());
  EXPECT_EQ(result->size(), 1);
  EXPECT_FALSE(result->childAt(0)->isNullAt(0));
}
...
// Java golden bytes generated by TestKllSketchFunctions.printGolden*Bytes()
// (presto-main-base/.../operator/scalar/TestKllSketchFunctions.java).
// The same hex strings are also embedded in:
//   - TestKllSketchFunctions.java (NATIVE_*_GOLDEN_HEX constants)
//   - AbstractTestKllSketchFunctions.java
//   (testKllSketchCrossEngineConstantFold)
...
// To regenerate:
//   Java → C++: Enable the tests then run ./mvnw -pl presto-main-base
//   -Dtest='TestKllSketchFunctions#printGolden*' test
//   C++ → Java: run <path/to/presto_kll_sketch_test>
//   --gtest_also_run_disabled_tests --gtest_filter="*printNative*"
