    ...
    if (auto scalarArgument =
      ...
      if (serializable.block.data.empty()) {
        ...
        auto value = exprConverter.getConstantValue(
            parser.parse(serializable.type), serializable.block);
        functionArg = std::make_shared<tvf::ScalarArgument>(
            value.inferType(),
      ...
      for (auto& arg : tableArgument->fields) {
        if (arg.name) {
          ...
          // Using empty string instead of null field names.
      ...
      if (!descriptorArgument->descriptor) {
        ...
        if (fieldTypes.size()) {
          auto functionDescriptor = std::make_shared<tvf::Descriptor>(
              std::move(fieldNames), std::move(fieldTypes));
          functionArg =
              std::make_shared<tvf::DescriptorArgument>(functionDescriptor);
        } else {
