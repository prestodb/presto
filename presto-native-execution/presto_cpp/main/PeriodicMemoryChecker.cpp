...
PeriodicMemoryChecker::PeriodicMemoryChecker(Config config)
  ...
  if (config_.mallocMemHeapDumpEnabled) {
    VELOX_CHECK(!config_.heapDumpLogDir.empty());
    VELOX_CHECK(!config_.heapDumpFilePrefix.empty());
  }
}
...
void PeriodicMemoryChecker::start() {
  if (!config_.systemMemPushbackEnabled) {
    ...
    PRESTO_STARTUP_LOG(INFO)
        << "Creating server memory pushback checker, system memory limit: "
...
std::string PeriodicMemoryChecker::createHeapDumpFilePath() const {
  ...
  // Format that can be consumed by strobelight:
  ...
  if (allocatedSize >= config_.mallocBytesUsageDumpThreshold &&
    ...
    if (dumpFilesByHeapMemUsageMinPq_.size() > config_.maxHeapDumpFiles) {
      ...
      LOG(INFO) << fmt::format(
          ...
          evicted.filePath);
      const int ret = remove(evicted.filePath.c_str());
      if (ret != 0) {
        LOG(ERROR) << "Error removing Heap profile (error code: " << ret
                   << "): " << evicted.filePath;
      }
...
void PeriodicMemoryChecker::pushbackMemory() const {
  ...
  const uint64_t deltaMemBytes = currentMemBytes - targetMemBytes;
  VELOX_CHECK_GT(deltaMemBytes, 0);

  auto* cache = velox::cache::AsyncDataCache::getInstance();
  auto systemConfig = SystemConfig::instance();
  auto freedBytes = cache != nullptr ? cache->shrink(deltaMemBytes) : 0;
  if (freedBytes < deltaMemBytes) {
    try {
      ...
      if (freedBytes < deltaMemBytes &&
        ...
        // reference.
        if (cache != nullptr) {
          freedBytes += cache->shrink(deltaMemBytes - freedBytes);
        }
        if (freedBytes < deltaMemBytes) {
          freedBytes += velox::memory::AllocationTraits::pageBytes(
              memoryManager->allocator()->unmap(
