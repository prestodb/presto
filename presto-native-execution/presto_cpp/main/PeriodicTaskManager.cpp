...
folly::Synchronized<std::shared_ptr<CacheShrink>>& curCacheShrinkP() {
  static folly::Synchronized<std::shared_ptr<CacheShrink>> kShrink;
  return kShrink;
}
...
void PeriodicTaskManager::start() {
  ...
  if (asyncDataCache_ != nullptr) {
    ...
    if (SystemConfig::instance()->asyncCacheShrinkEnabled()) {
      addCacheShrinkTask();
    }
  }
...
bool PeriodicTaskManager::registerCacheShrink(
    std::shared_ptr<CacheShrink> cacheShrinkP) {
  return curCacheShrinkP().withWLock([&](auto& curCacheShrinkP) {
    if (curCacheShrinkP == nullptr) {
      curCacheShrinkP = cacheShrinkP;
      return true;
    }
    LOG(INFO) << "Cache shrink function already registered";
    return false;
  });
}

void PeriodicTaskManager::addCacheShrinkTask() {
  curCacheShrinkP().withRLock([&](auto& curCacheShrinkP) {
    if (curCacheShrinkP != nullptr) {
      addTask(
          [curCacheShrinkP] { curCacheShrinkP->shrink(); },
          curCacheShrinkP->interval(),
          "cache shrink");
    }
  });
}
