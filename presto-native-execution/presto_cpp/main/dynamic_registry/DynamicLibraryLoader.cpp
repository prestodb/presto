...
bool isConfigSignatureRegistered(
    ...
    const facebook::velox::exec::FunctionSignaturePtr& localFnSignature) {
  return std::find_if(
...
std::optional<std::string> DynamicLibraryLoader::constructAbsoluteFilePath(
  ...
  for (const auto& it : lockedFunctionMap) {
    ...
    if (numNewRegistrations != it.second.size()) {
      LOG(ERROR) << it.first << " has " << numNewRegistrations
                 ...
                 << it.second.size() << " actual registrations";
    }
  ...
  for (const auto& it : JsonSignatureParser(
    ...
    for (const auto& function : it.second) {
      ...
      if (filePath.has_value()) {
        ...
        (*lockedEntrypointMap)[filePath.value()] = function.entrypoint;
      }
    }
  }
}
