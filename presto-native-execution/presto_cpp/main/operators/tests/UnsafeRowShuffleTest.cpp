...
int lexicographicalCompare(
      const BufferPtr& key1,
      const BufferPtr& key2) {
    ...
    bool lessThan = std::lexicographical_compare(data1, data1 + size1, data2, data2 + size2);
...
class TestShuffleWriter : public ShuffleWriter {
 public:
  ...
  void collect(int32_t partition, std::string_view key, std::string_view data) override {
...
class UnsafeRowShuffleTest : public exec::test::OperatorTestBase {
 ...
 protected:
  ...
  void runShuffleTest(
      ...
      const std::optional<std::vector<velox::core::SortOrder>>& sortOrders = std::nullopt,
      const std::optional<std::vector<velox::core::FieldAccessTypedExprPtr>>& fields = std::nullopt,
    ...
    auto writerTask = makeTask(writerTaskId, writerPlan, 0, std::move(queryConfig));
    ...
    auto shuffleWritter = TestShuffleWriter::getInstance();
    if (shuffleWritter) {
      auto serializedSortKeys = shuffleWritter->serializedSortKeys();
      if (sortOrders && fields) {
        for (auto i = 0; i < numPartitions; ++i) {
          auto resultSortingOrder = getSortOrder((*serializedSortKeys)[i]);
  ...
  void partitionAndSerializeWithThresholds(
    ...
    std::optional<std::vector<std::shared_ptr<const velox::core::FieldAccessTypedExpr>>> fields = std::nullopt;
    
    if (sorted) {
      data = makeRowVector({fuzzer.fuzzConstant(INTEGER(), inputRows), fuzzer.fuzzMap(
      ...
      fields = std::vector<std::shared_ptr<const velox::core::FieldAccessTypedExpr>>
    ...
    auto plan = exec::test::PlanBuilder()
                    .values({data}, false)
                    .addNode(addPartitionAndSerializeNode(2, true, {}, ordering, fields))
                    .planNode();
...
TEST_F(UnsafeRowShuffleTest, endToEndWithSortedShuffleRowLimit) {
  ...
  auto data = makeRowVector({
      ...
      makeFlatVector<StringView>({"key3", "key1", "key22", "key55", 
                                  "key44", "key66", "key111", "key222", "key333"}) // sorting column
  });
