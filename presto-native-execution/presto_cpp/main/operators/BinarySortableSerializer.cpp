...
BinarySortableSerializer::BinarySortableSerializer(
      ...
      : inputRowVector_(source),
        sortOrders_(std::move(sortOrders)) {
    auto inputRowType = velox::asRowType(source->type());
    fieldChannels_.reserve(fields.size());
    VELOX_CHECK_EQ(fields.size(), sortOrders_.size());
    for (int32_t i = 0; i < fields.size(); ++i) {
      fieldChannels_.emplace_back(
          i, exprToChannel(fields[i].get(), *inputRowType));
    }
}
...
void BinarySortableSerializer::serialize(
    ...
    RawBuffer* out) {
  auto* row =
      inputRowVector_->wrappedVector()->template asUnchecked<velox::RowVector>();
  auto wrappedIndex = inputRowVector_->wrappedIndex(index);
  const auto& children = row->children();
