...
/// If 'replicateNullsAndAny' is true, the output includes a third boolean column 
/// which indicates whether a row needs to be replicated to all partitions.
 ...
 public:
  ...
  const velox::RowTypePtr& outputType() const override {
    static const velox::RowTypePtr kOutputType{velox::ROW(
        {"partition", "key", "data"}, {velox::INTEGER(),  velox::VARBINARY(), velox::VARBINARY()})};

    static const velox::RowTypePtr kReplicateNullsAndAnyOutputType{velox::ROW(
        {"partition", "key", "data", "replicate"},
        {velox::INTEGER(),  velox::VARBINARY(), velox::VARBINARY(), velox::BOOLEAN()})};
  ...
  bool isSortedShuffle() const {
    return sortingOrders_ && sortingKeys_;
  }

  const std::optional<std::vector<velox::core::FieldAccessTypedExprPtr>>& sortingKeys() const {
    return sortingKeys_;
  }

  const  std::optional<std::vector<velox::core::SortOrder>>& sortingOrders() const {
