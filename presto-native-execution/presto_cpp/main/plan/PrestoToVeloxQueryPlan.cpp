...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  if (node->orderingScheme) {
    ...
    auto localMergeNode = std::make_shared<core::LocalMergeNode>(
        node->id, sortingKeys, sortingOrders, std::move(sourceNodes));
    validateNode(localMergeNode.get());
  ...
  for (auto i = 0; i < sourceNodes.size(); ++i) {
    ...
    sourceNodes[i] = std::make_shared<core::ProjectNode>(
        ...
        sourceNodes[i]);
    validateNode(sourceNodes[i].get());
  }

  if (type == core::LocalPartitionNode::Type::kGather) {
    auto localPartitionNode =
        core::LocalPartitionNode::gather(node->id, std::move(sourceNodes));
    validateNode(localPartitionNode.get());
    return localPartitionNode;
  }
  ...
  if (std::dynamic_pointer_cast<protocol::SystemPartitioningHandle>(
    ...
    auto localPartitionNode = buildLocalSystemPartitionNode(
        node,
        type,
        scaleWriters,
        outputType,
...
VeloxQueryPlanConverterBase::toColumnStatsSpec(
  ...
  const auto aggregationNode = std::make_shared<core::AggregationNode>(
      ...
      sourceVeloxPlan);
  validateNode(aggregationNode.get());
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto valuesNode =
      std::make_shared<core::ValuesNode>(node->id, std::vector{rowVector});
  validateNode(valuesNode.get());
  return valuesNode;
}
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  validateNode(tableScanNode.get());
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto groupIdNode = std::make_shared<core::GroupIdNode>(
      node->id,
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  if (node->criteria.empty()) {
    ...
    if (isNestedJoinType) {
      auto nestedLoopJoinNode = std::make_shared<core::NestedLoopJoinNode>(
          node->id,
          joinType,
      ...
      validateNode(nestedLoopJoinNode.get());
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  if (SessionProperties::instance()->useVeloxGeospatialJoin()) {
    auto spatialJoinNode = std::make_shared<core::SpatialJoinNode>(
        node->id,
        joinType,
        ...
        toVeloxQueryPlan(node->right, tableWriteInfo, taskId),
        toRowType(node->outputVariables, typeParser_));
    validateNode(spatialJoinNode.get());
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto indexLookupJoinNode = std::make_shared<core::IndexLookupJoinNode>(
      node->id,
  ...
  validateNode(indexLookupJoinNode.get());
  return indexLookupJoinNode;
}
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
    ...
    const std::shared_ptr<protocol::TableWriteInfo>& tableWriteInfo,
    const protocol::TaskId& taskId) {
  auto markDistinctNode = std::make_shared<core::MarkDistinctNode>(
      node->id,
      node->markerVariable.name,
      toVeloxExprs(node->distinctVariables),
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
  validateNode(markDistinctNode.get());
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto mergeJoinNode = std::make_shared<core::MergeJoinNode>(
      node->id,
      joinType,
      leftKeys,
      rightKeys,
      ...
      toVeloxQueryPlan(node->right, tableWriteInfo, taskId),
      toRowType(node->outputVariables, typeParser_));
  validateNode(mergeJoinNode.get());
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto topNNode = std::make_shared<core::TopNNode>(
      node->id,
      sortingKeys,
      sortingOrders,
      node->count,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  std::optional<core::ColumnStatsSpec> columnStatsSpec = toColumnStatsSpec(
      ...
      tableWriteInfo,
      taskId);
  auto tableWriteNode = std::make_shared<core::TableWriteNode>(
      node->id,
      ...
      getCommitStrategy(),
      sourceVeloxPlan);
  validateNode(tableWriteNode.get());
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto tableWriteMergeNode = std::make_shared<core::TableWriteMergeNode>(
      node->id, outputType, columnStatsSpec, sourceVeloxPlan);
  validateNode(tableWriteMergeNode.get());
  return tableWriteMergeNode;
}
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
    ...
    const std::shared_ptr<protocol::TableWriteInfo>& tableWriteInfo,
    const protocol::TaskId& taskId) {
  auto enforceSingleRowNode = std::make_shared<core::EnforceSingleRowNode>(
      node->id, toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
  validateNode(enforceSingleRowNode.get());
  return enforceSingleRowNode;
}
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto assignUniqueIdNode = std::make_shared<core::AssignUniqueIdNode>(
      node->id,
      node->idVariable.name,
      taskUniqueId,
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
  validateNode(assignUniqueIdNode.get());
  return assignUniqueIdNode;
}
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto rowNumberNode = std::make_shared<core::RowNumberNode>(
      node->id,
      partitionFields,
      rowNumberColumnName,
      limit,
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
  validateNode(rowNumberNode.get());
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto topNRowNumberNode = std::make_shared<core::TopNRowNumberNode>(
      node->id,
      ...
      node->maxRowCountPerPartition,
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
  validateNode(topNRowNumberNode.get());
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
    ...
    const std::shared_ptr<protocol::TableWriteInfo>& tableWriteInfo,
    const protocol::TaskId& taskId) {
  auto partitionedOutputNode = core::PartitionedOutputNode::single(
      node->id,
      toRowType(node->outputVariables, typeParser_),
      "Presto",
      toVeloxQueryPlan(node->source, tableWriteInfo, taskId));
...
core::PlanNodePtr VeloxInteractiveQueryPlanConverter::toVeloxQueryPlan(
  ...
  if (node->orderingScheme) {
    ...
    auto mergeExchangeNode = std::make_shared<core::MergeExchangeNode>(
        node->id,
        rowType,
        sortingKeys,
        sortingOrders,
        toVeloxSerdeKind(node->encoding));
    validateNode(mergeExchangeNode.get());
    return mergeExchangeNode;
  }
  auto exchangeNode = std::make_shared<core::ExchangeNode>(
      node->id, rowType, toVeloxSerdeKind(node->encoding));
  validateNode(exchangeNode.get());
  return exchangeNode;
}
...
core::PlanFragment VeloxBatchQueryPlanConverter::toVeloxQueryPlan(
  ...
  if (partitionedOutputNode->isBroadcast()) {
    ...
    validateNode(broadcastWriteNode.get());
  ...
  const auto partitionAndSerializeNode =
      std::make_shared<operators::PartitionAndSerializeNode>(
          ...
          sortingOrders,
          sortingKeys);
  validateNode(partitionAndSerializeNode.get());
...
core::PlanNodePtr VeloxBatchQueryPlanConverter::toVeloxQueryPlan(
  ...
  if (node->exchangeType == protocol::ExchangeNodeType::REPLICATE) {
    auto exchangeNode =
        std::make_shared<core::ExchangeNode>(node->id, rowType, "Presto");
    validateNode(exchangeNode.get());
    return exchangeNode;
  }
  // Partitioned shuffle exchange source.
  auto shuffleReadNode =
      std::make_shared<operators::ShuffleReadNode>(node->id, rowType);
  validateNode(shuffleReadNode.get());
  return shuffleReadNode;
}
