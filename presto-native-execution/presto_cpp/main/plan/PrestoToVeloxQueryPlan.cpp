...
template <typename T>
std::shared_ptr<T> validatePlanNode(
    std::shared_ptr<T> planNode,
    PlanConversionPurpose purpose) {
  if (purpose == PlanConversionPurpose::kValidation) {
    VELOX_CHECK(
        VeloxPlanChecker::isValidPlanNode(planNode.get()),
        "PlanNode not supported in validation: {}",
        planNode->toString());
  }
  return planNode;
}
...
std::shared_ptr<core::LocalPartitionNode> buildLocalSystemPartitionNode(
  ...
  if (isHashPartition(node)) {
    ...
    auto planNode = std::make_shared<core::LocalPartitionNode>(
        node->id,
        type,
        scaleWriters,
        std::make_shared<HashPartitionFunctionSpec>(outputType, keyChannels),
        std::move(sourceNodes));
    return validatePlanNode(std::move(planNode), purpose);
  }
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  if (node->orderingScheme) {
    ...
    return validatePlanNode(
        std::make_shared<core::LocalMergeNode>(
  ...
  for (auto i = 0; i < sourceNodes.size(); ++i) {
    ...
    sourceNodes[i] = validatePlanNode(
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  if (const auto semiJoin =
    ...
    auto hashJoinNode = validatePlanNode(
...
VeloxQueryPlanConverterBase::tryConvertOffsetLimit(
  ...
  if (gt && equal(gt->arguments[0], rowNumberVariable)) {
    ...
    if (auto offsetConstExpr =
      ...
      auto limitNode = validatePlanNode(
...
VeloxQueryPlanConverterBase::toColumnStatsSpec(
  ...
  const auto aggregationNode = validatePlanNode(
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::GroupIdNode>(
          node->id,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  auto aggregationNode = validatePlanNode(
      std::make_shared<core::AggregationNode>(
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  if (node->criteria.empty()) {
    ...
    if (isNestedJoinType) {
      return validatePlanNode(
          std::make_shared<core::NestedLoopJoinNode>(
              node->id,
              joinType,
              node->filter ? exprConverter_.toVeloxExpr(*node->filter)
                           : nullptr,
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  if (SessionProperties::instance()->useVeloxGeospatialJoin()) {
    return validatePlanNode(
        std::make_shared<core::SpatialJoinNode>(
            node->id,
            joinType,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::IndexLookupJoinNode>(
          node->id,
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
    ...
    const protocol::TaskId& taskId) {
  return validatePlanNode(
      std::make_shared<core::MarkDistinctNode>(
          node->id,
...
core::PlanNodePtr VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::MergeJoinNode>(
          node->id,
          joinType,
          leftKeys,
          rightKeys,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::OrderByNode>(
          node->id,
          sortingKeys,
          sortingOrders,
          node->isPartial,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  std::optional<core::ColumnStatsSpec> columnStatsSpec = toColumnStatsSpec(
      ...
      taskId);
  return validatePlanNode(
      std::make_shared<core::TableWriteNode>(
          node->id,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::TableWriteMergeNode>(
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::UnnestNode>(
          node->id,
          toVeloxExprs(node->replicateVariables),
          unnestFields,
          unnestNames,
          node->ordinalityVariable
              ? std::optional{node->ordinalityVariable->name}
              : std::nullopt,
          std::nullopt,
          toVeloxQueryPlan(node->source, tableWriteInfo, taskId)),
      purpose_);
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
    ...
    const protocol::TaskId& taskId) {
  return validatePlanNode(
      std::make_shared<core::EnforceSingleRowNode>(
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::AssignUniqueIdNode>(
          node->id,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::WindowNode>(
          node->id,
          partitionFields,
          sortFields,
          sortOrders,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::RowNumberNode>(
          node->id,
          partitionFields,
          rowNumberColumnName,
          limit,
...
VeloxQueryPlanConverterBase::toVeloxQueryPlan(
  ...
  return validatePlanNode(
      std::make_shared<core::TopNRowNumberNode>(
          node->id,
  ...
  if (auto systemPartitioningHandle =
    ...
    switch (systemPartitioningHandle->partitioning) {
      case protocol::SystemPartitioning::SINGLE:
        ...
        planFragment.planNode = validatePlanNode(
            core::PartitionedOutputNode::single(
      ...
      case protocol::SystemPartitioning::FIXED: {
        switch (systemPartitioningHandle->function) {
          ...
          case protocol::SystemPartitionFunction::BROADCAST: {
            planFragment.planNode = validatePlanNode(
                core::PartitionedOutputNode::broadcast(
                    partitionedOutputNodeId,
                    1,
                    outputType,
                    toVeloxSerdeKind(partitioningScheme.encoding),
      ...
      case protocol::SystemPartitioning::SCALED: {
        VELOX_CHECK(
            ...
            toJsonString(systemPartitioningHandle->function));
        planFragment.planNode = validatePlanNode(
            core::PartitionedOutputNode::arbitrary(
                partitionedOutputNodeId,
                std::move(outputType),
                toVeloxSerdeKind(partitioningScheme.encoding),
                std::move(sourceNode)),
...
core::PlanNodePtr VeloxInteractiveQueryPlanConverter::toVeloxQueryPlan(
  ...
  if (node->orderingScheme) {
    ...
    return validatePlanNode(
        std::make_shared<core::MergeExchangeNode>(
            node->id,
            rowType,
            sortingKeys,
            sortingOrders,
...
core::PlanFragment VeloxBatchQueryPlanConverter::toVeloxQueryPlan(
  ...
  if (partitionedOutputNode->isBroadcast()) {
    ...
    auto broadcastWriteNode = validatePlanNode(
        std::make_shared<operators::BroadcastWriteNode>(
            fmt::format("{}.bw", partitionedOutputNode->id()),
            *broadcastBasePath_,
            maxBroadcastBytesOpt.has_value()
                ? maxBroadcastBytesOpt.value()
  ...
  const auto partitionAndSerializeNode = validatePlanNode(
  ...
  planFragment.planNode = validatePlanNode(
      std::make_shared<operators::ShuffleWriteNode>(
