...
std::unique_ptr<SimpleTableFunctionAnalysis> SimpleTableFunction::analyze(
  ...
  analysis->tableFunctionHandle_ = std::make_shared<SimpleTableFunctionHandle>();
  analysis->returnType_ = std::make_shared<Descriptor>(returnNames, returnTypes);
  return analysis;
}
...
std::shared_ptr<TableFunctionResult> IdentityDataProcessor::apply(
  ...
  if (numRows == 0) {
    return std::make_shared<TableFunctionResult>(TableFunctionResult::TableFunctionState::kFinished);
  }
...
void registerIdentityFunction(const std::string& name) {
  TableArgumentSpecList argSpecs;
  argSpecs.insert(std::make_shared<TableArgumentSpecification>("INPUT", true, false, false));
  registerTableFunction(
      name,
      argSpecs,
      std::make_shared<GenericTableReturnType>(),
...
std::shared_ptr<TableFunctionResult> RepeatFunctionDataProcessor::apply(
  ...
  RowVectorPtr outputTable = RowVector::createEmpty(inputTable->rowType(), pool());
...
std::unique_ptr<RepeatFunctionAnalysis> RepeatFunction::analyze(
  ...
  analysis->tableFunctionHandle_ = std::make_shared<RepeatFunctionHandle>(count);
  analysis->returnType_ = std::make_shared<Descriptor>(returnNames, returnTypes);
  analysis->requiredColumns_ = requiredColumns;
  return analysis;
}
...
void registerRepeatFunction(const std::string& name) {
  TableArgumentSpecList argSpecs;
  argSpecs.insert(std::make_shared<TableArgumentSpecification>("INPUT", true, false, false));
  argSpecs.insert(std::make_shared<ScalarArgumentSpecification>("COUNT", BIGINT(), true));
  registerTableFunction(
      name,
      argSpecs,
      std::make_shared<GenericTableReturnType>(),
