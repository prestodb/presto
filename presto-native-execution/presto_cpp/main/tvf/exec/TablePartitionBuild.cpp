...
TablePartitionBuild::TablePartitionBuild(
    ...
    : pool_(pool),
      ...
      compareFlags_{makeCompareFlags(
          tableFunctionProcessorNode->partitionKeys().size(),
  ...
  const auto& partitionKeys = tableFunctionProcessorNode_->partitionKeys();
  const auto& sortingKeys = tableFunctionProcessorNode_->sortingKeys();
  const auto& sortingOrders = tableFunctionProcessorNode_->sortingOrders();
  std::tie(inputChannels_, inversedInputChannels_, inputType_) =
      reorderInputChannels(inputType_, partitionKeys, sortingKeys);
...
std::shared_ptr<TableFunctionPartition> TablePartitionBuild::emptyPartition() {
  ...
  return std::make_shared<TableFunctionPartition>(
      ...
      tableFunctionProcessorNode_->markerChannels(),
      tableFunctionProcessorNode_->passThroughColumns(),
      tableFunctionProcessorNode_->outputType(),
      pool_);
}
