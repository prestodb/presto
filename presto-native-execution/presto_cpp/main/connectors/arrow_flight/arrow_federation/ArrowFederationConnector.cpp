...
const std::vector<std::string> toArrowFederationColumnHandleList(
  ...
  for (size_t i = 0; i < outputType->size(); ++i) {
    ...
    field.type =
        std::make_shared<std::string>(outputType.get()->childAt(i)->toString());
...
ArrowFederationDataSource::ArrowFederationDataSource(
    ...
    : ArrowFlightDataSource(
      ...
      tableHandle_(
          std::dynamic_pointer_cast<const ArrowFederationTableHandle>(
              tableHandle)) {
  VELOX_CHECK(
      tableHandle_,
      "ArrowFederationDataSource received wrong type of table handle");

  // Precompute constant portion of request json
  requestJson_["tableHandleBytes"] = tableHandle_->tableHandleBytes();
  requestJson_["tableLayoutHandleBytes"] =
      tableHandle_->tableLayoutHandleBytes();
  requestJson_["transactionHandleBytes"] =
      tableHandle_->transactionHandleBytes();

  std::vector<protocol::Field> fields;
  requestJson_["columnHandlesBytes"] =
      toArrowFederationColumnHandleList(outputType_, columnHandles_, fields);
  requestJson_["fields"] = fields;
}
...
void ArrowFederationDataSource::addSplit(
  ...
  // Copy precomputed const JSON and patch in split-specific data
  nlohmann::json request = requestJson_;
  request["connectorId"] = federationSplit->connectorId;
  request["splitBytes"] = federationSplit->splitBytes();
