...
class ArrowFlightConnectorTlsTestBase : public FlightWithServerTestBase {
 protected:
  explicit ArrowFlightConnectorTlsTestBase(
      std::shared_ptr<velox::config::ConfigBase> config)
      : FlightWithServerTestBase(std::move(config)) {}

  flight::Location getServerLocation() override {
    AFC_ASSIGN_OR_RAISE(
        auto loc, flight::Location::ForGrpcTls(kBindHost, kListenPort));
    return loc;
  }
  ...
  void executeTest(
    ...
    ASSERT_OK_AND_ASSIGN(
        auto loc, flight::Location::ForGrpcTls(kConnectHost, kListenPort));
    auto locs = std::vector<flight::Location>{loc};
    if (isPositiveTest) {
      AssertQueryBuilder(plan)
          .splits(makeSplits({"sample-data"}, locs))
