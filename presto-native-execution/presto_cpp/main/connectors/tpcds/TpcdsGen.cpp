...
RowVectorPtr genTpcdsCatalogReturns(
  ...
  // This table is a dependent table on catalog_sales, this table will
  // be populated when catalog_sales is called so we call that first.
...
RowVectorPtr genTpcdsCatalogSales(
  ...
  // This table is a parent table of catalog_returns, this table will
  // be populated first and then data will be dumped in catalog_returns table.
  // Create schema and allocate vectors.
  auto catalogReturnsRowType = getTableSchema(Table::TBL_CATALOG_RETURNS);
  auto childChildren =
      allocateVectors(catalogReturnsRowType, catalogSalesUpperBound, pool);
...
RowVectorPtr genTpcdsStoreSales(
  ...
  // This table is a parent table of store_returns, this table will
  // be populated first and then data will be dumped into the store_returns
  // table. Create schema and allocate vectors.
  auto storeReturnsRowType = getTableSchema(Table::TBL_STORE_RETURNS);
  auto childChildren =
      allocateVectors(storeReturnsRowType, storeSalesUpperBound, pool);
...
RowVectorPtr genTpcdsTimeDim(
    ...
    vector_size_t parallel,
    vector_size_t child) {
  auto timeRowType = getTableSchema(Table::TBL_TIME_DIM);
  DSDGenIterator dsdGenIterator(scaleFactor, parallel, child);
  size_t vectorSize = getVectorSize(
      dsdGenIterator.getRowCount(static_cast<int>(Table::TBL_TIME_DIM)),
      maxRows,
      offset);
  auto children = allocateVectors(timeRowType, vectorSize, pool);

  auto table_id = static_cast<int>(Table::TBL_TIME_DIM);
...
RowVectorPtr genTpcdsWebReturns(
  ...
  DSDGenIterator dsdGenIterator(scaleFactor, parallel, child);
  size_t webSalesVectorSize = getVectorSize(
      dsdGenIterator.getRowCount(static_cast<int>(Table::TBL_WEB_SALES)),
      maxRows,
      offset);

  size_t webSalesUpperBound = webSalesVectorSize * 16;
  auto children = allocateVectors(webSalesRowType, webSalesUpperBound, pool);

  // This table is a dependent table on web_sales, this table will
  // be populated when web_sales is called so we call that first.
  // Create schema and allocate vectors.
  auto webReturnsRowType = getTableSchema(Table::TBL_WEB_RETURNS);
  auto childChildren =
      allocateVectors(webReturnsRowType, webSalesUpperBound, pool);

  auto table_id = static_cast<int>(Table::TBL_WEB_SALES);
  auto child_table_id = static_cast<int>(Table::TBL_WEB_RETURNS);
  dsdGenIterator.initTableOffset(table_id, offset);
  dsdGenIterator.initTableOffset(child_table_id, offset);
  dsdGenIterator.initializeTable(children, table_id);
  dsdGenIterator.initializeTable(childChildren, child_table_id);
  auto& tableDef = dsdGenIterator.getTableDefs();
  for (size_t i = 0; i < webSalesVectorSize; ++i) {
    dsdGenIterator.genRow(table_id, i + offset + 1);
  }

  auto webReturnsRowCount = tableDef[child_table_id]->rowIndex;
  for (auto& child : tableDef[child_table_id]->children) {
    child->resize(webReturnsRowCount);
  }
  return std::make_shared<RowVector>(
      pool,
      webReturnsRowType,
...
RowVectorPtr genTpcdsWebSales(
  ...
  auto webSalesRowType = getTableSchema(Table::TBL_WEB_SALES);
  DSDGenIterator dsdGenIterator(scaleFactor, parallel, child);
  // todo: Verify this vector size, for now using webSalesVectorSize.
  size_t webSalesVectorSize = getVectorSize(
      dsdGenIterator.getRowCount(static_cast<int>(Table::TBL_WEB_SALES)),
      maxRows,
      offset);
  ...
  // This table is a parent table of web_returns, this table will
  // be populated first and then data will be dumped in the web_returns table.
  // Create schema and allocate vectors.
  auto webReturnsRowType = getTableSchema(Table::TBL_WEB_RETURNS);
  auto childChildren =
      allocateVectors(webReturnsRowType, webSalesUpperBound, pool);
  ...
  auto webSalesRowCount = tableDef[table_id]->rowIndex;
  for (auto& child : tableDef[table_id]->children) {
    child->resize(webSalesRowCount);
  }
  return std::make_shared<RowVector>(
      pool,
      webSalesRowType,
...
RowVectorPtr genTpcdsWebSite(
    ...
    vector_size_t parallel,
    vector_size_t child) {
  auto websiteRowType = getTableSchema(Table::TBL_WEB_SITE);
  DSDGenIterator dsdGenIterator(scaleFactor, parallel, child);
  size_t vectorSize = getVectorSize(
      dsdGenIterator.getRowCount(static_cast<int>(Table::TBL_WEB_SITE)),
      maxRows,
      offset);
  auto children = allocateVectors(websiteRowType, vectorSize, pool);

  auto table_id = static_cast<int>(Table::TBL_WEB_SITE);
