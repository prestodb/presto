...
RowVectorPtr genTpcdsTableData(
  ...
  auto children = allocateVectors(rowType, vectorSize, pool);
...
RowVectorPtr genTpcdsParentAndChildTable(
  ...
  auto children = allocateVectors(parentTableType, parentTableUpperBound, pool);

  auto childTableType = getTableSchema(childTable);
  auto childChildren =
      allocateVectors(childTableType, parentTableUpperBound, pool);
