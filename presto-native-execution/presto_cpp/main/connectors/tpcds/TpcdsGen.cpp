...
std::unordered_map<std::string_view, Table> tableNames() {
  return {
...
std::string_view toTableName(Table table) {
  static const auto kTableNames = invertMap(tableNames());
  auto it = kTableNames.find(table);
  VELOX_CHECK(
      it != kTableNames.end(),
      "Invalid TPC-DS table : {}",
      static_cast<int>(table));
  return it->second;
}

Table fromTableName(const std::string_view& tableName) {
  static const auto kTableNames = tableNames();
  auto it = kTableNames.find(tableName);
  VELOX_CHECK(
      it != kTableNames.end(), "Invalid TPC-DS table name : {}", tableName);
  return it->second;
}
