...
int mk_w_call_center(
  ...
  struct CALL_CENTER_TBL *r,
      *rOldValues = &dsdGenContext.g_call_center_OldValues;

  r = &dsdGenContext.g_w_call_center;

  if (!dsdGenContext.mk_w_call_center_init) {
    ...
    dsdGenContext.mk_w_call_center_init = 1;
  }
  ...
  if (setSCDKeys(
     ...
     * written
     */
    nSuffix = (int)index / distsize("call_centers", dsdGenContext);
    dist_member(
        &cp,
        "call_centers",
        (int)(index % distsize("call_centers", dsdGenContext)) + 1,
        1,
        dsdGenContext);
    if (nSuffix > 0)
  ...
  genrand_integer(
      ...
      DIST_UNIFORM,
      1,
      nScale >= 1 ? int(CC_EMPLOYEE_MAX * nScale * nScale)
                  : int(CC_EMPLOYEE_MAX),
      0,
      CC_EMPLOYEES,
      dsdGenContext);
  ...
  append_varchar(info, std::to_string(r->cc_address.street_num));
  ...
  decimal_t gmt_offset;
  gmt_offset.number = r->cc_address.gmt_offset;
  append_decimal(info, &gmt_offset);
