...
int read_file(
  ...
  while (fgets(line, MAX_LINE_LEN, fp) != NULL) {
    ...
    if ((cp = strtok(line, " \t=\n")) != NULL) {
      ...
      if (dsdGenContext.options[index].flags & OPT_SET)
        continue;

      if (*cp) {
        switch (dsdGenContext.options[index].flags & TYPE_MASK) {
          case OPT_INT:
            if ((cp = strtok(cp, " \t\n")) != NULL)
              set_option(name, cp);
            break;
...
int IntegrateDist(
  ...
  while (nGoal >= 0) {
    nStartIndex++;
    nGoal -= dist_weight(
        NULL, szDistName, nStartIndex % nSize, nWeightSet, dsdGenContext);
  }
...
void dump_dist(const char* name, DSDGenContext& dsdGenContext) {
  ...
  for (i = 0; i < pIndex->length; i++) {
    ...
    for (j = 0; j < pIndex->w_width; j++) {
      if (j)
        printf(", ");
      printf("%d", dist_weight(NULL, name, i + 1, j + 1, dsdGenContext));
    }
    printf(");\n");
  }
...
int dist_active(
  ...
  for (i = 1; i <= nSize; i++) {
    if (dist_weight(NULL, szName, i, nWeightSet, dsdGenContext) != 0)
      nResult += 1;
  }
