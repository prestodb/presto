...
int setSCDKeys(
  ...
  static ds_key_t jMinimumDataDate, jMaximumDataDate, jH1DataDate, jT1DataDate,
      jT2DataDate;
  date_t dtTemp;
  int nTableID;

  if (!dsdGenContext.setSCDKeys_init) {
...
ds_key_t matchSCDSK(
  ...
  if (!dsdGenContext.matchSCDSK_init) {
    strtodt(&dtTemp, DATA_START_DATE);
    jMinimumDataDate = dtTemp.julian;
    strtodt(&dtTemp, DATA_END_DATE);
    jMaximumDataDate = dtTemp.julian;
    ...
    jT1DataDate = jMinimumDataDate + jT2DataDate;
    jT2DataDate += jT1DataDate;
    dsdGenContext.matchSCDSK_init = 1;
  }
