...
mk_master(void* info_arr, ds_key_t index, DSDGenContext& dsdGenContext) {
  ...
  {
    jDate += 1;
    kNewDateIndex += dateScaling(WEB_SALES, jDate, dsdGenContext);
  }
  ...
  genrand_integer(
      &nItemIndex, DIST_UNIFORM, 1, nItemCount, 0, WS_ITEM_SK, dsdGenContext);
...
mk_detail(void* info_arr, int bPrint, DSDGenContext& dsdGenContext) {
  ...
  if (dsdGenContext.mk_detail_init) {
    ...
    dsdGenContext.mk_detail_init = 1;
  }
  ...
  r->ws_item_sk = matchSCDSK(
      getPermutationEntry(pItemPermutation, nItemIndex),
      r->ws_sold_date_sk,
      ITEM,
      dsdGenContext);
  ...
  if (nTemp < WR_RETURN_PCT) {
    mk_w_web_returns(&w_web_returns, 1, dsdGenContext);
