...
mk_master(void* info_arr, ds_key_t index, DSDGenContext& dsdGenContext) {
  ...
  if (!dsdGenContext.mk_master_catalog_sales_init) {
    ...
    jDate = skipDays(CATALOG_SALES, &kNewDateIndex, dsdGenContext);
    pItemPermutation = makePermutation(
        NULL,
        (nItemCount = (int)getIDCount(ITEM, dsdGenContext)),
        CS_PERMUTE,
        dsdGenContext);
  ...
  {
    jDate += 1;
    kNewDateIndex += dateScaling(CATALOG_SALES, jDate, dsdGenContext);
  }
...
mk_detail(void* info_arr, int bPrint, DSDGenContext& dsdGenContext) {
  ...
  if (!dsdGenContext.mk_detail_catalog_sales_init) {
    strtodec(&dZero, "0.00");
    strtodec(&dHundred, "100.00");
    strtodec(&dOne, "1.00");
    strtodec(&dOneHalf, "0.50");
    ...
    dsdGenContext.mk_detail_catalog_sales_init = 1;
  }
