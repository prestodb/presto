...
mk_master(void* info_arr, ds_key_t index, DSDGenContext& dsdGenContext) {
  ...
  if (!dsdGenContext.mk_master_store_sales_init) {
    ...
    nMaxItemCount = 20;
    dsdGenContext.jDate = skipDays(STORE_SALES, &kNewDateIndex, dsdGenContext);
    dsdGenContext.pItemPermutation = makePermutation(
        NULL,
        dsdGenContext.nItemCount = (int)getIDCount(ITEM, dsdGenContext),
        SS_PERMUTATION,
        dsdGenContext);
  ...
  while (index > kNewDateIndex) /* need to move to a new date */
  {
    ...
    kNewDateIndex +=
        dateScaling(STORE_SALES, dsdGenContext.jDate, dsdGenContext);
  }
  ...
  genrand_integer(
      &dsdGenContext.nItemIndex,
      DIST_UNIFORM,
      1,
      dsdGenContext.nItemCount,
      0,
      SS_SOLD_ITEM_SK,
      dsdGenContext);
...
mk_detail(void* info_arr, int bPrint, struct W_STORE_SALES_TBL* r, DSDGenContext& dsdGenContext) {
   ...
   */
  if (++dsdGenContext.nItemIndex > dsdGenContext.nItemCount)
    dsdGenContext.nItemIndex = 1;
  int getperm = getPermutationEntry(
      dsdGenContext.pItemPermutation, dsdGenContext.nItemIndex);
  r->ss_sold_item_sk =
