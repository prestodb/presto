...
int mk_w_item(void* info_arr, ds_key_t index, DSDGenContext& dsdGenContext) {
  ...
  if (r->i_category_id) {
    dist_member(
        &bUseSize, "categories", (int)r->i_category_id, 3, dsdGenContext);
    pick_distribution(
  ...
  append_key(I_BRAND_ID, info, r->i_brand_id);
  append_varchar(I_BRAND, info, r->i_brand);
  append_key(I_CLASS_ID, info, r->i_class_id);
  append_varchar(I_CLASS, info, r->i_class);
  append_key(I_CATEGORY_ID, info, r->i_category_id);
  append_varchar(I_CATEGORY, info, r->i_category);
  append_key(I_MANUFACT_ID, info, r->i_manufact_id);
  ...
  append_key(I_MANAGER_ID, info, r->i_manager_id);
