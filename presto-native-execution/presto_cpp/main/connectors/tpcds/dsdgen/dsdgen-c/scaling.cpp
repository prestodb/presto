...
LogScale(int nTable, int nTargetGB, DSDGenContext& dsdGenContext) {
  ...
  fOffset = (float)(nTargetGB - dsdGenContext.arScaleVolume[i - 1]) /
      (float)(dsdGenContext.arScaleVolume[i] -
              dsdGenContext.arScaleVolume[i - 1]);
      ...
      if (dsdGenContext.arRowcount[nTable].kBaseRowcount >= 0) {
        if (nScale < 1) {
          ...
          if (!(mem == 1 && nMultiplier == 1)) {
            dsdGenContext.arRowcount[nTable].kBaseRowcount =
                int(dsdGenContext.arRowcount[nTable].kBaseRowcount * nScale);
          }
          if (dsdGenContext.arRowcount[nTable].kBaseRowcount == 0) {
