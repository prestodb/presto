...
/// generate TPC-DS datasets using the TPC-DS Connector. You can control the
/// generated table, scale factor, number of splits, and number of threads
/// (drivers) using the flags defined below.

DEFINE_string(table, "call_center", "TPC-DS table name to generate.");

DEFINE_int32(
    scale_factor,
    1,
    "Scale factor for the TPC-DS table being generated.");

DEFINE_int32(
    num_splits,
    1,
    "Number of splits to generate for a particular TPC-DS table scan.");
  ...
  TpcdsSpeedTest() {
    auto tpcdsConnector =
        connector::getConnectorFactory(
            ...
            ->newConnector(
                kTpcdsConnectorId_, std::make_shared<core::MemConfig>());
    connector::registerConnector(tpcdsConnector);
  }

  ~TpcdsSpeedTest() {
    connector::unregisterConnector(kTpcdsConnectorId_);
  }

  void run(tpcds::Table table, size_t scaleFactor, size_t numSplits) {
 ...
 private:
  void addSplits(exec::Task& task, core::PlanNodeId scanId, size_t numSplits) {
    for (size_t i = 0; i < numSplits; ++i) {
      task.addSplit(
          scanId,
          exec::Split(std::make_shared<connector::tpcds::TpcdsConnectorSplit>(
              kTpcdsConnectorId_, numSplits, i)));
    }
  ...
  const std::string kTpcdsConnectorId_{"test-tpcds"};
...
int main(int argc, char** argv) {
  ...
  TpcdsSpeedTest speedTest;
  speedTest.run(
      tpcds::fromTableName(FLAGS_table), FLAGS_scale_factor, FLAGS_num_splits);
  return 0;
}