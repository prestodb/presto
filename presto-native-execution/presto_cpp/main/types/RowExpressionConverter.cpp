...
json RowExpressionConverter::veloxToPrestoRowExpression(
  ...
  if (expr->isConstant()) {
    if (expr->inputs().empty()) {
      ...
      // compilation in velox, since they would throw an exception (Divide by
      // zero here) during evaluation (see function `tryFoldIfConstant` in
