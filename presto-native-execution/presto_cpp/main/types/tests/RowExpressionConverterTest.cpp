...
TEST_F(RowExpressionConverterTest, constant) {
  auto inputs = std::vector<exec::ExprPtr>{
      ...
      std::make_shared<exec::ConstantExpr>(makeConstant<StringView>("abcd", 1)),
      std::make_shared<exec::ConstantExpr>(
          makeNullConstant(TypeKind::BIGINT, 1)),
      std::make_shared<exec::ConstantExpr>(
          makeNullConstant(TypeKind::VARCHAR, 1))};
  testFile(inputs, "ConstantExpressions.json");
...
TEST_F(RowExpressionConverterTest, rowConstructor) {
  auto inputs = std::vector<exec::ExprPtr>{
      ...
      std::make_shared<exec::ConstantExpr>(makeConstantRow(
          ...
          1))};
  testFile(inputs, "RowConstructorExpressions.json");
}
...
TEST_F(RowExpressionConverterTest, variable) {
  ...
  testFile(inputs, "VariableExpressions.json");
}
...
TEST_F(RowExpressionConverterTest, special) {
  ...
  auto inputs = std::vector<exec::ExprPtr>{
      ...
      andExpr->constructSpecialForm(
          ...
          false,
          core::QueryConfig({}))};
  testFile(inputs, "CallAndSpecialFormExpressions.json");
}
