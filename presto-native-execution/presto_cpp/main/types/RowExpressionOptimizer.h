...
/// Helper class to optimize Presto expressions of type protocol::RowExpression
/// sent along the REST endpoint 'v1/expressions' in the sidecar.
class RowExpressionOptimizer {
 ...
 private:
  ...
  /// Evaluates special form expression AND when the inputs can be constant
  /// folded to true, false, or NULL.
  RowExpressionPtr optimizeAndSpecialForm(
      const SpecialFormExpressionPtr& specialFormExpr);

  /// Evaluates special form expression IF when the condition can be constant
  /// folded to true or false.
  RowExpressionPtr optimizeIfSpecialForm(
      const SpecialFormExpressionPtr& specialFormExpr);

  /// Evaluates special form expression IS_NULL when the input can be constant
  /// folded.
  RowExpressionPtr optimizeIsNullSpecialForm(
      const SpecialFormExpressionPtr& specialFormExpr);

  /// Evaluates special form expression OR when the inputs can be constant
  /// folded to true, false, or NULL.
  RowExpressionPtr optimizeOrSpecialForm(
      const SpecialFormExpressionPtr& specialFormExpr);

  /// Optimizes special form expression COALESCE by removing NULLs from the
  /// argument list.
  RowExpressionPtr optimizeCoalesceSpecialForm(
      const SpecialFormExpressionPtr& specialFormExpr);

  /// Optimizes special form expressions using optimization rules borrowed from
