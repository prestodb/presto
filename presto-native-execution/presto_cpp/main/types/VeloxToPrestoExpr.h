...
class VeloxToPrestoExprConverter {
 public:
  ...
  /// The input Presto protocol RowExpression, `inputRowExpr`, is returned in
  /// case the Velox to Presto expression conversion fails.
  RowExpressionPtr getRowExpression(
      const velox::core::TypedExprPtr& expr,
      const RowExpressionPtr& inputRowExpr = nullptr) const;

 private:
  /// `ValueBlock` in Presto `protocol::ConstantExpression` requires only the
  /// column from the serialized PrestoPage without the page header. This
  /// function is used to serialize a velox vector to `ValueBlock`.
