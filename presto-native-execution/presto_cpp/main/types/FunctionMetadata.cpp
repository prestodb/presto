...
const protocol::AggregationFunctionMetadata getAggregationFunctionMetadata(
  ...
  /// TODO: Set to true for now. To be read from a mapping of aggregate to
  /// order sensitivity which should be added in velox.
...
const protocol::RoutineCharacteristics getRoutineCharacteristics(
  ...
  if (functionKind == protocol::FunctionKind::SCALAR) {
    ...
    VELOX_USER_CHECK(functionMetadata.has_value());
    determinism = functionMetadata.value().deterministic
        ? protocol::Determinism::DETERMINISTIC
        : protocol::Determinism::NOT_DETERMINISTIC;
    nullCallClause = functionMetadata.value().defaultNullBehavior
...
json getJsonFunctionMetadata() {
  auto registeredFunctionNames = getSortedAggregateNames();
  auto scalarFunctionNames = getSortedScalarNames();
  for (const auto& scalarFunction : scalarFunctionNames) {
    registeredFunctionNames.emplace_back(scalarFunction);
  }
  auto windowFunctionNames = getSortedWindowNames();
