...
std::shared_ptr<exec::ConstantExpr> getConstantExpr(
  ...
  std::shared_ptr<exec::ConstantExpr> constExpr = nullptr;
  if constexpr (
    ...
    constExpr = std::make_shared<exec::ConstantExpr>(constVector);
  }
  return constExpr;
}
} // namespace
...
json::array_t RowExpressionOptimizer::optimizeExpressions(
  ...
  for (auto i = 0; i < numExpr; i++) {
    ...
    if (optimizerLevel == "EVALUATED") {
      if (compiledExpr->isConstant()) {
        resultJson = rowExpressionConverter_.veloxExprToRowExpression(
...
void RowExpressionOptimizer::optimize(
  ...
  try {
    ...
    auto optimizerLevel = message->getHeaders().getSingleOrEmpty(
    ...
    json output = optimizeExpressions(inputList, optimizerLevel, currentUser);
