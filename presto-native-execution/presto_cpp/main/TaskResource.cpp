...
void TaskResource::registerUris(http::HttpServer& server) {
  ...
  server.registerGet(
      R"(/v1/task/async/(.+)/results/([0-9]+)/([0-9]+)/acknowledge)",
      [&](proxygen::HTTPMessage* message,
          const std::vector<std::string>& pathMatch) {
        return acknowledgeResults(message, pathMatch);
      });

  // task/(.+)/batch must come before the /v1/task/async(.+) as it's more specific
  // otherwise all requests will be matched with /v1/task/async(.+)
  server.registerPost(
      R"(/v1/task/async/(.+)/batch)",
      [&](proxygen::HTTPMessage* message,
          const std::vector<std::string>& pathMatch) {
