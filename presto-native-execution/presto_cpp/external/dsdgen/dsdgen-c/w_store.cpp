...
int mk_w_store(void* info_arr, ds_key_t index, DSDGenContext& dsdGenContext) {
  ...
  if (!dsdGenContext.mk_w_store_init) {
    nHierarchyTotal = (int)get_rowcount(DIVISIONS, dsdGenContext);
    nHierarchyTotal *= (int)get_rowcount(COMPANY, dsdGenContext);
  ...
  mk_word(
      r->store_name, "syllables", (long)index, 5, W_STORE_NAME, dsdGenContext);
  changeSCD(
      SCD_CHAR,
      &r->store_name,
