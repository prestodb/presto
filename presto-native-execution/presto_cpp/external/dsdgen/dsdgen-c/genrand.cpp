...
void skip_random(int nStream, ds_key_t N, DSDGenContext& dsdGenContext) {
...
#endif
  M = Mult;
  Z = (ds_key_t)dsdGenContext.Streams[nStream].nInitialSeed;
  while (N > 0) {
  ...
  dsdGenContext.Streams[nStream].nSeed = (long)Z;
...
int genrand_integer(
  ...
  switch (dist) {
    ...
    case DIST_EXPONENTIAL:
      for (i = 0; i < 12; i++)
        fres += (double)(next_random(stream, dsdGenContext) / MAXINT) - 0.5;
...
int genrand_decimal(
  ...
  switch (dist) {
    ...
    case DIST_EXPONENTIAL:
      for (i = 0; i < 12; i++) {
        fres /= 2.0;
        fres += (double)((double)next_random(stream, dsdGenContext) /
                         (double)MAXINT) -
            0.5;
      }
...
void DSNthElementNthElement(
  ...
  Mult = Multiplier;
  Z = (HUGE_TYPE)dsdGenContext.Streams[nStream].nInitialSeed;
  while (N > 0) {
    if (N % 2 != 0) /* testing for oddness, this seems portable */
    {
#ifdef JMS
