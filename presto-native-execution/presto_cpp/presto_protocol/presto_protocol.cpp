...
void to_json(json& j, const std::shared_ptr<ConnectorHistogram>& p) {
  ...
  if (type == "uniform-distribution") {
    j = *std::static_pointer_cast<UniformDistributionHistogram>(p);
    return;
  }

  throw TypeError(type + " no abstract type ConnectorHistogram ");
}
...
void from_json(const json& j, std::shared_ptr<ConnectorHistogram>& p) {
  ...
  try {
    ...
    throw ParseError(
        std::string(e.what()) + " ConnectorHistogram  ConnectorHistogram");
  }

  if (type == "uniform-distribution") {
    std::shared_ptr<UniformDistributionHistogram> k =
        std::make_shared<UniformDistributionHistogram>();
    j.get_to(*k);
    p = std::static_pointer_cast<ConnectorHistogram>(k);
    return;
  }
...
namespace facebook::presto::protocol {
UniformDistributionHistogram::UniformDistributionHistogram() noexcept {
  _type = "uniform-distribution";
}

void to_json(json& j, const UniformDistributionHistogram& p) {
  j = json::object();
  j["@type"] = "uniform-distribution";
  to_json_key(
      j,
      "lowValue",
      p.lowValue,
      "UniformDistributionHistogram",
      "double",
      "lowValue");
  to_json_key(
      j,
