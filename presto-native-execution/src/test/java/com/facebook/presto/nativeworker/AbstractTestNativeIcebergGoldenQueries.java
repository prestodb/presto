        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testTemporalPartitioning()
    {
        ...
        try {
            javaQueryRunner.execute(
                    "CREATE TABLE golden_part_temporal (id BIGINT, ts TIMESTAMP) " +
                            "WITH (partitioning = ARRAY['year(ts)', 'month(ts)', 'day(ts)', 'hour(ts)'])");
            javaQueryRunner.execute(
                    "INSERT INTO golden_part_temporal VALUES " +
                            "(1, TIMESTAMP '2022-04-09 10:11:12.000'), " +
                            "(2, TIMESTAMP '2022-03-18 01:02:03.000'), " +
                            "(3, TIMESTAMP '1993-01-01 23:59:59.000')");
    ...
    {
        assertQuery("SELECT * FROM golden_types");
        assertQuery("SELECT id, dec, dt, bl FROM golden_types WHERE id = 1",
                "VALUES (1, DECIMAL '12.34', DATE '2022-04-09', true)");
