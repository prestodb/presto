        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testInsertFromTpchNation()
    {
        ...
        try {
            ...
            assertQuery(String.format("SELECT COUNT(*) FROM %s", targetTable), String.format("SELECT COUNT(*) FROM %s", sourceTable));
            assertQuery(String.format("SELECT nationkey, name FROM %s WHERE nationkey = 0", targetTable),
                    String.format("SELECT nationkey, name FROM %s WHERE nationkey = 0", sourceTable));
        }
        finally {
    ...
    public void testInsertFromTpchRegion()
    {
        ...
        try {
            ...
            assertQuery(String.format("SELECT COUNT(*) FROM %s", targetTable), String.format("SELECT COUNT(*) FROM %s", sourceTable));
            assertQuery(String.format("SELECT * FROM %s ORDER BY regionkey", targetTable),
                    String.format("SELECT * FROM %s ORDER BY regionkey", sourceTable));
        }
        finally {
            assertUpdate(String.format("DROP TABLE IF EXISTS %s", targetTable));
        }
