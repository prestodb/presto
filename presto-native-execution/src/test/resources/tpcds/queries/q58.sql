WITH
  ss_items AS (
   SELECT
     "i_item_id" "item_id"
   , "round"("sum"("ss_ext_sales_price"), 2) "ss_item_rev"
   FROM
     store_sales
   , item
   , date_dim
...
, cs_items AS (
   SELECT
     "i_item_id" "item_id"
   , "round"("sum"("cs_ext_sales_price"), 2) "cs_item_rev"
   FROM
     catalog_sales
   , item
   , date_dim
...
, ws_items AS (
   SELECT
     "i_item_id" "item_id"
   , "round"("sum"("ws_ext_sales_price"), 2) "ws_item_rev"
   FROM
     web_sales
   , item
   , date_dim
...
, CAST(((("ss_item_rev" / ((CAST("ss_item_rev" AS double) + "cs_item_rev") + "ws_item_rev")) / 3) * 100) AS double) "ss_dev"
, "cs_item_rev"
, CAST(((("cs_item_rev" / ((CAST("ss_item_rev" AS double) + "cs_item_rev") + "ws_item_rev")) / 3) * 100) AS double) "cs_dev"
, "ws_item_rev"
, CAST(((("ws_item_rev" / ((CAST("ss_item_rev" AS double) + "cs_item_rev") + "ws_item_rev")) / 3) * 100) AS double) "ws_dev"
