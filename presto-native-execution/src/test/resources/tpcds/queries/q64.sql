WITH
  cs_ui AS (
   SELECT
     "cs_item_sk"
   , "round"("sum"("cs_ext_list_price"), 2) "sale"
   , "round"("sum"((("cr_refunded_cash" + "cr_reversed_charge") + "cr_store_credit")), 2) "refund"
   FROM
     catalog_sales
   , catalog_returns
   ...
   GROUP BY "cs_item_sk"
   HAVING ("round"("sum"("cs_ext_list_price"), 2) > "round"((2 * "sum"((("cr_refunded_cash" + "cr_reversed_charge") + "cr_store_credit"))), 2))
) 
, cross_sales AS (
   SELECT
   ...
   , "d3"."d_year" "s2year"
   , "count"(*) "cnt"
   , "round"("sum"("ss_wholesale_cost"), 2) "s1"
   , "round"("sum"("ss_list_price"), 2) "s2"
   , "round"("sum"("ss_coupon_amt"), 2) "s3"
   FROM
     store_sales
   , store_returns
   , cs_ui
