WITH
  ws AS (
   ...
   , "round"("sum"("ws_quantity"), 2) "ws_qty"
   , "round"("sum"("ws_wholesale_cost"), 2) "ws_wc"
   , "round"("sum"("ws_sales_price"), 2) "ws_sp"
   FROM
     ((web_sales
...
, cs AS (
   ...
   , "round"("sum"("cs_quantity"), 2) "cs_qty"
   , "round"("sum"("cs_wholesale_cost"), 2) "cs_wc"
   , "round"("sum"("cs_sales_price"), 2) "cs_sp"
   FROM
     ((catalog_sales
...
, ss AS (
   ...
   , "ss_item_sk"
   , "ss_customer_sk"
   , "round"("sum"("ss_quantity"), 2) "ss_qty"
   , "round"("sum"("ss_wholesale_cost"), 2) "ss_wc"
   , "round"("sum"("ss_sales_price"), 2) "ss_sp"
   FROM
     ((store_sales
