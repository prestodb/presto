...
public class ThreadResource
{
    ...
    public String getUi()
            throws IOException
    {
        return Resources.toString(getResource(getClass(), "thread.html"), Charsets.UTF_8);
    }
    ...
    public List<Info> getThreadInfo()
    {
        ...
        for (ThreadInfo info : mbean.getThreadInfo(mbean.getAllThreadIds(), Integer.MAX_VALUE)) {
            builder.add(new Info(info.getThreadId(),
                    info.getThreadName(),
    ...
    private static List<StackLine> toStackTrace(StackTraceElement[] stackTrace)
    {
        ...
        for (StackTraceElement item : stackTrace) {
            builder.add(new StackLine(item.getFileName(),
                    item.getLineNumber(),
                    item.getClassName(),
                    item.getMethodName()));
        }
