...
import com.facebook.presto.utils.decoder.DecoderTestColumnHandle;
...
import static com.facebook.presto.utils.decoder.FieldDecoder.DEFAULT_FIELD_DECODER_NAME;
import static com.facebook.presto.utils.decoder.util.DecoderTestUtil.checkIsNull;
import static com.facebook.presto.utils.decoder.util.DecoderTestUtil.checkValue;
...
public class TestMillisecondsSinceEpochJsonFieldDecoder
{
    private static final Map<String, JsonFieldDecoder> DECODERS = ImmutableMap.of(DEFAULT_FIELD_DECODER_NAME, new JsonFieldDecoder(),
            MillisecondsSinceEpochJsonFieldDecoder.NAME, new MillisecondsSinceEpochJsonFieldDecoder());

    private static final ObjectMapperProvider PROVIDER = new ObjectMapperProvider();

    private static Map<DecodableColumnHandle, FieldDecoder<?>> buildMap(List<DecodableColumnHandle> columns)
    {
        ImmutableMap.Builder<DecodableColumnHandle, FieldDecoder<?>> map = ImmutableMap.builder();
        for (DecodableColumnHandle column : columns) {
            ...
            throws Exception
    {
        ...
        List<DecodableColumnHandle> columns = ImmutableList.of(row1, row2, row3, row4, row5, row6);
