...
export function getProgressBarPercentage(
  progress: number,
  queryState: string,
): number {
...
export function getProgressBarTitle(
  progress: any,
  queryState: string,
  ...
  if (progress && queryState === "RUNNING") {
    return (
      humanReadableState +
      " (" +
      getProgressBarPercentage(progress, queryState) +
      "%)"
    );
  }
...
export function addToHistory(value: number, valuesArray: number[]): number[] {
  ...
  return valuesArray
    .concat([value])
    .slice(Math.max(valuesArray.length - MAX_HISTORY, 0));
}

export function addExponentiallyWeightedToHistory(
  value: number,
  valuesArray: number[],
): number[] {
  if (valuesArray.length === 0) {
  ...
  let movingAverage =
    value * MOVING_AVERAGE_ALPHA +
    valuesArray[valuesArray.length - 1] * (1 - MOVING_AVERAGE_ALPHA);
  if (value < 1) {
    movingAverage = 0;
  }

  return valuesArray
    .concat([movingAverage])
    .slice(Math.max(valuesArray.length - MAX_HISTORY, 0));
}
