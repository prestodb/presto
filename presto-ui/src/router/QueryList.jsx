...
export class QueryListItem extends React.Component {
  ...
  render() {
    ...
    if (query.session.principal) {
      user = <span>{query.session.user}</span>;
    }
          ...
          <div className="col-4">
            <div className="row stat-row query-header query-header-queryid">
              ...
              >
                <a
                  href={
                    query.coordinatorUri + "/ui/query.html?" + query.queryId
                  }
                  target="_blank"
                  rel="noreferrer"
                >
              ...
              >
                <span>
                  {formatShortTime(
                    new Date(Date.parse(query.queryStats.createTime)),
                  )}
                </span>
              </div>
            </div>
...
const ERROR_TYPE = {
  ...
  INSUFFICIENT_RESOURCES: function (query) {
    return (
      query.state === "FAILED" && query.errorType === "INSUFFICIENT_RESOURCES"
    );
  },
