...
function OperatorSummary({ operator }) {
  ...
  const rowInputRate =
    totalWallTime === 0
      ? 0
      : (1.0 * operator.inputPositions) / (totalWallTime / 1000.0);
  const byteInputRate =
    totalWallTime === 0
      ? 0
      : (1.0 * parseDataSize(operator.inputDataSize)) /
...
function OperatorDetail({ operator, tasks }) {
  ...
  const rowInputRate =
    totalWallTime === 0 ? 0 : (1.0 * operator.inputPositions) / totalWallTime;
  const byteInputRate =
    totalWallTime === 0
      ? 0
      : (1.0 * parseDataSize(operator.inputDataSize)) /
  ...
  const rowOutputRate =
    totalWallTime === 0 ? 0 : (1.0 * operator.outputPositions) / totalWallTime;
  const byteOutputRate =
    totalWallTime === 0
      ? 0
      : (1.0 * parseDataSize(operator.outputDataSize)) /
        (totalWallTime / 1000.0);
...
function StageOperatorGraph({ stage }) {
  ...
  const computeOperatorGraphs = () => {
    ...
    stage.latestAttemptExecutionInfo.stats.operatorSummaries.forEach(
      (operator) => {
  ...
  const handleOperatorClick = (event) => {
    ...
    if (
      event.target.hasOwnProperty("__data__") &&
      ...
      const operatorSummaries =
        stage.latestAttemptExecutionInfo.stats.operatorSummaries;
      ...
      if (!detailContainer.current) {
        detailContainer.current = createRoot(
          document.getElementById("operator-detail"),
        );
      }
      detailContainer.current.render(
        <OperatorDetail
          index={event}
  ...
  const computeD3StageOperatorGraph = (graph, operator, sink, pipelineNode) => {
    ...
    if (sink !== null) {
      graph.setEdge(operatorNodeId, sink, {
        class: "plan-edge",
...
export default function StageView({ data, show }) {
  ...
  const stage = findStage(
    data.queryId + "." + state.selectedStageId,
    data.outputStage,
  );
                    ...
                    {allStages.map((stageId) => (
                      <li key={stageId}>
                        <a
                          ...
                          onClick={() => setState({ selectedStageId: stageId })}
                        >
                          {stageId}
                        </a>
                      </li>
