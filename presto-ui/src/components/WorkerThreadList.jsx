...
const QUERY_THREAD_REGEX = new RegExp(
  /([0-9])*_([0-9])*_([0-9])*_.*?\.([0-9])*\.([0-9])*-([0-9])*-([0-9])*/,
);
...
export class WorkerThreadList extends React.Component {
  ...
  captureSnapshot() {
    $.get(
      WorkerThreadList.getRequestQuery(
        getFirstParameter(window.location.search),
      ),
      function (threads) {
        this.setState({
  ...
  filterThreads(group, state) {
    return this.state.threads[group].filter(
      (t) => t.state === state || state === ALL_THREAD_STATE,
    );
  }
      ...
      <li key={group}>
        <a
          ...
          className={clsx(
            ...
            this.state.selectedGroup === group ? "selected" : "",
          )}
    ...
    return (
      <li key={threadState}>
        <a
          ...
          className={clsx(
            "dropdown-item text-dark",
            this.state.selectedThreadState === threadState ? "selected" : "",
          )}
    ...
    return (
      <div key={threadInfo.id}>
        <span className="font-white">
          {threadInfo.name} {threadInfo.state} #{threadInfo.id}{" "}
          {threadInfo.lockOwnerId}
        </span>
        <a
          className="copy-button"
          data-clipboard-target={"#stack-trace-" + threadInfo.id}
    ...
    if (threads === null) {
      if (this.state.initialized === false) {
        ...
        display = (
          <div className="row error-message">
            <div className="col-12">
              <h4 className="therad-snapshot-text">
              ...
              <tr>
                ...
                <td>
                  <div className="input-group-btn text-right">
                    <button
                      type="button"
                      className="btn btn-default dropdown-toggle bg-white text-dark rounded-0 dropdown-text"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                    >
                      <strong>Group:</strong> {this.state.selectedGroup}{" "}
                      <span className="caret" />
                    </button>
                    <ul className="dropdown-menu bg-white rounded-0 dropdown-text">
                      {Object.keys(threads).map((group) =>
                        this.renderGroupListItem(group),
                      )}
                    </ul>
                  </div>
                </td>
                <td>
                  <div className="input-group-btn text-right">
                    <button
                      ...
                      aria-expanded="false"
                    >
                      <strong>State:</strong> {this.state.selectedThreadState}{" "}
                      <span className="caret" />
                    </button>
                    <ul className="dropdown-menu bg-white rounded-0 dropdown-text">
                      {THREAD_STATES.map((state) =>
                        this.renderThreadStateListItem(state),
                      )}
                    </ul>
                  </div>
                </td>
      ...
      if (
        ...
        this.state.selectedThreadState === ALL_THREAD_STATE
      ) {
        displayedThreads = (
          <div className="row error-message">
            <div className="col-12">
        ...
        filteredThreads.length === 0 &&
        this.state.selectedGroup === ALL_THREADS
      ) {
        displayedThreads = (
          <div className="row error-message">
            <div className="col-12">
              <h4>No threads with state {this.state.selectedThreadState}</h4>
            </div>
          </div>
        );
        ...
        displayedThreads = (
          <pre>{filteredThreads.map((t) => this.renderThread(t))}</pre>
        );
      }

      display = <div id="stack-traces">{displayedThreads}</div>;
    }
          ...
          <div className="col-3">
            <h3 style={{ fontSize: "24px" }}>
              ...
              <a
                ...
                data-clipboard-target="#stack-traces"
                data-bs-toggle="tooltip"
                data-placement="right"
                title="Copy to clipboard"
              >
                <span
                  ...
                  style={{ color: "#98E8FF", fontSize: "14px" }}
                />
              </a>
              &nbsp;
            </h3>
