...
export function createClient(
  ...
  const opt: typeof PrestoClientConfig = {
...
class SelectListener extends SqlBaseListener {
  limit: number = -1;
  fetchFirstNRows: { text: string } | number = -1;
  isSelect = false;
  ...
  try {
    ...
    const parser = new SqlBaseParser(
      new antlr4.CommonTokenStream(
        new SqlBaseLexer(new UpperCaseCharStream(cleanSql)),
      ),
    );
    ...
    if (selectDetector.isSelect) {
      if (
        typeof selectDetector.limit === "string" ||
        selectDetector.limit > 100
      ) {
        ...
        selectDetector.limit === -1 &&
        selectDetector.fetchFirstNRows === -1
      ) {
        cleanSql += " limit 100";
      }
    }
