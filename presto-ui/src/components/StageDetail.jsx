...
class StageOperatorGraph extends React.Component {
  ...
  computeOperatorGraphs() {
    ...
    this.props.stage.latestAttemptExecutionInfo.stats.operatorSummaries.forEach(
      (operator) => {
  ...
  computeD3StageOperatorGraph(graph, operator, sink, pipelineNode) {
    ...
    if (operator.hasOwnProperty("child")) {
      this.computeD3StageOperatorGraph(
        graph,
        operator.child,
        operatorNodeId,
        pipelineNode,
  ...
  updateD3Graph() {
    ...
    if (operatorGraphs.size > 0) {
      ...
      svg
        .selectAll("g.operator-stats")
        .on("click", this.handleOperatorClick.bind(this));
...
export class StageDetail extends React.Component {
  ...
  render() {
    ...
    const stage = this.findStage(
      query.queryId + "." + this.state.selectedStageId,
      query.outputStage,
    );
    if (stage === null) {
      return (
                      ...
                      {allStages.map((stageId) => (
                        <li key={stageId}>
                          <a
                            ...
                            onClick={() =>
                              this.setState({ selectedStageId: stageId })
                            }
                          >
                            {stageId}
                          </a>
