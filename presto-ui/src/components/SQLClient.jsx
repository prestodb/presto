...
export default function SQLClientView() {
  ...
  const sessions: React.RefObject<SessionValues> = React.useRef<SessionValues>(
    {},
  );
  const views = [
    { name: "SQL", label: "SQL" },
  ...
  const executeSQL = (sql: string, catalog: string, schema: string) => {
    ...
    client
      .query(sql)
      .then((prestoQuery: typeof PrestoQuery) => {
  ...
  const switchView = (view: { label: string, name: string, ... }) => {
  ...
  return (
    <>
      <div
        ...
        role="alert"
      >
        SQL client directly accesses the coordinator APIs and submits SQL
        queries. Users who can access the Web UI can use this client to query,
        update, and even delete data in the catalogs. Be sure to enable the user
        authentication to protect the Web UI access if needed. By default, the
        SQL client uses the <strong>prestoui</strong> user id. You can set up{" "}
        <a
      ...
      <div className="container">
        <div className="row">
          ...
          <div className="col-12">
            ...
            />
            <SessionProps
              show={values.view === "Session"}
              changeHandler={sessionHandler}
            />
          </div>
          <div className="col-12">
