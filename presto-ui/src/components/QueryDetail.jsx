...
function TaskList({ tasks }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) =>
        formatDuration(parseDuration(row.stats.elapsedTimeInNanos + "ns")),
      sortable: true,
      minWidth: "75px",
    },
    {
      ...
      cell: (row) =>
        formatDuration(parseDuration(row.stats.totalCpuTimeInNanos + "ns")),
      sortable: true,
      minWidth: "75px",
    },
    {
        ...
        <tbody>
          <tr>
            ...
            <th className="expand-charts-container">
              ...
              >
                <span
                  className={"bi " + this.getExpandedIcon()}
                  style={GLYPHICON_HIGHLIGHT}
                  data-bs-toggle="tooltip"
                  data-bs-placement="top"
                  title="Show metrics"
          ...
          {Object.values(this.props.stats)
            ...
            .map((metric) => (
              <tr style={this.getExpandedStyle()} key={metric.name}>
                ...
                <td className="info-text">
                  {this.renderMetricValue(metric.unit, metric.min)}
                </td>
                <td className="info-text">
                  {this.renderMetricValue(metric.unit, metric.max)}
                </td>
              </tr>
            ))}
        </tbody>
...
class StageSummary extends React.Component {
  ...
  componentDidUpdate() {
    ...
    if (
      ...
      StageSummary.renderHistogram(
        "#scheduled-time-histogram-" + stageId,
        scheduledTimes,
        formatDuration,
      );
      StageSummary.renderHistogram(
        "#cpu-time-histogram-" + stageId,
        cpuTimes,
        formatDuration,
      );
      ...
      if (this.state.expanded) {
        ...
        for (let i = 0; i < numTasks; i++) {
          tooltipValueLookups["offset"][i] =
            getStageNumber(stage.stageId) + "." + i;
        }
  ...
  renderTaskList(tasks) {
    ...
    tasks = tasks.filter(
      (task) => this.state.taskFilter.predicate(task.taskStatus.state),
      this,
    );
    return (
  ...
  renderTaskFilterListItem(taskFilter) {
    return (
      <li>
        <a
          href="#"
          className={`dropdown-item text-dark ${this.state.taskFilter === taskFilter ? "selected" : ""}`}
                ...
                <td>
                  <div className="btn-group text-right">
                    <button
                      ...
                      aria-expanded="false"
                    >
                      Show: {this.state.taskFilter.text}{" "}
      ...
      <tr>
        <td className="stage-id">
          <div
            ...
            style={{ borderLeftColor: getStageStateColor(stage) }}
          >
            {stageId}
          </div>
        </td>
        <td>
          <table className="table single-stage-table">
            <tbody>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td className="stage-table-stat-text">
                          {
                            stage.latestAttemptExecutionInfo.stats
                              .totalScheduledTime
                          }
                        </td>
                      </tr>
              ...
              {this.renderStageExecutionAttemptsTasks([
                stage.latestAttemptExecutionInfo,
              ])}

              {this.renderStageExecutionAttemptsTasks(
                stage.previousAttemptsExecutionInfos,
              )}
            </tbody>
          </table>
        </td>
...
class StageList extends React.Component {
  ...
  render() {
    ...
    const renderedStages = stages.map((stage) => (
      <StageSummary key={stage.stageId} stage={stage} />
    ));
...
export class QueryDetail extends React.Component {
  ...
  static formatStackTraceHelper(info, parentStack, prefix, linePrefix) {
    ...
    if (info.stack) {
      ...
      if (parentStack !== null) {
        sharedStackFrames = QueryDetail.countSharedStackFrames(
          info.stack,
          parentStack,
        );
      }
  ...
  static getQueryURL(id) {
    ...
    return sanitizedId.length > 0
      ? `/v1/query/${encodeURIComponent(sanitizedId)}`
      : "/v1/query/undefined";
  }
  ...
  refreshLoop() {
    ...
    $.get(
      QueryDetail.getQueryURL(queryId),
      function (query) {
        ...
        if (
          alreadyEnded ||
          (lastRefresh === null && query.state === "RUNNING")
        ) {
          this.resetTimer();
          return;
        }
        ...
        if (elapsedSecsSinceLastRefresh >= 0) {
          const currentScheduledTimeRate =
            (parseDuration(query.queryStats.totalScheduledTime) -
              lastScheduledTime) /
          ...
          const currentRowInputRate =
            (query.queryStats.processedInputPositions - lastRowInput) /
            elapsedSecsSinceLastRefresh;
          const currentByteInputRate =
            (parseDataSize(query.queryStats.processedInputDataSize) -
              lastByteInput) /
            elapsedSecsSinceLastRefresh;
          this.setState({
            scheduledTimeRate: addToHistory(
              currentScheduledTimeRate,
              this.state.scheduledTimeRate,
            ),
            cpuTimeRate: addToHistory(
              currentCpuTimeRate,
              this.state.cpuTimeRate,
            ),
            rowInputRate: addToHistory(
              currentRowInputRate,
              this.state.rowInputRate,
            ),
            byteInputRate: addToHistory(
              currentByteInputRate,
  ...
  renderStageRefreshButton() {
    if (this.state.stageRefresh) {
      return (
        <button
          className="btn btn-info live-button rounded-0"
    ...
    for (let catalog in query.session.catalogProperties) {
      if (query.session.catalogProperties.hasOwnProperty(catalog)) {
        for (let property in query.session.catalogProperties[catalog]) {
          if (
            query.session.catalogProperties[catalog].hasOwnProperty(property)
          ) {
            properties.push(
              <span>
                -{" "}
                ...
                <tr>
                  ...
                  <td className="info-text">
                    {query.errorCode.name +
                      " (" +
                      this.state.query.errorCode.code +
                      ")"}
                  </td>
                </tr>
                <tr>
                ...
                <table className="table">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td className="info-text">
                        {formatDataSize(
                          query.queryStats.cumulativeUserMemory / 1000.0,
                        )}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td className="info-text">
                        {formatDataSize(
                          query.queryStats.cumulativeTotalMemory / 1000.0,
                        )}
                      </td>
                    </tr>
                    <tr>
                  ...
                  <tbody>
                    ...
                    <tr className="tr-noborder">
                      <td className="info-sparkline-text">
                        {formatCount(
                          this.state.cpuTimeRate[
                            this.state.cpuTimeRate.length - 1
                          ],
                        )}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td rowSpan="2">
                        <div className="query-stats-sparkline-container">
                          <span
                            className="sparkline"
                            id="scheduled-time-rate-sparkline"
                          >
                            <div className="loader">Loading ...</div>
                          </span>
                        </div>
                    ...
                    <tr className="tr-noborder">
                      <td className="info-sparkline-text">
                        {formatCount(
                          this.state.rowInputRate[
                            this.state.rowInputRate.length - 1
                          ],
                        )}
                      </td>
                    </tr>
