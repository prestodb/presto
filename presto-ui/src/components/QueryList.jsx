...
function getHumanReadableStateFromInfo(query) {
  ...
  return getHumanReadableState(
    query.queryState,
    query.queryState === "RUNNING" &&
      progress.queuedDrivers +
        progress.runningDrivers +
        progress.completedDrivers >
        0 &&
      progress.runningDrivers >= 0,
    progress.blocked,
    ...
    return (
      <a
        ...
        href={
          ...
          encodeURIComponent(groupId.slice(0, idx + 1).join("."))
        }
        key={idx}
      >
...
export class QueryListItem extends React.Component {
  static stripQueryTextWhitespace(queryText, isTruncated) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        leadingWhitespace > -1 &&
        (leadingWhitespace < minLeadingWhitespace ||
    ...
    for (let i = 0; i < lines.length; i++) {
      const trimmedLine = lines[i]
        .substring(minLeadingWhitespace)
        .replace(/\s+$/g, "");
    ...
    return isTruncated
      ? formattedQueryText + "..."
      : truncateString(formattedQueryText, maxQueryLength);
  }
  ...
  render() {
    ...
    const progressPercentage = getProgressBarPercentage(
      query.progress.progressPercentage,
      query.queryState,
    );
    ...
    const driverDetails = (
      <div className="col-12 tinystat-row">
        ...
        >
          ...
          />
          &nbsp;&nbsp;
          {query.queryState === "FINISHED" || query.queryState === "FAILED"
            ? 0
            : query.progress.runningDrivers}
        </span>
        <span
          className="tinystat"
          data-bs-toggle="tooltip"
    ...
    const newDriverDetails = (
      <div className="col-12 tinystat-row">
        ...
        >
          ...
          {query.queryState === "FINISHED" || query.queryState === "FAILED"
            ? 0
            : query.progress.runningNewDrivers}
        </span>
        <span
          className="tinystat"
          data-bs-toggle="tooltip"
          data-bs-placement="top"
          title="Queued drivers"
        >
          <span
          ...
          {query.queryState === "FINISHED" || query.queryState === "FAILED"
            ? 0
            : query.progress.queuedNewDrivers}
        </span>
      </div>
    );
    ...
    if (query.authenticated) {
      user = <span>{truncateString(query.user, 35)}</span>;
    }
        ...
        <div className="row">
          <div className="col-4">
            ...
            {query.progress.completedSplits ? (
              <>
                <div className="row stat-row">{newDriverDetails}</div>
                <div className="row stat-row">{splitDetails}</div>
              </>
            ) : (
              <div className="row stat-row">{driverDetails}</div>
            )}
            ...
            <div className="row query-row-bottom">
              <div className="col-12">
                <pre className="query-snippet">
                  <code className="sql">
                    {QueryListItem.stripQueryTextWhitespace(
                      query.query,
...
const FILTER_TYPE = {
  RUNNING: function (query) {
    return !(
      query.queryState === "QUEUED" ||
      query.queryState === "FINISHED" ||
      query.queryState === "FAILED"
    );
  },
  QUEUED: function (query) {
    return query.queryState === "QUEUED";
  },
  FINISHED: function (query) {
    return query.queryState === "FINISHED";
  },
};

const SORT_TYPE = {
  CREATED: function (query) {
    return Date.parse(query.createTime);
  },
  ELAPSED: function (query) {
    return query.progress.elapsedTimeMillis;
  },
  EXECUTION: function (query) {
    return query.progress.executionTimeMillis;
  },
  CPU: function (query) {
    return query.progress.cpuTimeMillis;
  },
  CUMULATIVE_MEMORY: function (query) {
    return query.progress.cumulativeUserMemory;
  },
  CURRENT_MEMORY: function (query) {
    return query.progress.currentMemoryBytes;
  },
};
...
const ERROR_TYPE = {
  USER_ERROR: function (query) {
    return (
      query.queryState === "FAILED" && query.errorCode.type === "USER_ERROR"
    );
  },
  INTERNAL_ERROR: function (query) {
    return (
      query.queryState === "FAILED" && query.errorCode.type === "INTERNAL_ERROR"
    );
  },
  INSUFFICIENT_RESOURCES: function (query) {
    return (
      query.queryState === "FAILED" &&
      query.errorCode.type === "INSUFFICIENT_RESOURCES"
    );
  },
  EXTERNAL: function (query) {
    return query.queryState === "FAILED" && query.errorCode.type === "EXTERNAL";
  },
};
...
export class QueryList extends React.Component {
    ...
    if (searchString === "") {
      ...
      return stateFilteredQueries.filter(function (query) {
        ...
        if (
          ...
          query.resourceGroupId.join(".").toLowerCase().indexOf(term) !== -1
        ) {
          return true;
        }
  ...
  refreshLoop() {
    ...
    $.get(
      "/v1/queryState?includeAllQueries=true&includeAllQueryProgressStats=true&excludeResourceGroupPathInfo=true",
      function (queryList) {
        ...
        if (
          this.state.reorderInterval !== 0 &&
          lastRefresh - lastReorder >= this.state.reorderInterval
        ) {
        ...
        if (
          this.state.maxQueries !== 0 &&
          updatedQueries.length > this.state.maxQueries
        ) {
  ...
  handleSortClick(sortType) {
    ...
    if (
      ...
      this.state.currentSortOrder === SORT_ORDER.DESCENDING
    ) {
  ...
  render() {
    let queryList = (
      <DisplayedQueriesList queries={this.state.displayedQueries} />
    );
    if (
      this.state.displayedQueries === null ||
      this.state.displayedQueries.length === 0
    ) {
        ...
        <div className="row toolbar-row">
          <div className="col-12 input-group gap-1 toolbar-col d-flex">
            ...
            <div className="input-group-btn d-flex align-items-center ms-auto">
              <span
                className="input-group-text input-group-prepend rounded-0"
                style={{
                  backgroundColor: "white",
                  color: "black",
                  height: "2rem",
            ...
            <div className="input-group-btn">
              <button
                type="button"
                className="btn btn-dark btn-sm dropdown-toggle bg-white text-btn-default .query-detail-buttons rounded-0"
                data-bs-toggle="dropdown"
                aria-haspopup="true"
                aria-expanded="false"
              ...
              <ul className="dropdown-menu bg-white text-dark rounded-0">
                ...
                {this.renderSortListItem(
                  SORT_TYPE.CURRENT_MEMORY,
                  "Current Memory",
                )}
                {this.renderSortListItem(
                  SORT_TYPE.CUMULATIVE_MEMORY,
            ...
            <div className="input-group-btn">
              <button
                type="button"
                className="btn btn-dark btn-sm dropdown-toggle bg-white text-btn-default rounded-0"
                data-bs-toggle="dropdown"
                aria-haspopup="true"
                aria-expanded="false"
                style={{ fontSize: "12px", height: "31px" }}
