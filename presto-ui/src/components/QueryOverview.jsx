...
function TaskList({ tasks }: { tasks: Task[] }): React.Node {
  ...
  const hasSplitStats = tasks.some(
    (task) => task.stats.completedSplits !== undefined,
  );
  ...
  const columns = [
    {
      ...
      cell: (row: Task) => (
        <a href={"/v1/taskInfo/" + row.taskId + "?pretty"}>
    ...
    },
    {
      ...
      cell: (row: Task) => (
        <a
          href={"worker.html?" + row.nodeId}
          className="font-light nowrap"
          target="_blank"
          rel="noreferrer"
        >
          {showingPortNumbers
            ? getHostAndPort(row.taskStatus.self)
            : getHostname(row.taskStatus.self)}
        </a>
      ),
      sortable: true,
      grow: 3,
    ...
    },
    {
      name: "State",
      selector: (row: Task) =>
        formatState(row.taskStatus.state, row.stats.fullyBlocked),
      sortable: true,
      minWidth: "80px",
    },
    ...(hasSplitStats
    ...
    },
    {
      name: "Rows/s",
      selector: (row: Task) =>
        computeRate(row.stats.rawInputPositions, calculateElapsedTime(row)),
      cell: (row: Task) =>
        formatCount(
          computeRate(row.stats.rawInputPositions, calculateElapsedTime(row)),
        ),
    ...
    },
    {
      ...
      cell: (row: Task) =>
        formatDataSizeBytes(row.stats.rawInputDataSizeInBytes),
      sortable: true,
      minWidth: "75px",
    },
    {
      ...
      selector: (row: Task) =>
        computeRate(
          row.stats.rawInputDataSizeInBytes,
          calculateElapsedTime(row),
        ),
      cell: (row: Task) =>
        formatDataSizeBytes(
          computeRate(
    ...
    },
    {
      name: "Elapsed",
      selector: (row: Task) =>
        parseDuration(row.stats.elapsedTimeInNanos + "ns"),
      cell: (row: Task) =>
        formatDuration(parseDuration(row.stats.elapsedTimeInNanos + "ns") || 0),
      sortable: true,
      minWidth: "75px",
    },
    {
      name: "CPU Time",
      selector: (row: Task) =>
        parseDuration(row.stats.totalCpuTimeInNanos + "ns"),
      cell: (row: Task) =>
        formatDuration(
          parseDuration(row.stats.totalCpuTimeInNanos + "ns") || 0,
        ),
      sortable: true,
      minWidth: "75px",
    },
    {
      ...
      cell: (row: Task) =>
        formatDataSizeBytes(row.outputBuffers.totalBufferedBytes),
      sortable: true,
      minWidth: "75px",
    },
  ];
          ...
          .map((metric, index) => (
            <tr style={getExpandedStyle()} key={index}>
              ...
              <td className="info-text">
                {renderMetricValue(metric.unit, metric.sum)}
              </td>
              <td className="info-text">{formatCount(metric.count)}</td>
              <td className="info-text">
                {renderMetricValue(metric.unit, metric.min)}
              </td>
              <td className="info-text">
                {renderMetricValue(metric.unit, metric.max)}
              </td>
            </tr>
          ))}
      </tbody>
...
function StageSummary({
  ...
  const renderStageExecutionAttemptsTasks = (
    attempts: StageExecutionInfo[],
  ) => {
  ...
  useEffect(() => {
    ...
    prestoStage.latestAttemptExecutionInfo.tasks.sort(
      (taskA, taskB) =>
        getTaskNumber(taskA.taskId) - getTaskNumber(taskB.taskId),
    );
    ...
    renderHistogram(
      "#scheduled-time-histogram-" + stageId,
      scheduledTimes,
      formatDuration,
    );
    ...
    for (let i = 0; i < numTasks; i++) {
      tooltipValueLookups["offset"][i] =
        getStageNumber(prestoStage.stageId) + "." + i;
    }
    ...
    <tr key={index}>
      <td className="stage-id">
        <div
          ...
          style={{ borderLeftColor: getStageStateColor(prestoStage) }}
        >
          {stageId}
        </div>
      </td>
        ...
        <table className="table single-stage-table">
          <tbody>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td className="stage-table-stat-text">
                        {
                          prestoStage.latestAttemptExecutionInfo.stats
                            .totalScheduledTime
                        }
                      </td>
                    </tr>
            ...
            {renderStageExecutionAttemptsTasks([
              prestoStage.latestAttemptExecutionInfo,
            ])}
            {renderStageExecutionAttemptsTasks(
              prestoStage.previousAttemptsExecutionInfos,
            )}
          </tbody>
        </table>
      </td>
...
function StageList({ outputStage }: { outputStage: OutputStage }): React.Node {
  ...
  const renderedStages = stages.map((stage, index) => (
    <StageSummary key={index} index={index} prestoStage={stage} />
  ));
...
export default function QueryOverview({
  ...
  const renderPreparedQuery = () => {
    const query = data;
    if (
      !query.hasOwnProperty("preparedQuery") ||
    ...
    for (let catalog in data.session.catalogProperties) {
      if (data.session.catalogProperties.hasOwnProperty(catalog)) {
        for (let property in data.session.catalogProperties[catalog]) {
          if (
            data.session.catalogProperties[catalog].hasOwnProperty(property)
          ) {
            properties.push(
              <span>
                -{" "}
                {catalog +
                  ...
                  property +
                  "=" +
                  data.session.catalogProperties[catalog][property]}{" "}
                <br />
              </span>,
            );
          }
              ...
              <table className="table">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="info-text">
                      {formatDataSize(
                        data.queryStats.cumulativeUserMemory / 1000.0,
                      )}
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td className="info-text">
                      {formatDataSize(
                        data.queryStats.cumulativeTotalMemory / 1000.0,
                      )}
                    </td>
                  </tr>
                  <tr>
                  ...
                  {(parseDataSize(data.queryStats.spilledDataSize) || 0) >
                    0 && (
                    <tr>
              ...
              <table className="table">
                <tbody>
                  ...
                  <tr className="tr-noborder">
                    <td className="info-sparkline-text">
                      {formatCount(
                        (parseDuration(data.queryStats.totalScheduledTime) ||
                          0) /
                          (elapsedTime * 1000),
                      )}
                    </td>
                  ...
                  <tr className="tr-noborder">
                    <td className="info-sparkline-text">
                      {formatCount(
                        data.queryStats.processedInputPositions / elapsedTime,
                      )}
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr className="tr-noborder">
                    <td className="info-sparkline-text">
                      {formatDataSize(
                        parseDataSize(data.queryStats.userMemoryReservation) ||
                          0,
                      )}
                    </td>
                  </tr>
