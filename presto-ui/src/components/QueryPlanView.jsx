...
import type { QueryData } from "./QueryOverview.jsx";
...
export default function PlanView({
  ...
  const updateD3Stage = (
    stage: StageNodeInfo,
    graph: any,
    allStages: Map<string, StageNodeInfo>,
  ) => {
    ...
    const html = ReactDOMServer.renderToString(
      <StageStatistics key={stage.id} stage={stage} />,
    );

    graph.setNode(stageRootNodeId, {
      class: "stage-stats text-center",
      label: html,
      labelType: "html",
    });
  ...
  const updateD3Graph = () => {
    ...
    const width =
      parseInt(
        window
          .getComputedStyle(document.getElementById("plan-viewer"), null)
