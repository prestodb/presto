    ...
    const columns = [
        {
            ...
            cell: (row: Task) => <a href={"/v1/taskInfo/" + row.taskId + "?pretty"}>{getTaskIdSuffix(row.taskId)}</a>,
            minWidth: "60px",
        },
        {
            name: "Host",
...
function StageSummary({ index, prestoStage }: { index: number; prestoStage: OutputStage }): React.ReactElement {
    ...
    const renderTaskFilterListItem = (taskFilter: TaskFilter) => {
        return (
            <li>
                <a
                    href="#"
                    className={`dropdown-item text-dark ${state.taskFilter === taskFilter ? "selected" : ""}`}
                    onClick={(event) => handleTaskFilterClick(taskFilter, event)}
                >
                    {taskFilter.text}
                </a>
            </li>
    ...
    useEffect(() => {
        ...
        // sort the x-axis
        prestoStage.latestAttemptExecutionInfo.tasks.sort(
            (taskA, taskB) => getTaskNumber(taskA.taskId) - getTaskNumber(taskB.taskId)
        );

        const scheduledTimes = prestoStage.latestAttemptExecutionInfo.tasks.map(
            (task) => parseDuration(task.stats.totalScheduledTimeInNanos + "ns") || 0
        );
        const cpuTimes = prestoStage.latestAttemptExecutionInfo.tasks.map(
            (task) => parseDuration(task.stats.totalCpuTimeInNanos + "ns") || 0
        );
    ...
    return (
        <tr key={index}>
            <td className="stage-id">
                <div className="stage-state-color" style={{ borderLeftColor: getStageStateColor(prestoStage) }}>
                    {stageId}
                </div>
            </td>
            <td>
                        ...
                        <tr>
                                ...
                                <table className="stage-table stage-table-memory">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td className="stage-table-stat-text">
                                                {formatDataSize(
                                                    prestoStage.latestAttemptExecutionInfo.stats.cumulativeUserMemory /
                                                        1000
                                                )}
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td className="stage-table-stat-text">
                                                {formatDataSize(
                                                    prestoStage.latestAttemptExecutionInfo.stats.cumulativeTotalMemory /
                                                        1000
                                                )}
                                            </td>
                                        </tr>
                            ...
                            <td>
                                <table className="stage-table stage-table-tasks">
                                    ...
                                    <tbody>
                                        <tr>
                                            {/* Planned is the first state of a task. There is no "Pending" state. Also
                                            see line 787. */}
                                            <td className="stage-table-stat-title">Planned</td>
                                            <td className="stage-table-stat-text">
                                                {
                                                    prestoStage.latestAttemptExecutionInfo.tasks.filter(
        ...
        for (let resource in estimates) {
            if (Object.prototype.hasOwnProperty.call(estimates, resource)) {
                ...
                renderedEstimates.push(
                    <span>
                        - {snakeCased + "=" + estimates[resource]} <br />
                    </span>
                );
            }
        }
                        ...
                        <table className="table">
                            <tbody>
                                <tr>
                                    ...
                                    <td className="info-text">{data.errorType}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td className="info-text">
                                        <pre id="stack-trace">{formatStackTrace(data.failureInfo)}</pre>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <table className="table">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td className="info-text wrap-text">{data.session.principal}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Source</td>
                                <td className="info-text wrap-text">{data.session.source}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Catalog</td>
                                <td className="info-text">{data.session.catalog}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Schema</td>
                                <td className="info-text">{data.session.schema}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Client Address</td>
                                <td className="info-text">{data.session.remoteUserAddress}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Client Tags</td>
                                <td className="info-text">{data.session.clientTags.join(", ")}</td>
                            </tr>
                            <tr>
                    ...
                    <table className="table">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td className="info-text">
                                    {new Date(data.queryStats.endTime).getTime() !== 0
                                        ? formatShortDateTime(new Date(data.queryStats.endTime))
                                        : ""}
                                </td>
                            </tr>
                            <tr>
                                <td className="info-title">Elapsed Time</td>
                                <td className="info-text">{data.queryStats.elapsedTime}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Prerequisites Wait Time</td>
                                <td className="info-text">{data.queryStats.waitingForPrerequisitesTime}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Queued Time</td>
                                <td className="info-text">{data.queryStats.queuedTime}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Planning Time</td>
                                <td className="info-text">{data.queryStats.totalPlanningTime}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Execution Time</td>
                                <td className="info-text">{data.queryStats.executionTime}</td>
                            </tr>
                            <tr>
                                <td className="info-title">Coordinator</td>
                                <td className="info-text">{getHostname(data.self)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                            ...
                            <table className="table">
                                <tbody>
                                    <tr>
                                        ...
                                        <td className="info-text">{data.queryStats.totalCpuTime}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">Scheduled Time</td>
                                        <td className="info-text">{data.queryStats.totalScheduledTime}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">Blocked Time</td>
                                        <td className="info-text">{data.queryStats.totalBlockedTime}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td className="info-text">{data.queryStats.processedInputDataSize}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">Raw Input Rows</td>
                                        <td className="info-text">{formatCount(data.queryStats.rawInputPositions)}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">Raw Input Data</td>
                                        <td className="info-text">{data.queryStats.rawInputDataSize}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">
                                            <span
                                        ...
                                        <td className="info-text">{formatCount(data.queryStats.shuffledPositions)}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">
                                            <span
                                        ...
                                        <td className="info-text">{data.queryStats.shuffledDataSize}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">Peak User Memory</td>
                                        <td className="info-text">{data.queryStats.peakUserMemoryReservation}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">Peak Total Memory</td>
                                        <td className="info-text">{data.queryStats.peakTotalMemoryReservation}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">Memory Pool</td>
                                        <td className="info-text">{data.memoryPool}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td className="info-text">{formatCount(data.queryStats.outputPositions)}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">Output Data</td>
                                        <td className="info-text">{data.queryStats.outputDataSize}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td className="info-text">{data.queryStats.writtenOutputLogicalDataSize}</td>
                                    </tr>
                                    <tr>
                                        <td className="info-title">Written Output Physical Data Size</td>
                                        <td className="info-text">{data.queryStats.writtenOutputPhysicalDataSize}</td>
                                    </tr>
                                    {(parseDataSize(data.queryStats.spilledDataSize) || 0) > 0 && (
                                        <tr>
                                            <td className="info-title">Spilled Data</td>
                                            <td className="info-text">{data.queryStats.spilledDataSize}</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                                ...
                                <tbody>
                                    ...
                                    <tr className="tr-noborder">
                                        <td className="info-sparkline-text">
                                            {formatCount(
                                                (parseDuration(data.queryStats.totalCpuTime) || 0) /
                                                    (elapsedTime * 1000)
                                            )}
                                        </td>
                                    </tr>
                                    ...
                                    <tr className="tr-noborder">
                                        <td className="info-sparkline-text">
                                            {formatCount(
                                                (parseDuration(data.queryStats.totalScheduledTime) || 0) /
                                                    (elapsedTime * 1000)
                                            )}
                                        </td>
                                    </tr>
                                    ...
                                    <tr className="tr-noborder">
                                        <td className="info-sparkline-text">
                                            {formatDataSize(
                                                (parseDataSize(data.queryStats.processedInputDataSize) || 0) /
                                                    elapsedTime
                                            )}
                                        </td>
                                    </tr>
