...
{
    private static final int MAX_DELETION_VECTOR_SIZE = 100 * 1024 * 1024; // 100MB
    @Test
            ...
            throws IOException
    {
        ...
        RoaringBitmapArray bitmapArray = DeletionVectors.readDeletionVectors(fs, dvFile.getAbsolutePath(), entry, MAX_DELETION_VECTOR_SIZE);
            ...
            throws IOException
    {
        ...
        DeletionVectorEntry entry = new DeletionVectorEntry(
                ...
                Optional.of(1), 34, 1);
        RoaringBitmapArray bitmapArray = DeletionVectors.readDeletionVectors(fs, null, entry, MAX_DELETION_VECTOR_SIZE);
        RoaringBitmapArray expected = new RoaringBitmapArray();
        expected.add(2);
        assertEquals(bitmapArray.toArray().length, expected.toArray().length);
        for (int i = 0; i < bitmapArray.toArray().length; i++) {
    ...
    public void testParseInlineDeletionVector()
    {
        DeletionVectorEntry entry = new DeletionVectorEntry(
                ...
                Optional.of(1), 34, 1);
        assertThrows(PrestoException.class, () -> DeletionVectors.readDeletionVectors(null, null, entry, MAX_DELETION_VECTOR_SIZE));
    }

    @Test
    public void testDeletionVectorSizeExceedsMaxSize()
            throws IOException
    {
        File dvFile = new File("src/test/resources/delta_v3/test_basic_dv/");
        assertTrue(dvFile.exists(), "Deletion vector file should exist");
        ...
        DeletionVectorEntry entry = new DeletionVectorEntry(
                "u", "6(tl#!bz!xPgt^pqEO+K", Optional.of(1), 34, 1);
        int tooSmallMaxSize = 10;
        assertThrows(PrestoException.class,
                () -> DeletionVectors.readDeletionVectors(fs, dvFile.getAbsolutePath(), entry, tooSmallMaxSize));
    }
}
