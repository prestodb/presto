        ...
        implements BytecodeNode
{
    ...
    {
        return invoke(methodName, type(returnType), Streams.stream(parameterTypes).map(ParameterizedType::type).collect(toImmutableList()), ImmutableList.copyOf(requireNonNull(parameters, "parameters is null")));
    }
