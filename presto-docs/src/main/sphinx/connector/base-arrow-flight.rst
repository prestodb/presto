...
``arrow-flight.client-ssl-certificate``     Path to SSL certificate that Flight client should use for mTLS authentication with Flight server
``arrow-flight.client-ssl-key``             Path to SSL key that Flight client should use for mTLS authentication with Flight server
