...
Views created using Presto support both ``SECURITY DEFINER`` (the default) and ``SECURITY INVOKER`` modes.
Presto stores the view security metadata in Redshift.

For existing views that were created directly in Redshift (outside of Presto), Presto defaults to ``SECURITY INVOKER`` mode.
When querying these views, permissions and access control rules on underlying tables are evaluated using the identity of the user running the query.
