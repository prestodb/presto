...
* Writing a value that has nothing left to store is not supported, because a data file needs
  something to store for every value: an ``ARRAY(UNKNOWN)``, a ``MAP`` whose values are
  ``unknown``, or a ``ROW`` whose fields are all ``unknown``. Iceberg does not allow ``unknown``
  map keys. A table whose top-level columns are all ``unknown`` is an exception: the Parquet or
  ORC file records no column data but still records the row count, so reads return the correct
  number of rows, each with a ``NULL`` value for every column.
