...
* ``s3Prefix``: S3 location prefix that the table location must start with, allowing different
  ...
  * The prefix must use the same scheme as the table locations it covers. ``s3a://my-bucket/``
    does not match an ``s3://my-bucket/`` location, so list each scheme in use. The scheme must be
    one of ``s3``, ``s3a``, or ``s3n``; any other scheme is rejected when the file is loaded, since
    such an entry could never match a location.
