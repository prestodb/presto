...
The Hive connector also allows querying data in Hive where data files are stored in Amazon S3. 
The SQL support for S3 tables remains the same as for HDFS tables.

For accessing tables stored in S3 the mandatory configuration properties required
are the AWS credential properties ``hive.s3.aws-access-key`` and ``hive.s3.aws-secret-key``. 
You can also use ``hive.s3.use-instance-credentials`` which if set to true, enables retrieving
temporary AWS credentials that have the same permissions as those associated 
with the IAM role associated with the EC2 instance in its instance profile.
