...
. code-block:: none
    hive.azure.abfs.storage-account=mydatalake
    hive.azure.abfs.oauth.client-endpoint=https://login.microsoftonline.com/<tenant-id>/oauth2/token
    hive.azure.abfs.oauth.client-id=<oauth-client-id>
    hive.azure.abfs.oauth.client-secret=<your_client_secret_here>
...
``hive.azure.abfs.oauth.client-endpoint``           OAuth 2.0 token endpoint(includes tenant ID)
                                                    (format: https://login.microsoftonline.com/<tenant-id>/oauth2/token


``hive.azure.abfs.oauth.client-id``                 OAuth 2.0 client/application ID

``hive.azure.abfs.oauth.client-secret``             OAuth 2.0 client secret
======================================================= ===============================================================
...
.. code-block:: none
    CREATE TABLE hive.default.orders (
        ...
        external_location = 'abfs://mycontainer@mydatalake.dfs.core.windows.net/orders/',
        format = 'PARQUET'
    );
...
.. code-block:: none
    ...
    WITH (location = 'abfs://mycontainer@mydatalake.dfs.core.windows.net/schemas/');
...
* Check access keys or OAuth credentials are correct
* For OAuth, ensure the service principal has Storage Blob Data Contributor or Reader role in Azure
...
* Check Azure storage firewall rules allow access from Presto nodes
...
* Verify URI format is correct (``wasb://`` or ``abfs://``)
* Confirm container exists in the storage account
