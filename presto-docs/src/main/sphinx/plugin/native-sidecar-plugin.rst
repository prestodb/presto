...
Using the native sidecar plugin in a native cluster, we extend the capabilities of the cluster by allowing the underlying execution engine to be the source of truth.
...
and regular C++ workers, or a cluster composed entirely of sidecar workers.
...
The Native Sidecar plugin allows modular configuration, enabling users to selectively activate individual components such as the function registry, session property provider, type manager, or the plan checker.
Each component can be independently enabled using config files based on the specific requirements of the deployment.

Function registry
-----------------

These properties must be configured under ``etc/function-namespace/native.properties`` to use the function namespace manager from the ``NativeSidecarPlugin``.
...
These properties must be configured under ``etc/session-property-providers/native-worker.properties`` to use the session property provider of the ``NativeSidecarPlugin``.
...
These properties must be configured under ``etc/type-managers/native.properties`` to use the type manager of the ``NativeSidecarPlugin``.
...
These properties must be configured under ``etc/plan-checker-providers/native.properties`` to use the native plan checker of the ``NativeSidecarPlugin``.
