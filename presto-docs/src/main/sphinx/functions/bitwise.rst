...
.. function:: bitwise_and(x, y) -> bigint
    ...
    Returns the bitwise AND of ``x`` and ``y`` in 2's complement arithmetic.

.. function:: bitwise_not(x) -> bigint

    Returns the bitwise NOT of ``x`` in 2's complement arithmetic.

.. function:: bitwise_or(x, y) -> bigint

    Returns the bitwise OR of ``x`` and ``y`` in 2's complement arithmetic.

.. function:: bitwise_xor(x, y) -> bigint

    Returns the bitwise XOR of ``x`` and ``y`` in 2's complement arithmetic.
