...
3. Create a JSON file in etc folder named ``resource-groups.json``. This file should contain the definitions
...
4. In the ``etc/resource-groups.properties`` file, add a line that specifies the location of the JSON file using the
...
5. To specify the database in the Presto configuration, add a line in ``etc/resource-groups.properties`` that
   ...
   Note : The supported databases are MySQL, PostgreSQL, and Oracle.
