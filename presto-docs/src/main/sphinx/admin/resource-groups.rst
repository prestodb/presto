...
* ``queryType`` (optional): String to match against the type of the query submitted:
  ...
  - ``CONTROL``: Transaction control queries like ``COMMIT``, ``ROLLBACK`` etc and session control queries like ``USE``,
    ``SET SESSION`` etc.
...
* Non-DDL queries with a source ``pipeline`` will run under the ``global.pipeline`` group, with a total concurrency of 45, and a per-user
