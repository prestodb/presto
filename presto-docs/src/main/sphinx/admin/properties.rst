...
If set to ``false`` no statistics are propgated, and the panner uses ``UNKNOWN`` as the stats estimate for this plan node. This can negatively impact query planning
