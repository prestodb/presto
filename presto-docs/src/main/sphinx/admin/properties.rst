...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    * **Default value:** ``JVM max memory * 0.1``
    ...
    User memory is allocated during execution for things
    that are directly attributable to or controllable by a user query.
    For example, memory used by the hash tables built during execution,
    memory used during sorting, etc. When a query hits this limit it will be killed
    by Presto.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    * **Default value:** ``JVM max memory * 0.3``
    ...
    System memory is allocated during execution for things
    that are not directly attributable to or controllable by a user query.
    For example, memory allocated by the readers, writers, and network buffers, etc.
    The value of ``query.max-memory-per-node`` must be less than or equal to
    ``query.max-total-memory-per-node``.
