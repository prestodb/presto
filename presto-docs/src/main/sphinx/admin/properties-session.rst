...
``optimizer.merge_max_by_aggregations``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

* **Type:** ``boolean``
* **Default value:** ``false``

Enable optimization to merge multiple ``max_by`` aggregations that share the same comparison key
into a single ``max_by`` aggregation with a ``ROW`` argument. This reduces computational overhead
