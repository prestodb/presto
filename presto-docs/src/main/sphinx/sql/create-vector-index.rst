...
- ``index_type``: The type of vector index (e.g., ``'ivf_rabitq4'``)
- ``distance_metric``: The distance metric to use (e.g., ``'cosine'``)
- ``index_options``: Additional index configuration (e.g., ``'nlist=100000,nb=8'``)
- ``partitioned_by``: Partition columns (e.g., ``ARRAY['ds']``)
...
Create a vector index on an embeddings table for specific date partitions::
    ...
    UPDATING FOR ds BETWEEN '2026-02-20' AND '2026-02-22'
