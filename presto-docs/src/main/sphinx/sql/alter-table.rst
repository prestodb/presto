...
.. code-block:: none
    ...
    ALTER TABLE [ IF EXISTS ] name ADD COLUMN [ IF NOT EXISTS ] column_name data_type [ DEFAULT default_expression ] [ COMMENT comment ] [[GENERATED ALWAYS] AS expression [VIRTUAL | PERSISTENT]] [ WITH ( property_name = expression [, ...] ) ]
...
Add column ``zip_derived`` to the ``users`` table with derived column expression::
...
Set default value for column ``country`` in the ``users`` table::

    ALTER TABLE users ALTER COLUMN country SET DEFAULT 'US';

Set default value for column ``country`` in the ``users`` table if the table ``users`` exists::

    ALTER TABLE IF EXISTS users ALTER COLUMN country SET DEFAULT 'US';
