...
* **Default value:** ``0.7``
  ...
  The max ratio of the number of in-memory cache entries being written to SSD cache 
  over the total number of cache entries. This is to control SSD cache write rate, 
  and once the ratio exceeds this threshold, then we stop writing to SSD cache.
...
* **Type:** ``string``
* **Default value:** ``0s``
  The interval for persisting in-memory cache to SSD. Setting this config to a non-zero value will 
  activate periodic cache persistence. The following time units are supported: 
  ns, us, ms, s, m, h, d.
...
* **Type:** ``bool``
* **Default value:** ``false``
  In file systems, such as btrfs, supporting cow (copy on write), the ssd cache can use all ssd 
