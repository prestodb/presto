...
2. For each worker, edit the ``config.properties`` file:

   * Set the ``plugin.dir`` property to the path of the ``plugin`` directory. If it is not specified, the path to this directory defaults to the ``plugin`` directory relative to the directory in which the process is being run.
