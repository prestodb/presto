...
**Data Correctness Issues or Critical Bugs**: Any change that introduces data correctness issues, major crashes, or severe stability problems
...
**Backwards Incompatible Client Changes**: Client libraries evolve slowly and many users cannot easily upgrade clients. Breaking changes to
...
**Backwards Incompatible SPI Changes Without Migration Path**: If a backwards incompatible change to the SPI is discovered that lacks the required migration path
(e.g., no deprecation period, no reasonable defaults for new methods), the change should be reverted
...
- **Consider configuration**: Can the optimization be made optional via session properties?
...
**Don't revert**:
- New functions with edge case bugs (fix the bugs)
- Optional features that can be disabled via configuration
