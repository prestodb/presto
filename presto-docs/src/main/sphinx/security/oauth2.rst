...
Presto can be configured to enable frontend OAuth2 authentication over HTTPS for clients such as the CLI, JDBC, and ODBC drivers. OAuth2 provides a secure and flexible way to authenticate users via an external identity provider (IdP), such as Okta, Auth0, Azure AD, or Google.
...
Access to the Presto coordinator must be secured via HTTPS. You must configure a valid TLS certificate and keystore on the coordinator. See the `TLS setup guide <https://trino.io/docs/current/security/tls.html>`_ for details.
