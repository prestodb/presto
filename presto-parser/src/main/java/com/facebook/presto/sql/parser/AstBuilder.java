        ...
        if (context.type() == null) {
            try {
                return new CreateType(
                        getQualifiedName(context.qualifiedName()),
                        context.sqlParameterDeclaration().stream().map(p -> ((Identifier) visit(p.identifier())).getValue()).collect(ImmutableList.toImmutableList()),
                        context.sqlParameterDeclaration().stream().map(p -> getType(p.type())).collect(ImmutableList.toImmutableList()));
            }
