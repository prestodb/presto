...
public final class SqlFormatter
{
            ...
            extends AstVisitor<Void, Integer>
    {
        ...
        protected Void visitExternalBodyReference(ExternalBodyReference node, Integer indent)
        {
            ...
            if (node.getIdentifier().isPresent() || node.getStringLiteral().isPresent()) {
                builder.append(" NAME ");
                if (node.getIdentifier().isPresent()) {
                    builder.append(node.getIdentifier().get().toString());
                }
                else if (node.getStringLiteral().isPresent()) {
                    builder.append(formatStringLiteral(node.getStringLiteral().get().getValue()));
                }
            }
