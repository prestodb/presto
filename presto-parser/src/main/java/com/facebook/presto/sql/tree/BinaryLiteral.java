        ...
        extends Literal
{
    ...
    private static final Pattern ecscapeSequencePattern = Pattern.compile("[ \\r\\n\\t]");
    private static final Pattern notHexDigitPattern = Pattern.compile(".*[^A-F0-9].*");
    ...
    public BinaryLiteral(Optional<NodeLocation> location, String value)
    {
        ...
        String hexString = ecscapeSequencePattern.matcher(value).replaceAll("").toUpperCase();
        if (notHexDigitPattern.matcher(hexString).matches()) {
