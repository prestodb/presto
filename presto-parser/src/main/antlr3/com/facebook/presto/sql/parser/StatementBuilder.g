...
createTable returns [Statement value]
    ...
    | ^(CREATE_TABLE qname tableElementList existClause? partitionElement? serdeElement? inputFormat? outputFormat? location?)
        { $value = new CreateTable(
            ...
            Optional.fromNullable($serdeElement.value),
            Optional.fromNullable($inputFormat.value),
            Optional.fromNullable($outputFormat.value),
            Optional.fromNullable($location.value)); }
    ;
...
serdeElement returns [String value]
    : ^(SERDE string)
        { $value = $string.value; }
    ;

inputFormat returns [String value]
    : ^(INPUTFORMAT string)
        { $value = $string.value; }
    ;

outputFormat returns [String value]
    : ^(OUTPUTFORMAT string)
        { $value = $string.value; }
    ;

location returns [String value]
    : ^(LOCATION string)
        { $value = $string.value; }
    ;
