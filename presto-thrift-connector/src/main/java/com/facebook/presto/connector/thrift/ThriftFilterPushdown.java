        ...
        implements ConnectorPlanOptimizer
{
    ...
    {
        // Filter pushdown is only supported in native execution mode.
        if (session instanceof FullConnectorSession && isNativeExecutionEnabled(((FullConnectorSession) session).getSession())) {
            ...
            extends ConnectorPlanRewriter<Void>
    {
        ...
        public PlanNode visitFilter(FilterNode filter, RewriteContext<Void> context)
        {
            ...
            if (!(indexSource.getTableHandle().getConnectorHandle() instanceof ThriftTableHandle)
                    || indexSource.getTableHandle().getLayoutHandle() == null
                    || !(indexSource.getTableHandle().getLayoutHandle() instanceof ThriftTableLayoutHandle)) {
                return filter;
            }
            ...
            if (FALSE_CONSTANT.equals(optimizedExpression)) {
                return createValuesNode(indexSource);
            }
        ...
        private ValuesNode createValuesNode(IndexSourceNode indexSource)
        {
