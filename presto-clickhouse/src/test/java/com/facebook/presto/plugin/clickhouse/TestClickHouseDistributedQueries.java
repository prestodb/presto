...
//                "   \"column_a\" date NOT NULL,\n" +
//                "   \"column_b\" date NOT NULL\n" +
//                ")";
//        assertUpdate("CREATE TABLE test_not_null_with_insert (column_a DATE NOT NULL, column_b DATE NOT NULL)");
//        assertQuery(
...
//                "column_a",
//                "column_b",
//                " WITH (engine = 'MergeTree', order_by = ARRAY['column_a'])");
...
//    {
//        assertUpdate("CREATE TABLE test_create (a bigint, b double, c varchar) WITH (engine = 'Log')");
...
//
//        assertQueryFails("CREATE TABLE test_create (a bad_type) WITH (engine = 'Log')", ".* Unknown type 'bad_type' for column 'a'");
//        assertFalse(getQueryRunner().tableExists(getSession(), "test_create"));
//
//        assertUpdate("CREATE TABLE test_create_table_if_not_exists (a bigint, b varchar, c double) WITH (engine = 'Log')");
...
//
//        assertUpdate("CREATE TABLE IF NOT EXISTS test_create_table_if_not_exists (d bigint, e varchar) WITH (engine = 'Log')");
//        assertTrue(getQueryRunner().tableExists(getSession(), "test_create_table_if_not_exists"));
//        assertTableColumnNames("test_create_table_if_not_exists", "a", "b", "c");
//
...
//        assertUpdate("CREATE TABLE test_create_original (a bigint, b double, c varchar) WITH (engine = 'Log')");
//        assertTrue(getQueryRunner().tableExists(getSession(), "test_create_original"));
//        assertTableColumnNames("test_create_original", "a", "b", "c");
//
//        assertUpdate("CREATE TABLE test_create_like (LIKE test_create_original, d boolean, e varchar) WITH (engine = 'Log')");
...
//                "   \"column_b\" date NOT NULL\n" +
//                ")  WITH (engine = 'Log')";
//        assertUpdate("CREATE TABLE test_not_null_with_insert (column_a DATE, column_b DATE NOT NULL)  WITH (engine = 'Log')");
//        assertQuery(
//                "SHOW CREATE TABLE test_not_null_with_insert",
//                "VALUES '" + createTableStatement + "'");
//
...
//
//        assertAccessDenied("CREATE TABLE foo (pk bigint)  WITH (engine = 'Log')", "Cannot create table .*.foo.*", privilege("foo", CREATE_TABLE));
...
//        assertUpdate("DROP TABLE test_insert");
//
//        assertUpdate("CREATE TABLE test_insert (a ARRAY<DOUBLE>, b ARRAY<BIGINT>)  WITH (engine = 'Log')");
//
...
//    private final QueryRunner queryRunner;
    ...
    //    protected TestClickHouseDistributedQueries(TestingClickHouseServer clickhouseServer)
//        throws Exception
//    {
//        this.queryRunner = createClickHouseQueryRunner(clickhouseServer,
//                ImmutableMap.of("http-server.http.port", "8080"),
//                ImmutableMap.of(),
//                TpchTable.getTables());
//        this.clickhouseServer = new TestingClickHouseServer();
