        ...
        extends PlanNode
{
            ...
            @JsonProperty("assignments") Map<VariableReferenceExpression, ColumnHandle> assignments)
    {
        ...
        this.temporaryTableInfo = Optional.empty();
    }
    ...
    public TableScanNode(
            ...
            TupleDomain<ColumnHandle> enforcedConstraint, Optional<TemporaryTableInfo> temporaryTableInfo)
    {
        this(sourceLocation, id, table, outputVariables, assignments, emptyList(), currentConstraint, enforcedConstraint, temporaryTableInfo);
    }
    ...
    public TableScanNode(
            ...
            TupleDomain<ColumnHandle> enforcedConstraint, Optional<TemporaryTableInfo> temporaryTableInfo)
    {
        this(sourceLocation, id, Optional.empty(), table, outputVariables, assignments, tableConstraints, currentConstraint, enforcedConstraint, temporaryTableInfo);
    }
            ...
            Optional<TemporaryTableInfo> temporaryTableInfo)
    {
        ...
        this.outputVariables = unmodifiableList(requireNonNull(outputVariables, "outputVariables is null"));
        this.assignments = unmodifiableMap(new HashMap<>(requireNonNull(assignments, "assignments is null")));
        this.temporaryTableInfo = requireNonNull(temporaryTableInfo, "temporaryTableInfo is null");
        checkArgument(assignments.keySet().containsAll(outputVariables), "assignments does not cover all of outputs");
    ...
    {
        return new TableScanNode(getSourceLocation(), getId(), statsEquivalentPlanNode, table, outputVariables, assignments, tableConstraints, currentConstraint, enforcedConstraint, temporaryTableInfo);
    }
