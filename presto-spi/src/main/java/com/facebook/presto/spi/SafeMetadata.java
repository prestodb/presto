        ...
        ITH extends ConnectorInsertTableHandle>
{
     ...
     */
    TH getTableHandle(ConnectorSession session, SchemaTableName tableName);
     ...
     */
    ConnectorTableMetadata getTableMetadata(TH table);
     ...
     */
    CH getSampleWeightColumnHandle(TH tableHandle);
     ...
     * @throws RuntimeException if table handle is no longer valid
     */
    Map<String, CH> getColumnHandles(TH tableHandle);
     ...
     */
    ColumnMetadata getColumnMetadata(TH tableHandle, CH columnHandle);
     ...
     */
    TH createTable(ConnectorSession session, ConnectorTableMetadata tableMetadata);
     ...
     */
    void dropTable(TH tableHandle);

    /**
     * Rename the specified table
     */
    void renameTable(TH tableHandle, SchemaTableName newTableName);

    /**
     * Begin the atomic creation of a table with data.
     */
    OTH beginCreateTable(ConnectorSession session, ConnectorTableMetadata tableMetadata);

    /**
     * Commit a table creation with data after the data is written.
     */
    void commitCreateTable(OTH tableHandle, Collection<String> fragments);

    /**
     * Begin insert query
     */
    ITH beginInsert(ConnectorSession session, TH tableHandle);

    /**
     * Commit insert query
     */
    void commitInsert(ITH insertHandle, Collection<String> fragments);
