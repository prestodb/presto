...
public class SubfieldPathTokenizer
        implements Iterator<SubfieldPath.PathElement>
{
    ...
    private SubfieldPath.PathElement next;
    ...
    public SubfieldPathTokenizer(String path)
    {
    ...
    @Override
    public final SubfieldPath.PathElement next()
    {
        if (!hasNext()) {
            throw new NoSuchElementException();
        }
        state = State.NOT_READY;
        SubfieldPath.PathElement result = next;
        next = null;
        return result;
    }
    ...
    private SubfieldPath.PathElement computeNext()
    {
        if (!hasNextCharacter()) {
            state = State.DONE;
            return null;
        ...
        if (tryMatch(DOT)) {
            SubfieldPath.PathElement token = matchPathSegment();
            firstSegment = false;
            return token;
        }

        if (tryMatch(OPEN_BRACKET)) {
            SubfieldPath.PathElement token = tryMatch(QUOTE) ? matchQuotedSubscript() : tryMatch(WILDCARD) ? matchWildcardSubscript() : matchUnquotedSubscript();
    ...
    private SubfieldPath.PathElement matchPathSegment()
    {
        ...
        return new SubfieldPath.NestedField(token);
    }

    private SubfieldPath.PathElement matchWildcardSubscript()
    {
    ...
    private SubfieldPath.PathElement matchUnquotedSubscript()
    {
        // seek until we see a special character or whitespace
        int start = index;
        ...
        return new SubfieldPath.LongSubscript(Long.valueOf(token));
    }
    ...
    private SubfieldPath.PathElement matchQuotedSubscript()
    {
            ...
            if (escaped) {
                ...
                switch (peekCharacter()) {
                    case BACKSLASH:
                        ...
                        break;
                    case QUOTE:
                        throw new IllegalStateException("Should be handled by loop condition");
                    default:
                        token.append(peekCharacter());
                }
            }
        ...
        if (escaped) {
            if (hasNextCharacter()) {
                throw new IllegalStateException("Loop terminated after escape while there is still input");
            }
            throw invalidSubfieldPath();
        }
    ...
    {
        return new PrestoException(INVALID_FUNCTION_ARGUMENT, format("Invalid subfield path: '%s'", path));
    }
