...
public final class ResourceEstimates
{
    ...
    {
        this(
                estimates.getExecutionTimeInMillis().map(MILLISECONDS::toNanos).map(Duration::succinctNanos),
                estimates.getCpuTimeInNanos().map(Duration::succinctNanos),
                estimates.getPeakMemoryInBytes().map(DataSize::succinctBytes),
                estimates.getPeakTaskMemoryInBytes().map(DataSize::succinctBytes));
    }

    public ThriftResourceEstimates toThrift()
    {
        ThriftResourceEstimates thriftResourceEstimates = new ThriftResourceEstimates();
        thriftResourceEstimates.setExecutionTimeInMillis(this.executionTime.map(Duration::toMillis).orElse(0L))
                ...
                .setPeakTaskMemoryInBytes(this.peakTaskMemory.map(DataSize::toBytes).orElse(0L));
