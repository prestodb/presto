...
public class TestLongDecimalType
{
    ...
    {
        Block decimalABlock = decimalAsBlock(decimalA);
        Block decimalBBlock = decimalAsBlock(decimalB);
        int actual = TYPE.compareTo(decimalABlock, 0, decimalBBlock, 0);
        assertEquals((int) signum(actual), (int) signum(expected), "bad comparison result for " + decimalA + ", " + decimalB);
    }

    private Block decimalAsBlock(String decimalString)
    {
        Slice slice = TYPE.bigDecimalToSlice(new BigDecimal(decimalString));
