...
template <class T>
VectorPtr createVeloxConstantVectorTemplated(
    Vector& input,
    exec::EvalCtx* context) {
  auto value = ::duckdb::ConstantVector::GetData<T>(input);
  return BaseVector::createConstant(
      *value, BaseVector::kMaxElements, context->pool());
}

template <>
VectorPtr createVeloxConstantVectorTemplated<StringView>(
    Vector& input,
    exec::EvalCtx* context) {
  auto value = ::duckdb::ConstantVector::GetData<string_t>(input);
  return BaseVector::createConstant(
      value->GetDataUnsafe(), BaseVector::kMaxElements, context->pool());
}

template <>
VectorPtr createVeloxConstantVectorTemplated<Timestamp>(
    Vector& input,
    exec::EvalCtx* context) {
  auto result_timestamp = ::duckdb::ConstantVector::GetData<timestamp_t>(input);
  auto velox_timestamp = duckdbTimestampToVelox(*result_timestamp);
  return BaseVector::createConstant(
      velox_timestamp, BaseVector::kMaxElements, context->pool());
}

VectorPtr createVeloxConstantVectorSwitch(
    Vector& input,
    exec::EvalCtx* context) {
  assert(input.GetVectorType() == VectorType::CONSTANT_VECTOR);
  if (::duckdb::ConstantVector::IsNull(input)) {
    return BaseVector::createConstant(
        variant::null(toVeloxType(input.GetType())->kind()),
        BaseVector::kMaxElements,
        context->pool());
  }
  switch (input.GetType().id()) {
    case LogicalTypeId::BOOLEAN:
      return createVeloxConstantVectorTemplated<bool>(input, context);
    case LogicalTypeId::TINYINT:
      return createVeloxConstantVectorTemplated<int8_t>(input, context);
    case LogicalTypeId::SMALLINT:
      return createVeloxConstantVectorTemplated<int16_t>(input, context);
    case LogicalTypeId::INTEGER:
      return createVeloxConstantVectorTemplated<int32_t>(input, context);
    case LogicalTypeId::BIGINT:
      return createVeloxConstantVectorTemplated<int64_t>(input, context);
    case LogicalTypeId::FLOAT:
      return createVeloxConstantVectorTemplated<float>(input, context);
    case LogicalTypeId::DOUBLE:
      return createVeloxConstantVectorTemplated<double>(input, context);
    case LogicalTypeId::VARCHAR:
      return createVeloxConstantVectorTemplated<StringView>(input, context);
    case LogicalTypeId::TIMESTAMP:
      return createVeloxConstantVectorTemplated<Timestamp>(input, context);
    default:
      throw std::runtime_error(
          "Unimplemented type for createVeloxConstantVector: " +
          input.GetType().ToString());
  }
}

VectorPtr
createVeloxConstantVector(Vector& input, size_t count, exec::EvalCtx* context) {
  auto constant = createVeloxConstantVectorSwitch(input, context);
  constant->setSize(count);
  return constant;
}
