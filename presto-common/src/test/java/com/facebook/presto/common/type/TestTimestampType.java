...
public class TestTimestampType
{
    ...
    {
        BlockBuilder longPrecision = createTimestampType(9).createFixedSizeBlockBuilder(4);
        assertTrue(longPrecision instanceof Fixed12ArrayBlockBuilder,
                "Expected Fixed12ArrayBlockBuilder for p=9, got: " + longPrecision.getClass().getSimpleName());
        longPrecision.writeLong(123L).writeInt(456).closeEntry();
        assertEquals(longPrecision.getLong(0, 0), 123L);
        assertEquals(longPrecision.getInt(0), 456);

        BlockBuilder shortPrecision = createTimestampType(3).createFixedSizeBlockBuilder(4);
        assertTrue(shortPrecision instanceof LongArrayBlockBuilder,
                "Expected LongArrayBlockBuilder for p=3, got: " + shortPrecision.getClass().getSimpleName());
        shortPrecision.writeLong(123L).closeEntry();
        assertEquals(shortPrecision.getLong(0), 123L);
    }
