        ...
        implements Comparable<Marker>, ThriftSerializable
{
    ...
    public Marker(ThriftMarker thriftMarker)
    {
        this(
                ...
                thriftMarker.getValueBlock().map(b -> (Block) BlockAdapter.fromThrift(b)),
                Bound.valueOf(thriftMarker.getBound().name()));
    }

    public ThriftMarker toThrift()
    {
        ThriftMarker thriftMarker = new ThriftMarker((ThriftType) type.toThriftInterface(), ThriftBound.valueOf(bound.name()));
        valueBlock.ifPresent(block -> thriftMarker.setValueBlock((ThriftBlock) block.toThriftInterface()));
        return thriftMarker;
    }
