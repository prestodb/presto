...
public class TablestoreRecordCursor
        extends TablestoreRowBasedRecordCursor
        implements RecordCursor
{
    public TablestoreRecordCursor(TablestoreFacade tablestoreFacade, ConnectorSession session, TablestoreSplit split,
    ...
    {
        return tablestoreFacade.rowIterator(session, split, Arrays.asList(columnHandles));
    }
    ...
    {
        checkClosed();
        if (rowIterator instanceof GetRangeRowIterator) {
            return ((GetRangeRowIterator) rowIterator).getCompletedBytes();
        }
