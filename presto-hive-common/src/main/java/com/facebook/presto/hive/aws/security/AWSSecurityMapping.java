...
public class AWSSecurityMapping
{
            ...
            @JsonProperty("secretKey") Optional<String> secretKey)
    {
        ...
        this.s3CacheKeyQualifier = computeS3CacheKeyQualifier(this.s3Prefixes);
    ...
    public Optional<String> getS3CacheKeyQualifier()
    {
        return s3CacheKeyQualifier;
    }
    ...
    private static Optional<String> computeS3CacheKeyQualifier(List<String> s3Prefixes)
    {
        if (s3Prefixes.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(Hashing.sha256()
                .hashString(String.join(CACHE_SCOPE_SEPARATOR, s3Prefixes), UTF_8)
                .toString());
    }
    ...
    private static String canonicalizeS3Prefix(String prefix)
    {
        ...
        checkArgument(S3_SCHEMES.contains(uri.getScheme()), "s3Prefix scheme must be one of %s: %s", S3_SCHEMES, prefix);
