...
public class CassandraTestingUtils
{
    ...
    public static void insertIntoTableClusteringKeys(CassandraSession session, SchemaTableName table, int rowsCount)
    {
        ...
        assertEquals(session.execute("SELECT COUNT(*) FROM " + table).one().getLong(0), rowsCount);
    }
    ...
    public static void insertIntoTableMultiPartitionClusteringKeys(CassandraSession session, SchemaTableName table)
    {
        ...
        assertEquals(session.execute("SELECT COUNT(*) FROM " + table).one().getLong(0), 9);
    }
    ...
    {
        // Driver 4.x: Create tuple type using DataTypes.tupleOf()
        TupleType tupleType = DataTypes.tupleOf(DataTypes.BIGINT, DataTypes.TEXT);

        for (int rowNumber = 1; rowNumber <= rowsCount; rowNumber++) {
            Insert insert = QueryBuilder.insertInto(table.getSchemaName(), table.getTableName())
                    .value("key", literal("key " + rowNumber))
                    .value("typeuuid", literal(UUID.fromString(String.format("00000000-0000-0000-0000-%012d", rowNumber))))
                    ...
                    .value("typetimestamp", literal(date.getTime()))
                    ...
                    .value("typetimeuuid", literal(UUID.fromString(String.format("d2177dd0-eaa2-11de-a572-001b779c76e%d", rowNumber))))
                    ...
                    .value("typetuple", literal(tupleType.newValue((long) rowNumber, "row=" + rowNumber)));
