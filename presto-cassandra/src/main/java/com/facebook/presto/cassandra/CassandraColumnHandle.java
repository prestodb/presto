...
public class CassandraColumnHandle implements ColumnHandle {
	...
	@JsonCreator
	public CassandraColumnHandle(@JsonProperty("connectorId") String connectorId,
	...
	public ColumnMetadata getColumnMetadata() {
		return new ColumnMetadata(CassandraCqlUtils.cqlNameToSqlName(name), cassandraType.getNativeType(), ordinalPosition,
				partitionKey);
	}
	...
	@Override
	public int hashCode() {
		return Objects.hashCode(connectorId, name, ordinalPosition, cassandraType, typeArguments,
	...
	public String toString() {
		final ToStringHelper helper = Objects.toStringHelper(this).add("connectorId", connectorId)
				.add("name", name).add("ordinalPosition", ordinalPosition)
				.add("cassandraType", cassandraType);
		if (!typeArguments.isEmpty())
			helper.add("typeArguments", typeArguments);
		return helper.add("partitionKey", partitionKey).add("clusteringKey", clusteringKey)
				.toString();
	}
	...
	public static Function<CassandraColumnHandle, Integer> ordinalPositionIndexGetter() {
		return new Function<CassandraColumnHandle, Integer>() {
			@Override
			public Integer apply(CassandraColumnHandle input) {
				return input.getOrdinalPosition();
			}
		};
	}
	...
	public static Predicate<CassandraColumnHandle> isPartitionKeyPredicate() {
		return new Predicate<CassandraColumnHandle>() {
			@Override
			public boolean apply(CassandraColumnHandle input) {
				return input.isPartitionKey();
			}
		};
	}

	public static Predicate<CassandraColumnHandle> isClusteringKeyPredicate() {
		return new Predicate<CassandraColumnHandle>() {
			@Override
			public boolean apply(CassandraColumnHandle input) {
				return input.isClusteringKey();
			}
		};
	}
}
