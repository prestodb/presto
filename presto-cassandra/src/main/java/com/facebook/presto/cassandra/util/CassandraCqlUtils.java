...
public class CassandraCqlUtils {
	...
	static final String EMPTY_COLUMN_NAME = "__empty__";
	
	private final Map<String, HostAddress> hostMap;
	
	public CassandraCqlUtils() {
		hostMap = new HashMap<>();
	}
	
	public HostAddress toHostAddress(Host host) {
		final String hostAddressName = host.getAddress().getHostAddress();
	...
	public static String validColumnName(String identifier) {
		if (identifier.isEmpty() || identifier.equals(EMPTY_COLUMN_NAME)) {
			return "\"\"";
		} else if (keywords.contains(identifier.toUpperCase())) {
			return '"' + identifier + '"';
		}
		return identifier.toLowerCase();
	}
	...
	public static void appendSelectColumns(StringBuilder sb, List<? extends ColumnHandle> columns) {
		appendSelectColumns(sb, columns, true);
	}

	private static void appendSelectColumns(StringBuilder sb, List<? extends ColumnHandle> columns,
			boolean first) {
		for (ColumnHandle column: columns) {
			...
			sb.append(validColumnName(((CassandraColumnHandle) column).getName()));
		}
	}
	...
	public static Select from(Selection selection, CassandraTableHandle tableHandle) {
		final String schema = validSchemaName(tableHandle.getSchemaName());
		final String table = validTableName(tableHandle.getTableName());
