...
public enum CassandraType implements FullCassandraType {
	ASCII(ColumnType.STRING, String.class), BIGINT(ColumnType.LONG, Long.class), BLOB(
			ColumnType.STRING, ByteBuffer.class), CUSTOM(ColumnType.STRING, ByteBuffer.class), BOOLEAN(ColumnType.BOOLEAN,
			Boolean.class), COUNTER(ColumnType.LONG, Long.class), DECIMAL(ColumnType.DOUBLE,
			BigDecimal.class), DOUBLE(ColumnType.DOUBLE, Double.class), FLOAT(ColumnType.DOUBLE,
			Float.class), INET(ColumnType.STRING, InetAddress.class), INT(ColumnType.LONG,
			Integer.class), TEXT(ColumnType.STRING, String.class), TIMESTAMP(ColumnType.LONG,
			Date.class), UUID(ColumnType.STRING, java.util.UUID.class), TIMEUUID(ColumnType.STRING,
			java.util.UUID.class), VARCHAR(ColumnType.STRING, String.class), VARINT(
			ColumnType.STRING, BigInteger.class), LIST(ColumnType.STRING, null), MAP(
			ColumnType.STRING, null), SET(ColumnType.STRING, null);
	...
	public static String getColumnValueForCql(Row row, int i, CassandraType cassandraType) {
		if (row.isNull(i)) {
			...
			case TEXT:
			case VARCHAR:
				return CassandraCqlUtils.quote(row.getString(i));
			case INT:
	...
	private static String objectToString(Object object, CassandraType elemType) {
		...
		case INET:
		case VARINT:
			return CassandraCqlUtils.quote(object.toString());
			
		case BLOB:
		case CUSTOM:
			return CassandraCqlUtils.quote(Bytes.toHexString((ByteBuffer) object));
