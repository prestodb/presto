        ...
        extends AbstractHudiDistributedQueryTestBase
{
    ...
    public void testPartitionPruneAndFileSkipping()
    {
        Arrays.stream(new String[] {HUDI_SKIPPING_TABLE, HUDI_SKIPPING_TABLE_NON_HIVE_STYLE}).forEach(f -> {
            ...
            List<String> parts = hudiPartitionManager.getEffectivePartitions(connectorSession, metastore, metaClient, table.get().getSchemaTableName(), predicate);
            ...
            List<String> parts1 = hudiPartitionManager.getEffectivePartitions(connectorSession, metastore, metaClient, table.get().getSchemaTableName(), predicate1);

            assertEquals(parts1.size(), 2);

            // test file skipping
            List<HudiColumnHandle> dataColumns = HudiMetadata.fromDataColumns(table.get().getDataColumns());
            List<String> partitions = hudiPartitionManager.getEffectivePartitions(connectorSession, metastore, metaClient, table.get().getSchemaTableName(), TupleDomain.all());
