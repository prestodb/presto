                    ...
                    if (table.isPresent()) {
                        ...
                        if (!table.get().getPartitionColumns().isEmpty()) {
                            ...
                            for (List<String> partitionNameBatch : Iterables.partition(partitionNames, 10)) {
                                Collection<Optional<Partition>> partitions = delegate.getPartitionsByNames(metastoreContext, schemaTableName.getSchemaName(), schemaTableName.getTableName(), MetastoreUtil.getPartitionsWithEmptyVersion(partitionNameBatch)).values();
                                partitions.stream()
                                        .filter(Optional::isPresent)
                                        .map(Optional::get)
