...
public class SemiTransactionalHiveMetastore
{
    ...
    private void commitShared()
    {
        ...
        try {
            for (Map.Entry<SchemaTableName, Action<TableAndMore>> entry : tableActions.entrySet()) {
                ...
                MetastoreContext metastoreContext = new MetastoreContext(action.getContext().getIdentity(), action.getContext().getQueryId().orElse(""), action.getContext().getClientInfo(), action.getContext().getSource());
                switch (action.getType()) {
                    case DROP:
                        committer.prepareDropTable(metastoreContext, schemaTableName);
                        break;
                    case ALTER:
                        committer.prepareAlterTable(action.getContext(), metastoreContext, action.getData());
                        break;
                    case ADD:
                        committer.prepareAddTable(metastoreContext, action.getContext(), action.getData());
                        break;
                    case INSERT_EXISTING:
                        committer.prepareInsertExistingTable(metastoreContext, action.getContext(), action.getData());
                        break;
                    default:
                        throw new IllegalStateException("Unknown action type");
                }
                ...
                for (Map.Entry<List<String>, Action<PartitionAndMore>> partitionEntry : tableEntry.getValue().entrySet()) {
                    ...
                    switch (action.getType()) {
                        case DROP:
                            ...
                            break;
                        case ALTER:
                            committer.prepareAlterPartition(metastoreContext, context, action.getData());
                            break;
                        case ADD:
                            committer.prepareAddPartition(metastoreContext, context, action.getData());
                            break;
                        case INSERT_EXISTING:
                            committer.prepareInsertExistingPartition(metastoreContext, context, action.getData());
                            break;
                        default:
                            throw new IllegalStateException("Unknown action type");
                    }
    ...
    private class Committer
    {
        ...
        private void prepareAlterTable(HdfsContext context, MetastoreContext metastoreContext, TableAndMore tableAndMore)
        {
            deleteOnly = false;

            Table table = tableAndMore.getTable();
            String targetLocation = table.getStorage().getLocation();
            Table oldTable = delegate.getTable(metastoreContext, table.getDatabaseName(), table.getTableName())
                    .orElseThrow(() -> new PrestoException(TRANSACTION_CONFLICT, "The table that this transaction modified was deleted in another transaction. " + table.getDatabaseName() + "." + table.getTableName()));
            ...
            alterTableOperations.add(new AlterTableOperation(metastoreContext, tableAndMore.getTable(), oldTable, tableAndMore.getPrincipalPrivileges()));
    ...
    private static class AlterTableOperation
    {
        ...
        public AlterTableOperation(MetastoreContext metastoreContext, Table newTable, Table oldTable, PrincipalPrivileges principalPrivileges)
        {
            this.metastoreContext = requireNonNull(metastoreContext, "metastoreContext is null");
        ...
        {
            undo = true;
            metastore.replaceTable(metastoreContext, newTable.getDatabaseName(), newTable.getTableName(), newTable, principalPrivileges);
        }
        ...
        public void undo(ExtendedHiveMetastore metastore)
        {
            ...
            metastore.replaceTable(metastoreContext, oldTable.getDatabaseName(), oldTable.getTableName(), oldTable, principalPrivileges);
        }
    }
